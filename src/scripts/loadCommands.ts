...
export const loadCommands = (quest: Quest): Cmd[] => {
  ...
  const commands = [
    ...
    cmdFactory.makeCmd('Look', {
      script() {
        ...
        return quest.settings.lookCountsAsTurn
          ? WorldStates.SUCCESS
    ...
    cmdFactory.makeCmd('Map', {
      script() {
        ...
        if (!flag) return quest.io.failedmsg(quest.i18n.lexicon.no_map);
        return WorldStates.SUCCESS_NO_TURNSCRIPTS;
      },
    }),
    ...
    cmdFactory.makeCmd('PurchaseFromList', {
      script() {
        ...
        quest.state.forEach((key, val) => {
          if (quest.parser.isForSale(val)) {
            ...
            const row = [
              ...
              //quest.world.Money(price),
            ];
            row.push(
              price > quest.game.player.money
                ? '-'
                : `{cmd:buy ${val.alias}:${price}}`,
            );
            l.push(row);
          }
        });
        ...
        quest.io.msg(
          `${quest.i18n.lexicon.current_money}: ${WorldStates.Money(quest.game.player.money)}`,
        );
    ...
    cmdFactory.makeCmd('Drop', {
      ...
      defmsg(char, item) {
        if (item.isAtLoc(char)) return quest.i18n.lexicon.cannot_drop;
    ...
    cmdFactory.makeCmd('Wear2', {
      ...
      defmsg(char, item) {
        return item.ensemble
          ? quest.i18n.lexicon.cannot_wear_ensemble
          : quest.i18n.lexicon.cannot_wear;
      },
    }),
    ...
    cmdFactory.makeCmd('Remove', {
      ...
      defmsg(char, item) {
        return item.ensemble
          ? quest.i18n.lexicon.cannot_wear_ensemble
          : quest.i18n.lexicon.not_wearing;
      },
    }),
    ...
    cmdFactory.makeCmd('Use', {
      ...
      script(objects, matches) {
        ...
        if (obj.useDefaultsTo) {
          const cmd = cmdFactory.findCmd(obj.useDefaultsTo());
          if (cmd) {
    ...
    cmdFactory.makeCmd('Say', {
      script(arr) {
        ...
        quest.state.forEach((key, val) => {
          if (
            val.sayCanHear &&
            val.sayCanHear(quest.game.player, arr[0])
          ) {
            l.push(val);
          }
        });
        ...
        if (l.length === 0) {
          quest.io.msg(
            quest.i18n.processor.say_no_one_here(quest.game.player, arr[0], arr[1]),
          );
          return WorldStates.SUCCESS;
        }
        if (quest.settings.givePlayerSayMsg)
          quest.io.msg(
            `${quest.i18n.processor.nounVerb(
              quest.game.player,
              arr[0],
              true,
            )}, '${sentenceCase(arr[1])}.'`,
        ...
        for (const chr of l) {
          if (
            chr.sayQuestion &&
            quest.state.get(chr.sayQuestion).sayResponse(chr, arr[1])
          )
            return WorldStates.SUCCESS;
        ...
        if (quest.settings.givePlayerSayMsg) {
          quest.io.msg(
            quest.i18n.processor.say_no_response(quest.game.player, arr[0], arr[1]),
          );
        } else {
          quest.io.msg(
            quest.i18n.processor.say_no_response_full(quest.game.player, arr[0], arr[1]),
          );
        }
        return WorldStates.SUCCESS;
      },
    ...
    cmdFactory.makeCmd('UseWith', {
      ...
      script(objects, matches) {
        ...
        if (obj.useWithDefaultsTo) {
          const cmd = cmdFactory.findCmd(obj.useWithDefaultsTo());
          if (cmd) {
        ...
        if (obj2.withUseDefaultsTo) {
          const cmd = cmdFactory.findCmd(obj2.withUseDefaultsTo());
          if (cmd) {
            const result = cmd.script(objects, matches);
            return result ? WorldStates.SUCCESS : WorldStates.FAILED;
          }
    ...
    cmdFactory.makeCmd('AskAbout', {
      rules: [cmdFactory.rules.canTalkTo],
      script(arr) {
        if (!quest.game.player.canTalk()) return WorldStates.FAILED;
        if (!arr[0][0].askabout) {
  ...
  if (quest.settings.playMode === 'dev') {
    commands.push(
      cmdFactory.makeCmd('DebugWalkThrough', {
        objects: [{ text: true }],
        script(objects) {
          ...
          for (const el of wt) {
            if (typeof el === 'string') {
              ...
              quest.settings.walkthroughMenuResponses = Array.isArray(el.menu)
                ? el.menu
                : [el.menu];
    ...
    commands.push(
      cmdFactory.makeCmd('DebugInspect', {
        script(arr) {
          const item = arr[0][0];
          quest.io.debugmsg(
            `See the console for details on the object ${item.name} (press F12 to WorldStates. the console)`,
          );
    ...
    commands.push(
      cmdFactory.makeCmd('DebugInspectByName', {
        script(arr) {
          ...
          quest.io.debugmsg(
            `See the console for details on the object ${item_name} (press F12 to WorldStates. the console)`,
          );
      ...
      cmdFactory.makeCmd('DebugTest', {
        script() {
          if (!quest.settings.tests) {
            ...
            quest.logger.info(
              'To activate testing in your game, set quest.settings.tests to true. More details here: https://github.com/ThePix/QuestJS/wiki/Unit-testing'
            );
            return WorldStates.SUCCESS_NO_TURNSCRIPTS;
          }
    ...
    commands.push(
      cmdFactory.makeCmd('DebugInspectCommand', {
        script(arr) {
          quest.io.debugmsg(`Looking for ${arr[0]}`);
          for (const cmd of commands) {
            if (
              cmd.name.toLowerCase() === arr[0] ||
              (cmd.category && cmd.category.toLowerCase() === arr[0])
            ) {
              quest.io.debugmsg(`Name: ${cmd.name}`);
              for (const key in cmd) {
                if (cmd.hasOwnProperty(key)) {
                  quest.io.debugmsg(` -= 1${key}: ${cmd[key]}`);
                }
              }
            }
          }
    ...
    commands.push(
      cmdFactory.makeCmd('DebugListCommands', {
        script(arr) {
          let count = 0;
          for (const cmd of commands) {
            if (!cmd.name.match(/\d$/)) {
    ...
    commands.push(
      cmdFactory.makeCmd('DebugListCommands2', {
        script(arr) {
          let count = 0;
          for (const cmd of commands) {
    ...
    commands.push(
      cmdFactory.makeCmd('DebugParserToggle', {
        script(arr) {
...
const initNpcCommands = (quest: Quest, cmdFactory: CommandFactory, commands: Cmd[]) => {
  for (const el of commands) {
    if (el.regex) {
      el.regexes = [el.regex];
    }
    if (el.npcCmd) {
      if (!Array.isArray(el.regexes)) quest.logger.info(el);
      // quest.logger.info("creating NPC command for " + el.name)
      ...
      Object.keys(quest.i18n.lexicon.tell_to_prefixes).forEach(key => {
        regexes.push(
          new RegExp(`^${quest.i18n.lexicon.tell_to_prefixes[key]}${regexAsStr}`),
        );
      });
      let script;
      if (el.useThisScriptForNpcs) script = el.script;

      const scope = [];
      for (const el2 of el.objects) {
        scope.push(
          el2 === quest.parser.isHeld ? quest.parser.isHeldByNpc : el2,
        );
        scope.push(
          el2 === quest.parser.isWorn ? quest.parser.isWornByNpc : el2,
        );
      }
  ...
  Object.keys(quest.i18n.lexicon.exit_list).forEach(key => {
    ...
    if (el.type !== 'nocmd') {
      let regex = `(${quest.i18n.lexicon.go_pre_regex})(${el.name
      ...
      Object.keys(quest.i18n.lexicon.tell_to_prefixes).forEach(key => {
        regexes.push(
          new RegExp(`^${quest.i18n.lexicon.tell_to_prefixes[key]}${regex}`),
        );
      })
      commands.push(
