...
export class Merch extends Item {
  ...
  getSellingPrice(char) {
    if (this.state.get(char.loc).buyingValue) {
      return Math.round(
        (this.getPrice() * this.state.get(char.loc).buyingValue) / 100,
      );
    }
  ...
  getBuyingPrice(char) {
    if (this.state.get(char.loc).sellingDiscount) {
      return Math.round(
        (this.getPrice() * (100 - this.state.get(char.loc).sellingDiscount)) / 100,
      );
    }
  ...
  isAtLoc(loc, situation?: number) {
    if (
      (situation === WorldStates.PARSER ||
        situation === WorldStates.SCOPING) &&
      this.isForSale(loc)
    )
      return true;
  ...
  purchase(isMultiple, char) {
    ...
    if (!this.isForSale(char.loc)) {
      if (this.doNotClone && this.isAtLoc(char.name)) {
        return this.io.failedmsg(
          this.lexicon.cannot_purchase_again,
          tpParams,
        );
      }
      return this.io.failedmsg(
        this.lexicon.cannot_purchase_here,
        tpParams,
      );
    }
    ...
    if (char.money < cost)
      return this.io.failedmsg(
        prefix(this, isMultiple) + this.lexicon.cannot_afford,
        tpParams,
      );
  ...
  purchaseScript(isMultiple, char, cost, tpParams) {
    ...
    this.io.msg(
      prefix(this, isMultiple) +
        this.lexicon.purchase_successful,
      tpParams,
    );
    if (this.doNotClone) {
  ...
  sell(isMultiple, char) {
    ...
    char.money += cost;
    this.io.msg(
      prefix(this, isMultiple) + this.lexicon.sell_successful,
      tpParams,
    );
    if (this.doNotClone) {
