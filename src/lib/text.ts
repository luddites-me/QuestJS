...
export class Text extends Base {
  ...
  processText(str, params) {
    ...
    // } catch (err) {
    //  this.io.errormsg("Text processor string caused an error, returning unmodified (reported error: " + err + ")");
    //  return str;
    // }
  }
  ...
  _findObject(name, params, arr) {
    if (params && params[name])
      return typeof params[name] === 'string'
        ? this.state.get(params[name])
        : params[name];
    if (name === 'player') return this.game.player;
    if (this.state.get(name)) return this.state.get(name);
  ...
  findSubject(arr, params) {
    ...
    if (params[arr[0]]) {
      ...
      if (typeof subject === 'string') subject = this.state.get(subject);
  ...
  handlePronouns(arr, params, pronoun) {
    ...
    return arr[1] === 'true'
      ? sentenceCase(subject.pronouns[pronoun])
  ...
  init() {
    ...
    this.text_processors.number = (arr, params) => {
      ...
      if (name.match(/^\d+$/)) return this.processor.toWords(toInt(name));
    ...
    this.text_processors.ordinal = (arr, params) => {
      const name = arr.shift();
      if (name.match(/^\d+$/)) return this.processor.toOrdinal(toInt(name));
      if (typeof params[name] === 'number')
    ...
    this.text_processors.money = (arr, params) => {
      const name = arr.shift();
      if (name.match(/^\d+$/)) return this.utils.displayMoney(toInt(name));
      if (typeof params[name] === 'number')
      ...
      if (obj.loc === this.game.player.name && obj.getSellingPrice) {
        return this.utils.displayMoney(
          obj.getSellingPrice(this.game.player),
        );
      }
      if (obj.loc === this.game.player.name && obj.getBuyingPrice) {
        return this.utils.displayMoney(
          obj.getBuyingPrice(this.game.player),
        );
      }
      if (obj.getPrice) {
    ...
    this.text_processors.dateTime = (arr, params: Options) => {
    ...
    this.text_processors.transitDest = (arr, params) => {
      const transit = arr[0]
        ? this.state.get(arr[0])
        : this.state.get(this.game.player.loc.name);
    ...
    this.text_processors.param = (arr, params) => {
      ...
      if (x === undefined) {
        // this.io.errormsg("In text processor param, could not find a value with the key '" + arr[0] + "'. Check the console (F12) to see what params is. [" + params.tpOriginalString + "]");
        this.log.info('params:');
        this.log.info(params);
        return false;
      }
    ...
    this.text_processors.terse = (arr, params) => {
      if (
        (this.game.verbosity === WorldStates.TERSE &&
          this.game.room.visited === 0) ||
        this.game.verbosity === WorldStates.VERBOSE
      ) {
    ...
    this.text_processors.nm = (arr, params: Options) => {
      const subject = this.findSubject(arr, params);
      if (!subject) return false;
      const opt = Object.assign({},params);
      if (arr[1] === 'the') opt.article = Known.DEFINITE;
      if (arr[1] === 'a') opt.article = Known.INDEFINITE;
      if (params[`${subject.name}_count`])
        opt[`${subject.name}_count`] = params[`${subject.name}_count`];
      return arr[2] === 'true'
        ? sentenceCase(this.processor.getName(subject, opt))
        : this.processor.getName(subject, opt);
    };
    this.text_processors.nms = (arr, params: Options) => {
      const subject = this.findSubject(arr, params);
      if (!subject) return false;
    ...
    this.text_processors.cj = (arr, params) => {
      ...
      if (!subject) return false;
      return arr[2] === 'true'
        ? sentenceCase(this.processor.conjugate(subject, arr[1]))
        : this.processor.conjugate(subject, arr[1]);
    };
    ...
    this.text_processors.pa2 = (arr, params) => {
      ...
      if (chr1.pronouns === chr2.pronouns) {
        ...
        return arr[1] === 'true'
          ? sentenceCase(this.processor.getName(chr1, opt))
          : this.processor.getName(chr1, opt);
      }

      return arr[1] === 'true'
        ? sentenceCase(chr1.pronouns.poss_adj)
