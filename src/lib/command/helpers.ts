...
export class Helpers extends Base {
  ...
  handleFillWithLiquid(char, vessel, liquid) {
    ...
    if (!char.canManipulate(vessel, 'fill')) return WorldStates.FAILED;
    if (!char.getAgreement('Fill', vessel)) return WorldStates.FAILED;
  ...
  handlePutInContainer(char, objects) {
    ...
    if (container.closed)
      return this.io.failedmsg(this.lexicon.container_closed, tpParams);
    if (!char.canManipulate(objects[0], 'put')) return WorldStates.FAILED;
    ...
    for (const obj of objects[0]) {
      ...
      if (flag) {
        if (!obj.isAtLoc(char.name)) {
          this.io.failedmsg(
            prefix(obj, multiple) + this.lexicon.not_carrying,
            {
              char,
              item: obj,
            },
          ...
          this.io.msg(
            prefix(obj, multiple) + this.lexicon.done_msg,
          );
          success = WorldStates.SUCCESS;
        }
      }
    }
    if (success === WorldStates.SUCCESS) char.pause();
    ...
    for (const obj of objects[0]) {
      ...
      if (flag) {
        if (!obj.isAtLoc(container.name)) {
          this.io.failedmsg(
            prefix(obj, multiple) +
              this.processor.not_inside(container, obj),
          );
        } else {
  ...
  handleUntieFrom(char, rope, obj2?) {
    ...
    if (obj2 === undefined) {
      ...
      if (rope.tiedTo1 && !rope.tiedTo2) {
        ...
        this.state.get(rope.tiedTo1).isHere() &&
        !this.state.get(rope.tiedTo2).isHere()
      ) {
        obj2 = this.state.get(rope.tiedTo1);
      } else if (
        !this.state.get(rope.tiedTo1).isHere() &&
