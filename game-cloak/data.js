...
const cloakHere = function () {
  ...
  if (
    QuestJs._w.cloak.isAtLoc('hook')
    && QuestJs._game.player.isAtLoc('cloakroom')
  ) return true;
  return false;
};
...
QuestJs._create.createItem('cloak', QuestJs._templates.WEARABLE(), {
  examine:
    'The cloak is black... Very black... So black it seems to absorb light.',
  worn : true,
  loc  : 'me',
});
  ...
  new QuestJs._command.Cmd('HangUp', {
    ...
    script(objects) {
      if (!objects[0][0].isAtLoc(QuestJs._game.player)) {
        return QuestJs._io.failedmsg("You're not carrying {sb:obj}.", {
          obj: objects[0][0],
        });
      }
      if (objects[0][0].worn) {
        return QuestJs._io.failedmsg("Not while you're wearing {sb:obj}!", {
          obj: objects[0][0],
        });
      }
      if (!QuestJs._game.player.isAtLoc('cloakroom')) {
        return QuestJs._io.failedmsg('Hang {sb:obj} where, exactly?', {
          obj: objects[0][0],
        });
      }
      objects[0][0].moveToFrom('hook');
      QuestJs._io.msg('You hang {nm:obj:the} on the hook.', {
        obj: objects[0][0],
      });
