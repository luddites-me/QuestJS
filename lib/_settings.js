...
const settings = {
  ...
  isHeldNotWorn(item) {
    return (
      item.isAtLoc(QuestJs._game.player.name, world.SIDE_PANE) &&
  ...
  isHere(item) {
    return (
      item.isAtLoc(QuestJs._game.player.loc, world.SIDE_PANE) &&
      world.ifNotDark(item)
    );
  },
  ...
  writeScript(folder) {
    ...
    if (settings.tests) {
      ...
      document.writeln(
        `<script src="${settings.folder}tests.js"></scr` + 'ipt>'
      );
    }
    document.writeln(
      `<script src="${folder ? 'lang/' : ''}${settings.lang}.js"></scr` + 'ipt>'
    );
    if (settings.customExits) {
      document.writeln(
        `<script src="${settings.folder}${settings.customExits}.js"></scr` +
          'ipt>'
      );
    }
    for (const file of settings.libraries) {
      document.writeln(
        `<script src="${folder ? 'lib/' : ''}${file}.js"></scr` + 'ipt>'
      );
    }
    for (const lib of settings.customLibraries) {
      for (const file of lib.files) {
        document.writeln(
          `<script src="${folder ? `${lib.folder}/` : ''}${file}.js"></scr` +
            'ipt>'
        );
      }
    }
    for (const file of settings.files) {
      document.writeln(
        `<script src="${settings.folder}${file}.js"></scr` + 'ipt>'
      );
    }
  },
};
