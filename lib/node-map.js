...
map.init = function() {
  ...
  while (rooms.length > 0) {
    ...
    for (let dir of QuestJs.LANG.exit_list) {
...
// Mapping from room to exitRoom, exit is the exit linking the two, dir is an object from QuestJs.LANG.exit_list
...
if (settings.playMode === 'dev') {
  commands.unshift(new QuestJs.command.Cmd('DebugMap', {
    regex:/^debug map$/,
    objects:[
    ],
    script:function() {
...
QuestJs.command.findCmd('Map').script = function() {
  if (settings.hideMap) {
    $('#quest-map').show()
