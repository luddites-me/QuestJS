...
map.mapRoomFromExit = function (room, exitRoom, exit, dir, rooms) {
  ...
  if (exitRoom.mapX === undefined) {
    ...
    if (exitRoom.mapX !== room.mapX + offsetX) {
      QuestJs._log.info(
        `WARNING: Mapping exit from ${room.name} to ${exit.name} - funny X offset (${
          exitRoom.mapX
        } vs ${room.mapX + offsetX})`,
      );
    ...
    if (exitRoom.mapY !== room.mapY - offsetY)
      QuestJs._log.info(
        `WARNING: Mapping exit from ${room.name} to ${exit.name} - funny Y offset (${
          exitRoom.mapY
        } vs ${room.mapY + offsetY})`,
      );
    if (exitRoom.mapZ !== room.mapZ + offsetZ)
      QuestJs._log.info(`WARNING: Mapping exit from ${room.name} to ${exit.name} - funny Z offset`);
  }
};
...
map.update = function () {
  ...
  QuestJs._io.draw(QuestJs._settings.mapWidth, QuestJs._settings.mapHeight, result, {
...
map.mapDraw = function (lists, region, level, room) {
  ...
  for (const dir of QuestJs._lang.exit_list) {
    ...
    if (exit.mapDrawString) {
      ...
      s += `" x2="${room.mapX + (dir.x * QuestJs._settings.mapScale) / 2}" y2="${
        room.mapY - (dir.y * QuestJs._settings.mapScale) / 2
      }`;
      s += '"/>';
...
map.moveableLocDraw = function (lists, region, level, room) {
  for (const el of this.locations) {
    ...
    destinationLayer.push(
      room.mapDrawBase ? room.mapDrawBase(level, el) : map.mapDrawDefault(room, el),
    );
    ...
    for (const dir of QuestJs._lang.exit_list) {
      ...
      if (exit.mapDrawBase) {
        ...
        for (const el of room.locations) {
          ...
          s += `" x2="${el.mapX + (dir.x * QuestJs._settings.mapScale) / 2}" y2="${
            el.mapY - (dir.y * QuestJs._settings.mapScale) / 2
          }`;
          s += '"/>';
          lists.exits.push(s);
        }
