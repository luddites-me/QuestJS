...
map.getStartingLocations = function () {
  const starts = QuestJs._settings.mapAutomapFrom
    ? QuestJs._settings.mapAutomapFrom.map((el) => QuestJs._w[el])
...
map.init = function () {
  ...
  const rooms = QuestJs._settings.mapGetStartingLocations
    ? QuestJs._settings.mapGetStartingLocations()
    : map.getStartingLocations();
  ...
  while (rooms.length > 0) {
    ...
    for (const dir of QuestJs._lang.exit_list) {
  ...
  map.layers = [
    ...
    {
      name: 'otherLevels',
      attrs: `stroke="${QuestJs._settings.mapBorderColour}" stroke-width="1" fill="${QuestJs._settings.mapLocationColour}" opacity="0.2" pointer-events="none"`,
    },
    // exits
    {
      name: 'exits',
      attrs: `style="stroke:${QuestJs._settings.mapExitColour};stroke-width:${QuestJs._settings.mapExitWidth}px;fill:${QuestJs._settings.mapExitColour}"`,
    },
    // rooms on this level
    {
      name: 'base',
      attrs: `stroke="${QuestJs._settings.mapBorderColour}" stroke-width="1" fill="${QuestJs._settings.mapLocationColour}"`,
    },
    ...
    {
      name: 'labels',
      attrs: `pointer-events="none" fill="${QuestJs._settings.mapLabelColour}" text-anchor="middle"`,
    },
  ];
};
...
map.mapRoomFromExit = function (room, exitRoom, exit, dir, rooms) {
  ...
  const offsetX =
    (exit.mapOffsetX ? exit.mapOffsetX : dir.x) * QuestJs._settings.mapScale;
  const offsetY =
    (exit.mapOffsetY ? exit.mapOffsetY : dir.y) * QuestJs._settings.mapScale;
  ...
  if (exitRoom.mapX === undefined) {
    ...
    if (exitRoom.mapX !== room.mapX + offsetX) {
      QuestJs._log.info(
        `WARNING: Mapping exit from ${room.name} to ${
          exit.name
        } - funny X offset (${exitRoom.mapX} vs ${room.mapX + offsetX})`,
      );
    ...
    if (exitRoom.mapY !== room.mapY - offsetY)
      QuestJs._log.info(
        `WARNING: Mapping exit from ${room.name} to ${
          exit.name
        } - funny Y offset (${exitRoom.mapY} vs ${room.mapY + offsetY})`,
      );
    if (exitRoom.mapZ !== room.mapZ + offsetZ)
      QuestJs._log.info(
        `WARNING: Mapping exit from ${room.name} to ${exit.name} - funny Z offset`,
      );
  }
};
...
map.update = function () {
  ...
  if (QuestJs._settings.mapExtras)
    result.push(...QuestJs._settings.mapExtras());
  result.push(
    QuestJs._settings.mapMarker
      ? QuestJs._settings.mapMarker(QuestJs._w[QuestJs._game.player.loc])
      : map.marker(
          QuestJs._w[QuestJs._game.player.loc].mapX,
          QuestJs._w[QuestJs._game.player.loc].mapY,
        ),
  );

  // Centre the view on the player, and draw it
  const x =
    QuestJs._w[QuestJs._game.player.loc].mapX - QuestJs._settings.mapWidth / 2;
  const y =
    -QuestJs._settings.mapHeight / 2 +
    QuestJs._w[QuestJs._game.player.loc].mapY;
  QuestJs._io.draw(
...
map.mapDraw = function (lists, region, level, room) {
  ...
  for (const dir of QuestJs._lang.exit_list) {
    ...
    if (exit.mapDrawString) {
      ...
      s += `" x2="${
        room.mapX + (dir.x * QuestJs._settings.mapScale) / 2
      }" y2="${room.mapY - (dir.y * QuestJs._settings.mapScale) / 2}`;
      s += '"/>';
      ...
      let s = `<line x1="${room.mapX}" y1="${room.mapY}`;
      s += `" x2="${(exitRoom.mapX + room.mapX) / 2}" y2="${
        (exitRoom.mapY + room.mapY) / 2
      }`;
      s += '"/>';
...
map.moveableLocDraw = function (lists, region, level, room) {
  for (const el of this.locations) {
    ...
    destinationLayer.push(
      room.mapDrawBase
        ? room.mapDrawBase(level, el)
        : map.mapDrawDefault(room, el),
    );
    ...
    for (const dir of QuestJs._lang.exit_list) {
      ...
      if (exit.mapDrawBase) {
        ...
        for (const el of room.locations) {
          ...
          s += `" x2="${
            el.mapX + (dir.x * QuestJs._settings.mapScale) / 2
          }" y2="${el.mapY - (dir.y * QuestJs._settings.mapScale) / 2}`;
          s += '"/>';
          lists.exits.push(s);
        }
      } else {
        let s = `<line x1="${el.mapX}" y1="${room.mapY}`;
        s += `" x2="${(exitRoom.mapX + el.mapX) / 2}" y2="${
          (exitRoom.mapY + el.mapY) / 2
        }`;
        s += '"/>';
        lists.exits.push(s);
      }
...
map.mapDrawLabelDefault = function (o, loc) {
  ...
  if (QuestJs._settings.mapLabelRotate)
    s += `" transform="rotate(${QuestJs._settings.mapLabelRotate},${loc.mapX},${
      loc.mapY - QuestJs._settings.mapLabelOffset
    })`;
  s += '">';
...
map.rect = function (isRoom, room, points, attrs) {
  let s = `<rect x="${room.mapX + points[0][0]}" y="${
    room.mapY + points[0][1]
  }`;
...
if (QuestJs._settings.playMode === 'dev') {
  QuestJs._commands.unshift(
    new QuestJs._command.Cmd('DebugMap', {
      regex: /^debug map$/,
      objects: [],
      script() {
        for (const key in QuestJs._w) {
          ...
          QuestJs._io.metamsg(
            `${QuestJs._w[key].name}: ${QuestJs._w[key].mapX}, ${QuestJs._w[key].mapY}, ${QuestJs._w[key].mapZ} Region=${QuestJs._w[key].mapRegion}`,
          );
        }
