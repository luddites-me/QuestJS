...
map.getStartingLocations = function() {
  const starts = QuestJs._settings.mapAutomapFrom ? QuestJs._settings.mapAutomapFrom.map(el => w[el]) : [w[game.player.loc]]
  let count = 0
...
map.update = function() {
  ...
  result.push(QuestJs._settings.mapMarker ? QuestJs._settings.mapMarker(w[game.player.loc]) : map.marker(w[game.player.loc].mapX, w[game.player.loc].mapY))

  // Centre the view on the player, and draw it
  const x = w[game.player.loc].mapX - QuestJs._settings.mapWidth/2
  const y = -QuestJs._settings.mapHeight/2 + w[game.player.loc].mapY
  ...
  QuestJs._commands.unshift(new QuestJs._command.Cmd('DebugMap', {
    ...
    script:function() {
      for (let key in w) {
        ...
        QuestJs._io.metamsg(w[key].name + ': ' + w[key].mapX + ', ' + w[key].mapY + ', ' + w[key].mapZ + ' Region=' + w[key].mapRegion)
      }
