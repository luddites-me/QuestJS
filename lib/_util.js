...
function printOrRun2(char, item, attname, options) {
  ...
  if (!item[attname]) {
    ...
    QuestJs._io.msg(attname, { char, item });
    return true;
  }
  if (typeof item[attname] === 'string') {
    // The attribute is a string
    QuestJs._io.msg(prefix(item, options.multi) + item[attname], { char, item });
    return true;
  }
  if (typeof item[attname] === 'function') {
    // The attribute is a function
...
function formatList(itemArray, options) {
  ...
  const l = itemArray.map((el) =>
    ...
    typeof el === 'string' ? el : QuestJs._lang.getName(el, options),
  );
...
util.niceDirection = function (dir) {
  const dirObj = QuestJs._lang.exit_list.find((el) => el.name === dir);
...
util.reverseDirectionObj = function (dir) {
  return QuestJs._lang.exit_list.find((el) => el.name === util.reverseDirection(dir));
};
...
util.getCustomDateTimeDict = function (options) {
  ...
  for (const el of QuestJs._settings.dateTime.data) {
...
util.getCustomDateTime = function (options) {
  ...
  for (const key in QuestJs._settings.dateTime.functions) {
...
util.changePOV = function (char, pronouns) {
  ...
  char.pronouns = pronouns || QuestJs._lang.pronouns.secondperson;
...
util.defaultExitUse = function (char, dir, exit) {
  ...
  if (exit.msg) {
    ...
    QuestJs._io.msg(QuestJs._lang.go_successful, { char, dir });
  }
