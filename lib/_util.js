...
import { io, errormsg, msg } from "./_io";
...
export function printOrRun2(char, item, attname, options) {
  if (options === undefined) options = {};
  let flag;
  let i;
  ...
  if (!item[attname]) {
    ...
    msg(attname, { char, item });
    return true;
  }
  if (typeof item[attname] === "string") {
    // The attribute is a string
    msg(prefix(item, options.multi) + item[attname], { char, item });
    return true;
  }
  if (typeof item[attname] === "function") {
    // The attribute is a function
...
export function formatList(itemArray, options) {
  ...
  const l = itemArray.map((el) =>
    ...
    typeof el === "string" ? el : lang.getName(el, options)
  );

  let s = "";
  if (settings.oxfordComma && l.length === 2 && options.lastJoiner)
...
export function displayMoney(n) {
  if (typeof settings.moneyFormat === "undefined") {
    errormsg(
      "No format for money set (set settings.moneyFormat in settings.js)."
    );
    return `${n}`;
  }
  ...
  errormsg(
    "settings.moneyFormat in settings.js expected to have either 1, 2 or 3 exclamation marks."
  );
  return `${n}`;
}
...
export function scopeHeldBy(chr, situation = world.PARSER) {
...
export function scopeHereListed() {
  ...
  for (const key in w) {
    ...
    if (
      !o.player &&
      o.isAtLoc(game.player.loc, world.LOOK) &&
      world.ifNotDark(o)
    ) {
      list.push(o);
    }
...
export function scopeAllNpcHere(ignoreDark) {
  ...
  for (const key in w) {
    const o = w[key];
    if (
      o.isAtLoc(game.player.loc, world.PARSER) &&
      o.npc &&
      (world.ifNotDark(o) || ignoreDark)
    ) {
      list.push(o);
...
util.testForRecursion = function (char, item) {
  ...
  while (w[contName]) {
    if (w[contName].loc === item.name)
      return falsemsg(lang.container_recursion, {
        char,
        container: this,
        item,
      });
...
util.nameModifierFunctionForContainer = function (o, list) {
  ...
  if (contents.length > 0 && (!o.closed || o.transparent)) {
    list.push(
      lang.contentsForData[o.contentsType].prefix +
        o.listContents(world.LOOK) +
        lang.contentsForData[o.contentsType].suffix
    );
  }
  // console.log(list)
};
...
util.niceDirection = function (dir) {
  const dirObj = lang.exit_list.find((el) => el.name === dir);
...
util.reverseDirectionObj = function (dir) {
  return lang.exit_list.find((el) => el.name === util.reverseDirection(dir));
};
...
util.getDateTimeDict = function (options) {
  if (!options) options = {};
  return settings.dateTime.formats
...
util.getCustomDateTimeDict = function (options) {
  ...
  for (const el of settings.dateTime.data) {
...
util.getCustomDateTime = function (options) {
  ...
  let s = options.format
    ? settings.dateTime.formats[options.format]
    : settings.dateTime.formats.def;
  for (const key in settings.dateTime.functions) {
...
util.changePOV = function (char, pronouns) {
  ...
  if (game.player) {
    ...
    game.player.regex = new RegExp(
      `^(${char.npcAlias ? char.npcAlias : char.alias})$`
    );
  }
  char.player = true;
  char.npcPronouns = char.pronouns;
  char.pronouns = pronouns || lang.pronouns.secondperson;
