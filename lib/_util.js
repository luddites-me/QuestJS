...
util.testForRecursion = function(char, item) {
  ...
  while (w[contName]) {
    if (w[contName].loc === item.name) return falsemsg(QuestJs._lang.container_recursion, {char:char, container:this, item:item})
...
util.defaultExitUse = function(char, dir, exit) {
  ...
  if (exit.isLocked()) {
    if (exit.lockedmsg) {
      ...
      msg(QuestJs._lang.locked_exit(char, exit))
    }
    return false
  }
  ...
  if (exit.msg) {
    ...
    msg(QuestJs._lang.go_successful, {char:char, dir:dir})
  }
...
util.useWithDoor = function(char, dir) {
  ...
  if (!obj.locked) {
    obj.closed = false
    msg(QuestJs._lang.open_and_enter, tpParams)
  ...
  if (obj.testKeys(char)) {
    obj.closed = false
    obj.locked = false
    msg(QuestJs._lang.unlock_and_enter, tpParams)
    world.setRoom(char, this.name, false)
    return true
  }        
  msg(QuestJs._lang.try_but_locked, tpParams)
  return false
}
...
util.cannotUse = function(char, dir) {
  ...
  msg(this.msg ? this.msg : QuestJs._lang.try_but_locked, tpParams)
  return false
}
