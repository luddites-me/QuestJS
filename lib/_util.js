...
//@DOC
// Creates a string from an array. If the array element is a string, that is used, if it is an item, `QuestJs.LANG.getName` is used (and passed the `options`). Items are sorted alphabetically, based on the "name" attribute.
//
// Options:
//
// * article:    DEFINITE (for "the") or INDEFINITE (for "a"), defaults to none (see `QuestJs.LANG.getName`)
...
// * modified:   item aliases modified (see `QuestJs.LANG.getName`) (defaults to false)
...
function formatList(itemArray, options) {
  ...
  const l = itemArray.map(el => {
    ...
    return typeof el === "string" ? el : QuestJs.LANG.getName(el, options)
  })
...
util.testForRecursion = function(char, item) {
  ...
  while (w[contName]) {
    if (w[contName].loc === item.name) return falsemsg(QuestJs.LANG.container_recursion, {char:char, container:this, item:item})
...
util.nameModifierFunctionForContainer = function(o, list) {
  ...
  if (contents.length > 0 && (!o.closed || o.transparent)) {
    list.push(QuestJs.LANG.contentsForData[o.contentsType].prefix + o.listContents(world.LOOK) + QuestJs.LANG.contentsForData[o.contentsType].suffix)
  }
  //console.log(list)
}
...
util.listContents = function(situation, modified = true) {
  return formatList(this.getContents(situation), {article:INDEFINITE, lastJoiner:QuestJs.LANG.list_and, modified:modified, nothing:QuestJs.LANG.list_nothing, loc:this.name})
}

util.niceDirection = function(dir) {
  const dirObj = QuestJs.LANG.exit_list.find(function(el) { return el.name === dir })
...
util.reverseDirectionObj = function(dir) {
  return QuestJs.LANG.exit_list.find(function(el) { return el.name === util.reverseDirection(dir) })
}
...
util.changePOV = function(char, pronouns) {
  ...
  char.pronouns = pronouns ? pronouns : QuestJs.LANG.pronouns.secondperson
...
util.defaultExitUse = function(char, dir, exit) {
  ...
  if (exit.isLocked()) {
    if (exit.lockedmsg) {
      ...
      msg(QuestJs.LANG.locked_exit(char, exit))
    }
    return false
  }
  ...
  if (exit.msg) {
    ...
    msg(QuestJs.LANG.go_successful, {char:char, dir:dir})
  }
...
util.useWithDoor = function(char, dir) {
  ...
  if (!obj.locked) {
    obj.closed = false
    msg(QuestJs.LANG.open_and_enter, tpParams)
  ...
  if (obj.testKeys(char)) {
    obj.closed = false
    obj.locked = false
    msg(QuestJs.LANG.unlock_and_enter, tpParams)
    world.setRoom(char, this.name, false)
    return true
  }        
  msg(QuestJs.LANG.try_but_locked, tpParams)
  return false
}
...
util.cannotUse = function(char, dir) {
  ...
  msg(this.msg ? this.msg : QuestJs.LANG.try_but_locked, tpParams)
  return false
}
