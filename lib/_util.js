...
function printOrRun(char, item, attname, options) {
  ...
  if (typeof item[attname] === 'string') {
    ...
    const s =
      'Unsupported type for printOrRun (' + attname + ' is a ' + typeof item[attname] + ').';
...
function printOrRun2(char, item, attname, options) {
  ...
  if (Array.isArray(item[attname])) {
    ...
    flag = true;
    for (i = 0; i < item[attname].length; i += 1) {
...
function formatList(itemArray, options) {
  ...
  let s = '';
  if (QuestJs._settings.oxfordComma && l.length === 2 && options.lastJoiner)
...
function displayMoney(n) {
  if (typeof QuestJs._settings.moneyFormat === 'undefined') {
    errormsg(
      'No format for money set (set QuestJs._settings.moneyFormat in QuestJs._settings.js).',
    );
    return '' + n;
  }
  ...
  if (ary.length === 2) {
    ...
    errormsg(
      'QuestJs._settings.moneyFormat in QuestJs._settings.js expected to have either 1, 2 or 3 exclamation marks.',
    );
    return '' + n;
  }
}
...
function displayNumber(n, control) {
  ...
  if (!regex.test(control)) {
    errormsg(
      'Unexpected format in displayNumber (' +
        control +
...
array.clone = function (ary, options) {
  ...
  if (options.attribute)
    newary = newary.map((el) =>
      typeof el[options.attribute] === 'function' ? el[options.attribute]() : el[options.attribute],
    );
...
function scopeReachable() {
  ...
  for (let key in QuestJs._w) {
    if (
      QuestJs._w[key].scopeStatus === QuestJs._world.REACHABLE &&
      QuestJs._world.ifNotDark(QuestJs._w[key])
    ) {
...
function scopeHereListed() {
  ...
  for (let key in QuestJs._w) {
    ...
    if (
      !o.player &&
      o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.LOOK) &&
      QuestJs._world.ifNotDark(o)
    ) {
      list.push(o);
    }
  }
...
function scopeNpcHere(ignoreDark) {
  ...
  for (let key in QuestJs._w) {
    ...
    if (
      o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.LOOK) &&
      o.npc &&
      (QuestJs._world.ifNotDark(o) || ignoreDark)
    ) {
      list.push(o);
    }
  }
...
function scopeAllNpcHere(ignoreDark) {
  ...
  for (let key in QuestJs._w) {
    const o = QuestJs._w[key];
    if (
      o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) &&
      o.npc &&
      (QuestJs._world.ifNotDark(o) || ignoreDark)
    ) {
      list.push(o);
...
util.testForRecursion = function (char, item) {
  ...
  while (QuestJs._w[contName]) {
    if (QuestJs._w[contName].loc === item.name)
      return QuestJs._io.falsemsg(QuestJs._lang.container_recursion, {
        char: char,
        container: this,
        item: item,
      });
...
util.nameModifierFunctionForContainer = function (o, list) {
  ...
  if (contents.length > 0 && (!o.closed || o.transparent)) {
    list.push(
      QuestJs._lang.contentsForData[o.contentsType].prefix +
        o.listContents(QuestJs._world.LOOK) +
        QuestJs._lang.contentsForData[o.contentsType].suffix,
    );
  }
...
util.niceDirection = function (dir) {
  const dirObj = QuestJs._lang.exit_list.find(function (el) {
    return el.name === dir;
  });
...
util.reverseDirectionObj = function (dir) {
  return QuestJs._lang.exit_list.find(function (el) {
    return el.name === util.reverseDirection(dir);
  });
};
...
util.getDateTimeDict = function (options) {
  if (!options) options = {};
  return QuestJs._settings.dateTime.formats
...
util.getCustomDateTime = function (options) {
  ...
  let s = options.format
    ? QuestJs._settings.dateTime.formats[options.format]
    : QuestJs._settings.dateTime.formats.def;
...
util.seconds = function (seconds, minutes = 0, hours = 0, days = 0) {
  if (QuestJs._settings.dateTime.convertSeconds)
    return QuestJs._settings.dateTime.convertSeconds(seconds, minutes, hours, days);
...
util.changePOV = function (char, pronouns) {
  if (typeof char === 'string') {
    if (!QuestJs._w[char])
      return QuestJs._io.errormsg("Failed to change POV, no object called '" + char + "'");
  ...
  if (QuestJs._game.player) {
    ...
    QuestJs._game.player.regex = new RegExp(
...
util.useWithDoor = function (char, dir) {
  ...
  if (obj === undefined) {
    QuestJs._io.errormsg(
      "Not found an object called '" +
        this.door +
        "'. Any exit that uses the 'util.useWithDoor' function must also set a 'door' attribute.",
    );
  }
