...
util.testForRecursion = function(char, item) {
  ...
  while (w[contName]) {
    if (w[contName].loc === item.name) return QuestJs._io.falsemsg(QuestJs._lang.container_recursion, {char:char, container:this, item:item})
...
util.nameModifierFunctionForContainer = function(o, list) {
  ...
  if (contents.length > 0 && (!o.closed || o.transparent)) {
    list.push(QuestJs._lang.contentsForData[o.contentsType].prefix + o.listContents(world.LOOK) + QuestJs._lang.contentsForData[o.contentsType].suffix)
  }
...
util.getCustomDateTimeDict = function(options) {
  const dict = {}
  let time = QuestJs._settings.dateTime.startTime + game.elapsedTime
...
util.changePOV = function(char, pronouns) {
  if (typeof char === 'string') {
    if (!w[char]) return QuestJs._io.errormsg("Failed to change POV, no object called '" + char + "'")
    char = w[char]
  }
