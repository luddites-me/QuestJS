...
function printOrRun2(char, item, attname, options) {
  ...
  if (typeof item[attname] === 'string') {
    // The attribute is a string
    QuestJs._io.msg(prefix(item, options.multi) + item[attname], {
      char,
      item,
    });
    return true;
...
util.addChangeListener = function (
  object,
  attName,
  func,
...
util.reverseDirectionObj = function (dir) {
  return QuestJs._lang.exit_list.find(
    (el) => el.name === util.reverseDirection(dir),
  );
};
...
util.getDateTime = function (options) {
  if (!QuestJs._settings.dateTime.formats) {
    const time = new Date(
      QuestJs._game.elapsedTime * 1000 + QuestJs._game.startTime.getTime(),
    );
    return time.toLocaleString(
      QuestJs._settings.dateTime.locale,
...
util.getStdDateTimeDict = function (options) {
  ...
  const time = new Date(
    timeInSeconds * 1000 + QuestJs._game.startTime.getTime(),
  );
...
util.seconds = function (seconds, minutes = 0, hours = 0, days = 0) {
  if (QuestJs._settings.dateTime.convertSeconds)
    return QuestJs._settings.dateTime.convertSeconds(
      seconds,
      minutes,
      hours,
      days,
...
util.elapsed = function (seconds, minutes = 0, hours = 0, days = 0) {
  return (
    util.seconds(seconds, minutes, hours, days) >= QuestJs._game.elapsedTime
  );
};

util.isAfter = function (timeString) {
  if (typeof timeString === 'number')
    return QuestJs._game.elapsedTime > timeString;
...
util.changePOV = function (char, pronouns) {
  if (typeof char === 'string') {
    ...
    QuestJs._io.errormsg('Failed to change POV, char not defined.');
...
util.cannotUse = function (char, dir) {
  const tpParams = { char };
  QuestJs._io.msg(
    QuestJs._io.msg ? QuestJs._io.msg : QuestJs._lang.try_but_locked,
    tpParams,
  );
  return false;
};
