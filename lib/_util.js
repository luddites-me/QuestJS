...
//       QuestJs._io.msg("This was the first time.")
//     }, function() {
//       QuestJs._io.msg("This was NOT the first time.")
//     }, function() {
//
...
QuestJs._tools.firsttime = firsttime;
...
function printOrRun(char, item, attname, options) {
  ...
  if (typeof item[attname] === "string") {
    ...
    QuestJs._io.msg(s, {char:char, item:item});
    return true;
  }
    ...
    QuestJs._io.errormsg(s + " F12 for more.")
    throw new Error(s)
  }
}

QuestJs._tools.printOrRun = printOrRun;
...
function printOrRun2(char, item, attname, options) {
  ...
  if (Array.isArray(item[attname])) {
    ...
    //QuestJs._io.debugmsg(0, "Array: " + attname);
    flag = true;
  ...
  if (Array.isArray(attname)) {
    ...
    if (typeof attname === "function"){
      ...
      QuestJs._io.msg(attname, {char:char, item:item});
      return true;
    }
  }  
  else if (typeof item[attname] === "string") {
    // The attribute is a string
    QuestJs._io.msg(prefix(item, options.multi) + item[attname], {char:char, item:item});
    return true;
  }
  else if (typeof item[attname] === "function"){
    // The attribute is a function
    ...
    QuestJs._io.errormsg("Unsupported type for printOrRun");
    return false;
  }
}
  ...
  for (let dice of s.split("+")) {
    ...
    if (/^\d+$/.test(dice)) {
      ...
      if (parts.length === 2 && /^\d+$/.test(parts[0]) && /^[0-9\:]+$/.test(parts[1])) {
        ...
        QuestJs._log.info("Can't parse dice type (but will attempt to use what I can): " + dice)
...
QuestJs._tools.sentenceCase = sentenceCase;
...
// In the command, you can have `QuestJs._io.msg(prefix(item, isMultiple) + "Done"), and it is sorted.
function prefix(item, isMultiple) {
  if (!isMultiple) { return ""; }
  return sentenceCase(item.alias) + ": ";
}

QuestJs._tools.prefix = prefix;
...
function formatList(itemArray, options) {
  ...
  const l = itemArray.map(el => {
    ...
    return typeof el === "string" ? el : QuestJs._lang.getName(el, options)
  })
  
  
  let s = "";
  if (QuestJs._settings.oxfordComma && l.length === 2 && options.lastJoiner) return l[0] + ' ' + options.lastJoiner + ' ' + l[1]
  do {
    s += l.shift()
    if (l.length === 1 && options.lastJoiner) {
      if (QuestJs._settings.oxfordComma) s += options.sep
...
QuestJs._tools.formatList = formatList;
...
QuestJs._tools.toRoman = toRoman;



//@DOC
// Returns the given number as a string formatted as money. The formatting is defined by QuestJs._settings.moneyFormat.
function displayMoney(n) {
  if (typeof QuestJs._settings.moneyFormat === "undefined") {
    errormsg ("No format for money set (set QuestJs._settings.moneyFormat in QuestJs._settings.js).");
    return "" + n;
  }
  const ary = QuestJs._settings.moneyFormat.split("!");
  if (ary.length === 2) {
    return QuestJs._settings.moneyFormat.replace("!", "" + n);
  }
    ...
    errormsg ("QuestJs._settings.moneyFormat in QuestJs._settings.js expected to have either 1, 2 or 3 exclamation marks.")
    return "" + n;
  }
}

QuestJs._tools.displayMoney = displayMoney;
...
QuestJs._tools.displayNumber = displayNumber;
...
function scopeReachable() {
  ...
  for (let key in QuestJs._w) {
    if (QuestJs._w[key].scopeStatus === QuestJs._world.REACHABLE && QuestJs._world.ifNotDark(QuestJs._w[key])) {
...
QuestJs._scope.scopeReachable = scopeReachable;

//@DOC
// Returns an array of objects held by the given character.
function scopeHeldBy(chr, situation = QuestJs._world.PARSER) {
  return chr.getContents(situation)
}

QuestJs._scope.scopeHeldBy = scopeHeldBy;
...
function scopeHereListed() {
  const list = [];
  for (let key in QuestJs._w) {
    const o = QuestJs._w[key]
    if (!o.player && o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.LOOK) && QuestJs._world.ifNotDark(o)) {
      list.push(o);
    }
  }
  return list;
...
QuestJs._scope.scopeHereListed = scopeHereListed;
...
function scopeNpcHere(ignoreDark) {
  ...
  for (let key in QuestJs._w) {
    const o = QuestJs._w[key]
    if (o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.LOOK) && o.npc && (QuestJs._world.ifNotDark(o) || ignoreDark)) {
      list.push(o);
    }
  }
  return list;
...
function scopeAllNpcHere(ignoreDark) {
  ...
  for (let key in QuestJs._w) {
    const o = QuestJs._w[key]
    if (o.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) && o.npc && (QuestJs._world.ifNotDark(o) || ignoreDark)) {
      list.push(o);
    }
  }
  return list;
...
QuestJs._scope.scopeAllNpcHere = scopeAllNpcHere;
...
QuestJs._scope.scopeBy = scopeBy;
...
util.getContents = function(situation) {
  const list = [];
  for (let key in QuestJs._w) {
    if (QuestJs._w[key].isAtLoc(this.name, situation)) {
      list.push(QuestJs._w[key]);
    }
  }
  return list;
...
util.testForRecursion = function(char, item) {
  ...
  while (QuestJs._w[contName]) {
    if (QuestJs._w[contName].loc === item.name) return QuestJs._io.falsemsg(QuestJs._lang.container_recursion, {char:char, container:this, item:item})
    contName = QuestJs._w[contName].loc
  }
  return true;
}

util.nameModifierFunctionForContainer = function(o, list) {
  //QuestJs._log.info("here")
  const contents = o.getContents(QuestJs._world.LOOK);
  //QuestJs._log.info(contents)
  if (contents.length > 0 && (!o.closed || o.transparent)) {
    list.push(QuestJs._lang.contentsForData[o.contentsType].prefix + o.listContents(QuestJs._world.LOOK) + QuestJs._lang.contentsForData[o.contentsType].suffix)
  }
  //QuestJs._log.info(list)
}
...
util.addChangeListener = function(object, attName, func, test = util.defaultChangeListenerTest) {
  if (QuestJs._world.isCreated && !QuestJs._settings.saveDisabled) {
    QuestJs._io.errormsg("Attempting to use addChangeListener after set up.")
    return
  }
...
util.respond = function respond(params, list, func) {
  //QuestJs._log.info(params)
  ...
  if (!response) {
    if (func) func(params)
    QuestJs._io.errormsg("Failed to find a response")
    QuestJs._log.info(params)
    QuestJs._log.info(list)
    return false
  }
  //QuestJs._log.info(response)
  ...
  if (response.msg) {
    if (params.actor) {
      ...
      QuestJs._io.msg(response.msg, params)
    }
  }
  if (!response.script && !response.msg && !response.failed) {
    QuestJs._io.errormsg("No script or QuestJs._io.msg for response")
    QuestJs._log.info(response)
  }
...
util.getResponseList = function getResponseList(params, list, result) {
  if (!result) result = []
  for (let item of list) {
    if (item.name) {
      params.text = item.name.toLowerCase()
      //QuestJs._log.info("check item: " + item.name)
      if (item.test) {
...
util.verifyResponses = function(list, level) {
  ...
  if (list[list.length - 1].test) {
    QuestJs._log.info("WARNING: Last entry at depth " + level + " has a test condition:")
    QuestJs._log.info(list)
  }
  for (let item of list) {
    if (item.responses) {
      //QuestJs._log.info(item.name)
      if (item.responses.length === 0) {
        QuestJs._log.info("Zero responses at depth " + level + " for: " + item.name)
      }
      else {
...
util.listContents = function(situation, modified = true) {
  return formatList(this.getContents(situation), {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:modified, nothing:QuestJs._lang.list_nothing, loc:this.name})
}

util.niceDirection = function(dir) {
  const dirObj = QuestJs._lang.exit_list.find(function(el) { return el.name === dir })
...
util.reverseDirectionObj = function(dir) {
  return QuestJs._lang.exit_list.find(function(el) { return el.name === util.reverseDirection(dir) })
}

util.exitList = function(char = QuestJs._game.player) {
  const list = []
...
util.getDateTime = function(options) {
  if (!QuestJs._settings.dateTime.formats) {
    const time = new Date(QuestJs._game.elapsedTime * 1000 + QuestJs._game.startTime.getTime())
    return time.toLocaleString(QuestJs._settings.dateTime.locale, QuestJs._settings.dateTime)
  }
...
util.getDateTimeDict = function(options) {
  if (!options) options = {}
  return QuestJs._settings.dateTime.formats ? util.getCustomDateTimeDict(options) : util.getStdDateTimeDict(options)
}

util.getStdDateTimeDict = function(options) {
  const dict = {}
  let timeInSeconds = QuestJs._game.elapsedTime
  if (options.add) timeInSeconds += options.add
  const time = new Date(timeInSeconds * 1000 + QuestJs._game.startTime.getTime())
...
util.getCustomDateTimeDict = function(options) {
  const dict = {}
  let time = QuestJs._settings.dateTime.startTime + QuestJs._game.elapsedTime
  if (options.is) time = QuestJs._settings.dateTime.startTime + options.is
  if (options.add) time += options.add
  for (let el of QuestJs._settings.dateTime.data) {
...
util.getCustomDateTime = function(options) {
  ...
  let s = options.format ? QuestJs._settings.dateTime.formats[options.format] : QuestJs._settings.dateTime.formats.def
  for (let key in QuestJs._settings.dateTime.functions) {
    s = s.replace('%' + key + '%', QuestJs._settings.dateTime.functions[key](dict))
  }
  return s
}

util.seconds = function(seconds, minutes = 0, hours = 0, days = 0) {
  if (QuestJs._settings.dateTime.convertSeconds) return QuestJs._settings.dateTime.convertSeconds(seconds, minutes, hours, days)
  return ((((days * 24) + hours) * 60) + minutes) * 60 + seconds
}

util.elapsed = function(seconds, minutes = 0, hours = 0, days = 0) {
  return util.seconds(seconds, minutes, hours, days) >= QuestJs._game.elapsedTime
}

util.isAfter = function(timeString) {
  if (typeof timeString === 'number') return QuestJs._game.elapsedTime > timeString
  ...
  const nowTime = new Date(QuestJs._game.elapsedTime * 1000 + QuestJs._game.startTime.getTime())
  const targetTime = Date.parse(timeString)
  if (targetTime) return nowTime > targetTime
  return QuestJs._io.errormsg("Failed to parse date-time string: " + timeString)
}


util.changePOV = function(char, pronouns) {
  if (typeof char === 'string') {
    if (!QuestJs._w[char]) return QuestJs._io.errormsg("Failed to change POV, no object called '" + char + "'")
    char = QuestJs._w[char]
  }
  else if (!char) QuestJs._io.errormsg("Failed to change POV, char not defined.")

  if (QuestJs._game.player) {
    QuestJs._game.player.player = false
    QuestJs._game.player.pronouns = QuestJs._game.player.npcPronouns
    QuestJs._game.player.regex = new RegExp('^(' + (char.npcAlias ? char.npcAlias : char.alias) + ')$')
  }
  char.player = true
  char.npcPronouns = char.pronouns
  char.pronouns = pronouns ? pronouns : QuestJs._lang.pronouns.secondperson
  char.regex = new RegExp('^(me|myself|player|' + (char.npcAlias ? char.npcAlias : char.alias) + ')$')
  QuestJs._game.player = char
  QuestJs._game.update();
  if (QuestJs._w.background) QuestJs._world.setBackground();
}
...
util.defaultExitUse = function(char, dir, exit) {
  ...
  if (exit.isLocked()) {
    if (exit.lockedmsg) {
      QuestJs._io.msg(exit.lockedmsg)
    }
    else {
      QuestJs._io.msg(QuestJs._lang.locked_exit(char, exit))
    }
    return false
  }
  ...
  for (let el of char.onGoCheckList) {
    if (!QuestJs._w[el].onGoCheck(char, exit.name, dir)) return false
  }

  QuestJs._io.msg(QuestJs._lang.stop_posture(char))
  if (exit.msg) {
    printOrRun(char, exit, "QuestJs._io.msg")
  }
  else {
    QuestJs._io.msg(QuestJs._lang.go_successful, {char:char, dir:dir})
  }
  QuestJs._world.setRoom(char, exit.name, false)
  return true
}
...
util.useWithDoor = function(char, dir) {
  const obj = QuestJs._w[this.door];
  if (obj === undefined) {
    QuestJs._io.errormsg("Not found an object called '" + this.door + "'. Any exit that uses the 'util.useWithDoor' function must also set a 'door' attribute.");
  }
  const tpParams = {char:char, doorName:this.doorName ? this.doorName : "door"}
  if (!obj.closed) {
    QuestJs._world.setRoom(char, this.name, dir)
    return true
  }
  if (!obj.locked) {
    obj.closed = false
    QuestJs._io.msg(QuestJs._lang.open_and_enter, tpParams)
    QuestJs._world.setRoom(char, this.name, false)
    return true
  }
  if (obj.testKeys(char)) {
    obj.closed = false
    obj.locked = false
    QuestJs._io.msg(QuestJs._lang.unlock_and_enter, tpParams)
    QuestJs._world.setRoom(char, this.name, false)
    return true
  }        
  QuestJs._io.msg(QuestJs._lang.try_but_locked, tpParams)
  return false
}


// Helper function for exits.
// You can optionally set "QuestJs._io.msg" in the exit attributes
util.cannotUse = function(char, dir) {
  const tpParams = {char:char}
  QuestJs._io.msg(QuestJs._io.msg ? QuestJs._io.msg : QuestJs._lang.try_but_locked, tpParams)
  return false
}
