...
test.printTitle = function() {
  QuestJs._io.debugmsg(test.currentTitle + ": Error (test " + test.subCount + ")");
  test.failCount++;
};
    ...
    for (let i = 0; i < Math.max(test.testOutput.length, expected.length); i++) {
      if (typeof expected[i] === "string") {
        if (expected[i] !== test.testOutput[i]) {
          ...
          QuestJs._io.debugmsg("...Found (A): " + test.testOutput[i]);
          if (extraOutput) {
        ...
        if (test.testOutput[i] === undefined || !expected[i].test(test.testOutput[i])) {
          ...
          QuestJs._io.debugmsg("...Found: " + test.testOutput[i]);
        }
      }
...
test.fail = function(QuestJs._io.msg) {
  test.printTitle();
...
test.results = function(time) {
  ...
  QuestJs._io.debugmsg("Number of tests: " + test.totalCount);
  QuestJs._io.debugmsg("Number of fails: " + test.failCount);
  QuestJs._io.debugmsg("Elapsed time: " + elapsed + " ms (" + (Math.round(elapsed / test.totalCount * 10) / 10) + " ms/test)");
};
