...
QuestJs._test.runTests = function () {
  const time = parseInt(Date.now(), 10);
  QuestJs._test.tests();
...
QuestJs._test.assertCmd = function (cmdStr, expected, extraOutput) {
  ...
  if (
    ...
    QuestJs._test.testOutput.every((value, index) => {
    ...
    for (
      let i = 0;
      i < Math.max(QuestJs._test.testOutput.length, expected.length);
      i += 1
    ) {
            ...
            if (
              ...
              for (let j = 0; j < expected[i].length; j += 1) {
                if (expected[i][j] !== QuestJs._test.testOutput[i][j]) {
                  ...
                  QuestJs._log.info(
                    `Found: ${QuestJs._test.testOutput[i].charCodeAt(j)}`,
                  );
                }
              }
            } else {
