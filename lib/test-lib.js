...
QuestJs._test.printTitle = function() {
  QuestJs._io.debugmsg(QuestJs._test.currentTitle + ": Error (test " + QuestJs._test.subCount + ")");
  QuestJs._test.failCount++;
};

QuestJs._test.assertCmd = function(cmdStr, expected, extraOutput) {
  QuestJs._test.totalCount++;
  QuestJs._test.subCount++;
  ...
  if (QuestJs._test.testOutput.length === expected.length && QuestJs._test.testOutput.every(function(value, index) {
    ...
    for (let i = 0; i < Math.max(QuestJs._test.testOutput.length, expected.length); i++) {
      if (typeof expected[i] === "string") {
        if (expected[i] !== QuestJs._test.testOutput[i]) {
          ...
          if (extraOutput) {
            if (typeof expected[i] === "string" && typeof QuestJs._test.testOutput[i] === "string") {
        ...
        if (QuestJs._test.testOutput[i] === undefined || !expected[i].test(QuestJs._test.testOutput[i])) {
        ...
        QuestJs._io.debugmsg("Found an unrecognised type for expected (should be string or regex): " + (typeof expected[i]));
      }
    }
  }
};
...
QuestJs._test.fail = function(QuestJs._io.msg) {
  QuestJs._test.printTitle();
  QuestJs._io.debugmsg("Failure: " + QuestJs._io.msg);
};
...
QuestJs._test.results = function(time) {
  ...
  QuestJs._io.debugmsg("Elapsed time: " + elapsed + " ms (" + (Math.round(elapsed / QuestJs._test.totalCount * 10) / 10) + " ms/test)");
};
