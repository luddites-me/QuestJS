


const quest = {
  INITIAL:0,
  ACTIVE:1,
  MOOT:2,
  FAILED:3,
  SUCCESS:4,
  data:[],
  progressNames:['', 'Active', 'Moot', 'Failed', 'Success'],
}




quest.create = function(name, stages, data) {
  if (!data) data = {}
  data.name = name
  data.stages = stages
  data.key = name.replace(/ /g, '_').replace(/[^a-zA-Z0-9_]/g, '')
  quest.data.push(data)
}



quest.getState = function(name, char) {
  if (!char) char = QuestJs._game.player
  const result = {}
  result.quest = typeof name === 'string' ? quest.data.find(el => el.name === name) : name
  if (!result.quest) {
    QuestJs._log.error("Failed to find a quest called " + name)
    QuestJs._log.info('Giving up...')
  }
  result.stateName = 'quest_state_' + result.quest.key
  result.progressName = 'quest_progress_' + result.quest.key
  QuestJs._log.info(char)
  QuestJs._log.info(result.stateName)
  QuestJs._log.info(char[result.stateName])
  result.state = char[result.stateName]
  result.progress = char[result.progressName]
  QuestJs._log.info(result)
  return result
}

quest.comment = function(q, n, s) {
  metamsg (s + ": {i:" + q.name + "}")
  if (n !== false) metamsg (q.stages[n].text)
}

quest.start = function(name) {
  QuestJs._log.info(name)
  const result = quest.getState(name, QuestJs._game.player)
  if (result.progress !== undefined) return false // quest already started
  QuestJs._game.player[result.progressName] = quest.ACTIVE
  QuestJs._game.player[result.stateName] = 0
  quest.comment(result.quest, 0, "Quest started")
  return true
}

quest.restart = function(name, n) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress === quest.ACTIVE) return false // quest already started
  QuestJs._game.player[result.progressName] = quest.ACTIVE
  QuestJs._game.player[result.stateName] = n ? n : 0
  quest.comment(result.quest, 0, "Quest started")
  return true
}

quest.next = function(name) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress !== quest.ACTIVE) return false
  QuestJs._game.player[result.stateName]++
  if (quest.stages.length >= QuestJs._game.player[result.stateName]) return quest.complete(data.quest)
  quest.comment(result.quest, result.stateName, "Quest progress")
  return true
}

quest.complete = function(name) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress !== quest.ACTIVE) return false
  QuestJs._game.player[result.progressName] = quest.SUCCESS
  delete QuestJs._game.player[result.stateName]
  quest.comment(result.quest, false, "Quest completed")
  return true
}

quest.fail = function(name) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress !== quest.ACTIVE) return false
  QuestJs._game.player[result.progressName] = quest.FAILED
  delete QuestJs._game.player[result.stateName]
  quest.comment(result.quest, false, "Quest failed")
  return true
}

quest.moot = function(name) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress !== quest.ACTIVE) return false
  QuestJs._game.player[result.progressName] = quest.MOOT
  delete QuestJs._game.player[result.stateName]
  quest.comment(result.quest, false, "Quest moot")
  return true
}

quest.set = function(name, n) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress !== quest.ACTIVE) return false
  if (result.state <= n) return false
  QuestJs._game.player[result.stateName] = n
  quest.comment(result.quest, result.stateName, "Quest progress")
  return true
}





quest.progress = function(name, all) {
  const data = quest.getState(name, QuestJs._game.player)
  if (result.progress === undefined) return false
  if (result.progress !== quest.ACTIVE && all) return false
  QuestJs._io.metamsg(data.name + ', {i:' + quest.progressNames[result.progress] + '}')
  if (result.progress === quest.ACTIVE) QuestJs._io.metamsg(data.quest.stages[result.stateName].text) 
  return true
}










quest.list = function(all) {
  QuestJs._io.metamsg(all ? 'Active Quests' : 'All Quests')
  let flag = false
  for (let q of quest.data) {
    flag = flag || quest.getStatus(q, all)
  }
  if (!flag) metamsg ("None")
  if (!all) metamsg ("[Do QUESTS ALL to include completed and failed quests]")
}

QuestJs._commands.unshift(new QuestJs._command.Cmd('MetaQuests', {
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(?:quest|quests|q)$/,
  objects:[
  ],
  script:function(item) {
    quest.list(false)
  },
}))

QuestJs._commands.unshift(new QuestJs._command.Cmd('MetaQuestsAll', {
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(?:quest|quests|q) all$/,
  objects:[
  ],
  script:function(item) {
    quest.list(true)
  },
}))
  
  
