...
quest.restart = function (name, n) {
  ...
  QuestJs._game.player[result.progressName] = quest.ACTIVE;
  QuestJs._game.player[result.stateName] = n || 0;
  quest.comment(result.quest, 0, 'Quest started');
  return true;
};
...
quest.next = function (name) {
  ...
  QuestJs._game.player[result.stateName] += 1;
...
quest.progress = function (name, all) {
  ...
  QuestJs._io.metamsg(
    `${data.name}, {i:${quest.progressNames[result.progress]}}`,
  );
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('MetaQuests', {
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('MetaQuestsAll', {
    rules   : [QuestJs.cmdRules.isHere],
