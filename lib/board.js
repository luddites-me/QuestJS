...
board.setup = function (settings) {
  ...
  this.QuestJs._settings.cosAngle = Math.cos(
    (this.QuestJs._settings.angle * Math.PI) / 180,
  );
  this.QuestJs._settings.sinAngle = Math.sin(
    (this.QuestJs._settings.angle * Math.PI) / 180,
  );
  ...
  $(
    `<div id="map" style="position: fixed;bottom: 0px; left: 200px;width:${this.QuestJs._settings.width}px;height:${this.QuestJs._settings.height}px;">This will be the board...</div>`,
  ).insertAfter('#dialog');
...
board.update = function () {
  ...
  this.map.push('<defs>');
  this.map.push(
    '<linearGradient id="leftBaseGradient" gradientTransform="rotate(120)"><stop offset="0%"  stop-color="black" /><stop offset="30%" stop-color="white" /></linearGradient>',
  );
  this.map.push(
    '<linearGradient id="rightBaseGradient" gradientTransform="rotate(60)"><stop offset="45%"  stop-color="black" /><stop offset="80%" stop-color="white" /></linearGradient>',
  );
  ...
  for (let i = 0; i < this.QuestJs._settings.size; i += 1) {
    for (let j = 0; j < i; j += 1) {
      const x = j;
  ...
  for (
    let i = this.QuestJs._settings.size;
    i <= 2 * this.QuestJs._settings.size;
    i += 1
  ) {
    for (let j = 0; j < 2 * this.QuestJs._settings.size - i; j += 1) {
  ...
  if (this.QuestJs._settings.compassPane)
    this.map.push(
      board.getCompass(
        this.QuestJs._settings.compass.x,
        this.QuestJs._settings.compass.y,
      ),
    );
  if (this.QuestJs._settings.switches) this.map.push(board.getSwitches());
  if (this.QuestJs._settings.extras)
    this.map.push(this.QuestJs._settings.extras());
  $('#map').html(
    `<svg width="${this.QuestJs._settings.width}" height="${
      this.QuestJs._settings.height
    }" viewBox="0 0 ${this.QuestJs._settings.width} ${
      this.QuestJs._settings.height
    }" xmlns="http://www.w3.org/2000/svg">${this.map.join()}${this.labels.join()}</svg>`,
  );
  return true;
};
...
board.handleCell = function (x, y) {
  ...
  for (const el of this.QuestJs._settings.cellPoly) {
    s += `${x2 + el[0]},${y2 + el[1]} `;
  }
  s += '" ';
  s += board.QuestJs._settings.cellBorder
    ? board.QuestJs._settings.cellBorder(x, y)
    : 'stroke="none"';
  ...
  if (typeof this.QuestJs._settings.getLeftBorder === 'function') {
    ...
    if (leftBorder) {
      s += `<line x1="${x2 + this.QuestJs._settings.cellPoly[0][0]}" y1="${
        y2 + this.QuestJs._settings.cellPoly[0][1]
      }" `;
      s += `x2="${x2 + this.QuestJs._settings.cellPoly[3][0]}" y2="${
        y2 + this.QuestJs._settings.cellPoly[3][1]
      }" ${leftBorder}/>`;
    }
    const rightBorder = this.QuestJs._settings.getRightBorder(x, y);
    if (rightBorder) {
      s += `<line x1="${x2 + this.QuestJs._settings.cellPoly[2][0]}" y1="${
        y2 + this.QuestJs._settings.cellPoly[2][1]
      }" `;
      s += `x2="${x2 + this.QuestJs._settings.cellPoly[3][0]}" y2="${
        y2 + this.QuestJs._settings.cellPoly[3][1]
      }" ${rightBorder}/>`;
  ...
  for (const el of this.QuestJs._settings.getFeaturesAt(x, y)) {
    const feature = this.QuestJs._settings.features[el];
    if (feature === undefined) {
      QuestJs._log.info(
        `WARNING: Failed to find a feature called "${el}" when drawing board`,
      );
      continue;
    }
    if (feature.script) {
      ...
      const y3 =
        y2 - feature.height + 30.5 - 0.26 * this.QuestJs._settings.cellSize;
      s += `<image href="${QuestJs._settings.imagesFolder}${feature.flatFile}" `;
      s += `width="${Math.round(
        this.QuestJs._settings.cellSize * this.QuestJs._settings.rootTwo,
      )}"`;
      s += ` x="${x3}" y="${y3}" transform-origin="`;
      s += `${x3}px ${y3}px" transform="scale(1, `;
      s += `${
        this.QuestJs._settings.cosAngle / this.QuestJs._settings.sinAngle
      }) rotate(45)"/>`;
    }
  }
  this.map.push(s);
...
board.getTitle = function () {
  ...
  let s = `<text x="${x}" y="${y}`;
  if (this.QuestJs._settings.titleStyle)
    s += `" style="${this.QuestJs._settings.titleStyle}`;
...
board.getSwitches = function () {
  ...
  if (this.QuestJs._settings.switchesWidth) {
    ...
    s += `${
      this.QuestJs._settings.switches.length * 18 + 14
    }" fill="#eee" stroke="black"/>`;
  }
  let offset = 0;
  for (const el of this.QuestJs._settings.switches) {
    ...
    s += '"/>';
    s += `<text x="${x + 26}" y="${y + 20 + offset}" fill="black">${
      el.text
    }</text>`;
    offset += 20;
  }
...
board.getBase = function () {
  let s = '<polygon points="';
  s += `${
    this.QuestJs._settings.offsetX + this.QuestJs._settings.cellPoly[0][0]
  },`;
  s += `${this.QuestJs._settings.height / 2 + this.QuestJs._settings.offsetY} `;

  s += `${
    this.QuestJs._settings.offsetX +
    this.QuestJs._settings.sinAngle *
      this.QuestJs._settings.size *
      this.QuestJs._settings.cellSize +
    this.QuestJs._settings.cellPoly[0][0]
  },`;
  s += `${
    this.QuestJs._settings.height / 2 +
    this.QuestJs._settings.offsetY +
    this.QuestJs._settings.cosAngle *
      this.QuestJs._settings.size *
      this.QuestJs._settings.cellSize
  } `;
  ...
  s += `${
    ...
    this.QuestJs._settings.cosAngle *
      this.QuestJs._settings.size *
      this.QuestJs._settings.cellSize +
    this.QuestJs._settings.baseHeight
  } `;
