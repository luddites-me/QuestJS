...
board.setup = function (settings) {
  ...
  $(
    '<div id="map" style="position: fixed;bottom: 0px; left: 200px;width:' +
      this.QuestJs._settings.width +
      'px;height:' +
      this.QuestJs._settings.height +
      'px;">This will be the board...</div>',
  ).insertAfter('#dialog');
...
board.handleCell = function (x, y) {
  ...
  if (typeof this.QuestJs._settings.getLeftBorder === 'function') {
    ...
    if (leftBorder) {
      s +=
        '<line x1="' +
        (x2 + this.QuestJs._settings.cellPoly[0][0]) +
        '" y1="' +
        (y2 + this.QuestJs._settings.cellPoly[0][1]) +
        '" ';
      s +=
        'x2="' +
        (x2 + this.QuestJs._settings.cellPoly[3][0]) +
        '" y2="' +
    ...
    if (rightBorder) {
      s +=
        '<line x1="' +
        (x2 + this.QuestJs._settings.cellPoly[2][0]) +
        '" y1="' +
  ...
  for (let el of this.QuestJs._settings.getFeaturesAt(x, y)) {
    ...
    if (feature.script) {
      ...
      s +=
        'width="' +
        Math.round(this.QuestJs._settings.cellSize * this.QuestJs._settings.rootTwo) +
        '"';
...
board.getCompass = function (x, y) {
  let s =
    ...
    'compass45.png" width="160" height="159" x="';
  s += x + '" y="' + y;
  s +=
    '" transform="scale(1, ' +
...
board.getSwitches = function () {
  ...
  if (this.QuestJs._settings.switchesWidth) {
    s +=
      ...
      y +
      '" width="' +
      this.QuestJs._settings.switchesWidth +
      '" height="';
