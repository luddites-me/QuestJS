...
import { DEFAULT_ROOM } from './_defaults';
import { createItem, createRoom } from './_world';
import * as $ from 'jquery';
import { tp } from './_text';
...
export function getDisplayAlias(obj,art=INDEFINITE){
...
export function enterButtonPress(cmd){
...
export function clickedCmdLink(s){
	if (s) {
		if (io.savedCommands[io.savedCommands.length - 1] !== s) {
		  io.savedCommands.push(s);
		}
...
export function getObjectLink(obj,isScopeHere=false,addArticle=true){
...
export function handleObjLnkClick(cmd,el,verb,objAlias){
...
export function updateDropdownVerblists(){
	//parser.debug = true
...
export function switchDropdownVerb(el, newVerb, objName){
...
export function disableExistingObjectLinks(bool=false){
...
export function disableObjectLink(el){
	let objName = $(el).attr("obj")
