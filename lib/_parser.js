...
parser.parse = function(inputText) {
  ...
  if (inputText) {
    ...
    if (typeof res === "string") {
      ...
      QuestJs._world.endTurn(QuestJs._world.PARSER_FAILURE);
      return;
    }
    ...
    for (let j = 0; j < parser.currentCommand.objects[i].length; j++) {
      if (parser.currentCommand.objects[i][j] instanceof Array) {
        if (parser.currentCommand.objects[i][j].length === 1) {
          ...
          QuestJs._io.showMenuWithNumbers(QuestJs._lang.disambig_msg, parser.currentCommand.objects[i][j], function(result) {
...
//
// Used by askQuestion in QuestJs._IO.
...
parser.execute = function() {
  ...
  try {
    ...
    inEndTurnFlag = true
    QuestJs._world.endTurn(outcome)
  } catch (err) {
    if (inEndTurnFlag) {
      QuestJs._log.error("Hit a coding error trying to process world.endTurn after that command.")
    }
    else {
      QuestJs._log.error("Hit a coding error trying to process the command '" + parser.currentCommand.cmdString + "'.")
    }
    QuestJs._log.info('Look through the trace below to find the offending code. It is probably the first entry in the list, but may not be.')
    QuestJs._log.info(err)
...
parser.convertInputTextToCommandCandidate = function(inputText) {
  ...
  if (QuestJs._settings.convertNumbersInParser) {
  ...
  const candidates = QuestJs._commands.filter(function(el) {
    if (!Array.isArray(el.regexes)) QuestJs._log.info(el)  // it will crash in the next line!
  ...
  if (!command) QuestJs._log.info(inputText)
...
parser.matchItemsToCmd = function(s, cmd) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (cmd.objects[i].text) {
      ...
      if (!cmd.objects[i].scope) QuestJs._log.info("WARNING: Command without scope - " + cmd.name)
      let list = cmd.objects[i].scope ? parser.scope(cmd.objects[i].scope) : fallbackScope;
      let exclude = [QuestJs._game.player];
      ...
      if (!cmd.objects[i].scope) QuestJs._log.info("WARNING: No scope (or scope not found) in command " + cmd.name)
      let scopes = cmd.objects[i].scope ? [parser.scope(cmd.objects[i].scope), fallbackScope] : [fallbackScope];
      //QuestJs._log.info(scopes)
...
parser.isReachable = function(item) {
  return item.scopeStatus === QuestJs._world.REACHABLE && QuestJs._world.ifNotDark(item);
}
// Anywhere in the location (used by the parser for the fallback)
parser.isVisible = function(item) {
  return item.scopeStatus && QuestJs._world.ifNotDark(item);
}
...
parser.isPresentOrMe = function(item) {
  return parser.isHere(item) || parser.isHeld(item) || item === QuestJs._game.player;
}
// ... but not in a container
parser.isHeldNotWorn = function(item) {
  return item.isAtLoc(QuestJs._game.player.name, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item) && !item.getWorn();
}
parser.isHeld = function(item) {
  return item.isAtLoc(QuestJs._game.player.name, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item);
}
parser.isHeldByNpc = function(item) {
  const npcs = parser.scope(parser.isReachable).filter(el => el.npc);
  for (let npc of npcs) {
    if (item.isAtLoc(npc.name, QuestJs._world.PARSER)) return true;
  }
  return false;
}
...
parser.isWornByNpc = function(item) {
  const npcs = parser.scope(parser.isReachable).filter(el => el.npc);
  for (let npc of npcs) {
    if (item.isAtLoc(npc.name, QuestJs._world.PARSER) && item.getWorn()) return true;
  }
  return false;
}

parser.isNpcOrHere = function(item) {
  return (item.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item)) || item.npc || item.player;
}
parser.isNpcAndHere = function(item) {
  return item.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) && (item.npc || item.player);
}
parser.isHere = function(item) {
  return item.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item);
}

parser.isForSale = function(item) {
  return item.isForSale && item.isForSale(QuestJs._game.player.loc) && QuestJs._world.ifNotDark(item);
}

//parser.isInside = function(item, options) {
//  return item.isAtLoc(options.container.name, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item);
//}
...
parser.isContained = function(item) {
  ...
  for (let container of containers) {
    ...
    if (item.isAtLoc(container.name, QuestJs._world.PARSER)) return true;
  }
  return false;
}
