...
parser.parse = function (inputText) {
  ...
  let flag = false;
  for (let i = 0; i < parser.currentCommand.objects.length; i += 1) {
    for (let j = 0; j < parser.currentCommand.objects[i].length; j += 1) {
      if (parser.currentCommand.objects[i][j] instanceof Array) {
        if (parser.currentCommand.objects[i][j].length === 1) {
          ...
          QuestJs._io.showMenuWithNumbers(
            QuestJs._lang.disambig_msg,
            parser.currentCommand.objects[i][j],
            function (result) {
              parser.currentCommand.objects[parser.currentCommand.disambiguate1][
                parser.currentCommand.disambiguate2
              ] = result;
              parser.parse(null);
            },
          );
...
parser.execute = function () {
  ...
  try {
    if (
      parser.currentCommand.objects.length > 0 &&
    ...
    if (inEndTurnFlag) {
      ...
      QuestJs._log.error(
        "Hit a coding error trying to process the command '" +
          parser.currentCommand.cmdString +
          "'.",
      );
    }
    QuestJs._log.info(
...
parser.matchItemsToCmd = function (s, cmd) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (cmd.objects[i].text) {
      ...
      QuestJs._lang.all_regex.test(arr[i]) ||
      QuestJs._lang.all_exclude_regex.test(arr[i])
    ) {
      ...
      if (QuestJs._lang.all_exclude_regex.test(arr[i])) {
        ...
        objectNames = s.split(joiner_regex).map(function (el) {
          return el.trim();
        });
      ...
      objectNames = arr[i].split(QuestJs._lang.joiner_regex).map(function (el) {
        return el.trim();
      });
      ...
      if (!cmd.objects[i].scope)
        QuestJs._log.info('WARNING: No scope (or scope not found) in command ' + cmd.name);
      ...
      for (let s of objectNames) {
        ...
        if (objNameMatch === null) {
          QuestJs._io.errormsg(
            "Failed to match to article_filter_regex with '" +
              s +
...
parser.findInScope = function (s, listOfLists, cmdParams) {
  ...
  for (let key in QuestJs._lang.pronouns) {
    if (
      s === QuestJs._lang.pronouns[key].objective &&
      parser.pronouns[QuestJs._lang.pronouns[key].objective]
    ) {
...
parser.scoreObjectMatch = function (s, item, cmdParams) {
  ...
  if (cmdParams.items && cmdParams.items.includes(item.name)) {
    ...
    item.parserItemNameParts.some(function (el) {
      return el === s;
    })
  ) {
    ...
    item.parserAltNames &&
    item.parserAltNames.some(function (el) {
...
parser.isHeldNotWorn = function (item) {
  return (
    item.isAtLoc(QuestJs._game.player.name, QuestJs._world.PARSER) &&
    QuestJs._world.ifNotDark(item) &&
    !item.getWorn()
  );
};
parser.isHeld = function (item) {
  return (
    item.isAtLoc(QuestJs._game.player.name, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item)
  );
};
...
parser.isNpcOrHere = function (item) {
  return (
    (item.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) &&
      QuestJs._world.ifNotDark(item)) ||
    item.npc ||
    item.player
  );
...
parser.isHere = function (item) {
  return (
    item.isAtLoc(QuestJs._game.player.loc, QuestJs._world.PARSER) && QuestJs._world.ifNotDark(item)
  );
};

parser.isForSale = function (item) {
  return (
    item.isForSale && item.isForSale(QuestJs._game.player.loc) && QuestJs._world.ifNotDark(item)
  );
};
