          ...
          QuestJs._io.showMenuWithNumbers(
            ...
            (result) => {
              parser.currentCommand.objects[
                parser.currentCommand.disambiguate1
              ][parser.currentCommand.disambiguate2] = result;
              parser.parse(null);
            },
          );
        }
...
parser.matchItemsToCmd = function (s, cmd) {
  const res = {
    cmd, objectTexts: [], objects: [], matches: [],
  };
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (cmd.objects[i].text) {
      ...
      let list = cmd.objects[i].scope
        ? parser.scope(cmd.objects[i].scope)
        : fallbackScope;
      ...
      objectNames = arr[i]
        .split(QuestJs._lang.joiner_regex)
        .map((el) => el.trim());
...
parser.isReachable = function (item) {
  return (
    item.scopeStatus === QuestJs._world.REACHABLE
...
parser.isPresentOrMe = function (item) {
  return (
    parser.isHere(item) || parser.isHeld(item) || item === QuestJs._game.player
  );
};
...
parser.isForSale = function (item) {
  return (
    item.isForSale
    && item.isForSale(QuestJs._game.player.loc)
    && QuestJs._world.ifNotDark(item)
  );
};
