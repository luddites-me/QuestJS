...
parser.matchItemsToCmd = function (s, cmd) {
  const res = { cmd, objectTexts: [], objects: [], matches: [] };
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (cmd.objects[i].text) {
      ...
      const exclude = [QuestJs._game.player];
      ...
      if (QuestJs._lang.all_exclude_regex.test(arr[i])) {
        ...
        objectNames = s.split(joiner_regex).map((el) => el.trim());
      ...
      objectNames = arr[i].split(QuestJs._lang.joiner_regex).map((el) => el.trim());
...
parser.findInScope = function (s, listOfLists, cmdParams) {
  ...
  // First handle IT etc.
  for (const key in QuestJs._lang.pronouns) {
    if (
...
parser.msg = function (...ary) {
  if (parser.debug) {
    for (const s of ary) QuestJs._io.debugmsg(`PARSER&gt; ${s}`);
  }
};
