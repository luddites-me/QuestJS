...
const parser = {};
      ...
      for (let j = 0; j < parser.currentCommand.objects[i].length; j++) {
        if (parser.currentCommand.objects[i][j] instanceof Array) {
          if (parser.currentCommand.objects[i][j].length === 1) {
            ...
            showMenuWithNumbers(QuestJs._lang.disambig_msg, parser.currentCommand.objects[i][j], function(result) {
  ...
  parser.quickCmd = function(cmd, item) {
    parser.QuestJs._io.msg("quickCmd: " + cmd.name)
    if (item) parser.QuestJs._io.msg("quickCmd: " + item.name)
    parser.currentCommand = {
  ...
  parser.execute = function() {
    ...
    try {
      ...
      console.log(err)
      io.print({tag:'p', cssClass:"error", text:QuestJs._lang.error})
    }
  };   
  ...
  parser.convertInputTextToCommandCandidate = function(inputText) {
    ...
    if (settings.convertNumbersInParser) {
      cmdString = QuestJs._lang.convertNumbers(cmdString);
    }
    
    // Get a list of candidate commands that match the regex
    const candidates = QuestJs._commands.filter(function(el) {
    ...
    if (candidates.length === 0) {
      return QuestJs._lang.not_known_msg;
    }
    parser.QuestJs._io.msg("Number of commands that have a regex match:" + candidates.length)
    ...
    let error = QuestJs._lang.general_obj_error;
    ...
    candidates.forEach(function(el) {
      ...
      parser.QuestJs._io.msg("* Looking at candidate: " + el.name);
      const res = parser.matchItemsToCmd(cmdString, el);
      if (!res) {
        parser.QuestJs._io.msg("No result!");
        error = "Res is " + res;
      }
      parser.QuestJs._io.msg("Result score is: " + res.score);
      if (res.score === -1) {
        error = res.error;
      }
      else {
        parser.QuestJs._io.msg("Candidate accepted!");
        matchedCandidates.push(res);
      }
    });
    parser.QuestJs._io.msg("Number of candidates accepted: " + matchedCandidates.length);
    if (matchedCandidates.length === 0) {
      parser.QuestJs._io.msg("No matches, returning error: " + error);
      return error;
    }
    // pick between matchedCandidates based on score
    let command = matchedCandidates[0];
    if (matchedCandidates.length > 1) {
      parser.QuestJs._io.msg("Need to pick just one; start with the first (score " + command.score + ").");
      for (let candidate of matchedCandidates) {
        // give preference to earlier commands
        if (command.score < candidate.score) {
          parser.QuestJs._io.msg("This one is better:" + command.cmd.name + " (score " + candidate.score + ")");
          command = candidate;
        }
      }
    }
    ...
    parser.QuestJs._io.msg("This is the one:" + command.cmd.name);
    return command;
  };
  ...
  parser.matchItemsToCmd = function(s, cmd) {
    ...
    parser.QuestJs._io.msg("..Base score: " + res.score);

    for (let i = 0; i < arr.length; i++) {
      if (!cmd.objects[i]) {
        QuestJs._io.errormsg("That command seems to have an error. It has more capture groups than there are elements in the 'objects' attribute.");
        return false;
      }
      ...
      if (cmd.objects[i].text) {
      ...
      else if (QuestJs._lang.all_regex.test(arr[i]) || QuestJs._lang.all_exclude_regex.test(arr[i])) {
        ...
        if (QuestJs._lang.all_exclude_regex.test(arr[i])) {
        ...
        if (list.length > 1 && !cmd.objects[i].multiple) {
          res.error = QuestJs._lang.no_multiples_msg;
          res.score = -1;
          return res;
        }
        if (list.length === 0) {
          res.error = cmd.nothingForAll ? cmd.nothingForAll : QuestJs._lang.nothing_msg;
          res.score = -1;
          return res;
        }
        score = 2;
        ...
        objectNames = arr[i].split(QuestJs._lang.joiner_regex).map(function(el){ return el.trim() });
        ...
        for (let s of objectNames) {
          const objNameMatch = QuestJs._lang.article_filter_regex.exec(s);
          if (objNameMatch === null) {
            QuestJs._io.errormsg("Failed to match to article_filter_regex with '" + s + "', - probably an error in article_filter_regex!");
            return null;
          }
      ...
      parser.QuestJs._io.msg("...Adding to the score: " + score);
      res.score += score;
    }
    return res;
  };
  ...
  parser.findInScope = function(s, listOfLists, cmdParams) {
    parser.QuestJs._io.msg("Now matching: " + s)
    // First handle IT etc.
    for (let key in QuestJs._lang.pronouns) {
      if (s === QuestJs._lang.pronouns[key].objective && parser.pronouns[QuestJs._lang.pronouns[key].objective]) {
        return [parser.pronouns[QuestJs._lang.pronouns[key].objective], 1];
      }
    }
        
    for (let i = 0; i < listOfLists.length; i++) {
      parser.QuestJs._io.msg("..Looking for a match for: " + s + " (scope " + (i + 1) + ")")
  ...
  parser.findInList = function(s, list, cmdParams) {
    ...
    let n;
    for (let item of list) {
      parser.QuestJs._io.msg("-> Considering: " + item.name)
      n = this.scoreObjectMatch(s, item, cmdParams);
      if (n >=0 ) parser.QuestJs._io.msg(item.name + " scores " + n)
      if (n > score) {
        res = [];
        score = n;
      }
    ...
    parser.QuestJs._io.msg(res.length > 1 ? "Cannot decide between: " + res.map(el => el.name).join(", ") : (res.length === 1 ? "..Going with: " + res[0].name : "Found no suitable objects"))
    return res;
  };
  ...
  parser.scoreObjectMatch = function(s, item, cmdParams) {
    ...
    if (cmdParams.items && cmdParams.items.includes(item.name)) {
      ...
      parser.QuestJs._io.msg('The command specifically mentions this item, so highest priority, score 100')
      res = 100;
    }
    else if (s === item.parserItemName) {
      parser.QuestJs._io.msg('The player has used the exact alias, score 60')
      res = 60;
    }
    else if (item.regex && item.regex.test(s)) {
      parser.QuestJs._io.msg('The player has used the exact string allowed in the regex, score 60')
      parser.QuestJs._io.msg('' + item.regex)
      parser.QuestJs._io.msg('>' + s + '<')
      res = 55;
    }
    else if (item.parserItemNameParts && item.parserItemNameParts.some(function(el) { return el === s})) {
      parser.QuestJs._io.msg('The player has matched a complete word, but not the full phrase, score 50')
      res = 50;
    }
    else if (item.parserItemName.startsWith(s)) {
      parser.QuestJs._io.msg('the player has used a string that matches the start of the alias, score length + 15')
      res = s.length + 15;
    }
    else if (item.parserAltNames && item.parserAltNames.some(function(el) { return el.startsWith(s)})) {
      parser.QuestJs._io.msg('the player has used a string that matches the start of an alt name, score length + 10')
      res = s.length + 10;
    }
    else if (item.parserItemNameParts && item.parserItemNameParts.some(function(el) { return el.startsWith(s)})) {
      parser.QuestJs._io.msg('the player has used a string that matches the start of an alt name, score length')
      res = s.length;
    }
    else {
      return -1;
    ...
    if (item[cmdParams.attName]) {
      parser.QuestJs._io.msg('bonus 20 as item has attribute ' + cmdParams.attName)
      res += 20;
    }
    if (item.parsePriority) {
      parser.QuestJs._io.msg('item.parsePriority is ' + item.parsePriority)
  ...
  parser.QuestJs._io.msg = function(...ary) {
    if (parser.debug) {
      for (let s of ary) QuestJs._io.debugmsg("PARSER&gt; " + s)
    }
  }
