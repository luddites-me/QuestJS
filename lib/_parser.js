...
const parser = {};
      ...
      for (let j = 0; j < parser.currentCommand.objects[i].length; j++) {
        if (parser.currentCommand.objects[i][j] instanceof Array) {
          if (parser.currentCommand.objects[i][j].length === 1) {
            ...
            showMenuWithNumbers(QuestJs.LANG.disambig_msg, parser.currentCommand.objects[i][j], function(result) {
  ...
  parser.convertInputTextToCommandCandidate = function(inputText) {
    ...
    if (settings.convertNumbersInParser) {
      cmdString = QuestJs.LANG.convertNumbers(cmdString);
    }
    ...
    if (candidates.length === 0) {
      return QuestJs.LANG.not_known_msg;
    }
    ...
    let error = QuestJs.LANG.general_obj_error;
  ...
  parser.matchItemsToCmd = function(s, cmd) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (cmd.objects[i].text) {
      ...
      else if (QuestJs.LANG.all_regex.test(arr[i]) || QuestJs.LANG.all_exclude_regex.test(arr[i])) {
        ...
        if (QuestJs.LANG.all_exclude_regex.test(arr[i])) {
        ...
        if (list.length > 1 && !cmd.objects[i].multiple) {
          res.error = QuestJs.LANG.no_multiples_msg;
          res.score = -1;
          return res;
        }
        if (list.length === 0) {
          res.error = cmd.nothingForAll ? cmd.nothingForAll : QuestJs.LANG.nothing_msg;
          res.score = -1;
          return res;
        }
        score = 2;
        ...
        objectNames = arr[i].split(QuestJs.LANG.joiner_regex).map(function(el){ return el.trim() });
        ...
        for (let s of objectNames) {
          const objNameMatch = QuestJs.LANG.article_filter_regex.exec(s);
  ...
  parser.findInScope = function(s, listOfLists, cmdParams) {
    ...
    // First handle IT etc.
    for (let key in QuestJs.LANG.pronouns) {
      if (s === QuestJs.LANG.pronouns[key].objective && parser.pronouns[QuestJs.LANG.pronouns[key].objective]) {
        return [parser.pronouns[QuestJs.LANG.pronouns[key].objective], 1];
      }
    }
