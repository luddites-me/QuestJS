import {
  en_grammar as Grammar, en_regex as Regex, en_lang as Language, en_responses as Responses,
...
function Cmd(name, hash) {
  ...
  this.noobjecterror = function (s) {
    return Language.object_unknown_msg(s);
  };
  ...
  if (!this.regex && !this.regexes) {
    this.regexes = Array.isArray(Regex[this.name])
      ? Regex[this.name]
      : [Regex[this.name]];
  }
}
...
function NpcCmd(name, hash) {
  ...
  this.script = function (objects) {
    ...
    if (!npc.npc) {
      QuestJs._io.failedmsg(Responses.not_npc, {
...
function ExitCmd(name, dir, hash) {
  ...
  (this.script = function (objects) {
    if (!QuestJs._game.room.hasExit(this.dir)) {
      QuestJs._io.failedmsg(Responses.not_that_way, {
        char : QuestJs._game.player,
        dir  : this.dir,
      });
      return QuestJs._world.FAILED;
...
function initCommands() {
  ...
  for (const el of QuestJs._commands) {
    ...
    if (el.npcCmd) {
      ...
      cmd.regexes = [];
      for (const key in Responses.tell_to_prefixes) {
        cmd.regexes.push(
          new RegExp(`^${Responses.tell_to_prefixes[key]}${regexAsStr}`),
        );
      }
  ...
  for (const el of Grammar.exit_list) {
    if (el.type !== 'nocmd') {
      let regex = `(${Grammar.go_pre_regex})(${
        el.name
      ...
      for (const key in Responses.tell_to_prefixes) {
        regexes.push(
          new RegExp(`^${Responses.tell_to_prefixes[key]}${regex}`),
        );
      }
      QuestJs._commands.push(
        new NpcExitCmd(
...
cmdRules.isHeldNotWorn = function (cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, QuestJs._world.PARSER)) {
    return QuestJs._io.falsemsg(
      QuestJs._tools.prefix(item, isMultiple) + Responses.already_wearing,
      { char, garment: item },
    );
  }
  ...
  if (item.loc) {
    ...
    if (holder.npc || holder.player) {
      return QuestJs._io.falsemsg(
        QuestJs._tools.prefix(item, isMultiple) + Responses.char_has_it,
        { holder, item },
      );
    }
  }
  ...
  return QuestJs._io.falsemsg(
    QuestJs._tools.prefix(item, isMultiple) + Responses.not_carrying,
    { char, item },
  );
};
...
cmdRules.isWorn = function (cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, QuestJs._world.PARSER)) {
    return QuestJs._io.falsemsg(
      QuestJs._tools.prefix(item, isMultiple) + Responses.not_wearing,
      { char, item },
    );
  }
...
cmdRules.isHere = function (cmd, char, item, isMultiple) {
  ...
  return QuestJs._io.falsemsg(
    QuestJs._tools.prefix(item, isMultiple) + Responses.not_here,
    {
      char,
      item,
    },
...
cmdRules.isHereNotHeld = function (cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    ...
    if (already && holder === QuestJs._game.player) {
      return QuestJs._io.falsemsg(
        QuestJs._tools.prefix(item, isMultiple) + Responses.already_have,
        { char: holder, item },
      );
    }
    if (holder.npc || holder.player) {
...
cmdRules.canTalkTo = function (cmd, char, item, isMultiple) {
  if (!char.canTalk(item)) return false;
  if (!item.npc) {
    return QuestJs._io.falsemsg(
      QuestJs._tools.prefix(item, isMultiple) + Responses.not_able_to_hear,
      { char, item },
    );
  }
  return true;
