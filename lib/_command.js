...
function ExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    if (!game.room.hasExit(this.dir)) {
      QuestJs._io.failedmsg(QuestJs._lang.not_that_way, {char:game.player, dir:this.dir});
      return world.FAILED;
    }
    else {
...
function extractChar(cmd, objects) {
  ...
  if (cmd.forNpc) {
    char = objects[0][0];
    if (!char.npc) {
      QuestJs._io.failedmsg(QuestJs._lang.not_npc, {char:game.player, item:char});
      return world.FAILED; 
    }
    objects.shift();
  }
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.already_wearing, {char:char, garment:item})
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_wearing, {char:char, item:item})
...
cmdRules.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    const holder = w[item.loc]
    if (already && holder === game.player) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
