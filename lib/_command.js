...
function Cmd(name, hash) {
  ...
  this.script = function(objects, matches) {
    ...
    if (objects[0].length === 0) {
      metamsg(QuestJs._lang.nothing_msg)
      return world.FAILED; 
    }
...
function ExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    if (!game.room.hasExit(this.dir)) {
      failedmsg(QuestJs._lang.not_that_way, {char:game.player, dir:this.dir});
      return world.FAILED;
    }
    else {
...
function NpcExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    const npc = objects[0][0];
    if (!game.room.hasExit(this.dir)) {
      failedmsg(QuestJs._lang.not_that_way, {char:npc, dir:this.dir});
      return world.FAILED;
    }
...
function extractChar(cmd, objects) {
  ...
  if (cmd.forNpc) {
    char = objects[0][0];
    if (!char.npc) {
      failedmsg(QuestJs._lang.not_npc, {char:game.player, item:char});
      return world.FAILED; 
    }
    objects.shift();
  }
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.already_wearing, {char:char, garment:item})

  if (item.loc) {
    const holder = w[item.loc];
    if (holder.npc || holder.player) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
    
  return falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_carrying, {char:char, item:item})
}
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_wearing, {char:char, item:item})
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  ...
  return falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_here, {char:char, item:item})
}
...
cmdRules.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    const holder = w[item.loc]
    if (already && holder === game.player) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
...
cmdRules.canTalkTo = function(cmd, char, item) {
  ...
  if (!item.npc) return falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_able_to_hear, {char:char, item:item})
  return true
}
