...
function Cmd(name, hash) {
  ...
  this.script = function(objects, matches) {
    ...
    if (objects[0].length === 0) {
      metamsg(QuestJs.LANG.nothing_msg)
      return world.FAILED; 
    }
  ...
  this.noobjecterror = function(s) {
    return QuestJs.LANG.object_unknown_msg(s)
  }
  ...
  if (!this.regex && !this.regexes) {
    this.regexes = Array.isArray(QuestJs.LANG.regex[this.name]) ? QuestJs.LANG.regex[this.name] : [QuestJs.LANG.regex[this.name]]
  }
}

QuestJs.command.Cmd = Cmd;
...
function ExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    if (!game.room.hasExit(this.dir)) {
      failedmsg(QuestJs.LANG.not_that_way, {char:game.player, dir:this.dir});
      return world.FAILED;
    }
    else {
...
QuestJs.command.ExitCmd = ExitCmd;
...
function NpcExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    const npc = objects[0][0];
    if (!game.room.hasExit(this.dir)) {
      failedmsg(QuestJs.LANG.not_that_way, {char:npc, dir:this.dir});
      return world.FAILED;
    }
...
QuestJs.command.NpcExitCmd = ExitCmd;
...
function initCommands() {
  ...
  for (let el of commands) {
    ...
    if (el.npcCmd) {
      ...
      cmd.regexes = []
      for (let key in QuestJs.LANG.tell_to_prefixes) {
        cmd.regexes.push(new RegExp("^" + QuestJs.LANG.tell_to_prefixes[key] + regexAsStr))
      }
  ...
  for (let el of QuestJs.LANG.exit_list) {
    if (el.type !== 'nocmd') {
      let regex = "(" + QuestJs.LANG.go_pre_regex + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      ...
      const regexes = []
      for (let key in QuestJs.LANG.tell_to_prefixes) {
        regexes.push(new RegExp("^" + QuestJs.LANG.tell_to_prefixes[key] + regex))
      }
...
QuestJs.command.initCommands = initCommands;
...
function extractChar(cmd, objects) {
  ...
  if (cmd.forNpc) {
    char = objects[0][0];
    if (!char.npc) {
      failedmsg(QuestJs.LANG.not_npc, {char:game.player, item:char});
      return world.FAILED; 
    }
    objects.shift();
  }
...
QuestJs.command.extractChar = extractChar;

function findCmd(name) {
  return commands.find(el => el.name === name)
}

QuestJs.command.findCmd = findCmd;
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.already_wearing, {char:char, garment:item})

  if (item.loc) {
    const holder = w[item.loc];
    if (holder.npc || holder.player) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.char_has_it, {holder:holder, item:item})
  }
    
  return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.not_carrying, {char:char, item:item})
}
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.not_wearing, {char:char, item:item})
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  ...
  return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.not_here, {char:char, item:item})
}
...
cmdRules.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    const holder = w[item.loc]
    if (already && holder === game.player) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.char_has_it, {holder:holder, item:item})
  }
...
cmdRules.canTalkTo = function(cmd, char, item) {
  ...
  if (!item.npc) return falsemsg(prefix(item, isMultiple) + QuestJs.LANG.not_able_to_hear, {char:char, item:item})
  return true
}
