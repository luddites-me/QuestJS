...
function Cmd(name, hash) {
  ...
  this.default = function(item, isMultiple, char) { 
    if (typeof this.defmsg === "string") {
      QuestJs._io.failedmsg(QuestJs._tools.prefix(item, isMultiple) + this.defmsg, {char:char, item:item});
    }
    else if (typeof this.defmsg === "function") {
      QuestJs._io.failedmsg(QuestJs._tools.prefix(item, isMultiple) + this.defmsg(char, item), {char:char, item:item});
    }
    else {
  ...
  this.script = function(objects, matches) {
    ...
    const multi = objects[0] && (objects[0].length > 1 || QuestJs._parser.currentCommand.all)
    ...
    for (let i = 0; i < objects[0].length; i++) {
      if (!objects[0][i][this.attName]) {
        ...
        let result = this.processCommand(QuestJs._game.player, objects[0][i], multi, matches[0][i], verb);
    ...
    if (success) {
      return (this.noTurnscripts || suppressEndturn ? QuestJs._world.SUCCESS_NO_TURNSCRIPTS : QuestJs._world.SUCCESS);
    }
    else {
      return QuestJs._world.FAILED; 
    }
  ...
  this.processCommand = function(char, item, multi, match, verb) {
    for (let rule of this.rules) {
      if (typeof rule !== "function") {
        QuestJs._io.errormsg("Failed to process command '" + this.name + "' as one of its rules is not a function (F12 for more).");
        QuestJs._log.info ("Failed to process command '" + this.name + "' as one of its rules is not a function:")
    ...
    let result = QuestJs._tools.printOrRun(char, item, this.attName, {multi:multi, match:match, verb:verb});
    if (typeof result !== "boolean" && result !== QuestJs._world.SUCCESS_NO_TURNSCRIPTS) {
  ...
  if (!this.regex && !this.regexes) {
    this.regexes = Array.isArray(QuestJs._lang.regex[this.name]) ? QuestJs._lang.regex[this.name] : [QuestJs._lang.regex[this.name]]
  }
}
...
function NpcCmd(name, hash) {
  ...
  this.script = function(objects) {
    ...
    if (!npc.npc) {
      QuestJs._io.failedmsg(QuestJs._lang.not_npc, {char:QuestJs._game.player, item:npc});
      return QuestJs._world.FAILED; 
    }
    let success = false, handled;
    if (objects.length !== 2) {
      QuestJs._io.errormsg("The command " + name + " is trying to use a facility for NPCs to do it, but there is no object list; this facility is only for commands in the form verb-object.");
      return QuestJs._world.FAILED;
    }
    ...
    if (success) {
      npc.pause();
      return (this.noTurnscripts ? QuestJs._world.SUCCESS_NO_TURNSCRIPTS : QuestJs._world.SUCCESS);
    }
    else {
      return QuestJs._world.FAILED; 
    }
...
function ExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    if (!QuestJs._game.room.hasExit(this.dir)) {
      QuestJs._io.failedmsg(QuestJs._lang.not_that_way, {char:QuestJs._game.player, dir:this.dir});
      return QuestJs._world.FAILED;
    }
    else {
      ...
      if (typeof ex === "object"){
        ...
        if (typeof ex.use !== 'function') {
          QuestJs._io.errormsg("Exit's 'use' attribute is not a function (or does not exist). Press F12 for more.");
        ...
        if (typeof flag !== "boolean") {
          QuestJs._io.errormsg("Exit failed to return a Boolean value, indicating success of failure; assuming success");
...
function NpcExitCmd(name, dir, hash) {
  ...
  this.exitCmd = true;
  this.dir = dir;
  this.objects = [{scope:QuestJs._parser.isHere, attName:"npc"}, {ignore:true}, {ignore:true}, ],
  this.script = function(objects) {
    const npc = objects[0][0];
    if (!QuestJs._game.room.hasExit(this.dir)) {
      QuestJs._io.failedmsg(QuestJs._lang.not_that_way, {char:npc, dir:this.dir});
      return QuestJs._world.FAILED;
    }
...
function initCommands() {
  const newCmds = [];
  for (let el of QuestJs._commands) {
    if (el.regex) {
      el.regexes = [el.regex]
    }
    if (el.npcCmd) {
      ...
      cmd.regexes = []
      for (let key in QuestJs._lang.tell_to_prefixes) {
        cmd.regexes.push(new RegExp("^" + QuestJs._lang.tell_to_prefixes[key] + regexAsStr))
      }
      if (el.useThisScriptForNpcs) cmd.script = el.script
      cmd.scope = []
      for (let el2 of el.objects) {
        cmd.scope.push(el2 === QuestJs._parser.isHeld ? QuestJs._parser.isHeldByNpc : el2)
        cmd.scope.push(el2 === QuestJs._parser.isWorn ? QuestJs._parser.isWornByNpc : el2)
      }
      newCmds.push(cmd)
    }
  }
  ...
  for (let el of QuestJs._lang.exit_list) {
    if (el.type !== 'nocmd') {
      let regex = "(" + QuestJs._lang.go_pre_regex + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      if (el.alt) { regex += "|" + el.alt; }
      regex += ")$";
      QuestJs._commands.push(new ExitCmd("Go" + QuestJs._tools.sentenceCase(el.name), el.name, { regexes:[new RegExp("^" + regex)] }));
      
      const regexes = []
      for (let key in QuestJs._lang.tell_to_prefixes) {
        regexes.push(new RegExp("^" + QuestJs._lang.tell_to_prefixes[key] + regex))
      }
      QuestJs._commands.push(new NpcExitCmd("NpcGo" + QuestJs._tools.sentenceCase(el.name) + "2", el.name, { regexes:regexes }))
    }
  }
}
...
function extractChar(cmd, objects) {
  ...
  if (cmd.forNpc) {
    char = objects[0][0];
    if (!char.npc) {
      QuestJs._io.failedmsg(QuestJs._lang.not_npc, {char:QuestJs._game.player, item:char});
      return QuestJs._world.FAILED; 
    }
    objects.shift();
  }
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  if (!item.getWorn() && item.isAtLoc(char.name, QuestJs._world.PARSER)) return true

  if (item.isAtLoc(char.name, QuestJs._world.PARSER)) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.already_wearing, {char:char, garment:item})

  if (item.loc) {
    const holder = QuestJs._w[item.loc];
    if (holder.npc || holder.player) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
    
  return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_carrying, {char:char, item:item})
}
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, QuestJs._world.PARSER)) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_wearing, {char:char, item:item})
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  ...
  return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_here, {char:char, item:item})
}
...
cmdRules.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    const holder = QuestJs._w[item.loc]
    if (already && holder === QuestJs._game.player) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
  
  if (item.scopeStatus === QuestJs._world.REACHABLE || item.multiLoc) return true
  return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_here, {char:char, item:item})
}
...
cmdRules.canTalkTo = function(cmd, char, item) {
  ...
  if (!item.npc) return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_able_to_hear, {char:char, item:item})
  return true
}
