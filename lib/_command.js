...
function Cmd(name, hash) {
  ...
  this.default = function(item, isMultiple, char) { 
    if (typeof this.defmsg === "string") {
      QuestJs._io.failedmsg(prefix(item, isMultiple) + this.defmsg, {char:char, item:item});
    }
    else if (typeof this.defmsg === "function") {
      QuestJs._io.failedmsg(prefix(item, isMultiple) + this.defmsg(char, item), {char:char, item:item});
    }
    else {
...
function initCommands() {
  ...
  for (let el of QuestJs._lang.exit_list) {
    if (el.type !== 'nocmd') {
      ...
      regex += ")$";
      QuestJs._commands.push(new ExitCmd("Go" + sentenceCase(el.name), el.name, { regexes:[new RegExp("^" + regex)] }));
      
      const regexes = []
      for (let key in QuestJs._lang.tell_to_prefixes) {
        regexes.push(new RegExp("^" + QuestJs._lang.tell_to_prefixes[key] + regex))
      }
      QuestJs._commands.push(new NpcExitCmd("NpcGo" + sentenceCase(el.name) + "2", el.name, { regexes:regexes }))
    }
  }
}
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.already_wearing, {char:char, garment:item})

  if (item.loc) {
    const holder = w[item.loc];
    if (holder.npc || holder.player) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
    
  return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_carrying, {char:char, item:item})
}
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_wearing, {char:char, item:item})
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  ...
  return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_here, {char:char, item:item})
}
...
cmdRules.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  ...
  if (item.loc) {
    const holder = w[item.loc]
    if (already && holder === game.player) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.char_has_it, {holder:holder, item:item})
  }
...
cmdRules.canTalkTo = function(cmd, char, item) {
  ...
  if (!item.npc) return QuestJs._io.falsemsg(prefix(item, isMultiple) + QuestJs._lang.not_able_to_hear, {char:char, item:item})
  return true
}
