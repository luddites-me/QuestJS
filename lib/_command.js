...
function Cmd(name, hash) {
  ...
  this.default = function (item, isMultiple, char) {
    if (typeof this.defmsg === 'string') {
      QuestJs._io.failedmsg(QuestJs._tools.prefix(item, isMultiple) + this.defmsg, {
        char: char,
        item: item,
      });
    } else if (typeof this.defmsg === 'function') {
      QuestJs._io.failedmsg(QuestJs._tools.prefix(item, isMultiple) + this.defmsg(char, item), {
        char: char,
        item: item,
      });
    } else {
      ...
      if (typeof rule !== 'function') {
        QuestJs._io.errormsg(
          "Failed to process command '" +
            this.name +
            "' as one of its rules is not a function (F12 for more).",
        );
        QuestJs._log.info(
...
function initCommands() {
  ...
  for (let el of QuestJs._lang.exit_list) {
    if (el.type !== 'nocmd') {
      ...
      QuestJs._commands.push(
        new NpcExitCmd('NpcGo' + QuestJs._tools.sentenceCase(el.name) + '2', el.name, {
          regexes: regexes,
        }),
      );
    }
...
cmdRules.isHere = function (cmd, char, item, isMultiple) {
  ...
  return QuestJs._io.falsemsg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.not_here, {
    char: char,
    item: item,
  });
};
