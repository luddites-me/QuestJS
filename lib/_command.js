...
import { errormsg, failedmsg, msg, metamsg } from './_io';
import { w, world } from './_world';
...
this.isHeldNotWorn = function(cmd, char, item, isMultiple) {
...
this.isWorn = function(cmd, char, item, isMultiple) {
  if (item.getWorn() && item.isAtLoc(char.name, world.PARSER)) return true
...
this.isHeld = function(cmd, char, item, isMultiple) {
...
this.isHere = function(cmd, char, item, isMultiple) {
...
// Item's location is the char's location or the char
// or item is reachable, but not held by someone else
this.isHereNotHeld = function(cmd, char, item, isMultiple, already) {
  if (item.isAtLoc(char.loc, world.PARSER)) return true
...
this.isHereNotHeldAlready = function(cmd, char, item, isMultiple) {
  return this.isHereNotHeld(cmd, char, item, isMultiple, true)
}


this.canManipulate = function(cmd, char, item) {
  if (!char.canManipulate(item, cmd.name)) return false
  return true
}

this.canTalkTo = function(cmd, char, item) {
...
this.canPosture = function(cmd, char, item) {
  if (!char.canPosture(cmd.name)) return false
  return true
}
}

export const cmdRules = new CmdRules();