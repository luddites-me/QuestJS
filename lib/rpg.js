...
const SPELLBOOK = function(list) {
  ...
  res.examine = function() {
    msg(this.examineX + ' It contains the spells ' + formatList(this.spellsAvailableToLearn.map(el => '<i>' + el + '</i>'), {lastJoiner:QuestJs._lang.list_and}) + '.')
  }
  res.icon = () => 'spell12'
  return res 
}
...
QuestJs._commands.push(new QuestJs._command.Cmd('Attack', {
  ...
  default:function(item, isMultiple, char) {
    msg(prefix(item, isMultiple) + "No point attacking " + QuestJs._lang.getName(item, {article:DEFINITE}) + ".");
    return false;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('Equip', {
  ...
  default:function(item, isMultiple, char) {
    msg(prefix(item, isMultiple) + QuestJs._lang.pronounVerb(item, "be", true) + " not something you can equip.");
    return false;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('LearnSpell', {
  ...
  script:function(objects) {
    ...
    msg("You learn {i:" + spell.name + "} from " + QuestJs._lang.getName(source, {article:DEFINITE}) + ".")
