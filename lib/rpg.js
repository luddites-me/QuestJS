/*

character attacks foe with weapon and skill

Start from skill, as it may involve more than one attack
Calc base chance and damage

Pass to weapon to modify

Pass to character to modify

Pass to room to modify

Pass to foe to modify

Determine if it hits
Apply damage

, using weapon


*/

const elements = {
  list: [
    { name: 'fire', opposed: 'frost' },
    { name: 'frost', opposed: 'fire' },

    { name: 'storm', opposed: 'earthmight' },
    { name: 'earthmight', opposed: 'storm' },

    { name: 'shadow', opposed: 'rainbow' },
    { name: 'rainbow', opposed: 'shadow' },

    { name: 'divine', opposed: 'necrotic' },
    { name: 'necrotic', opposed: 'divine' },

    { name: 'chaos', opposed: 'law' },
    { name: 'law', opposed: 'chaos' },

    { name: 'life', opposed: 'corruption' },
    { name: 'corruption', opposed: 'life' },
  ],
  opposed(s) {
    if (!s)
      QuestJs._io.errormsg(
        `elements.opposed was sent something that evaulates to false (type is ${typeof s})`,
      );
    for (const el of this.list) {
      if (el.name === s) return el.opposed;
    }
    QuestJs._io.errormsg(`elements.opposed was sent an unrecognised element: ${s}`);
    return null;
  },
};

class Skill {
  constructor(name, data) {
    this.name = name;
    this.reportText = '{nv:attacker:attack:true} {nm:target:the}.';
    this.targetText = 'Targeting {nm:target:the}.';
    for (const key in data) this[key] = data[key];
  }
}

class MetaSkill extends Skill {
  constructor(name, data) {
    super(name, data);
    this.meta = true;
  }
}

class Effect extends Skill {
  constructor(name, data) {
    super(name, data);
    this.effect = true;
  }
}

class Spell extends Skill {
  constructor(name, data) {
    super(name, data);
    this.spell = true;
    this.noWeapon = true;
    this.reportText = '{nv:attacker:cast:true} {i:{nm:skill}}.';
    this.statForOffensiveBonus = 'spellCasting';
  }
}

class SpellSelf extends Spell {
  constructor(name, data) {
    super(name, data);
    this.getPrimaryTargets = function () {
      return false;
    };
    this.noTarget = true;
    this.automaticSuccess = true;
    this.reportText = '{nv:attacker:cast:true} the {i:{nm:skill}} spell.';
  }
}

class SpellInanimate extends Spell {
  constructor(name, data) {
    super(name, data);
    this.inanimateTarget = true;
  }
}

const defaultSkill = new Skill('Basic attack', {
  icon: 'sword1',
  tooltip: 'A simple attack',
  modifyOutgoingAttack(attack) {},
});

const skills = {
  list: [defaultSkill],
  add(skill) {
    this.list.push(skill);
  },

  find(skillName) {
    return this.list.find(
      (el) => skillName === el.name.toLowerCase() || (el.regex && skillName.match(el.regex)),
    );
  },

  findName(skillName) {
    return this.list.find((el) => skillName === el.name);
  },

  terminate(spell, item) {
    QuestJs._array.remove(item.activeEffects, spell.name);
    delete item[`countdown_${spell.name}`];
    let s = `The {i:${spell.name}} spell${
      item === QuestJs._game.player
        ? ''
        : ` on ${QuestJs._lang.getName(item, { article: QuestJs._consts.DEFINITE })}`
    } terminates.`;
    if (spell.terminatingScript) s += spell.terminatingScript(item);
    return s;
  },
};

// The Attack object is used when someone casts a spell or makes a weapon or unarmed attack.
// The target could be the attacker, the room, a weapon or other item, or a number of characters.
// The Attack object should only be created once we are sure the command is successful
// (that is, the player input is okay - we can still throw errors if the code is wrong!).
// The Attack object may get modified at several points in the process,
// each can add to the report array to say what it has done
// If the primaryTargets is false, the target is the attacker himself
class Attack {
  static createAttack(attacker, target, skill) {
    const attack = new Attack();

    attack.attacker = attacker;
    attack.skill = skill;
    attack.target = target;

    // Find the skill
    if (attacker === QuestJs._game.player && skill === undefined) {
      attack.skill = skillUI.getSkillFromButtons();
      skillUI.resetButtons();
    }
    if (!attack.skill) attack.skill = defaultSkill;

    // Set targets
    if (attack.skill.noTarget) {
      attack.primaryTargets = attack.skill.getPrimaryTargets
        ? attack.skill.getPrimaryTargets()
        : [];
    } else {
      attack.primaryTargets = attack.skill.getPrimaryTargets
        ? attack.skill.getPrimaryTargets(target)
        : [target];
    }
    attack.secondaryTargets = attack.skill.getSecondaryTargets
      ? attack.skill.getSecondaryTargets(target)
      : [];

    // Set some defaults first
    attack.attackNumber = 1;
    attack.report = [];
    attack.armourModifier = 0;
    attack.armourMultiplier = 1;
    attack.primarySuccess = attack.skill.primarySuccess || 'A hit!';
    attack.primaryFailure = attack.skill.primaryFailure || 'A miss!';
    attack.secondarySuccess = attack.skill.secondarySuccess || 'A hit!';
    attack.secondaryFailure = attack.skill.secondaryFailure || 'A miss!';
    attack.msg(attack.skill.reportText, 1);

    // Get the weapon (for most monsters, the monster IS the weapon)
    // Base the attack on the weapon
    // Some skills use no weapon
    if (attack.skill.noWeapon) {
      attack.damage = attack.skill.damage;
      if (attack.damage) attack.setDamageAtts(attack.damage);
      attack.offensiveBonus = attack.skill.offensiveBonus || 0;
      attack.element = attack.skill.element;
    } else {
      attack.weapon = attacker.getEquippedWeapon();
      attack.offensiveBonus = attack.weapon.offensiveBonus || 0;
      attack.element = attack.weapon.element;
      attack.damage = attack.weapon.damage;
      attack.setDamageAtts(attack.damage);
    }
    if (attack.skill.secondaryDamage)
      attack.setDamageAtts(attack.skill.secondaryDamage, 'secondaryDamage');

    // modify for the skill
    if (attack.skill.modifyOutgoingAttack) attack.skill.modifyOutgoingAttack(attack);

    // Now take into account the attacker's stats
    attacker.modifyOutgoingAttack(attack);

    // Now take into account the attacker's weapon's active spell
    if (!attack.skill.noWeapon) attack.applyActiveEffects(attack.weapon);

    // Now take into account the attacker's active spells
    attack.applyActiveEffects(attack.attacker);

    const items = QuestJs._scope.scopeHeldBy(attack.attacker);
    for (const el of items) {
      if (el.modifyOutgoingAttack) el.modifyOutgoingAttack(attack);
    }

    // Now take into account the target's room
    const room = target ? QuestJs._w[target.loc] : QuestJs._w[attacker.loc];
    if (room.modifyOutgoingAttack) room.modifyOutgoingAttack(attack);
    attack.applyActiveEffects(room);

    QuestJs._log.info(attack);

    return attack;
  }

  // Once we have an attack object set up, call this to apply the attack to each target.
  // Creates a clone of this attack and calls resolve on it for each target to do the real work
  apply() {
    if (this.abort) return this;

    // target self
    if (!this.primaryTargets) {
      this.resolve(this.attacker, true, false);
      return this;
    }

    // target inanimate object
    // TODO!!!
    if (this.inanimateTarget) {
      this.resolveForInanimate(this.attacker, this.primaryTargets);
      return this;
    }

    // Iterate through the targets and apply the attack
    // The attack may be modified by the target, so we send a clone
    for (const target of this.primaryTargets) {
      for (let i = 0; i < this.attackNumber; i += 1) {
        this.clone().resolve(target, true, i);
      }
    }
    for (const target of this.secondaryTargets) {
      for (let i = 0; i < this.attackNumber; i += 1) {
        this.clone().resolve(target, false, i);
      }
    }
    return this;
  }

  // Once we have an attack object set up, call this to apply the attack to each target.
  // This applies all effects, and puts the descriptive text in report
  // (report is the attribute of the parent attack, not this clone)
  resolve(target, isPrimary, count) {
    this.target = target;

    // Now take into account the target's active spell
    this.applyActiveEffects(target);

    const items = QuestJs._scope.scopeHeldBy(target);
    for (const el of items) {
      if (el.modifyIncomingAttack) el.modifyIncomingAttack(this);
    }

    // Now take into account the target
    if (target.modifyIncomingAttack) target.modifyIncomingAttack(this);
    if (this.element && target.element) this.modifyElementalAttack(target.element, this, isPrimary);

    // Is the target affected (hit)?
    if (this.skill.automaticSuccess) {
      QuestJs._io.msg(
        QuestJs._text.processText(this.skill.targetText, {
          skill: this.skill,
          attacker: this.attacker,
          target,
        }),
        3,
      );
      QuestJs._io.msg(`Element: ${this.element}`, 4);
      QuestJs._io.msg('Automatic success', 4);
    } else {
      this.roll = QuestJs._random.int(1, 20);
      this.result = this.offensiveBonus - target.defensiveBonus + this.roll - 10;
      QuestJs._io.msg(
        QuestJs._text.processText(this.skill.targetText, {
          skill: this.skill,
          attacker: this.attacker,
          target,
        }),
        3,
      );
      if (this.element) QuestJs._io.msg(`Element: ${this.element}`, 4);
      QuestJs._io.msg(`Offensive bonus: ${this.offensiveBonus}`, 4);
      QuestJs._io.msg(`Roll: ${this.roll}`, 4);

      if (this.result < 0) {
        if (isPrimary && this.primaryFailure) {
          QuestJs._io.msg(QuestJs._text.processText(this.primaryFailure, this), 1);
        } else if (!isPrimary && this.secondaryFailure) {
          QuestJs._io.msg(QuestJs._text.processText(this.secondaryFailure, this), 1);
        } else {
          QuestJs._io.msg('A miss...\n', 1);
        }
        if (isPrimary && this.skill.onPrimaryFailure) this.skill.onPrimaryFailure(this.prototype);
        return;
      }

      if (isPrimary && this.primarySuccess) {
        QuestJs._io.msg(QuestJs._text.processText(this.primarySuccess, this), 1);
      } else if (!isPrimary && this.secondarySuccess) {
        QuestJs._io.msg(QuestJs._text.processText(this.secondarySuccess, this), 1);
      } else {
        QuestJs._io.msg('A hit!', 1);
      }
    }

    // check for any spell effects that will be terminated by this for use later
    const terminatedSpells = [];
    if (this.skill.incompatible) {
      for (const spellName of target.activeEffects) {
        for (const regex of this.skill.incompatible) {
          if (spellName.match(regex)) terminatedSpells.push(spellName);
        }
      }
    }

    if (this.skill.ongoing) target.activeEffects.push(this.skill.name);
    if (this.skill.duration) target[`countdown_${this.skill.name}`] = this.skill.duration;
    if (this.skill.targetEffect) {
      this.skill.targetEffect(this, isPrimary, count);
    }

    if (isPrimary) {
      this.applyDamage(target);
    } else {
      this.applyDamage(target, 'secondaryDamage');
    }

    for (const spellName of terminatedSpells) {
      QuestJs._io.msg(skills.terminate(skills.findName(spellName), target));
    }
  }

  modifyElementalAttack(element) {
    if (this.element === element) {
      QuestJs._io.msg('Damage halved as same element', 4);
      this.damageMultiplier *= 0.5;
      this.secondaryDamageMultiplier *= 0.5;
    }
    if (this.element === elements.opposed(element)) {
      QuestJs._io.msg('Damage doubled as opposed element', 4);
      this.damageMultiplier *= 2;
      this.secondaryDamageMultiplier *= 2;
    }
  }

  msg(s, n) {
    this.report.push({ t: QuestJs._text.processText(s, this), level: n || 1 });
  }

  output() {
    QuestJs._settings.output(this.report);
  }

  clone() {
    const copy = new Attack(); // !!!!!
    for (const key in this) copy[key] = this[key];
    copy.prototype = this;
    return copy;
  }

  setDamageAtts(string, prefix = 'damage') {
    const regexMatch = /^(\d*)d(\d+)([\+|\-]\d+)?$/i.exec(string);
    if (regexMatch === null) {
      QuestJs._io.errormsg(`Weapon ${this.weapon.name} has a bad damage attribute (${string}).`);
      return;
    }
    this[`${prefix}Number`] = regexMatch[1] === '' ? 1 : parseInt(regexMatch[1]);
    this[`${prefix}Sides`] = parseInt(regexMatch[2]);
    this[`${prefix}Bonus`] = regexMatch[3] === undefined ? 0 : parseInt(regexMatch[3]);
    this[`${prefix}Multiplier`] = 1;
  }

  applyDamage(target, prefix = 'damage') {
    // calculate base damage
    if (this[`${prefix}Bonus`] || this[`${prefix}Number`]) {
      QuestJs._io.msg(
        `Damage: ${this.damageNumber}d${this[`${prefix}Sides`]}+${this[`${prefix}Bonus`]}`,
        3,
      );
      let damage = this[`${prefix}Bonus`];
      for (let i = 0; i < this[`${prefix}Number`]; i += 1) {
        const roll = QuestJs._random.int(1, this[`${prefix}Sides`]);
        damage += roll;
        QuestJs._io.msg(`Damage roll (d${this[`${prefix}Sides`]}): ${roll}`, 5);
      }
      QuestJs._io.msg(`Damage before armour: ${damage}`, 4);
      QuestJs._io.msg(`Armour: ${target.armour}`, 4);
      damage -=
        this[`${prefix}Number`] * (target.armour * this.armourMultiplier + this.armourModifier);
      QuestJs._io.msg(`Damage after armour: ${damage}`, 4);
      if (damage < 1) damage = 1;
      QuestJs._io.msg(`Damage before multiplier: ${damage}`, 6);
      const modifiedDamage = this[`${prefix}Multiplier`] * damage;
      QuestJs._io.msg(`Damage: ${modifiedDamage}`, 1);
      target.health -= modifiedDamage;
      QuestJs._io.msg(`Health now: ${target.health}`, 2);
    }
  }

  applyActiveEffects(source) {
    if (!source || !source.activeEffects) return;
    for (const el of source.activeEffects) {
      const effect = skills.findName(el);
      if (!effect) return QuestJs._io.errormsg(`applyActiveEffects: Failed to find skill [${el}]`);
      if (effect.modifyOutgoingAttack) effect.modifyOutgoingAttack(this, source);
    }
  }
}

const rpg = {};

// Get a list of foes in the current room, with target first (whether a foe or not)
rpg.getFoes = function (target) {
  const l = QuestJs._scope
    .scopeHereListed()
    .filter((el) => el.npc && el.isHostile() && el !== target);
  if (target !== undefined) l.unshift(target);
  return l;
};

// Get a list of NPCs in the current room, with target first
rpg.getAll = function (target) {
  const l = QuestJs._scope.scopeHereListed().filter((el) => el.npc && el !== target);
  if (target !== undefined) l.unshift(target);
  return l;
};

// Get a list of foes in the current room, with target first (whether a foe or not)
rpg.getFoesBut = function (target) {
  const l = QuestJs._scope
    .scopeHereListed()
    .filter((el) => el.npc && el.isHostile() && el !== target);
  return l;
};

// Get a list of NPCs in the current room, with target first
rpg.getAllBut = function (target) {
  const l = QuestJs._scope.scopeHereListed().filter((el) => el.npc && el !== target);
  return l;
};

// Give a character a modifyOutgoingAttack function to have it modify an attack the character is making
// or modifyIncomingAttack for an attack it is receiving
const RPG_TEMPLATE = {
  offensiveBonus: 0,
  defensiveBonus: 0,
  armour: 0,
  activeEffects: [],
  skillsLearnt: [],

  attack(isMultiple, char) {
    // Create an attack, based on the current skill, weapon and given target
    const attack = Attack.createAttack(QuestJs._game.player, this).apply().output();

    // QuestJs._io.msg(attack.result.join(''))
    return true;
  },

  modifyOutgoingAttack(attack) {
    if (!attack.skill.statForOffensiveBonus) {
      attack.offensiveBonus += this.offensiveBonus;
    } else if (this[attack.skill.statForOffensiveBonus] !== undefined) {
      attack.offensiveBonus += this[attack.skill.statForOffensiveBonus];
    } else {
      QuestJs._io.errormsg(
        `ERROR: Trying to use a stat the character ${this.name} does not have: ${attack.statForOffensiveBonus}`,
      );
    }
  },

  getEquippedWeapon() {
    return this;
  },

  isHostile() {
    return true;
  },

  getArmour() {
    return this.armour;
  },
};

const RPG_PLAYER = function () {
  const res = QuestJs._templates.PLAYER();

  for (const key in RPG_TEMPLATE) res[key] = RPG_TEMPLATE[key];

  // res.getEquippedWeapon = function() { return this.equipped ? QuestJs._w[this.equipped] : QuestJs._w.weapon_unarmed; }

  res.getEquippedWeapon = function () {
    const carried = QuestJs._scope.scopeHeldBy(this);
    return carried.find((el) => el.equipped && el.weapon) || QuestJs._w.weapon_unarmed;
  };

  res.getArmour = function () {
    const garments = QuestJs._scope.scopeHeldBy(this).filter((el) => el.worn);
    let armour = 0;
    for (const el of garments) armour += el.getArmour();
    return armour / QuestJs._settings.armourScaling;
  };

  return res;
};

const RPG_NPC = function (female) {
  const res = QuestJs._npc.NPC(female);

  for (const key in RPG_TEMPLATE) res[key] = RPG_TEMPLATE[key];

  res.oldRpgOnCreation = res.onCreation;
  res.onCreation = function (o) {
    o.oldRpgOnCreation(o);
    o.verbFunctions.push((o, verbList) => {
      verbList.push(QuestJs._lang.verbs.attack);
    });
    o.nameModifierFunctions.push((o, list) => {
      if (o.dead) list.push(QuestJs._lang.invModifiers.dead);
    });
  };

  return res;
};

const RPG_CORPOREAL_UNDEAD = function (female) {
  const res = RPG_NPC(female);

  res.element = necrotic;

  return res;
};

const RPG_NON_CORPOREAL_UNDEAD = function (female) {
  const res = RPG_CORPOREAL_UNDEAD(female);

  res.noCorpse = true;

  res.modifyIncomingAttack = function (attack) {
    if (attack.element || attack.isMagic || attack.spell) {
      attack.damageMultiplier = 0;
      attack.primarySuccess = attack.primarySuccess.replace(
        /[.!]/,
        ', but it passes straight through {sb:target}.',
      );
    }
  };

  return res;
};

// The attack agenda item will have the monster wait until the player appears, then attack, fllowing if the player moves away
QuestJs._npc.agenda.attack = function (npc, arr) {
  // do stuff
  return false;
};

const EQUIPPABLE = function () {
  const res = $.extend({}, QuestJs._templates.TAKEABLE_DICTIONARY);

  res.equippable = true;
  (res.activeEffects = []),
    (res.getObstructing = function (char) {
      return QuestJs._scope.scopeHeldBy(char).filter((el) => el.equipped && this.match(el));
    });

  res.oldRpgOnCreation = res.onCreation;
  res.onCreation = function (o) {
    o.oldRpgOnCreation(o);
    o.verbFunctions.push((o, verbList) => {
      if (o.isAtLoc(QuestJs._game.player)) {
        verbList.push(o.equipped ? QuestJs._lang.verbs.unequip : QuestJs._lang.verbs.equip);
      }
    });
    o.nameModifierFunctions.push((o, list) => {
      if (o.equipped && o.isAtLoc(QuestJs._game.player.name))
        list.push(QuestJs._lang.invModifiers.equipped);
    });
  };

  res.drop = function (isMultiple, char) {
    if (this.equipped) {
      delete this.equipped;
    }
    const tpParams = { char, item: this };
    QuestJs._io.msg(
      QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.drop_successful,
      tpParams,
    );
    this.moveToFrom(char.loc, this.loc);
    return true;
  };

  res.equip = function (isMultiple, char) {
    const equipped = this.getObstructing(char);
    if (equipped.includes(this)) {
      QuestJs._io.msg('It already is.');
      return false;
    }
    if (equipped.length === 0) {
      QuestJs._io.msg('{nv:char:draw:true} {nm:item:the}.', { char, item: this });
    } else if (equipped.length === 1) {
      const params = { char, item: this };
      QuestJs._io.msg(
        `{nv:char:put:true} away ${QuestJs._tools.formatList(equipped, {
          article: QuestJs._consts.DEFINITE,
          joiner: ' and ',
        })}, and equip {nm:item:the}.`,
        params,
      );
      for (const el of equipped) delete el.equipped;
    }
    this.equipped = true;
    return true;
  };

  res.unequip = function (isMultiple, char) {
    if (!this.equipped) {
      QuestJs._io.msg('It already is.');
      return false;
    }
    delete this.equipped;
    const params = { char, weapon: this };
    QuestJs._io.msg('{nv:char:put:true} away {nm:weapon:the}.', params);
    return true;
  };

  return res;
};

const WEAPON = function (damage) {
  const res = $.extend({}, EQUIPPABLE());
  res.weapon = true;
  res.damage = damage;
  res.match = function (item) {
    return item.weapon;
  };
  res.icon = () => 'weapon12';
  return res;
};

const LIMITED_USE_WEAPON = function (damage, ammo) {
  const res = $.extend({}, WEAPON(damage));
  res.ammo = ammo;
  res.activeEffects.push('Limited ammo');
  return res;
};

skills.add(
  new MetaSkill('Limited ammo', {
    modifyOutgoingAttack(attack) {
      if (attack.weapon.ammo === 0) {
        attack.msg('Out of ammo!');
        attack.abort = true;
      } else {
        attack.weapon.ammo -= 1;
      }
    },
  }),
);

/*
skills.add(new MetaSkill("Deteriorating", {
  // really needs to be on success only
  modifyOutgoingAttack:function(attack) {
    if (attack.weapon.ammo === 0) {
      attack.msg("Out of ammo!")
      attack.abort = true
    }
    else {
      attack.weapon.ammo -= 1
    }
  },
}))
*/

const SHIELD = function (bonus) {
  const res = $.extend({}, EQUIPPABLE());
  res.shield = true;
  res.shieldBonus = bonus;
  res.match = function (item) {
    return item.shield;
  };
  res.modifyIncomingAttack = function (attack) {
    if (!this.equipped) return;
    attack.offensiveBonus -= this.shieldBonus;
  };
  res.icon = () => 'shield12';
  return res;
};

const SPELLBOOK = function (list) {
  const res = $.extend({}, QuestJs._templates.TAKEABLE_DICTIONARY);
  res.spellbook = true;
  res.spellsAvailableToLearn = list;
  res.examineX = '';
  res.examine = function () {
    QuestJs._io.msg(
      `${this.examineX} It contains the spells ${QuestJs._tools.formatList(
        this.spellsAvailableToLearn.map((el) => `<i>${el}</i>`),
        { lastJoiner: QuestJs._lang.list_and },
      )}.`,
    );
  };
  res.icon = () => 'spell12';
  return res;
};

QuestJs._create.createItem('weapon_unarmed', WEAPON(), {
  loc: 'me',
  image: 'fist',
  damage: 'd4',
  offensiveBonus: -2,
  alias: 'unarmed',
  scenery: true,
  isAtLoc(loc, situation) {
    if (situation !== QuestJs._world.SCOPING) return false;
    if (typeof loc !== 'string') loc = loc.name;
    return this.loc === loc;
  },
});

QuestJs._create.createItem('spell_handler', {
  eventPeriod: 1,
  eventActive: true,
  eventScript() {
    const objs = QuestJs._scope.scopeBy((o) => o.activeEffects !== undefined);
    // QuestJs._log.info(objs)
    for (const el of objs) {
      for (const spellName of el.activeEffects) {
        if (el[`countdown_${spellName}`]) {
          el[`countdown_${spellName}`] -= 1;
          if (el[`countdown_${spellName}`] <= 0) {
            QuestJs._io.msg(skills.terminate(skills.findName(spellName), el));
          }
        }
      }
    }
  },
  isAtLoc() {
    return false;
  },
});

QuestJs._commands.push(
  new QuestJs._command.Cmd('Attack', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(attack|att) (.+)$/,
    objects: [{ ignore: true }, { scope: QuestJs._parser.isPresent }],
    default(item, isMultiple, char) {
      QuestJs._io.msg(
        `${QuestJs._tools.prefix(item, isMultiple)}No point attacking ${QuestJs._lang.getName(
          item,
          { article: QuestJs._consts.DEFINITE },
        )}.`,
      );
      return false;
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('Equip', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHeld],
    regex: /^(equip|brandish|draw) (.+)$/,
    objects: [{ ignore: true }, { scope: QuestJs._parser.isHeld }],
    default(item, isMultiple, char) {
      QuestJs._io.msg(
        `${
          QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.pronounVerb(item, 'be', true)
        } not something you can equip.`,
      );
      return false;
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('Unequip', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHeld],
    regex: /^(unequip|holster|sheath|put away) (.+)$/,
    objects: [{ ignore: true }, { scope: QuestJs._parser.isHeld }],
    default(item, isMultiple, char) {
      QuestJs._io.msg(
        `${
          QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.pronounVerb(item, 'be', true)
        } not something you can equip.`,
      );
      return false;
    },
  }),
);

QuestJs._world.isSpellAvailable = function (char, spell) {
  for (const el of QuestJs._scope.scopeHeldBy(char)) {
    if (el.spellsAvailableToLearn && el.spellsAvailableToLearn.includes(spell.name)) {
      return el;
    }
  }
  return false;
};

QuestJs._commands.push(
  new QuestJs._command.Cmd('LearnSpell', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(learn) (.+)$/,
    objects: [{ ignore: true }, { text: true }],
    script(objects) {
      const spell = skills.find(objects[0]);
      if (!spell || !spell.spell)
        return QuestJs._io.failedmsg(`There is no spell called ${objects[0]}.`);

      const source = QuestJs._world.isSpellAvailable(QuestJs._game.player, spell);
      if (!source)
        return QuestJs._io.failedmsg(
          `You do not have anything you can learn {i:${spell.name}} from.`,
        );

      QuestJs._game.player.skillsLearnt.push(spell.name);
      QuestJs._io.msg(
        `You learn {i:${spell.name}} from ${QuestJs._lang.getName(source, {
          article: QuestJs._consts.DEFINITE,
        })}.`,
      );
      return QuestJs._world.SUCCESS;
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('CastSpell', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(cast|invoke) (.+)$/,
    objects: [{ ignore: true }, { text: true }],
    script(objects) {
      const spell = skills.find(objects[0]);
      if (!spell || !spell.spell)
        return QuestJs._io.failedmsg(`There is no spell called ${objects[0]}.`);

      if (!QuestJs._game.player.skillsLearnt.includes(spell.name))
        return QuestJs._io.failedmsg(`You do not know the spell {i:${spell.name}}.`);

      if (!spell.noTarget)
        return QuestJs._io.failedmsg(`You need a target for the spell {i:${spell.name}}.`);

      const attack = Attack.createAttack(QuestJs._game.player, QuestJs._game.player, spell)
        .apply()
        .output();

      // return skills.castSpell(spell, QuestJs._game.player, QuestJs._game.player)
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('CastSpellAt', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(cast|invoke) (.+) (at|on) (.+)$/,
    objects: [
      { ignore: true },
      { text: true },
      { ignore: true },
      { scope: QuestJs._parser.isPresent },
    ],
    script(objects) {
      const spell = skills.find(objects[0]);
      if (!spell || !spell.spell)
        return QuestJs._io.failedmsg(`There is no spell called ${objects[0]}.`);

      if (!QuestJs._game.player.skillsLearnt.includes(spell.name))
        return QuestJs._io.failedmsg(`You do not know the spell {i:${spell.name}}.`);

      const target = objects[1][0];

      // check target

      if (spell.damage && target.health === undefined)
        return QuestJs._io.failedmsg("You can't attack that.");

      const attack = Attack.createAttack(QuestJs._game.player, target, spell).apply().output();
      return QuestJs._world.SUCCESS;
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('DebugRPG', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHeld],
    regex: /^(rpg)$/,
    objects: [{ ignore: true }],
    script(objects) {
      QuestJs._settings.attackOutputLevel = 10;
      QuestJs._io.metamsg('All output from attacks will now be seen.');
      return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
);
