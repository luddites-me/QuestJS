...
class Attack {
  static createAttack(attacker, target, skill) {
    ...
    attack.QuestJs._io.msg(attack.skill.reportText, 1)
  ...
  resolve(target, isPrimary, count) {
    ...
    if (this.skill.automaticSuccess) {
      this.QuestJs._io.msg(processText(this.skill.targetText, {skill:this.skill, attacker:this.attacker, target:target}), 3)
      this.QuestJs._io.msg("Element: " + this.element, 4)
      this.QuestJs._io.msg("Automatic success", 4)
    }
    else {
      this.roll = random.int(1, 20)
      this.result = this.offensiveBonus - target.defensiveBonus + this.roll - 10
      this.QuestJs._io.msg(processText(this.skill.targetText, {skill:this.skill, attacker:this.attacker, target:target}), 3)
      if (this.element) this.QuestJs._io.msg("Element: " + this.element, 4)
      this.QuestJs._io.msg("Offensive bonus: " + this.offensiveBonus, 4)
      this.QuestJs._io.msg("Roll: " + this.roll, 4)
      
      if (this.result < 0) {
        if (isPrimary && this.primaryFailure) {
          this.QuestJs._io.msg(processText(this.primaryFailure, this), 1)
        }
        else if (!isPrimary && this.secondaryFailure) {
          this.QuestJs._io.msg(processText(this.secondaryFailure, this), 1)
        }
        else {
          this.QuestJs._io.msg("A miss...\n", 1)
        }
      ...
      if (isPrimary && this.primarySuccess) {
        this.QuestJs._io.msg(processText(this.primarySuccess, this), 1)
      }
      else if (!isPrimary && this.secondarySuccess) {
        this.QuestJs._io.msg(processText(this.secondarySuccess, this), 1)
      }
      else {
        this.QuestJs._io.msg("A hit!", 1)
      }
    }
    ...
    for (let spellName of terminatedSpells) {
      this.QuestJs._io.msg(skills.terminate(skills.findName(spellName), target))
    }
  }

  modifyElementalAttack(element) {
    if (this.element === element) {
      this.QuestJs._io.msg("Damage halved as same element", 4)
      this.damageMultiplier *= 0.5
      this.secondaryDamageMultiplier *= 0.5
    }
    if (this.element === elements.opposed(element)) {
      this.QuestJs._io.msg("Damage doubled as opposed element", 4)
  ...
  applyDamage(target, prefix = 'damage') {
    ...
    if (this[prefix + 'Bonus'] || this[prefix + 'Number']) {
      this.QuestJs._io.msg(`Damage: ${this.damageNumber}d${this[prefix + 'Sides']}+${this[prefix + 'Bonus']}`, 3)
      let damage = this[prefix + 'Bonus'];
      for (let i = 0; i < this[prefix + 'Number']; i++) {
        const roll = random.int(1, this[prefix + 'Sides'])
        damage += roll
        this.QuestJs._io.msg(`Damage roll (d${this[prefix + 'Sides']}): ${roll}`, 5)
      }
      this.QuestJs._io.msg("Damage before armour: " + damage, 4)
      this.QuestJs._io.msg("Armour: " + target.armour, 4)
      damage -= this[prefix + 'Number'] * (target.armour * this.armourMultiplier + this.armourModifier)
      this.QuestJs._io.msg("Damage after armour: " + damage, 4)
      if (damage < 1) damage = 1;
      this.QuestJs._io.msg("Damage before multiplier: " + damage, 6)
      const modifiedDamage = this[prefix + 'Multiplier'] * damage
      this.QuestJs._io.msg("Damage: " + modifiedDamage, 1)
      target.health -= modifiedDamage
      this.QuestJs._io.msg("Health now: " + target.health, 2)
    }
  }
...
skills.add(new MetaSkill("Limited ammo", {
  modifyOutgoingAttack:function(attack) {
    if (attack.weapon.ammo === 0) {
      attack.QuestJs._io.msg("Out of ammo!")
      attack.abort = true
    }
    else {
      attack.weapon.ammo--
