...
class Attack {
  ...
  applyActiveEffects(source) {
    ...
    for (let el of source.activeEffects) {
      ...
      if (!effect)
        return QuestJs._io.errormsg('applyActiveEffects: Failed to find skill [' + el + ']');
...
const RPG_TEMPLATE = {
  ...
  modifyOutgoingAttack: function (attack) {
    if (!attack.skill.statForOffensiveBonus) {
      ...
      QuestJs._io.errormsg(
        'ERROR: Trying to use a stat the character ' +
          this.name +
          ' does not have: ' +
          attack.statForOffensiveBonus,
      );
    }
  },
...
const EQUIPPABLE = function () {
  ...
  res.equip = function (isMultiple, char) {
    ...
    if (equipped.length === 0) {
      ...
      QuestJs._io.msg(
        '{nv:char:put:true} away ' +
          QuestJs._tools.formatList(equipped, {
...
QuestJs._create.createItem('spell_handler', {
  ...
  eventScript: function () {
    const objs = QuestJs._scope.scopeBy(function (o) {
      return o.activeEffects !== undefined;
    });
    ...
    script: function (objects) {
      ...
      QuestJs._io.msg(
        'You learn {i:' +
          spell.name +
          '} from ' +
          QuestJs._lang.getName(source, { article: QuestJs._consts.DEFINITE }) +
          '.',
      );
