...
const elements = {
  ...
  opposed:function(s) {
    if (!s) QuestJs._io.errormsg("elements.opposed was sent something that evaulates to false (type is " + (typeof s) + ")")
...
const skills = {
  ...
  terminate:function(spell, item) {
    ...
    let s = "The {i:" + spell.name + "} spell" + (item === QuestJs._game.player ? '' : " on " + QuestJs._lang.getName(item, {article:QuestJs._consts.DEFINITE})) + " terminates."
...
class Attack {
  ...
  resolve(target, isPrimary, count) {
    ...
    if (this.skill.automaticSuccess) {
      QuestJs._io.msg(QuestJs._text.processText(this.skill.targetText, {skill:this.skill, attacker:this.attacker, target:target}), 3)
  ...
  applyDamage(target, prefix = 'damage') {
    ...
    if (this[prefix + 'Bonus'] || this[prefix + 'Number']) {
      QuestJs._io.msg(`Damage: ${this.damageNumber}d${this[prefix + 'Sides']}+${this[prefix + 'Bonus']}`, 3)
...
rpg.getFoes = function(target) {
  const l = QuestJs._scope.scopeHereListed().filter(function(el) {
...
const RPG_TEMPLATE = {
  ...
  modifyOutgoingAttack:function(attack) {
    if (!attack.skill.statForOffensiveBonus) {
      ...
      QuestJs._io.errormsg("ERROR: Trying to use a stat the character " + this.name + " does not have: " + attack.statForOffensiveBonus)
    }
  },
...
const EQUIPPABLE = function() {
  ...
  res.onCreation = function(o) {
    ...
    o.nameModifierFunctions.push(function(o, list) {
      if (o.equipped && o.isAtLoc(QuestJs._game.player.name)) list.push(QuestJs._lang.invModifiers.equipped)
    })
  }
  ...
  res.equip = function(isMultiple, char) {
    ...
    if (equipped.length === 0) {
      QuestJs._io.msg("{nv:char:draw:true} {nm:item:the}.", {char:char, item:this})
    }
    else if (equipped.length === 1) {
      const params = {char:char, item:this}
      QuestJs._io.msg("{nv:char:put:true} away " + QuestJs._tools.formatList(equipped, {article:QuestJs._consts.DEFINITE, joiner:" and "}) + ", and equip {nm:item:the}.", params)
...
const SPELLBOOK = function(list) {
  ...
  res.examine = function() {
    QuestJs._io.msg(this.examineX + ' It contains the spells ' + QuestJs._tools.formatList(this.spellsAvailableToLearn.map(el => '<i>' + el + '</i>'), {lastJoiner:QuestJs._lang.list_and}) + '.')
  }
  res.icon = () => 'spell12'
  return res 
}
...
QuestJs._create.createItem("spell_handler", {
  ...
  eventScript:function() {
    const objs = QuestJs._scope.scopeBy(function(o) { return o.activeEffects !== undefined })
...
QuestJs._commands.push(new QuestJs._command.Cmd('Attack', {
  npcCmd:true,
  ...
  default:function(item, isMultiple, char) {
    QuestJs._io.msg(QuestJs._tools.prefix(item, isMultiple) + "No point attacking " + QuestJs._lang.getName(item, {article:QuestJs._consts.DEFINITE}) + ".");
    return false;
  },
}));


QuestJs._commands.push(new QuestJs._command.Cmd('Equip', {
  npcCmd:true,
  ...
  default:function(item, isMultiple, char) {
    QuestJs._io.msg(QuestJs._tools.prefix(item, isMultiple) + QuestJs._lang.pronounVerb(item, "be", true) + " not something you can equip.");
    return false;
  },
}));


QuestJs._commands.push(new QuestJs._command.Cmd('Unequip', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHeld],
...
QuestJs._world.isSpellAvailable = function(char, spell) {
  for (let el of QuestJs._scope.scopeHeldBy(char)) {
...
QuestJs._commands.push(new QuestJs._command.Cmd('LearnSpell', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(learn) (.+)$/,
  objects:[
  ...
  script:function(objects) {
    ...
    if (!spell || !spell.spell) return QuestJs._io.failedmsg("There is no spell called " + objects[0] + ".")
      
    const source = QuestJs._world.isSpellAvailable(QuestJs._game.player, spell)
    if (!source) return QuestJs._io.failedmsg("You do not have anything you can learn {i:" + spell.name + "} from.")
    
    QuestJs._game.player.skillsLearnt.push(spell.name)
    QuestJs._io.msg("You learn {i:" + spell.name + "} from " + QuestJs._lang.getName(source, {article:QuestJs._consts.DEFINITE}) + ".")
...
QuestJs._commands.push(new QuestJs._command.Cmd('CastSpell', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(cast|invoke) (.+)$/,
  objects:[
  ...
  script:function(objects) {
    ...
    if (!QuestJs._game.player.skillsLearnt.includes(spell.name)) return QuestJs._io.failedmsg("You do not know the spell {i:" + spell.name + "}.")
    
    if (!spell.noTarget) return QuestJs._io.failedmsg("You need a target for the spell {i:" + spell.name + "}.")
  
    const attack = Attack.createAttack(QuestJs._game.player, QuestJs._game.player, spell).apply().output()
...
QuestJs._commands.push(new QuestJs._command.Cmd('CastSpellAt', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
  ...
  script:function(objects) {
    ...
    if (spell.damage && target.health === undefined) return QuestJs._io.failedmsg("You can't attack that.")
...
QuestJs._commands.push(new QuestJs._command.Cmd('DebugRPG', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHeld],
  regex:/^(rpg)$/,
  objects:[
