  ...
  opposed(s) {
    if (!s)
      QuestJs._io.errormsg(
        `elements.opposed was sent something that evaulates to false (type is ${typeof s})`,
      );
    for (const el of this.list) {
      if (el.name === s) return el.opposed;
    }
    QuestJs._io.errormsg(
      `elements.opposed was sent an unrecognised element: ${s}`,
    );
    return null;
  },
};
...
const skills = {
  ...
  find(skillName) {
    return this.list.find(
      (el) =>
        skillName === el.name.toLowerCase() ||
        (el.regex && skillName.match(el.regex)),
    );
  },
  ...
  terminate(spell, item) {
    QuestJs._array.remove(item.activeEffects, spell.name);
    delete item[`countdown_${spell.name}`];
    let s = `The {i:${spell.name}} spell${
      item === QuestJs._game.player
        ? ''
        : ` on ${QuestJs._lang.getName(item, {
...
class Attack {
  static createAttack(attacker, target, skill) {
    ...
    // Find the skill
    if (attacker === QuestJs._game.player && skill === undefined) {
    ...
    if (attack.skill.noTarget) {
      attack.primaryTargets = attack.skill.getPrimaryTargets
        ? attack.skill.getPrimaryTargets()
        : [];
    } else {
      attack.primaryTargets = attack.skill.getPrimaryTargets
        ? attack.skill.getPrimaryTargets(target)
        : [target];
    }
    attack.secondaryTargets = attack.skill.getSecondaryTargets
      ? attack.skill.getSecondaryTargets(target)
      : [];
    ...
    if (attack.skill.secondaryDamage)
      attack.setDamageAtts(attack.skill.secondaryDamage, 'secondaryDamage');

    // modify for the skill
    if (attack.skill.modifyOutgoingAttack)
      attack.skill.modifyOutgoingAttack(attack);
  ...
  apply() {
    ...
    for (const target of this.primaryTargets) {
      for (let i = 0; i < this.attackNumber; i += 1) {
  ...
  resolve(target, isPrimary, count) {
    ...
    const items = QuestJs._scope.scopeHeldBy(target);
    for (const el of items) {
    ...
    if (this.element && target.element)
      this.modifyElementalAttack(target.element, this, isPrimary);

    // Is the target affected (hit)?
    if (this.skill.automaticSuccess) {
      QuestJs._io.msg(
        QuestJs._text.processText(this.skill.targetText, {
          skill: this.skill,
      ...
      QuestJs._io.msg(`Element: ${this.element}`, 4);
      QuestJs._io.msg('Automatic success', 4);
    } else {
      this.roll = QuestJs._random.int(1, 20);
      this.result =
        this.offensiveBonus - target.defensiveBonus + this.roll - 10;
      QuestJs._io.msg(
        QuestJs._text.processText(this.skill.targetText, {
          skill: this.skill,
          attacker: this.attacker,
      ...
      if (this.element) QuestJs._io.msg(`Element: ${this.element}`, 4);
      QuestJs._io.msg(`Offensive bonus: ${this.offensiveBonus}`, 4);
      QuestJs._io.msg(`Roll: ${this.roll}`, 4);

      if (this.result < 0) {
        if (isPrimary && this.primaryFailure) {
          QuestJs._io.msg(
            QuestJs._text.processText(this.primaryFailure, this),
            1,
          );
        } else if (!isPrimary && this.secondaryFailure) {
          QuestJs._io.msg(
            QuestJs._text.processText(this.secondaryFailure, this),
            1,
          );
        } else {
          QuestJs._io.msg('A miss...\n', 1);
        }
        if (isPrimary && this.skill.onPrimaryFailure)
          this.skill.onPrimaryFailure(this.prototype);
        return;
      }

      if (isPrimary && this.primarySuccess) {
        QuestJs._io.msg(
          QuestJs._text.processText(this.primarySuccess, this),
          1,
        );
      } else if (!isPrimary && this.secondarySuccess) {
        QuestJs._io.msg(
          QuestJs._text.processText(this.secondarySuccess, this),
          1,
        );
      } else {
        QuestJs._io.msg('A hit!', 1);
      }
    }
    ...
    if (this.skill.incompatible) {
      for (const spellName of target.activeEffects) {
        for (const regex of this.skill.incompatible) {
    ...
    if (this.skill.duration)
      target[`countdown_${this.skill.name}`] = this.skill.duration;
    ...
    for (const spellName of terminatedSpells) {
      QuestJs._io.msg(skills.terminate(skills.findName(spellName), target));
    }
  }

  modifyElementalAttack(element) {
    if (this.element === element) {
      QuestJs._io.msg('Damage halved as same element', 4);
      this.damageMultiplier *= 0.5;
      this.secondaryDamageMultiplier *= 0.5;
    }
    if (this.element === elements.opposed(element)) {
      QuestJs._io.msg('Damage doubled as opposed element', 4);
  ...
  msg(s, n) {
    this.report.push({ t: QuestJs._text.processText(s, this), level: n || 1 });
  }

  output() {
    QuestJs._settings.output(this.report);
  }
  ...
  setDamageAtts(string, prefix = 'damage') {
    ...
    if (regexMatch === null) {
      QuestJs._io.errormsg(
        `Weapon ${this.weapon.name} has a bad damage attribute (${string}).`,
      );
      return;
    }
    this[`${prefix}Number`] =
      regexMatch[1] === '' ? 1 : parseInt(regexMatch[1]);
    this[`${prefix}Sides`] = parseInt(regexMatch[2]);
    this[`${prefix}Bonus`] =
      regexMatch[3] === undefined ? 0 : parseInt(regexMatch[3]);
  ...
  applyDamage(target, prefix = 'damage') {
    ...
    if (this[`${prefix}Bonus`] || this[`${prefix}Number`]) {
      QuestJs._io.msg(
        `Damage: ${this.damageNumber}d${this[`${prefix}Sides`]}+${
          this[`${prefix}Bonus`]
        }`,
        3,
      );
      let damage = this[`${prefix}Bonus`];
      for (let i = 0; i < this[`${prefix}Number`]; i += 1) {
        const roll = QuestJs._random.int(1, this[`${prefix}Sides`]);
        damage += roll;
        QuestJs._io.msg(`Damage roll (d${this[`${prefix}Sides`]}): ${roll}`, 5);
      }
      QuestJs._io.msg(`Damage before armour: ${damage}`, 4);
      QuestJs._io.msg(`Armour: ${target.armour}`, 4);
      damage -=
        this[`${prefix}Number`] *
        (target.armour * this.armourMultiplier + this.armourModifier);
      QuestJs._io.msg(`Damage after armour: ${damage}`, 4);
      if (damage < 1) damage = 1;
      QuestJs._io.msg(`Damage before multiplier: ${damage}`, 6);
      const modifiedDamage = this[`${prefix}Multiplier`] * damage;
      QuestJs._io.msg(`Damage: ${modifiedDamage}`, 1);
      target.health -= modifiedDamage;
      QuestJs._io.msg(`Health now: ${target.health}`, 2);
    }
  }
    ...
    for (const el of source.activeEffects) {
      ...
      if (!effect)
        return QuestJs._io.errormsg(
          `applyActiveEffects: Failed to find skill [${el}]`,
        );
      if (effect.modifyOutgoingAttack)
        effect.modifyOutgoingAttack(this, source);
    }
  }
}
...
rpg.getFoes = function (target) {
  const l = QuestJs._scope
    .scopeHereListed()
    .filter((el) => el.npc && el.isHostile() && el !== target);
...
const RPG_TEMPLATE = {
  ...
  attack(isMultiple, char) {
    // Create an attack, based on the current skill, weapon and given target
    const attack = Attack.createAttack(QuestJs._game.player, this)
      .apply()
      .output();

    // QuestJs._io.msg(attack.result.join(''))
    return true;
  },
  ...
  modifyOutgoingAttack(attack) {
    if (!attack.skill.statForOffensiveBonus) {
      ...
      QuestJs._io.errormsg(
        `ERROR: Trying to use a stat the character ${this.name} does not have: ${attack.statForOffensiveBonus}`,
      );
    }
  },
...
const RPG_PLAYER = function () {
  const res = QuestJs._templates.PLAYER();

  for (const key in RPG_TEMPLATE) res[key] = RPG_TEMPLATE[key];

  // res.getEquippedWeapon = function() { return this.equipped ? QuestJs._w[this.equipped] : QuestJs._w.weapon_unarmed; }

  res.getEquippedWeapon = function () {
    const carried = QuestJs._scope.scopeHeldBy(this);
    return (
      carried.find((el) => el.equipped && el.weapon) ||
      QuestJs._w.weapon_unarmed
    );
  };

  res.getArmour = function () {
    const garments = QuestJs._scope.scopeHeldBy(this).filter((el) => el.worn);
    let armour = 0;
    for (const el of garments) armour += el.getArmour();
    return armour / QuestJs._settings.armourScaling;
  };
...
const RPG_NPC = function (female) {
  const res = QuestJs._npc.NPC(female);
  ...
  res.onCreation = function (o) {
    ...
    o.verbFunctions.push((o, verbList) => {
      verbList.push(QuestJs._lang.verbs.attack);
    });
    o.nameModifierFunctions.push((o, list) => {
      if (o.dead) list.push(QuestJs._lang.invModifiers.dead);
    });
  };
  ...
  res.modifyIncomingAttack = function (attack) {
    if (attack.element || attack.isMagic || attack.spell) {
      ...
      attack.primarySuccess = attack.primarySuccess.replace(
        /[.!]/,
        ', but it passes straight through {sb:target}.'
      );
    }
  };
...
QuestJs._npc.agenda.attack = function (npc, arr) {
  // do stuff
  return false;
};

const EQUIPPABLE = function () {
  const res = $.extend({}, QuestJs._templates.TAKEABLE_DICTIONARY);
  ...
  (res.activeEffects = []),
    (res.getObstructing = function (char) {
      return QuestJs._scope
      .scopeHeldBy(char)
      .filter((el) => el.equipped && this.match(el));
    });
  ...
  res.onCreation = function (o) {
    ...
    o.verbFunctions.push((o, verbList) => {
      if (o.isAtLoc(QuestJs._game.player)) {
        verbList.push(
          o.equipped ? QuestJs._lang.verbs.unequip : QuestJs._lang.verbs.equip,
        );
      }
    });
    o.nameModifierFunctions.push((o, list) => {
      if (o.equipped && o.isAtLoc(QuestJs._game.player.name))
        list.push(QuestJs._lang.invModifiers.equipped);
    });
  };
  ...
  res.equip = function (isMultiple, char) {
    ...
    if (equipped.length === 0) {
      QuestJs._io.msg('{nv:char:draw:true} {nm:item:the}.', {
        char,
        item: this,
      });
    } else if (equipped.length === 1) {
      const params = { char, item: this };
      QuestJs._io.msg(
        `{nv:char:put:true} away ${QuestJs._tools.formatList(equipped, {
          article: QuestJs._consts.DEFINITE,
          joiner: ' and ',
        })}, and equip {nm:item:the}.`,
        params,
      );
      for (const el of equipped) delete el.equipped;
    }
    this.equipped = true;
    return true;
  };
  ...
  res.unequip = function (isMultiple, char) {
    ...
    delete this.equipped;
    const params = { char, weapon: this };
    QuestJs._io.msg('{nv:char:put:true} away {nm:weapon:the}.', params);
    return true;
  };
...
skills.add(
  new MetaSkill('Limited ammo', {
    modifyOutgoingAttack(attack) {
      if (attack.weapon.ammo === 0) {
        attack.msg('Out of ammo!');
        attack.abort = true;
      } else {
        attack.weapon.ammo -= 1;
      }
    },
  }),
);
...
const SPELLBOOK = function (list) {
  ...
  res.examine = function () {
    QuestJs._io.msg(
      `${this.examineX} It contains the spells ${QuestJs._tools.formatList(
        this.spellsAvailableToLearn.map((el) => `<i>${el}</i>`),
...
QuestJs._create.createItem('weapon_unarmed', WEAPON(), {
  loc: 'me',
  image: 'fist',
  damage: 'd4',
  offensiveBonus: -2,
  ...
  isAtLoc(loc, situation) {
    if (situation !== QuestJs._world.SCOPING) return false;
    if (typeof loc !== 'string') loc = loc.name;
    return this.loc === loc;
  },
});

QuestJs._create.createItem('spell_handler', {
  eventPeriod: 1,
  eventActive: true,
  eventScript() {
    const objs = QuestJs._scope.scopeBy((o) => o.activeEffects !== undefined);
    // QuestJs._log.info(objs)
    for (const el of objs) {
      for (const spellName of el.activeEffects) {
        if (el[`countdown_${spellName}`]) {
          el[`countdown_${spellName}`] -= 1;
          if (el[`countdown_${spellName}`] <= 0) {
            QuestJs._io.msg(skills.terminate(skills.findName(spellName), el));
          }
        }
      }
    }
    ...
    default(item, isMultiple, char) {
      QuestJs._io.msg(
        `${QuestJs._tools.prefix(
          item,
          isMultiple,
        )}No point attacking ${QuestJs._lang.getName(item, {
          article: QuestJs._consts.DEFINITE,
        })}.`,
      );
      return false;
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('Equip', {
    npcCmd: true,
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('Unequip', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHeld],
...
QuestJs._world.isSpellAvailable = function (char, spell) {
  for (const el of QuestJs._scope.scopeHeldBy(char)) {
    if (
      el.spellsAvailableToLearn &&
      el.spellsAvailableToLearn.includes(spell.name)
    ) {
      return el;
    }
  }
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('LearnSpell', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(learn) (.+)$/,
    objects: [{ ignore: true }, { text: true }],
    script(objects) {
      const spell = skills.find(objects[0]);
      if (!spell || !spell.spell)
        return QuestJs._io.failedmsg(`There is no spell called ${objects[0]}.`);

      const source = QuestJs._world.isSpellAvailable(
        QuestJs._game.player,
        spell,
      );
      if (!source)
        return QuestJs._io.failedmsg(
          `You do not have anything you can learn {i:${spell.name}} from.`,
        );

      QuestJs._game.player.skillsLearnt.push(spell.name);
      QuestJs._io.msg(
        `You learn {i:${spell.name}} from ${QuestJs._lang.getName(source, {
          article: QuestJs._consts.DEFINITE,
        })}.`,
      );
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('CastSpell', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    regex: /^(cast|invoke) (.+)$/,
    objects: [{ ignore: true }, { text: true }],
    script(objects) {
      ...
      if (!QuestJs._game.player.skillsLearnt.includes(spell.name))
        return QuestJs._io.failedmsg(
          `You do not know the spell {i:${spell.name}}.`,
        );

      if (!spell.noTarget)
        return QuestJs._io.failedmsg(
          `You need a target for the spell {i:${spell.name}}.`,
        );

      const attack = Attack.createAttack(
        QuestJs._game.player,
        QuestJs._game.player,
        spell,
      )
      ...
      // return skills.castSpell(spell, QuestJs._game.player, QuestJs._game.player)
    },
  }),
);

QuestJs._commands.push(
  new QuestJs._command.Cmd('CastSpellAt', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHere],
    ...
    script(objects) {
      ...
      if (spell.damage && target.health === undefined)
        return QuestJs._io.failedmsg("You can't attack that.");

      const attack = Attack.createAttack(QuestJs._game.player, target, spell)
        .apply()
        .output();
      return QuestJs._world.SUCCESS;
    },
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('DebugRPG', {
    npcCmd: true,
    rules: [QuestJs.cmdRules.isHeld],
    regex: /^(rpg)$/,
    objects: [{ ignore: true }],
    script(objects) {
      ...
      QuestJs._io.metamsg('All output from attacks will now be seen.');
