...
const skills = {
  ...
  terminate:function(spell, item) {
    ...
    let s = "The {i:" + spell.name + "} spell" + (item === game.player ? '' : " on " + QuestJs._lang.getName(item, {article:QuestJs._consts.DEFINITE})) + " terminates."
...
const EQUIPPABLE = function() {
  ...
  res.onCreation = function(o) {
    ...
    o.nameModifierFunctions.push(function(o, list) {
      if (o.equipped && o.isAtLoc(game.player.name)) list.push(QuestJs._lang.invModifiers.equipped)
    })
  }
...
QuestJs._commands.push(new QuestJs._command.Cmd('CastSpell', {
  ...
  script:function(objects) {
    ...
    if (!game.player.skillsLearnt.includes(spell.name)) return QuestJs._io.failedmsg("You do not know the spell {i:" + spell.name + "}.")
