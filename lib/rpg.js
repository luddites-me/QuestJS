...
const skills = {
  ...
  terminate:function(spell, item) {
    ...
    let s = "The {i:" + spell.name + "} spell" + (item === game.player ? '' : " on " + QuestJs._lang.getName(item, {article:DEFINITE})) + " terminates."
...
const EQUIPPABLE = function() {
  ...
  res.equip = function(isMultiple, char) {
    ...
    if (equipped.length === 0) {
      ...
      QuestJs._io.msg("{nv:char:put:true} away " + formatList(equipped, {article:DEFINITE, joiner:" and "}) + ", and equip {nm:item:the}.", params)
...
const SPELLBOOK = function(list) {
  ...
  res.examine = function() {
    QuestJs._io.msg(this.examineX + ' It contains the spells ' + formatList(this.spellsAvailableToLearn.map(el => '<i>' + el + '</i>'), {lastJoiner:QuestJs._lang.list_and}) + '.')
  }
  res.icon = () => 'spell12'
  return res 
}
...
QuestJs._commands.push(new QuestJs._command.Cmd('Attack', {
  ...
  default:function(item, isMultiple, char) {
    QuestJs._io.msg(prefix(item, isMultiple) + "No point attacking " + QuestJs._lang.getName(item, {article:DEFINITE}) + ".");
    return false;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('Equip', {
  ...
  default:function(item, isMultiple, char) {
    QuestJs._io.msg(prefix(item, isMultiple) + QuestJs._lang.pronounVerb(item, "be", true) + " not something you can equip.");
    return false;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('LearnSpell', {
  ...
  script:function(objects) {
    ...
    QuestJs._io.msg("You learn {i:" + spell.name + "} from " + QuestJs._lang.getName(source, {article:DEFINITE}) + ".")
