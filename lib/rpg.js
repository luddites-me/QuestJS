...
const skills = {
  ...
  terminate:function(spell, item) {
    ...
    let s = "The {i:" + spell.name + "} spell" + (item === game.player ? '' : " on " + QuestJs.LANG.getName(item, {article:DEFINITE})) + " terminates."
...
const RPG_NPC = function(female) {
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(QuestJs.LANG.verbs.attack)
    })
    o.nameModifierFunctions.push(function(o, list) {
      if (o.dead) list.push(QuestJs.LANG.invModifiers.dead)
    })
  }
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      if (o.isAtLoc(game.player)) {
        verbList.push(o.equipped ? QuestJs.LANG.verbs.unequip : QuestJs.LANG.verbs.equip)
      }
    })
    o.nameModifierFunctions.push(function(o, list) {
      if (o.equipped && o.isAtLoc(game.player.name)) list.push(QuestJs.LANG.invModifiers.equipped)
    })
  }
...
const SPELLBOOK = function(list) {
  ...
  res.examine = function() {
    msg(this.examineX + ' It contains the spells ' + formatList(this.spellsAvailableToLearn.map(el => '<i>' + el + '</i>'), {lastJoiner:QuestJs.LANG.list_and}) + '.')
  }
  res.icon = () => 'spell12'
  return res 
}
...
commands.push(new QuestJs.command.Cmd('Attack', {
  npcCmd:true,
  ...
  default:function(item, isMultiple, char) {
    msg(prefix(item, isMultiple) + "No point attacking " + QuestJs.LANG.getName(item, {article:DEFINITE}) + ".");
    return false;
  },
}));


commands.push(new QuestJs.command.Cmd('Equip', {
  npcCmd:true,
  ...
  default:function(item, isMultiple, char) {
    msg(prefix(item, isMultiple) + QuestJs.LANG.pronounVerb(item, "be", true) + " not something you can equip.");
    return false;
  },
}));


commands.push(new QuestJs.command.Cmd('Unequip', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHeld],
...
commands.push(new QuestJs.command.Cmd('LearnSpell', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(learn) (.+)$/,
  objects:[
  ...
  script:function(objects) {
    ...
    msg("You learn {i:" + spell.name + "} from " + QuestJs.LANG.getName(source, {article:DEFINITE}) + ".")
...
commands.push(new QuestJs.command.Cmd('CastSpell', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
  regex:/^(cast|invoke) (.+)$/,
  objects:[
...
commands.push(new QuestJs.command.Cmd('CastSpellAt', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHere],
...
commands.push(new QuestJs.command.Cmd('DebugRPG', {
  npcCmd:true,
  rules:[QuestJs.cmdRules.isHeld],
  regex:/^(rpg)$/,
  objects:[
