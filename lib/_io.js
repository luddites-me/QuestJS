...
function sound(filename) {
  //console.log(QuestJs._settings.ssFolder)
...
function draw(width, height, data, options) {
  ...
  s += data.join('') + '</svg>'
  if (QuestJs._settings.reportAllSvg) console.log(s.replace(/></g, '>\n<'))
...
function createToolbar() {
  ...
  if (QuestJs._settings.toolbar.roomdisplay) {
    html += ' <div>' + sentenceCase(QuestJs._lang.getName(w[game.player.loc], { article:DEFINITE })) + '</div>';
  }
  html += '</div>';
...
QuestJs._IO.input = function(title, options, allowText, reactFunction, displayFunction) {
  ...
  if (test.testing) {
    if (test.menuResponseNumber === undefined) {
      ...
      QuestJs._IO.menuResponse(test.menuResponseNumber);
  ...
  if (QuestJs._settings.walkthroughMenuResponses.length > 0) {
    ...
    console.log("QuestJs._settings.walkthroughMenuResponses.length: " + QuestJs._settings.walkthroughMenuResponses.length)
...
QuestJs._IO.enable = function(level) {
  //console.log('enable ' + level + ' (' + QuestJs._IO.disableLevel + ')')
  if (!level) level = 1
    ...
    for (let st of QuestJs._settings.status) {
      if (typeof st === "string") {
        if (game.player[st] !== undefined) {
          let s = '<tr><td width="' + QuestJs._settings.statusWidthLeft + '">' + sentenceCase(st) + "</td>";
...
QuestJs._IO.clickExit = function(dir) {
  ...
  let cmd = QuestJs._IO.getCommand("Go" + sentenceCase(dir))
  if (!cmd) cmd = QuestJs._IO.getCommand(sentenceCase(dir))
  if (!cmd) cmd = QuestJs._IO.getCommand("Meta" + sentenceCase(dir))
...
QuestJs._IO.createPanes = function() {
  if (!['right', 'left', 'none'].includes(QuestJs._settings.panes)) {
    console.error('ERROR: Your QuestJs._settings.panes value is "' + QuestJs._settings.panes + '". It must be one of "right", "left" or "none" (all lower-case). It is probably set in the file setiings.js.')
    return
  }
