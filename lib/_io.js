...
function wait(delay, text) {
  ...
  if (delay === undefined) {
    QuestJs._IO.addToOutputQueue({ action: 'wait', text, cssClass: 'continue' });
  } else {
...
QuestJs._IO.typewriterEffect = function (data) {
  ...
  const el = $(`#n${data.id}`);
  el.html(
    `${data.text.slice(0, data.position)}<span class="typewriter-active">${data.text.slice(
      data.position,
      data.position + 1,
    )}</span>`,
  );
...
QuestJs._IO.getCommand = function (name) {
  const found = QuestJs._commands.find((el) => el.name === name);
  return found;
};
    ...
    slide(event, ui) {
      ...
      $(`#${id}-text`).html(
        `${QuestJs._settings.sliders[id].name}: ${
          QuestJs._settings.sliders[id].values ? QuestJs._settings.sliders[id].values[value] : value
        }`,
      );
