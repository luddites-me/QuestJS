...
function errormsg(s) {
  if (world.isCreated) {
    io.print({tag:'p', cssClass:"error", text:QuestJs.LANG.error})
  }
...
function showYesNoMenu(title, fn) {
  showMenu(title, QuestJs.LANG.yesNo, fn)
}

function showYesNoMenuWithNumbers(title, fn) {
  showMenuWithNumbers(title, QuestJs.LANG.yesNo, fn)
}

function showYesNoDropDown(title, fn) {
  showDropDown(title, QuestJs.LANG.yesNo, fn)
}
...
function endTurnUI(update) {
  if (settings.panes !== 'none' && update) {
    // set the QuestJs.LANG.exit_list
...
function createToolbar() {
  ...
  if (settings.toolbar.roomdisplay) {
    html += ' <div>' + sentenceCase(QuestJs.LANG.getName(w[game.player.loc], { article:DEFINITE })) + '</div>';
  }
  html += '</div>';
...
const io = {
  ...
  scriptShow:function(opts) {
    ...
    if (opts === 'w') {
      ...
      let html = '';
      html += '<!DOCTYPE html><html QuestJs.LANG="en"><head><meta charset="utf-8"/><title>Quest 6 Transcript for '
      html += settings.title
...
io.outputFromQueue = function() {
  ...
  if (data.action === 'wait') {
    ...
    if (!data.text) data.text = QuestJs.LANG.click_to_continue
    io.print(data)
  }
...
io.createPanes = function() {
  ...
  document.writeln(QuestJs.LANG.game_over_html);
  document.writeln('</div>');
  document.writeln('</div>');
...
io.writeExit = function(n) {
  document.write('<td class="compass-button" title="' + QuestJs.LANG.exit_list[n].name + '">')
  document.write('<span class="compass-button" id="exit-' + QuestJs.LANG.exit_list[n].name)
  document.write('" onclick="io.clickExit(\'' + QuestJs.LANG.exit_list[n].name + '\')">')
  document.write(settings.symbolsForCompass ? io.displayIconsCompass(QuestJs.LANG.exit_list[n]) : QuestJs.LANG.exit_list[n].abbrev);
...
io.againOrOops = function(isAgain) {
  if (io.savedCommands.length === 0) {
    metamsg(QuestJs.LANG.again_not_available)
    return world.FAILED
  }
