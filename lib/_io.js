...
function createToolbar() {
  ...
  if (QuestJs._settings.toolbar.roomdisplay) {
    html += ' <div>' + QuestJs._tools.sentenceCase(QuestJs._lang.getName(w[game.player.loc], { article:QuestJs._consts.DEFINITE })) + '</div>';
  }
  html += '</div>';
    ...
    for (let st of QuestJs._settings.status) {
      if (typeof st === "string") {
        if (game.player[st] !== undefined) {
          ...
          s += '<td width="' + QuestJs._settings.statusWidthRight + '">' + game.player[st] + "</td></tr>";
