...
function _msg(s, params, options) {
  ...
  const processed = params ? QuestJs._text.processText(s, params).trim() : s.trim();
...
function picture(filename, width, height) {
  const src = filename.includes('/') ? filename : QuestJs._settings.imagesFolder + filename
...
function video(filename) {
  //QuestJs._log.info(QuestJs._settings.ssFolder)
  _msg('Your browser does not support the <code>video</code> element.', {}, {action:'output', autoplay:true, tag:'video', src:QuestJs._settings.videoFolder + '/' + filename})
}
...
// It does not use the text processor.
function debugmsg(s) {
  if (QuestJs._settings.playMode === 'dev' || QuestJs._settings.playMode === 'meta') {
...
function wait(delay, text) {
  ...
  if (delay === undefined) {
    QuestJs._IO.addToOutputQueue({action:'wait', text:text, cssClass:'continue'})
  }
  else {
    QuestJs._IO.addToOutputQueue({action:'delay', delay:delay, text:text, cssClass:'continue'})
  }
}
...
function showMenu(title, options, fn) {
  ...
  QuestJs._IO.input(title, options, false, fn, function(options) {
...
function showMenuWithNumbers(title, options, fn) {
  const opts = {article:QuestJs._consts.DEFINITE, capital:true}
  QuestJs._parser.overrideWith(function(s) {QuestJs._IO.menuResponse(s)})
  QuestJs._IO.input(title, options, true, fn, function(options) {
    for (let i = 0; i < options.length; i++) {
      let s = (i + 1) + '. <a class="menu-option" onclick="QuestJs._IO.menuResponse(' + i + ')">';
      s += (typeof options[i] === 'string' ? options[i] : QuestJs._lang.getName(options[i], opts))
      s += '</a>';
      msg(s);
    }
...
function showDropDown(title, options, fn) {
  ...
  QuestJs._IO.input(title, options, false, fn, function(options) {
    ...
    s += 'onchange=\"QuestJs._IO.menuResponse($(\'#menu-select\').find(\':selected\').val())\">';
...
function endTurnUI(update) {
  if (QuestJs._settings.panes !== 'none' && update) {
    ...
    for (let exit of QuestJs._lang.exit_list) {
      if (QuestJs._game.room.hasExit(exit.name, {excludeScenery:true}) || exit.type === 'nocmd') {
        $('#exit-' + exit.name).show();
      }
      else {
        $('#exit-' + exit.name).hide();
  ...
  for (let o of QuestJs._IO.modulesToUpdate) {
    o.update(update)
  }
...
function createToolbar() {
  ...
  if (QuestJs._settings.toolbar.roomdisplay) {
    html += ' <div>' + QuestJs._tools.sentenceCase(QuestJs._lang.getName(QuestJs._w[QuestJs._game.player.loc], { article:QuestJs._consts.DEFINITE })) + '</div>';
  }
  html += '</div>';
    
  html += '<div class="links">'
  for (let link of QuestJs._settings.toolbar.buttons) {
...
QuestJs._IO = {
  ...
  scriptShow:function(opts) {
    ...
    if (opts === 'w') {
      ...
      let html = '';
      html += '<!DOCTYPE html><html QuestJs._lang="en"><head><meta charset="utf-8"/><title>Quest 6 Transcript for '
...
QuestJs._IO.input = function(title, options, allowText, reactFunction, displayFunction) {
  ...
  if (QuestJs._test.testing) {
    if (QuestJs._test.menuResponseNumber === undefined) {
      debugmsg("Error when testing menu (possibly due to disambiguation?), QuestJs._test.menuResponseNumber = " + QuestJs._test.menuResponseNumber);
    }
    else {
  ...
  if (QuestJs._settings.walkthroughMenuResponses.length > 0) {
    ...
    QuestJs._log.info("QuestJs._settings.walkthroughMenuResponses.length: " + QuestJs._settings.walkthroughMenuResponses.length)
...
QuestJs._IO.allowedHtmlAttrs = ['width', 'height', 'onclick', 'src', 'autoplay' ]
...
QuestJs._IO.print = function(data) {
  ...
  if (data.html) {
    ...
    for (let s of QuestJs._IO.allowedHtmlAttrs) if (data[s]) html += ' ' + s + '="' + data[s] + '"'
...
QuestJs._IO.typewriterEffect = function(data) {
  ...
  setTimeout(QuestJs._IO.forceOutputFromQueue, QuestJs._settings.textEffectDelay)
}
...
QuestJs._IO.unscamblePick = function() {
  let c = String.fromCharCode(QuestJs._random.int(33, 125))
  return c === '<' ? '~' : c
}
...
QuestJs._IO.setTitleAndInit = function(s) {
  document.title = s
  for (let o of QuestJs._IO.modulesToInit) {
    o.init()
  }
...
QuestJs._IO.calcMargins = function() {
  ...
  if (typeof imagePane !== 'undefined') {
    if (!QuestJs._settings.hideImagePane && QuestJs._settings.imageWidth > mapImageWidth) mapImageWidth = QuestJs._settings.imageWidth
  }
  ...
  if (QuestJs._settings.panes !== 'none') {
    ...
    if (QuestJs._IO.resizePanesListener.matches) { // If media query matches
      // hide sidepane
      $('#main').css(margin, (QuestJs._IO.mainGutter) + 'px')
      $('#panes').css('display', 'none')
    } else {
      // show sidepane
      $('#main').css(margin, (QuestJs._IO.panesWidth + QuestJs._IO.mainGutter) + 'px')
      $('#panes').css('display', 'block')
    }
  }

  let margin = QuestJs._settings.panes === 'right' ? 'margin-left' : 'margin-right'
  if (QuestJs._settings.mapImageSide) margin = QuestJs._settings.mapImageSide === 'left' ? 'margin-left' : 'margin-right'
  if (QuestJs._IO.resizeMapImageListener.matches) { // If media query matches
    // hide image
    $('#main').css(margin, (QuestJs._IO.mainGutter) + 'px')
...
QuestJs._IO.resizePanesListener = window.matchMedia('(max-width: ' + QuestJs._settings.panesCollapseAt + 'px)')
QuestJs._IO.resizeMapImageListener = window.matchMedia('(max-width: ' + QuestJs._settings.mapAndImageCollapseAt + 'px)')
...
QuestJs._IO.updateUIItems = function() {
  if (QuestJs._settings.panes === 'none' || !QuestJs._settings.inventoryPane) { return; }

  for (let inv of QuestJs._settings.inventoryPane) {
    $('#' + inv.alt).empty();
  }
...
QuestJs._IO.updateStatus = function() {
  if (QuestJs._settings.statusPane) {
    $("#status-pane").empty();
    for (let st of QuestJs._settings.status) {
      if (typeof st === "string") {
        if (QuestJs._game.player[st] !== undefined) {
          let s = '<tr><td width="' + QuestJs._settings.statusWidthLeft + '">' + QuestJs._tools.sentenceCase(st) + "</td>";
...
QuestJs._IO.menuResponse = function(n) {
  ...
  for (let i = QuestJs._IO.menuStartId; i < QuestJs._IO.nextid; i++) $('#n' + i).remove()
  if (n === undefined) {
    QuestJs._IO.menuFn()
  }
  else if (n !== -1) {
    if (QuestJs._IO.transcript) QuestJs._IO.scriptAppend({cssClass:'menu', text:(QuestJs._IO.menuOptions[n].alias ? QuestJs._IO.menuOptions[n].alias : QuestJs._IO.menuOptions[n]), n:n});
...
QuestJs._IO.clickItem = function(itemName) {
  ...
  for (let item of QuestJs._IO.currentItemList) {
    if (item === itemName) {
      $('.' + item + '-actions').toggle();
    }
    else {
...
QuestJs._IO.appendItem = function(item, htmlDiv, loc, isSubItem) {
  $('#' + htmlDiv).append('<div id="' + item.name + '-item"><p class="item' + (isSubItem ? ' sub-item' : '') + '" onclick="QuestJs._IO.clickItem(\'' + item.name + '\')">' + QuestJs._IO.getIcon(item) + item.getListAlias(loc) + "</p></div>");
  QuestJs._IO.currentItemList.push(item.name);
  const verbList = item.getVerbs(loc);
  if (verbList === undefined) { errormsg("No verbs for " + item.name); QuestJs._log.info(item); }
  ...
  if (item.container && !item.closed) {
    if (typeof item.getContents !== 'function') {
      QuestJs._log.info("item flagged as container but no getContents function:");
...
QuestJs._IO.createPanes = function() {
  if (!['right', 'left', 'none'].includes(QuestJs._settings.panes)) {
    QuestJs._log.error('ERROR: Your QuestJs._settings.panes value is "' + QuestJs._settings.panes + '". It must be one of "right", "left" or "none" (all lower-case). It is probably set in the file setiings.js.')
    return
  }

  $('#input').html('<span id="cursor">' + QuestJs._settings.cursor + '</span><input type="text" name="textbox" id="textbox"  autofocus/>')

  if (QuestJs._settings.panes === 'none') { return; }
  document.writeln('<div id="panes" class="side-panes side-panes' + QuestJs._settings.panes + ' panes-narrow">');
  ...
  if (QuestJs._settings.statusPane) {
    document.writeln('<div class="pane-div">');
    document.writeln('<h4 class="side-pane-heading">' + QuestJs._settings.statusPane + '</h4>');
...
QuestJs._IO.writeExit = function(n) {
  document.write('<td class="compass-button" title="' + QuestJs._lang.exit_list[n].name + '">')
  document.write('<span class="compass-button" id="exit-' + QuestJs._lang.exit_list[n].name)
  document.write('" onclick="QuestJs._IO.clickExit(\'' + QuestJs._lang.exit_list[n].name + '\')">')
  document.write(QuestJs._settings.symbolsForCompass ? QuestJs._IO.displayIconsCompass(QuestJs._lang.exit_list[n]) : QuestJs._lang.exit_list[n].abbrev);
...
QuestJs._IO.getCommand = function(name) {
  const found = QuestJs._commands.find(function(el) {
    return el.name === name;
  });
  return found;
};
...
QuestJs._IO.msgInputText = function(s) {
  ...
  $("#output").append('<p id="n' + QuestJs._IO.nextid + '" class="input-text">&gt; ' + s + "</p>");
  QuestJs._IO.nextid++;
...
$(document).ready(function() {
  $('#textbox').keydown(function(event){
    const keycode = (event.keyCode ? event.keyCode : event.which);
      ...
      if(keycode === 13){
        // enter
        if (event.ctrlKey && (QuestJs._settings.playMode === 'dev' || QuestJs._settings.playMode === 'beta')) {
      ...
      if(keycode === 38){
        ...
        if (QuestJs._IO.savedCommandsPos < 0) { QuestJs._IO.savedCommandsPos = 0; }
        $('#textbox').val(QuestJs._IO.savedCommands[QuestJs._IO.savedCommandsPos]);
      ...
      if(keycode === 40){
        // down arrow
        QuestJs._IO.savedCommandsPos += 1;
        if (QuestJs._IO.savedCommandsPos >= QuestJs._IO.savedCommands.length) { QuestJs._IO.savedCommandsPos = QuestJs._IO.savedCommands.length - 1; }
        $('#textbox').val(QuestJs._IO.savedCommands[QuestJs._IO.savedCommandsPos]);
      }
      if(keycode === 27){
        // ESC
      ...
      if(keycode === 96 && (QuestJs._settings.playMode === 'dev' || QuestJs._settings.playMode === 'beta')){
  ...
  if (QuestJs._settings.soundFiles) {
    const main = $('#main')
    for (let el of QuestJs._settings.soundFiles) {
      main.append('<audio id="' + el + '" src="' + QuestJs._settings.soundsFolder + el + QuestJs._settings.soundsFileExt + '"/>')
    }
  }
...
QuestJs._IO.speak = function(str, altVoice){
  if (!QuestJs._IO.voice) {
    QuestJs._IO.voice = QuestJs._IO.synth.getVoices().find(function(el) {
  ...
  if (!QuestJs._IO.voice2) {
    QuestJs._IO.voice2 = QuestJs._IO.synth.getVoices().find(function(el) {
...
QuestJs._IO.showHtml = function(title, html) {
  ...
  $("#showHtml").dialog({
    width: 860,
    close:function() { $("#showHtml").remove(); QuestJs._IO.dialogShowing = false; },
  });
  return true;
}
...
QuestJs._IO.addSlider = function(id, values, func) {
  ...
  QuestJs._settings.sliders[id] = {name:name, max:max, func:func}
  ...
  $('#' + id + '-slider').slider({
    slide: function(event, ui) {
      ...
      $('#' + id + '-text').html(QuestJs._settings.sliders[id].name + ': ' + (QuestJs._settings.sliders[id].values ? QuestJs._settings.sliders[id].values[value] : value))
...
QuestJs._IO.getIcon = function(item) {
  ...
  if (!item.icon) return ''
  if (item.icon() === '') return ''
  return '<img src="' + QuestJs._settings.iconsFolder + (QuestJs._settings.darkModeActive ? 'l_' : 'd_') + item.icon() + '.png" />'
}
...
QuestJs._IO.againOrOops = function(isAgain) {
  ...
  if (isAgain) {
    QuestJs._parser.parse(QuestJs._IO.savedCommands[QuestJs._IO.savedCommands.length - 1])
  }
  else {
    $('#textbox').val(QuestJs._IO.savedCommands[QuestJs._IO.savedCommands.length - 1])
