...
function draw(width, height, data, options) {
  ...
  s += `${data.join('')}</svg>`;
  if (QuestJs._settings.reportAllSvg)
    QuestJs._log.info(s.replace(/></g, '>\n<'));
...
QuestJs._IO.print = function (data) {
  ...
  if (data.html) {
    ...
    for (const s of QuestJs._IO.allowedHtmlAttrs)
...
QuestJs._IO.calcMargins = function () {
  ...
  if (QuestJs._settings.panes !== 'none') {
    const margin =
      QuestJs._settings.panes === 'left' ? 'margin-left' : 'margin-right';
  ...
  let margin =
    QuestJs._settings.panes === 'right' ? 'margin-left' : 'margin-right';
  if (QuestJs._settings.mapImageSide)
    margin =
      QuestJs._settings.mapImageSide === 'left'
        ? 'margin-left'
        : 'margin-right';
...
QuestJs._IO.menuResponse = function (n) {
  ...
  for (let i = QuestJs._IO.menuStartId; i < QuestJs._IO.nextid; i += 1)
    $(`#n${i}`).remove();
  if (n === undefined) {
    QuestJs._IO.menuFn();
  } else if (n !== -1) {
...
QuestJs._IO.clickExit = function (dir) {
  ...
  if (!cmd)
    cmd = QuestJs._IO.getCommand(`Meta${QuestJs._tools.sentenceCase(dir)}`);
...
QuestJs._IO.msgInputText = function (s) {
  ...
  if (QuestJs._IO.transcript)
    QuestJs._IO.scriptAppend({ cssClass: 'input', text: s });
};
...
QuestJs._IO.speak = function (str, altVoice) {
  if (!QuestJs._IO.voice) {
    ...
    if (!QuestJs._IO.voice)
      QuestJs._IO.voice = QuestJs._IO.synth.getVoices()[0];
  }
  if (!QuestJs._IO.voice2) {
    ...
    if (!QuestJs._IO.voice2)
      QuestJs._IO.voice2 = QuestJs._IO.synth.getVoices()[0];
  }
