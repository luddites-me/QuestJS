...
function picture(filename, width, height) {
  ...
  _msg(
    '',
    {},
    { action: 'output', width: width, height: height, tag: 'img', src: src, printBlank: true },
  );
}
...
function ambient(filename, volume) {
  ...
  _msg(
    'Your browser does not support the <code>audio</code> element.',
    {},
    { action: 'ambient', name: filename, volume: volume },
  );
}
...
function showMenuWithNumbers(title, options, fn) {
  const opts = { article: QuestJs._consts.DEFINITE, capital: true };
  QuestJs._parser.overrideWith(function (s) {
...
function createPaneBox(position, title, content) {
  $('div.pane-div:nth-child(' + position + ')').before(
    '<div class="pane-div"><h4 class="side-pane-heading">' +
      title +
      '</h4><div class="">' +
      content +
      '</div></div>',
...
function createToolbar() {
  ...
  if (QuestJs._settings.toolbar.content)
    html += ' <div>' + QuestJs._settings.toolbar.content() + '</div>';
  html += '</div>';
...
QuestJs._IO.input = function (title, options, allowText, reactFunction, displayFunction) {
  ...
  if (QuestJs._test.testing) {
    if (QuestJs._test.menuResponseNumber === undefined) {
      debugmsg(
        'Error when testing menu (possibly due to disambiguation?), QuestJs._test.menuResponseNumber = ' +
          QuestJs._test.menuResponseNumber,
      );
    } else {
  ...
  if (QuestJs._settings.walkthroughMenuResponses.length > 0) {
    ...
    QuestJs._log.info(
      'QuestJs._settings.walkthroughMenuResponses.length: ' +
        QuestJs._settings.walkthroughMenuResponses.length,
    );
...
QuestJs._IO.typewriterEffect = function (data) {
  ...
  const el = $('#n' + data.id);
  el.html(
    data.text.slice(0, data.position) +
      '<span class="typewriter-active">' +
      data.text.slice(data.position, data.position + 1) +
      '</span>',
  );
  data.position++;
...
QuestJs._IO.calcMargins = function () {
  ...
  if (typeof imagePane !== 'undefined') {
    if (!QuestJs._settings.hideImagePane && QuestJs._settings.imageWidth > mapImageWidth)
  ...
  if (QuestJs._settings.mapImageSide)
    margin = QuestJs._settings.mapImageSide === 'left' ? 'margin-left' : 'margin-right';
      ...
      if (typeof st === 'string') {
        if (QuestJs._game.player[st] !== undefined) {
          let s =
            ...
            '">' +
            QuestJs._tools.sentenceCase(st) +
            '</td>';
          s +=
            '<td width="' +
            QuestJs._settings.statusWidthRight +
            '">' +
            QuestJs._game.player[st] +
            '</td></tr>';
...
QuestJs._IO.appendItem = function (item, htmlDiv, loc, isSubItem) {
  $('#' + htmlDiv).append(
    '<div id="' +
      ...
      '-item"><p class="item' +
      (isSubItem ? ' sub-item' : '') +
      '" onclick="QuestJs._IO.clickItem(\'' +
      item.name +
      '\')">' +
      QuestJs._IO.getIcon(item) +
      item.getListAlias(loc) +
      '</p></div>',
  );
  ...
  for (let verb of verbList) {
    ...
    let s =
      '<div class="' +
      item.name +
      '-actions item-action" onclick="QuestJs._IO.clickItemAction(\'' +
      item.name +
      "', '" +
      verb.action +
      '\')">';
...
QuestJs._IO.createPanes = function () {
  if (!['right', 'left', 'none'].includes(QuestJs._settings.panes)) {
    QuestJs._log.error(
      'ERROR: Your QuestJs._settings.panes value is "' +
        QuestJs._settings.panes +
        '". It must be one of "right", "left" or "none" (all lower-case). It is probably set in the file setiings.js.',
    );
    return;
  }
  ...
  $('#input').html(
    '<span id="cursor">' +
      QuestJs._settings.cursor +
      '</span><input type="text" name="textbox" id="textbox"  autofocus/>',
  );
...
$(document).ready(function () {
  $('#textbox').keydown(function (event) {
    ...
    if (keycode === 13) {
      ...
      if (
        event.ctrlKey &&
        (QuestJs._settings.playMode === 'dev' || QuestJs._settings.playMode === 'beta')
      ) {
    ...
    for (let el of QuestJs._settings.soundFiles) {
      main.append(
        '<audio id="' +
          el +
          '" src="' +
          QuestJs._settings.soundsFolder +
          el +
          QuestJs._settings.soundsFileExt +
          '"/>',
      );
    }
  }
