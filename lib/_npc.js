...
const NPC = function(isFemale) {
  ...
  res.topics = function() {
    if (this.askOptions.length === 0 && this.tellOptions.length === 0) {
      metamsg(QuestJs._lang.topics_no_ask_tell);
    ...
    for (let action of ['ask', 'tell']) {
      ...
      if (arr2.length !== 0) {
        metamsg(QuestJs._lang['topics_' + action + '_list'], {item:this, list:arr2.sort().join('; ')});
        flag = true;
      }
    }

    if (!flag) {
      metamsg(QuestJs._lang.topics_none_found, {item:this});
    }
...
const CONSULTABLE = function() {
  ...
  res.askTellDone = function(params, response) {
    if (!response) {
      msg(QuestJs._lang.npc_no_interest_in, params)
      return
    }
