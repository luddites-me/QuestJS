...
const AGENDA_FOLLOWER = function() {
  ...
  res.doAgenda = function() {
    ...
    if (this.followers.length !== 0) {
      ...
      this.alias = formatList(this.followers, {lastJoiner:QuestJs._lang.list_and});
...
QuestJs.npc.AGENDA_FOLLOWER = AGENDA_FOLLOWER;

const agenda = {
  debug:function(s, npc, arr) {
    if (QuestJs._settings.agendaDebugging && QuestJs._settings.playMode === 'dev') QuestJs._io.debugmsg('AGENDA for ' + npc.name + ': ' + s + '; ' + formatList(arr, {doNotSort:true}))
  },
  debugS:function(s) {
  ...
  setItemAtt:function(npc, arr) {
    ...
    const att = arr.shift();
    let value = arr.shift();
    if (!w[item]) QuestJs._io.errormsg("Item '" + item + "' not recognised in the agenda of " + npc.name);
  ...
  moveItem:function(npc, arr) {
    ...
    if (!w[item]) QuestJs._io.errormsg("Item '" + item + "' was not recognised in the agenda of " + npc.name);
    if (!w[dest]) QuestJs._io.errormsg("Location '" + dest + "' was not recognised in the agenda of " + npc.name);
  ...
  moveTo:function(npc, arr) {
    ...
    if (!w[dest]) QuestJs._io.debugmsg("Location '" + dest + "' not recognised in the agenda of " + npc.name);
    if (!w[dest].room) dest = dest.loc;
    if (!w[dest]) QuestJs._io.errormsg("Location '" + dest + "' not recognized in the agenda of " + npc.name);
  ...
  walkRandom:function(npc, arr) {
    ...
    if (!w[exit.name]) QuestJs._io.errormsg("Location '" + exit.name + "' not recognised in the agenda of " + npc.name);
