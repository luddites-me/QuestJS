...
const NPC = function (isFemale) {
  ...
  res.isAtLoc = function (loc, situation) {
    ...
    if (situation === QuestJs._world.SIDE_PANE && this === QuestJs._game.player)
      return false;
    return this.loc === loc;
  };
...
const CONSULTABLE = function () {
  ...
  (res.askabout = function (text1, text2) {
    ...
    (res.asktellabout = function (text1, text2, intro, list, action) {
      ...
      if (QuestJs._settings.givePlayerAskTellMsg)
      QuestJs._io.msg(intro(this, text1, text2));
  ...
  return {
    ...
    runscript() {
      ...
      for (const s of this.nowShow) {
        obj = QuestJs._w[s];
        if (obj === undefined)
          QuestJs._io.errormsg(`No topic called ${s} found.`);
        obj.showTopic = true;
      }
