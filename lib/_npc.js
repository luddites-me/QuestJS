...
const NPC = function (isFemale) {
  ...
  res.pronouns = isFemale
    ? QuestJs._lang.pronouns.female
    : QuestJs._lang.pronouns.male;
  ...
  res.topics = function () {
    ...
    for (const action of ['ask', 'tell']) {
      const arr = QuestJs._util.getResponseList(
        { actor: this, action },
        this[`${action}Options`],
      );
      const arr2 = [];
      for (const res of arr) {
        if (
          res.silent
          && !QuestJs._game.player.mentionedTopics.includes(res.name)
        ) continue;
        arr2.push(res.name);
      }
      if (arr2.length !== 0) {
...
const npc_utilities = {
  talkto() {
    ...
    if (QuestJs._settings.dropdownForConv) {
      QuestJs._io.showDropDown(
        QuestJs._lang.speak_to_menu_title(this),
        topics,
        (result) => {
  ...
  getTopics() {
    ...
    for (const key in QuestJs._w) {
      if (
        QuestJs._w[key].isTopicVisible
        && QuestJs._w[key].isTopicVisible(this)
      ) {
  ...
  res.doAgenda = function () {
    ...
    if (typeof agenda[fn] !== 'function') {
      QuestJs._io.errormsg(
        `Unknown function \`${fn}' in agenda for ${this.name}`,
      );
      return;
    }
...
const agenda = {
  debug(s, npc, arr) {
    if (
      QuestJs._settings.agendaDebugging
      && QuestJs._settings.playMode === 'dev'
    ) {
      QuestJs._io.debugmsg(
        `AGENDA for ${npc.name}: ${s}; ${QuestJs._tools.formatList(arr, {
          doNotSort: true,
        })}`,
      );
    }
  ...
  setItemAtt(npc, arr) {
    ...
    if (!QuestJs._w[item]) {
      QuestJs._io.errormsg(
        `Item '${item}' not recognised in the agenda of ${npc.name}`,
      );
    }
  ...
  wait(npc, arr) {
    ...
    if (isNaN(arr[0])) {
      QuestJs._io.errormsg(
        `Expected wait to be given a number in the agenda of '${npc.name}'`,
      );
    }
  ...
  moveItem(npc, arr) {
    ...
    if (!QuestJs._w[item]) {
      QuestJs._io.errormsg(
        `Item '${item}' was not recognised in the agenda of ${npc.name}`,
      );
    }
    if (!QuestJs._w[dest]) {
      QuestJs._io.errormsg(
        `Location '${dest}' was not recognised in the agenda of ${npc.name}`,
      );
    }
  ...
  moveTo(npc, arr) {
    ...
    if (!QuestJs._w[dest]) {
      QuestJs._io.debugmsg(
        `Location '${dest}' not recognised in the agenda of ${npc.name}`,
      );
    }
    if (!QuestJs._w[dest].room) dest = dest.loc;
    if (!QuestJs._w[dest]) {
      QuestJs._io.errormsg(
        `Location '${dest}' not recognized in the agenda of ${npc.name}`,
      );
    }
  ...
  walkRandom(npc, arr) {
    ...
    if (!QuestJs._w[exit.name]) {
      QuestJs._io.errormsg(
        `Location '${exit.name}' not recognised in the agenda of ${npc.name}`,
      );
    }
  ...
  walkTo(npc, arr) {
    ...
    if (!route) {
      QuestJs._io.errormsg(
        `Location '${dest}' not reachable in the agenda of ${npc.name}`,
      );
    }
  ...
  while (currentList.length > 0 && length < maxlength) {
    ...
    for (const room of currentList) {
      ...
      for (const exit of exits) {
        ...
        if (dest === undefined) {
          QuestJs._io.errormsg(
            `Dest is undefined: ${exit.name} (room ${room.name}). Giving up.`,
          );
        ...
        if (
          dest.pathfinderNote
          && dest.pathfinderNote.id === QuestJs._game.pathID
        ) continue;
