...
const agenda = {
  debug:function(s, npc, arr) {
    if (settings.agendaDebugging && settings.playMode === 'dev') QuestJs._io.debugmsg('AGENDA for ' + npc.name + ': ' + s + '; ' + formatList(arr, {doNotSort:true}))
  },
  debugS:function(s) {
    if (settings.agendaDebugging && settings.playMode === 'dev') QuestJs._io.debugmsg('AGENDA comment: ' + s)
  },
...
const CONSULTABLE = function() {
  ...
  res.asktellabout = function(text1, text2, intro, list, action) {
    ...
    if (settings.noAskTell !== false) {
      QuestJs._io.metamsg(settings.noAskTell);
      return false;
    }
    ...
    if (settings.givePlayerAskTellMsg) QuestJs._io.msg(intro(this, text1, text2));
