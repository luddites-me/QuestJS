...
const NPC = function(isFemale) {
  ...
  const res = Object.assign({}, QuestJs._templates.CHARACTER(), CONSULTABLE(), AGENDA_FOLLOWER());
  ...
  res.topics = function() {
    ...
    for (let action of ['ask', 'tell']) {
      const arr = QuestJs._util.getResponseList({actor:this, action:action}, this[action + 'Options']);
      const arr2 = []
      for (let res of arr) {
      ...
      if (arr2.length !== 0) {
        QuestJs._io.metamsg(QuestJs._lang['topics_' + action + '_list'], {item:this, list:arr2.sort().join('; ')});
        flag = true;
      }
    }
...
const npc_utilities = {
  talkto:function() {
    ...
    if (QuestJs._settings.dropdownForConv) {
      QuestJs._io.showDropDown(QuestJs._lang.speak_to_menu_title(this), topics, function(result) {
      ...
      QuestJs._io.showMenu(QuestJs._lang.speak_to_menu_title(this), topics, function(result) {
        if (result !== QuestJs._lang.never_mind) {
          result.runscript();
        }
      });
...
const AGENDA_FOLLOWER = function() {
  ...
  res.doAgenda = function() {
    ...
    if (this.followers.length !== 0) {
      ...
      this.alias = QuestJs._tools.formatList(this.followers, {lastJoiner:QuestJs._lang.list_and});
...
const agenda = {
  debug:function(s, npc, arr) {
    if (QuestJs._settings.agendaDebugging && QuestJs._settings.playMode === 'dev') QuestJs._io.debugmsg('AGENDA for ' + npc.name + ': ' + s + '; ' + QuestJs._tools.formatList(arr, {doNotSort:true}))
  },
  debugS:function(s) {
    if (QuestJs._settings.agendaDebugging && QuestJs._settings.playMode === 'dev') QuestJs._io.debugmsg('AGENDA comment: ' + s)
  },
  ...
  setItemAtt:function(npc, arr) {
    ...
    const att = arr.shift();
    let value = arr.shift();
    if (!QuestJs._w[item]) QuestJs._io.errormsg("Item '" + item + "' not recognised in the agenda of " + npc.name);
  ...
  wait:function(npc, arr) {
    ...
    if (isNaN(arr[0])) QuestJs._io.errormsg("Expected wait to be given a number in the agenda of '" + npc.name + "'");
  ...
  moveItem:function(npc, arr) {
    ...
    if (!QuestJs._w[item]) QuestJs._io.errormsg("Item '" + item + "' was not recognised in the agenda of " + npc.name);
    if (!QuestJs._w[dest]) QuestJs._io.errormsg("Location '" + dest + "' was not recognised in the agenda of " + npc.name);
  ...
  moveTo:function(npc, arr) {
    ...
    if (!QuestJs._w[dest]) QuestJs._io.debugmsg("Location '" + dest + "' not recognised in the agenda of " + npc.name);
    if (!QuestJs._w[dest].room) dest = dest.loc;
    if (!QuestJs._w[dest]) QuestJs._io.errormsg("Location '" + dest + "' not recognized in the agenda of " + npc.name);
  ...
  walkRandom:function(npc, arr) {
    ...
    if (!QuestJs._w[exit.name]) QuestJs._io.errormsg("Location '" + exit.name + "' not recognised in the agenda of " + npc.name);
  ...
  walkTo:function(npc, arr) {
    ...
    if (!QuestJs._w[dest].room) {
      ...
      if (QuestJs._w[dest] === undefined) {
        QuestJs._io.errormsg("Object location '" + dest + "' not recognised in the agenda of " + npc.name);
        return true;
      }
    }
    if (npc.isAtLoc(dest)) {
      ...
      if (!route) QuestJs._io.errormsg("Location '" + dest + "' not reachable in the agenda of " + npc.name);
...
const CONSULTABLE = function() {
  ...
  res.askabout = function(text1, text2) { return this.asktellabout(text1, text2, QuestJs._lang.ask_about_intro, this.askOptions, "ask"); },
  res.tellabout = function(text1, text2) { return this.asktellabout(text1, text2, QuestJs._lang.tell_about_intro, this.tellOptions, "tell"); },
  res.asktellabout = function(text1, text2, intro, list, action) {
    ...
    if (!list || list.length === 0) {
      QuestJs._io.metamsg(QuestJs._settings.noAskTell);
      return QuestJs._io.errormsg("No " + action + "Options set for " + this.name + " and I think there should at least be default saying why.")
    }
  ...
  res.askTellDone = function(params, response) {
    ...
    if (response.mentions) {
      for (let s of response.mentions) {
        if (!QuestJs._game.player.mentionedTopics.includes(s)) QuestJs._game.player.mentionedTopics.push(s)
      }
    }
    params.actor.pause();
  } 
...
const TOPIC = function(fromStart) {
  const res = {
    ...
    runscript:function() {
      ...
      this.script(obj);
      if (typeof this.nowShow === "string") QuestJs._io.errormsg("nowShow for topic " + this.name + " is a string.");
      for (let s of this.nowShow) {
        obj = QuestJs._w[s];
        if (obj === undefined) QuestJs._io.errormsg("No topic called " + s + " found.");
        obj.showTopic = true;
      }
      if (typeof this.nowHide === "string") QuestJs._io.errormsg("nowHide for topic " + this.name + " is a string.");
