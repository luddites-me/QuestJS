...
const NPC = function(isFemale) {
  ...
  res.pronouns = isFemale ? QuestJs.LANG.pronouns.female : QuestJs.LANG.pronouns.male;
  ...
  res.heading = function(dir) {
    return QuestJs.LANG.go_successful
  };
  ...
  res.moveWithDescription = function(dest) {
    ...
    QuestJs.LANG.npc_leaving_msg(this, dest);
    
    // Move NPC (and followers)
    this.loc = dest;
    for (let follower of this.followers) follower.loc = dest;
    
    QuestJs.LANG.npc_entering_msg(this, origin);
  };
  ...
  res.topics = function() {
    if (this.askOptions.length === 0 && this.tellOptions.length === 0) {
      metamsg(QuestJs.LANG.topics_no_ask_tell);
    ...
    for (let action of ['ask', 'tell']) {
      ...
      if (arr2.length !== 0) {
        metamsg(QuestJs.LANG['topics_' + action + '_list'], {item:this, list:arr2.sort().join('; ')});
        flag = true;
      }
    }

    if (!flag) {
      metamsg(QuestJs.LANG.topics_none_found, {item:this});
    }
...
const npc_utilities = {
  talkto:function() {
    ...
    if (settings.dropdownForConv) {
      showDropDown(QuestJs.LANG.speak_to_menu_title(this), topics, function(result) {
      ...
      showMenu(QuestJs.LANG.speak_to_menu_title(this), topics, function(result) {
        if (result !== QuestJs.LANG.never_mind) {
          result.runscript();
        }
      });
...
const AGENDA_FOLLOWER = function() {
  ...
  res.doAgenda = function() {
    ...
    if (this.followers.length !== 0) {
      ...
      this.pronouns = QuestJs.LANG.pronouns.plural;
      this.followers.unshift(this);
      this.alias = formatList(this.followers, {lastJoiner:QuestJs.LANG.list_and});
...
const CONSULTABLE = function() {
  ...
  res.askabout = function(text1, text2) { return this.asktellabout(text1, text2, QuestJs.LANG.ask_about_intro, this.askOptions, "ask"); },
  res.tellabout = function(text1, text2) { return this.asktellabout(text1, text2, QuestJs.LANG.tell_about_intro, this.tellOptions, "tell"); },
  ...
  res.askTellDone = function(params, response) {
    if (!response) {
      msg(QuestJs.LANG.npc_no_interest_in, params)
      return
    }
