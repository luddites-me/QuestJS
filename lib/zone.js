...
const zoneExit = function(char, dir) {
  ...
  for (let el of this.origin.exits) {
    if (char.positionX === el.x && char.positionY === el.y && dir === el.dir) {
      ...
      if (el.blocking) {
        msg(el.blockedmsg || QuestJs.LANG.not_that_way, tpParams)
        return false
      }
      if (el.isLocked) {
        msg(el.lockedmsg || QuestJs.LANG.locked_exit, tpParams)
        return false
      }
      //console.log(this)
      msg(el.msg || QuestJs.LANG.go_successful, tpParams)
...
const ZONE = function(defaultToBlocked) {
  ...
  res.getDirection = function(char, objX, objY, range) {
    ...
    if (x > 0 && theta <= 22.5 && theta >= -22.5) return QuestJs.LANG.exit_list[7].name
    if (x > 0 && theta <= 67.5 && theta >= 22.5) return QuestJs.LANG.exit_list[2].name
    if (x > 0 && theta >= -67.5 && theta <= -22.5) return QuestJs.LANG.exit_list[12].name
    
    if (x < 0 && theta <= 22.5 && theta >= -22.5) return QuestJs.LANG.exit_list[5].name
    if (x < 0 && theta <= 67.5 && theta >= 22.5) return QuestJs.LANG.exit_list[10].name
    if (x < 0 && theta >= -67.5 && theta <= -22.5) return QuestJs.LANG.exit_list[0].name

    return y > 0 ? QuestJs.LANG.exit_list[1].name : QuestJs.LANG.exit_list[11].name
  }
