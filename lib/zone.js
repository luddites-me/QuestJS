...
const zoneExit = function(char, dir) {
  ...
  for (let el of this.origin.exits) {
    if (char.positionX === el.x && char.positionY === el.y && dir === el.dir) {
      ...
      if (el.blocking) {
        msg(el.blockedmsg || QuestJs._lang.not_that_way, tpParams)
        return false
      }
      if (el.isLocked) {
        msg(el.lockedmsg || QuestJs._lang.locked_exit, tpParams)
        return false
      }
      //console.log(this)
      msg(el.msg || QuestJs._lang.go_successful, tpParams)
