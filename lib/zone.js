...
const ZONE = function(defaultToBlocked) {
  ...
  res.hasExit = function(dir, options) {
    ...
    for (let el of this.exits) {
      ...
      if (QuestJs._game.player.positionX === el.x && QuestJs._game.player.positionY === el.y && dir === el.dir) {
  ...
  res.desc = function() {
    for (let el of this.descs) {
      if (el.when !== undefined) {
        if (el.when(QuestJs._game.player.positionX, QuestJs._game.player.positionY)) return this.getDesc(el)
      }
      else if (el.x !== undefined) {
        if (el.x === QuestJs._game.player.positionX && el.y === QuestJs._game.player.positionY) return this.getDesc(el)
      }
      else {
  ...
  res.getFeatureDescs = function() {
    ...
    for (let name in QuestJs._w) {
      ...
      if (QuestJs._game.player.positionX === el.positionX && QuestJs._game.player.positionY === el.positionY && el.featureLookHere) {
  ...
  res.beforeEnter = function() {
    ...
    for (let name in QuestJs._w) {
      const o = QuestJs._w[name]
      if (o.zoneElsewhere === this.name && o.positionX === QuestJs._game.player.positionX && o.positionY === QuestJs._game.player.positionY) {
        o.loc = this.name
...
const ZONE_FEATURE = function(loc, x, y, range, adjacent) {
  const res = {
    ...
    isAtLoc:function(loc, situation) {
      ...
      if (adjacent) {
        return loc === this.zone && Math.abs(QuestJs._game.player.positionX - this.x) < 2 && Math.abs(QuestJs._game.player.positionY -this.y) < 2
      }
      else {  
        return loc === this.zone && QuestJs._game.player.positionX === this.x && QuestJs._game.player.positionY === this.y
      }
    }
  };
  return res;
