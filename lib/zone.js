const zoneExit = function (char, dir) {
  ...
  for (const el of this.origin.exits) {
  ...
  for (const name in QuestJs._w) {
    const o = QuestJs._w[name];
    if (
      o.zone === this.origin.name &&
      newX === o.x &&
      newY === o.y &&
      o.featureNoExit
    ) {
...
const ZONE = function (defaultToBlocked) {
  ...
  res.desc = function () {
    for (const el of this.descs) {
      if (el.when !== undefined) {
        ...
        if (
          el.x === QuestJs._game.player.positionX &&
          el.y === QuestJs._game.player.positionY
        )
          return this.getDesc(el);
      } else {
    ...
    return `ERROR: No description found for zone at x=${QuestJs._game.player.positionX}, y=${QuestJs._game.player.positionY}`;
  };

  res.getDesc = function (el) {
    return (
      (typeof el.desc === 'function' ? el.desc() : el.desc) +
      this.getFeatureDescs()
    );
  };
  ...
  res.getFeatureDescs = function () {
    ...
    for (const el of this.getBorders()) {
      if (el.isAdjacentTo(QuestJs._game.player) && el.borderDesc)
        s += ` ${el.borderDesc}`;
    }
    return s;
  };
  ...
  res.beforeEnter = function () {
    if (QuestJs._game.player.positionX === undefined)
      QuestJs._game.player.positionX = 0;
    if (QuestJs._game.player.positionY === undefined)
  ...
  res.getDirection = function (char, objX, objY, range) {
    ...
    if (x > 0 && theta <= 22.5 && theta >= -22.5)
      return QuestJs._lang.exit_list[7].name;
    if (x > 0 && theta <= 67.5 && theta >= 22.5)
      return QuestJs._lang.exit_list[2].name;
    if (x > 0 && theta >= -67.5 && theta <= -22.5)
      return QuestJs._lang.exit_list[12].name;

    if (x < 0 && theta <= 22.5 && theta >= -22.5)
      return QuestJs._lang.exit_list[5].name;
    if (x < 0 && theta <= 67.5 && theta >= 22.5)
      return QuestJs._lang.exit_list[10].name;
    if (x < 0 && theta >= -67.5 && theta <= -22.5)
      return QuestJs._lang.exit_list[0].name;

    return y > 0
      ? QuestJs._lang.exit_list[1].name
      : QuestJs._lang.exit_list[11].name;
  };
  ...
  res.drawMap = function () {
    ...
    for (let x = -this.size; x <= this.size; x += 1) {
      for (let y = -this.size; y <= this.size; y += 1) {
        ...
        if (this.getBorderAt(x, y)) {
          cells.push(
            `<rect x="${x2}" y="${y2}" width="${this.cellSize}" height="${this.cellSize}" stroke="none" fill="${this.outsideColour}"/>`,
          );
        } else {
          cells.push(
            `<rect x="${x2}" y="${y2}" width="${this.cellSize}" height="${this.cellSize}" stroke="none" fill="${this.insideColour}"/>`,
          );
        }
        ...
        if (feature) {
          ...
          features.push(
            `<circle cx="${x2 + this.cellSize / 2}" cy="${
              y2 + this.cellSize / 2
            }" r="${this.cellSize / 2 - 1}" stroke="none" fill="${colour}"/>`,
          );
          if (feature.zoneMapName)
            labels.push(
              `<text x="${x2 + this.cellSize}" y="${y2 + 5}" style="font: ${
                this.mapFont
              }; fill: black;">${feature.zoneMapName}</text>`,
            );
        }
      }
    }

    const map = cells.concat(
      this.mapCells,
      features,
      this.mapFeatures,
      labels,
    ...
    map.push(
      `<rect x="${x2 + 4}" y="${y2 + 4}" width="${this.cellSize - 8}" height="${
        this.cellSize - 8
      }" stroke="none" fill="${this.playerColour}"/>`,
    );

    const svgSize = (this.size * 2 + 1) * this.cellSize;
    if (this.mapBorder)
      map.push(
        `<rect x="0" y="0" width="${svgSize}" height="${svgSize}" stroke="black" fill="none"/>`,
      );
...
const ZONE_BORDER = function (loc) {
  return {
    ...
    isAdjacentTo(char) {
      ...
      for (let x = char.positionX - 1; x <= char.positionX + 1; x += 1) {
        for (let y = char.positionY - 1; y <= char.positionY + 1; y += 1) {
