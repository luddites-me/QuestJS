...
const TAKEABLE_DICTIONARY = {
  onCreation:function(o) {
    //QuestJs._log.info('takeable: ' + o.name)
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(QuestJs._game.player.name) ? QuestJs._lang.verbs.drop : QuestJs._lang.verbs.take)
    })
  ...
  drop:function(isMultiple, char) {
    ...
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.drop_successful, tpParams);
...
QuestJs._templates.TAKEABLE_DICTIONARY = TAKEABLE_DICTIONARY;

const TAKEABLE = () => TAKEABLE_DICTIONARY;

QuestJs._templates.TAKEABLE = TAKEABLE;
...
QuestJs._templates.SHIFTABLE = SHIFTABLE;


const createEnsemble = function(name, members, dict) {
  const res = QuestJs._create.createItem(name, dict);
  res.ensemble = true;
  res.members = members;
  ...
  res.nameModifierFunctions = [function(o, list) {
    if (o.members[0].getWorn() && o.isAllTogether() && o.members[0].isAtLoc(QuestJs._game.player.name)) list.push(QuestJs._lang.invModifiers.worn)
  }]
  ...
  res.isAtLoc = function(loc, situation) {
    if (situation !== QuestJs._world.PARSER && situation !== QuestJs._world.SCOPING) return false;
...
QuestJs._templates.createEnsemble = createEnsemble;
...
const MERCH = function(value, locs) {
  const res = {
    ...
    getSellingPrice:function(char) { 
      if (QuestJs._w[char.loc].buyingValue) {
        return Math.round(this.getPrice() * (QuestJs._w[char.loc].buyingValue) / 100);
      }
    ...
    getBuyingPrice:function(char) {
      if (QuestJs._w[char.loc].sellingDiscount) {
        return Math.round(this.getPrice() * (100 - QuestJs._w[char.loc].sellingDiscount) / 100);
      }
      return this.getPrice(); 
    },
    
    isAtLoc:function(loc, situation) {
      if ((situation === QuestJs._world.PARSER || situation === QuestJs._world.SCOPING) && this.isForSale(loc)) return true;
      if (typeof loc !== "string") loc = loc.name;
      if (!QuestJs._w[loc]) QuestJs._io.errormsg("The location name `" + loc + "`, does not match anything in the game.");
    ...
    canBeSoldHere:function(loc) {
      return QuestJs._w[loc].willBuy && QuestJs._w[loc].willBuy(this);
    },
    ...
    purchase:function(isMultiple, char) {
      ...
      if (!this.isForSale(char.loc)) {
        if (this.doNotClone && this.isAtLoc(char.name)) {
          return QuestJs._io.failedmsg(QuestJs._lang.cannot_purchase_again, tpParams)
        }
        else {
          return QuestJs._io.failedmsg(QuestJs._lang.cannot_purchase_here, tpParams)
        }
      }
      
      const cost = this.getBuyingPrice(char);
      tpParams.money = cost
      if (char.money < cost) return QuestJs._io.failedmsg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_afford, tpParams);
      this.purchaseScript(isMultiple, char, cost, tpParams)
    },
    
    purchaseScript:function(isMultiple, char, cost, tpParams) {
      char.money -= cost;
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.purchase_successful, tpParams);
      if (this.doNotClone) {
        this.moveToFrom(char.name, char.loc);
        delete this.salesLoc;
      }
      else {
        QuestJs._create.cloneObject(this, char.name);
      }
    ...
    sell:function(isMultiple, char) {
      ...
      if (!this.canBeSoldHere(char.loc)) {
        return QuestJs._io.failedmsg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_sell_here, tpParams);
      }
      const cost = this.getSellingPrice(char);
      tpParams.money = cost
      char.money += cost;
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.sell_successful, tpParams);
      if (this.doNotClone) {
        this.moveToFrom(char.loc, char.name);
...
QuestJs._templates.MERCH = MERCH;
...
const COUNTABLE = function(countableLocs) {
  ...
  res.getListAlias = function(loc) {
    return QuestJs._tools.sentenceCase(this.pluralAlias ? this.pluralAlias : this.listalias + "s") + " (" + this.countAtLoc(loc) + ")";
  };
  ...
  res.takeFrom = function(loc, count) {
    if (this.countableLocs[loc] !== QuestJs._consts.INFINITY) this.countableLocs[loc] -= count;
    if (this.countableLocs[loc] <= 0) { delete this.countableLocs[loc]; }
    QuestJs._w[loc].itemTaken(this, count);
  };
  
  res.giveTo = function(loc, count) {
    if (!this.countableLocs[loc]) { this.countableLocs[loc] = 0; }
    if (this.countableLocs[loc] !== QuestJs._consts.INFINITY) this.countableLocs[loc] += count;
    QuestJs._w[loc].itemDropped(this, count);
  };
  ...
  res.findSource = function(sourceLoc, tryContainers) {
    ...
    if (tryContainers) {
      const containers = QuestJs._scope.scopeReachable().filter(el => el.container);
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!sourceLoc) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.none_here, tpParams);
      return false;
    }
  ...
  res.drop = function(isMultiple, char) {
    ...
    if (m === 0) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.none_held, tpParams);
      return false;
    }

    if (!n) { m === QuestJs._consts.INFINITY ? 1 : n = m; }  // no number specified
    if (n > m)  { n = m; }  // too big number specified
...
QuestJs._templates.COUNTABLE = COUNTABLE;
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.useDefaultsTo = function(char) {
    return char === QuestJs._game.player ? 'Wear' : 'NpcWear'
  }
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      if (!o.isAtLoc(QuestJs._game.player.name)) {
        verbList.push(QuestJs._lang.verbs.take)
      }
      else if (o.getWorn()) {
        if (!o.getWearRemoveBlocker(QuestJs._game.player, false)) verbList.push(QuestJs._lang.verbs.remove)
      }
      else {
        verbList.push(QuestJs._lang.verbs.drop)
        if (!o.getWearRemoveBlocker(QuestJs._game.player, true)) verbList.push(QuestJs._lang.verbs.wear)
      }
    })
    //QuestJs._log.info(o.verbFunctions.length)

    o.nameModifierFunctions.push(function(o, list) {
      if (o.worn && o.isAtLoc(QuestJs._game.player.name)) list.push(QuestJs._lang.invModifiers.worn)
    })
  }
  ...
  res.canWearRemove = function(char, toWear) {
    ...
    if (outer) {
      ...
      if (toWear) {
        QuestJs._io.msg(QuestJs._lang.cannot_wear_over, tpParams);
      }
      else {
        QuestJs._io.msg(QuestJs._lang.cannot_remove_under, tpParams);
      }
      return false;
    }
    return true;
  ...
  res.wear = function(isMultiple, char) {
    ...
    if (this.wearMsg) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + this.wearMsg(char, this), {garment:this, actor:char});
    }
    else {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.wear_successful, {garment:this, char:char});
    }
    this.worn = true;
  ...
  res.remove = function(isMultiple, char) {
    ...
    if (this.removeMsg) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + this.removeMsg(char, this), {garment:this, actor:char});
    }
    else {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.remove_successful, {garment:this, char:char});
    }
    this.worn = false;
...
QuestJs._templates.WEARABLE = WEARABLE;

const OPENABLE_DICTIONARY = {
  open:function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (!this.openable) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_open, {item:this});
      return false;
    }
    else if (!this.closed) {
    ...
    if (this.locked) {
      if (this.testKeys(char)) {
        this.locked = false;
        this.closed = false;
        QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.unlock_successful, tpParams);
        this.openMsg(isMultiple, tpParams);
        return true;
      }
      else {
        QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.locked, tpParams);
        return false;
      }
    }
    this.closed = false;
  ...
  close:function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (!this.openable) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_close, {item:this});
      return false;
    }
    else if (this.closed) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
  ...
  closeMsg:function(isMultiple, tpParams) {
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.close_successful, tpParams);
  },
...
const CONTAINER = function(openable) {
  ...
  res.getContents = QuestJs._util.getContents
  res.testForRecursion = QuestJs._util.testForRecursion
  res.listContents = QuestJs._util.listContents
  res.transparent = false
  
  res.onCreation = function(o) {
    //QuestJs._log.info('container: ' + o.name)
    o.verbFunctions.push(function(o, verbList) {
      if (o.openable) {
        verbList.push(o.closed ? QuestJs._lang.verbs.open : QuestJs._lang.verbs.close);
      }
    })
    //QuestJs._log.info(o.verbFunctions.length)
    o.nameModifierFunctions.push(QuestJs._util.nameModifierFunctionForContainer) 
    //QuestJs._log.info(o.nameModifierFunctions)
  },

  res.lookinside = function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (this.closed && !this.transparent) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.nothing_inside, {char:char});
      return false;
    }
    //tpParams.list = QuestJs._tools.formatList(this.getContents(QuestJs._world.LOOK), {article:QuestJs._consts.INDEFINITE, lastJoiner:QuestJs._lang.list_and, nothing:QuestJs._lang.list_nothing})
    tpParams.list = this.listContents(QuestJs._world.LOOK, true)
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.look_inside, tpParams);
    return true;
  };
  
  res.openMsg = function(isMultiple, tpParams) {
    tpParams.list = this.listContents(QuestJs._world.LOOK)
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.open_successful + " " + (tpParams.list === QuestJs._lang.list_nothing ? QuestJs._lang.it_is_empty : QuestJs._lang.look_inside), tpParams)
  };
...
QuestJs._templates.CONTAINER = CONTAINER;
...
const SURFACE = function() {
  ...
  res.testForRecursion = QuestJs._util.testForRecursion
  res.listContents = QuestJs._util.listContents
  res.onCreation = function(o) { o.nameModifierFunctions.push(QuestJs._util.nameModifierFunctionForContainer) }
  res.closed = false;
  res.openable = false;
...
QuestJs._templates.SURFACE = SURFACE;
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.onCreation = function(o) {
    //QuestJs._log.info('openable: ' + o.name)
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.closed ? QuestJs._lang.verbs.open : QuestJs._lang.verbs.close);
    })
    o.nameModifierFunctions.push(function(o, list) {
      if (!o.closed) list.push(QuestJs._lang.invModifiers.open)
    })
  }

  res.openMsg = function(isMultiple, tpParams) {
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.open_successful, tpParams);
  };

  return res;
};

QuestJs._templates.OPENABLE = OPENABLE;
...
const LOCKED_WITH = function(keyNames) {
  ...
  const res = {
    ...
    lock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (this.locked) return QuestJs._io.falsemsg(QuestJs._lang.already, tpParams)
      if (!this.testKeys(char, true)) return QuestJs._io.falsemsg(QuestJs._lang.no_key, tpParams)

      if (!this.closed) {
        this.closed = true
        this.locked = true
        QuestJs._io.msg(QuestJs._lang.close_and_lock_successful, tpParams)
      }
      else {
        this.locked = true
        QuestJs._io.msg(QuestJs._lang.lock_successful, tpParams)
      }
      return true
    },
    ...
    unlock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (!this.locked) return QuestJs._io.falsemsg(QuestJs._lang.already, {item:this})
      if (!this.testKeys(char, false)) return QuestJs._io.falsemsg(QuestJs._lang.no_key, tpParams)
      QuestJs._io.msg(QuestJs._lang.unlock_successful, tpParams)
      this.locked = false
    ...
    testKeys:function(char, toLock) {
      for (let s of keyNames) {
        if (!QuestJs._w[s]) {
          QuestJs._io.errormsg("The key name for this container, `" + s + "`, does not match any key in the game.");
          return false;
        }
        if (QuestJs._w[s].isAtLoc(char.name)) { 
          return true; 
        }
      }
      return false;
...
QuestJs._templates.LOCKED_WITH = LOCKED_WITH;
...
const LOCKED_DOOR = function(key, loc1, loc2, name1, name2) {
  ...
  res._setup = function() {
    const room1 = QuestJs._w[this.loc1]
    if (!room1) return QuestJs._io.errormsg("Bad location name '" + this.loc1 + "' for door " + this.name)
    const exit1 = room1.findExit(this.loc2)
    if (!exit1) return QuestJs._io.errormsg("No exit to '" + this.loc2 + "' for door " + this.name)
    this.dir1 = exit1.dir
    if (!room1[this.dir1]) return QuestJs._io.errormsg("Bad exit '" + this.dir1 + "' in location '" + room1.name + "' for door: " + this.name + " (possibly because the room is defined after the door?)")

    const room2 = QuestJs._w[this.loc2]
    if (!room2) return QuestJs._io.errormsg("Bad location name '" + this.loc2 + "' for door " + this.name)
    const exit2 = room2.findExit(this.loc1)
    if (!exit2) return QuestJs._io.errormsg("No exit to '" + this.loc1 + "' for door " + this.name)
    this.dir2 = exit2.dir
    if (!room2[this.dir2]) return QuestJs._io.errormsg("Bad exit '" + this.dir2 + "' in location '" + room2.name + "' for door: " + this.name + " (possibly because the room is defined after the door?)")

    room1[this.dir1].use = QuestJs._util.useWithDoor
    room1[this.dir1].door = this.name
    room1[this.dir1].doorName = this.name1 || 'door to ' + QuestJs._lang.getName(QuestJs._w[this.loc2], {article:QuestJs._consts.DEFINITE})

    room2[this.dir2].use = QuestJs._util.useWithDoor
    room2[this.dir2].door = this.name
    room2[this.dir2].doorName = this.name2 || 'door to ' + QuestJs._lang.getName(QuestJs._w[this.loc1], {article:QuestJs._consts.DEFINITE})
  }
  
  res.isAtLoc = function(loc, situation) {
    if (typeof loc !== "string") loc = loc.name
    if (situation !== QuestJs._world.PARSER && this.scenery) return false;
...
QuestJs._templates.LOCKED_DOOR = LOCKED_DOOR;
...
const READABLE = function(mustBeHeld) {
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (o.loc === QuestJs._game.player.name || !o.mustBeHeld) verbList.push(QuestJs._lang.verbs.read)
    })
  }
  return res;
}  

QuestJs._templates.READABLE = READABLE;
...
const FURNITURE = function(options) {
  const res = {
    ...
    getoff:function(isMultiple, char) {
      if (!char.posture) {
        char.msg(QuestJs._lang.already, {item:char});
        return false;
      }
      if (char.posture) {
        char.msg(QuestJs._lang.stop_posture(char));  // stop_posture handles details
        return true;
      }  
    },
  }
  res.useDefaultsTo = function(char) {
    ...
    return char === QuestJs._game.player ? cmd : 'Npc' + cmd
  }

  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (QuestJs._game.player.posture && QuestJs._game.player.postureFurniture === o.name) {
        verbList.push(QuestJs._lang.verbs.getOff)
        return
      }
      if (QuestJs._game.player.posture) return
      if (o.siton) verbList.push(QuestJs._lang.verbs.sitOn)
      if (o.standon) verbList.push(QuestJs._lang.verbs.standOn)
      if (o.reclineon) verbList.push(QuestJs._lang.verbs.reclineOn)
    })
  }
  ...
  res.assumePosture = function(isMultiple, char, posture, name, adverb) {
    ...
    if (char.posture && char.postureFurniture !== this.name) {
      ...
      char.msg(QuestJs._lang[name + '_on_successful'], tpParams)
    }
    ...
    const eventName = 'on' + QuestJs._tools.sentenceCase(name)
...
QuestJs._templates.FURNITURE = FURNITURE;
...
const SWITCHABLE = function(alreadyOn, nameModifier) {
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (!o.mustBeHeldToOperate || o.isAtLoc(QuestJs._game.player)) {
        verbList.push(o.switchedon ? QuestJs._lang.verbs.switchoff : QuestJs._lang.verbs.switchon)
      }
    })
    o.nameModifierFunctions.push(function(o, list) {
  ...
  res.switchon = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.switchedon) {
      char.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
      return false;
    }
    if (!this.checkCanSwitchOn()) {
      return false;
    }
    char.msg(QuestJs._lang.turn_on_successful, tpParams);
    this.doSwitchon();
    return true;
  };
  
  res.doSwitchon = function() {
    let lighting = QuestJs._game.dark;
    this.switchedon = true;
    QuestJs._game.update();
    if (lighting !== QuestJs._game.dark) {
      QuestJs._game.room.description();
    }
  ...
  res.switchoff = function(isMultiple, char) {
    ...
    char.msg(QuestJs._lang.turn_off_successful, tpParams);
    this.doSwitchoff();
    return true;
  };
...
QuestJs._templates.SWITCHABLE = SWITCHABLE;
...
const COMPONENT = function(nameOfWhole) {
  const res = {
    ...
    isAtLoc:function(loc, situation) {
      if (typeof loc !== "string") loc = loc.name
      if (situation !== QuestJs._world.PARSER) return false;
      let cont = QuestJs._w[this.loc];
      if (cont.isAtLoc(loc)) { return true; }
      return cont.isAtLoc(loc);
    },
    take:function(isMultiple, char) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_take_component, {char:char, item:this, whole:QuestJs._w[this.loc]});
      return false;
    },
  };
  if (!QuestJs._w[nameOfWhole]) QuestJs._io.debugmsg("Whole is not define: " + nameOfWhole);
  QuestJs._w[nameOfWhole].componentHolder = true;
  return res;
};

QuestJs._templates.COMPONENT = COMPONENT;
...
const EDIBLE = function(isLiquid) {
  ...
  res.eat = function(isMultiple, char) {
    ...
    if (this.isLiquid) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_eat, tpParam);
      return false;
    }
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.eat_successful, tpParam);
    this.loc = null;
  ...
  res.drink = function(isMultiple, char) {
    const tpParam = {char:char, item:this}
    if (!this.isLiquid) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_drink, tpParam);
      return false;
    }
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.drink_successful, tpParam);
    this.loc = null;
    if (this.onIngesting) this.onIngesting(char);
    return true;
  };
  ...
  res.icon = () => 'edible12'
  res.onCreation = function(o) {
    //QuestJs._log.info('edible: ' + o.name)
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(QuestJs._game.player.name) ? QuestJs._lang.verbs.drop : QuestJs._lang.verbs.take)
      if (o.isAtLoc(QuestJs._game.player)) verbList.push(o.isLiquid ? QuestJs._lang.verbs.drink : QuestJs._lang.verbs.eat)
    })
    //QuestJs._log.info(o.verbFunctions.length)
  }
  return res;
...
QuestJs._templates.EDIBLE = EDIBLE;
...
const VESSEL = function(capacity) {
  ...
  res.fill = function(isMultiple, char, liquid) {
    ...
    if (!source) return QuestJs._io.falsemsg(QuestJs._lang.none_here, {char:char, item:liquid});
    if (!this.mix && this.containedLiquidName !== liquid.name && this.volumeContained > 0) return QuestJs._io.falsemsg(QuestJs._lang.cannot_mix(char, this));
    if (this.volumeContained >= this.capacity) return QuestJs._io.falsemsg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
    ...
    if (this.mix && liquid.name !== this.containedLiquidName !== liquid.name) {
      ...
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.fill_successful, tpParams)
    }
    return true;
  };
  ...
  res.empty = function(isMultiple, char) {
    ...
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.empty_successful, tpParams);
    return true;
  };
...
QuestJs._templates.VESSEL = VESSEL;
...
const LIQUID = function(locs) {
  ...
  res.liquid = true;
  res.pronouns = QuestJs._lang.pronouns.massnoun;
  res.pluralAlias = '*';
  res.drink = function(isMultiple, char, options) {
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + "drink: " + options.verb + " char: " + char.name);
  };
  res.sourcedAtLoc = function(loc) { 
    if (typeof loc !== "string") loc = loc.name;
    return QuestJs._w[loc].isSourceOf(this.name);
  }
  res.source = function(chr) { 
    if (chr === undefined) chr = QuestJs._game.player;
    // Is character a source?
    if (chr.isSourceOf && chr.isSourceOf(this.name)) return chr;
    // Is the room a source?
    if (QuestJs._w[chr.loc].isSourceOf && QuestJs._w[chr.loc].isSourceOf(this.name)) return QuestJs._w[chr.loc];
    const items = QuestJs._scope.scopeHeldBy(chr).concat(QuestJs._scope.scopeHeldBy(chr.loc));
...
QuestJs._templates.LIQUID = LIQUID;
...
const ROPE = function(tetheredTo) {
  const res = $.extend({
    ...
    tiedTo1:tetheredTo,
    locs:[],
    attachVerb:QuestJs._lang.rope_attach_verb,
    attachedVerb:QuestJs._lang.rope_attached_verb,
    detachVerb:QuestJs._lang.rope_detach_verb,
    isAtLoc:function(loc, situation) {
      if (this.loc) {
        this.locs = [this.loc]
        delete this.loc
      ...
      if (situation === QuestJs._world.SIDE_PANE && this.locs.includes(QuestJs._game.player.name) && loc !== QuestJs._game.player.name) return false
    ...
    examineAddendum:function() {
      ...
      const obj1 = (this.tiedTo1 && QuestJs._w[this.tiedTo1].isHere()) ? QuestJs._w[this.tiedTo1] : false
      const obj2 = (this.tiedTo2 && QuestJs._w[this.tiedTo2].isHere()) ? QuestJs._w[this.tiedTo2] : false

      // Handle the easy cases, only one loc in locs
      if (this.locs.length === 1) {
        if (obj1 && obj2) return QuestJs._text.processText(QuestJs._lang.examineAddBothEnds, {rope:this, obj1:obj1, obj2:obj2})
        if (obj1) return QuestJs._text.processText(QuestJs._lang.rope_examine_attached_one_end, {rope:this, obj1:obj1})
        if (obj2) return QuestJs._text.processText(QuestJs._lang.rope_examine_attached_one_end, {rope:this, obj1:obj2})
        return ''  // just in one place, like any ordinary object
      }

      // Who is it held by (and we can see)
      const end1 = QuestJs._w[this.locs[0]]
      const holder1 = (end1.npc || end1.player) && end1.isHere() ? end1 : false
      const end2 = QuestJs._w[this.locs[this.locs.length - 1]]
      const holder2 = (end2.npc || end2.player) && end2.isHere() ? end2 : false
      
      // What locations does it go to
      const index = this.locs.findIndex(el => el === QuestJs._game.player.loc)
      const loc1 = (index > 0 && QuestJs._w[this.locs[index - 1]].room) ? QuestJs._w[this.locs[index - 1]] : false
      const loc2 = (index < (this.locs.length - 1) && QuestJs._w[this.locs[index + 1]].room) ? QuestJs._w[this.locs[index + 1]] : false
      
      let s = ''
      let flag = false
      if (obj1 || holder1 || loc1) {
        s += ' ' + QuestJs._lang.rope_one_end + ' '
        flag = true
      }
      if (obj1) {
        s += QuestJs._lang.rope_examine_end_attached.replace('obj', 'obj1')
      }
      else if (holder1) {
        s += QuestJs._lang.rope_examine_end_held.replace('holder', 'holder1')
      }
      else if (loc1) {
        s += QuestJs._lang.rope_examine_end_headed.replace('loc', 'loc1')
      }
      
      if (obj2 || holder2 || loc2) {
        s += ' ' + (flag ? QuestJs._lang.rope_other_end : QuestJs._lang.rope_one_end) + ' '
        flag = true
      }
      if (obj2) {
        s += QuestJs._lang.rope_examine_end_attached.replace('obj', 'obj2')
      }
      else if (holder2) {
        s += QuestJs._lang.rope_examine_end_held.replace('holder', 'holder2')
      }
      else if (loc2) {
        s += QuestJs._lang.rope_examine_end_headed.replace('loc', 'loc2')
      }
      
      return QuestJs._text.processText(s, {rope:this, obj1:obj1, obj2:obj2, holder1:holder1, holder2:holder2, loc1:loc1, loc2:loc2})
    },
    ...
    useWith:function(char, item) {
      return handleTieTo(char, this, item) === QuestJs._world.SUCCESS
    },
    detachFrom:function(item) {
      if (this.tiedTo1 === item.name) {
        if (this.locs.length === 2 && this.locs.includes(QuestJs._game.player.name)) this.locs.shift() // remove this room
        if (this.locs[0] !== QuestJs._game.player.name) {
          this.locs.unshift(QuestJs._game.player.name)
        }
        delete this.tiedTo1
      }
      else {
        if (this.locs.length === 2 && this.locs.includes(QuestJs._game.player.name)) this.locs.pop() // remove this room
        if (this.locs[this.locs.length - 1] !== QuestJs._game.player.name) this.locs.push(QuestJs._game.player.name)
        delete this.tiedTo2
      }
  ...
  res.moveToFrom = function(toLoc, fromLoc) {
    ...
    if (!QuestJs._w[fromLoc]) QuestJs._io.errormsg("The location name `" + fromLoc + "`, does not match anything in the game.")
    if (!QuestJs._w[toLoc]) QuestJs._io.errormsg("The location name `" + toLoc + "`, does not match anything in the game.")
    ...
    if (QuestJs._w[toLoc].onGoCheckList && !QuestJs._w[toLoc].onGoCheckList.includes(this.name)) QuestJs._w[toLoc].onGoCheckList.push(this.name)
    if (QuestJs._w[toLoc].onGoActionList && !QuestJs._w[toLoc].onGoActionList.includes(this.name)) QuestJs._w[toLoc].onGoActionList.push(this.name)
    QuestJs._w[fromLoc].itemTaken(this, end);
    QuestJs._w[toLoc].itemDropped(this, end);
...
/*  res.drop = function(isMultiple, char) {
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.drop_successful(char, this));
    //this.moveToFrom(char.loc);
    if (this.tiedTo1 === char.name) this.tiedTo1 = char.loc
    if (this.tiedTo2 === char.name) this.tiedTo2 = char.loc
    QuestJs._w[char.name].itemTaken(this);
    QuestJs._w[char.loc].itemDropped(this);
  ...
  res.take = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.isAtLoc(char.name)) return QuestJs._io.falsemsg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.already_have, tpParams)
    if (!char.canManipulate(this, "take")) return false;
    if (this.tiedTo1 && this.tiedTo2) return QuestJs._io.falsemsg(QuestJs._tools.prefix(this, isMultiple) + "It is tied up at both ends.")
    QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.take_successful, tpParams);
  ...
  res.onGoCheck = function(char, dest) {
    ...
    QuestJs._io.msg("{nv:rope:be:true} not long enough, you cannot go any further.", {rope:this})
    return false
  }
...
const BUTTON_DICTIONARY = {
  ...
  onCreation:function(o) {
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(QuestJs._lang.verbs.push)
    })
  },
}
...
const BUTTON = function() {
  ...
  res.push = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    QuestJs._io.msg(QuestJs._lang.push_button_successful, tpParams)
...
QuestJs._templates.BUTTON = BUTTON;
...
const TRANSIT_BUTTON = function(nameOfTransit) {
  ...
  res.isAtLoc = function(loc, situation) {
    if (situation === QuestJs._world.LOOK) return false;
    if (typeof loc !== "string") loc = loc.name
    return (this.loc === loc)
  }

  res.push = function(isMultiple, char) {
    const transit = QuestJs._w[this.loc];

    if (typeof transit.transitCheck === "function" && !transit.transitCheck()) {
      if (transit.transitAutoMove) QuestJs._world.setRoom(QuestJs._game.player, QuestJs._game.player.previousLoc, transit.transitDoorDir)
      return false;
    }

    const exit = transit[transit.transitDoorDir];
    if (this.locked) {
      QuestJs._tools.printOrRun(QuestJs._game.player, this, "transitLocked");
      return false;
    }
    else if (exit.name === this.transitDest) {
      QuestJs._tools.printOrRun(QuestJs._game.player, this, "transitAlreadyHere");
      return false;
    }
    else {
      QuestJs._tools.printOrRun(QuestJs._game.player, this, "transitGoToDest")
...
QuestJs._templates.TRANSIT_BUTTON = TRANSIT_BUTTON;
...
const TRANSIT = function(exitDir) {
  const res = {
    ...
    beforeEnter:function() {
      this.update(QuestJs._game.player.previousLoc)
    },

    getTransitButtons:function(includeHidden, includeLocked) {
      return this.getContents(QuestJs._world.LOOK).filter(function(el) {
    ...
    findTransitButton:function(dest) {
      for (let key in QuestJs._w) {
        if (QuestJs._w[key].loc === this.name && QuestJs._w[key].transitDest === dest) return QuestJs._w[key]
      }
      return null
    },
    ...
    update:function(transitButton, callEvent) {
      ...
      if (typeof map !== 'undefined') {
        QuestJs._log.info("MAPPING")
        QuestJs._log.info(transitButton.transitDest)
        ...
        if (this.mapCurrentConnection === -1) {
          QuestJs._io.errormsg('Failed to find a location called "' + transitButton.transitDest + '"')
          QuestJs._log.info(this.locations)
          return
        }
        QuestJs._log.info(this.mapCurrentConnection)
        const loc = this.locations[this.mapCurrentConnection]
        QuestJs._log.info(loc)
    ...
    transitOfferMenu:function() {
      if (typeof this.transitCheck === "function" && !this.transitCheck()) {
        if (this.transitAutoMove) QuestJs._world.setRoom(QuestJs._game.player, QuestJs._game.player.previousLoc, this.transitDoorDir)
        return false;
      }
      const buttons = this.getTransitButtons(true, false);
      const transitDoorDir = this.transitDoorDir;
      const room = this;
      QuestJs._io.showMenu(this.transitMenuPrompt, buttons.map(el => el.transitDestAlias), function(result) {
        for (let button of buttons) {
          if (buttons[i].transitDestAlias === result) {
            if (room[transitDoorDir].name === button.transitDest) {
              QuestJs._tools.printOrRun(QuestJs._game.player, button, "transitAlreadyHere");
            }
            else {
              QuestJs._tools.printOrRun(QuestJs._game.player, button, "transitGoToDest")
              transit.update(button.transitDest, true)
            }
            
            room[transitDoorDir].name = button.transitDest
            if (room.transitAutoMove) QuestJs._world.setRoom(QuestJs._game.player, button.transitDest, room[transitDoorDir])
          }
        }
      })
    },
...
QuestJs._templates.TRANSIT = TRANSIT;
...
const CHARACTER = function() {
  const res = {
    ...
    canSeeThrough:() => true,
    getAgreement:() => true,
    getContents:QuestJs._util.getContents,
    pause:QuestJs._consts.NULL_FUNC,  
    canManipulate:() => true,
    canMove:() => true,
    canPosture:() => true,
    canTakeDrop:() => true,
    ...
    getHolding:function() {
      return this.getContents(QuestJs._world.LOOK).filter(function(el) { return !el.getWorn(); });
    },
    
    getWearing:function() {
      return this.getContents(QuestJs._world.LOOK).filter(function(el) { return el.getWorn() && !el.ensemble; });
    },
  
    getStatusDesc:function() {
      if (!this.posture) return false;
      return this.posture + " " + this.postureAdverb + " " + QuestJs._lang.getName(QuestJs._w[this.postureFurniture], {article:QuestJs._consts.DEFINITE});
    },
    
    isAtLoc:function(loc, situation) {
      if (situation === QuestJs._world.LOOK) return false;
      if (situation === QuestJs._world.SIDE_PANE) return false;
      if (typeof loc !== "string") loc = loc.name
      return (this.loc === loc);
    },
    ...
    getOuterWearable:function(slot) {
      const clothing = this.getWearing().filter(function(el) {
        if (typeof el.getSlots !== "function") {
          QuestJs._log.info("Item with worn set to true, but no getSlots function");
    ...
    onCreation:function(o) {
      o.nameModifierFunctions.push(function(o, l) {
        ...
        if (held.length > 0) {
          list.push(QuestJs._lang.invHoldingPrefix + ' ' + QuestJs._tools.formatList(held, {article:QuestJs._consts.INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:false, nothing:QuestJs._lang.list_nothing, loc:o.name, npc:true}));
        }
        if (worn.length > 0) {
          list.push(QuestJs._lang.invWearingPrefix + ' ' + QuestJs._tools.formatList(worn, {article:QuestJs._consts.INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:false, nothing:QuestJs._lang.list_nothing, loc:o.name, npc:true}));
        }
        if (list.length > 0) l.push(list.join('; '))
      })
      o.verbFunctions.push(function(o, verbList) {
        verbList.shift()
        verbList.push(QuestJs._lang.verbs.lookat)
        if (!QuestJs._settings.noTalkTo) verbList.push(QuestJs._lang.verbs.talkto)
      })
    },
...
QuestJs._templates.CHARACTER = CHARACTER;

const PLAYER = function() {
  const res = CHARACTER()
  res.pronouns = QuestJs._lang.pronouns.secondperson
  res.player = true
  return res;
}

QuestJs._templates.PLAYER = PLAYER;
