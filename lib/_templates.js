...
const TAKEABLE_DICTIONARY = {
  ...
  drop:function(isMultiple, char) {
    ...
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.drop_successful, tpParams);
...
const MERCH = function(value, locs) {
  const res = {
    ...
    purchase:function(isMultiple, char) {
      ...
      tpParams.money = cost
      if (char.money < cost) return QuestJs._io.failedmsg(prefix(this, isMultiple) + QuestJs._lang.cannot_afford, tpParams);
      this.purchaseScript(isMultiple, char, cost, tpParams)
    },
    
    purchaseScript:function(isMultiple, char, cost, tpParams) {
      char.money -= cost;
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.purchase_successful, tpParams);
      if (this.doNotClone) {
    ...
    sell:function(isMultiple, char) {
      ...
      if (!this.canBeSoldHere(char.loc)) {
        return QuestJs._io.failedmsg(prefix(this, isMultiple) + QuestJs._lang.cannot_sell_here, tpParams);
      }
      const cost = this.getSellingPrice(char);
      tpParams.money = cost
      char.money += cost;
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.sell_successful, tpParams);
      if (this.doNotClone) {
        this.moveToFrom(char.loc, char.name);
...
const COUNTABLE = function(countableLocs) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!sourceLoc) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.none_here, tpParams);
      return false;
    }
  ...
  res.drop = function(isMultiple, char) {
    ...
    if (m === 0) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.none_held, tpParams);
      return false;
    }
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.wear = function(isMultiple, char) {
    ...
    if (this.wearMsg) {
      QuestJs._io.msg(prefix(this, isMultiple) + this.wearMsg(char, this), {garment:this, actor:char});
    }
    else {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.wear_successful, {garment:this, char:char});
    }
    this.worn = true;
  ...
  res.remove = function(isMultiple, char) {
    ...
    if (this.removeMsg) {
      QuestJs._io.msg(prefix(this, isMultiple) + this.removeMsg(char, this), {garment:this, actor:char});
    }
    else {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.remove_successful, {garment:this, char:char});
    }
    this.worn = false;
...
const OPENABLE_DICTIONARY = {
  open:function(isMultiple, char) {
    ...
    if (!this.openable) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.cannot_open, {item:this});
      return false;
    }
    else if (!this.closed) {
    ...
    if (this.locked) {
      if (this.testKeys(char)) {
        this.locked = false;
        this.closed = false;
        QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.unlock_successful, tpParams);
        this.openMsg(isMultiple, tpParams);
        return true;
      }
      else {
        QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.locked, tpParams);
        return false;
      }
    }
    this.closed = false;
  ...
  close:function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (!this.openable) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.cannot_close, {item:this});
      return false;
    }
    else if (this.closed) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
  ...
  closeMsg:function(isMultiple, tpParams) {
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.close_successful, tpParams);
  },
...
const CONTAINER = function(openable) {
  ...
  res.lookinside = function(isMultiple, char) {
    ...
    if (this.closed && !this.transparent) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.nothing_inside, {char:char});
      return false;
    }
    //tpParams.list = formatList(this.getContents(world.LOOK), {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, nothing:QuestJs._lang.list_nothing})
    tpParams.list = this.listContents(world.LOOK, true)
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.look_inside, tpParams);
    return true;
  };
  
  res.openMsg = function(isMultiple, tpParams) {
    tpParams.list = this.listContents(world.LOOK)
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.open_successful + " " + (tpParams.list === QuestJs._lang.list_nothing ? QuestJs._lang.it_is_empty : QuestJs._lang.look_inside), tpParams)
  };
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.openMsg = function(isMultiple, tpParams) {
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.open_successful, tpParams);
  };
...
const LOCKED_DOOR = function(key, loc1, loc2, name1, name2) {
  ...
  res._setup = function() {
    ...
    room1[this.dir1].doorName = this.name1 || 'door to ' + QuestJs._lang.getName(w[this.loc2], {article:DEFINITE})

    room2[this.dir2].use = util.useWithDoor
    room2[this.dir2].door = this.name
    room2[this.dir2].doorName = this.name2 || 'door to ' + QuestJs._lang.getName(w[this.loc1], {article:DEFINITE})
  }
...
const SWITCHABLE = function(alreadyOn, nameModifier) {
  ...
  res.switchon = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.switchedon) {
      char.msg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
      return false;
    }
...
const COMPONENT = function(nameOfWhole) {
  const res = {
    ...
    take:function(isMultiple, char) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.cannot_take_component, {char:char, item:this, whole:w[this.loc]});
      return false;
    },
  };
...
const EDIBLE = function(isLiquid) {
  ...
  res.eat = function(isMultiple, char) {
    ...
    if (this.isLiquid) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.cannot_eat, tpParam);
      return false;
    }
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.eat_successful, tpParam);
    this.loc = null;
  ...
  res.drink = function(isMultiple, char) {
    const tpParam = {char:char, item:this}
    if (!this.isLiquid) {
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.cannot_drink, tpParam);
      return false;
    }
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.drink_successful, tpParam);
    this.loc = null;
    if (this.onIngesting) this.onIngesting(char);
    return true;
  };
...
const VESSEL = function(capacity) {
  ...
  res.fill = function(isMultiple, char, liquid) {
    ...
    if (this.volumeContained >= this.capacity) return QuestJs._io.falsemsg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
    ...
    if (this.mix && liquid.name !== this.containedLiquidName !== liquid.name) {
      ...
      QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.fill_successful, tpParams)
    }
    return true;
  };
  ...
  res.empty = function(isMultiple, char) {
    ...
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.empty_successful, tpParams);
    return true;
  };
...
const LIQUID = function(locs) {
  ...
  res.drink = function(isMultiple, char, options) {
    QuestJs._io.msg(prefix(this, isMultiple) + "drink: " + options.verb + " char: " + char.name);
  };
...
/*  res.drop = function(isMultiple, char) {
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.drop_successful(char, this));
  ...
  res.take = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.isAtLoc(char.name)) return QuestJs._io.falsemsg(prefix(this, isMultiple) + QuestJs._lang.already_have, tpParams)
    if (!char.canManipulate(this, "take")) return false;
    if (this.tiedTo1 && this.tiedTo2) return QuestJs._io.falsemsg(prefix(this, isMultiple) + "It is tied up at both ends.")
    QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.take_successful, tpParams);
...
const CHARACTER = function() {
  const res = {
    ...
    getStatusDesc:function() {
      ...
      return this.posture + " " + this.postureAdverb + " " + QuestJs._lang.getName(w[this.postureFurniture], {article:DEFINITE});
    },
    ...
    onCreation:function(o) {
      o.nameModifierFunctions.push(function(o, l) {
        ...
        if (held.length > 0) {
          list.push(QuestJs._lang.invHoldingPrefix + ' ' + formatList(held, {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:false, nothing:QuestJs._lang.list_nothing, loc:o.name, npc:true}));
        }
        if (worn.length > 0) {
          list.push(QuestJs._lang.invWearingPrefix + ' ' + formatList(worn, {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:false, nothing:QuestJs._lang.list_nothing, loc:o.name, npc:true}));
        }
