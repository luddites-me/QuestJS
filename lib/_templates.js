...
const TAKEABLE_DICTIONARY = {
  onCreation:function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(game.player.name) ? QuestJs.LANG.verbs.drop : QuestJs.LANG.verbs.take)
    })
  ...
  drop:function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + QuestJs.LANG.drop_successful, tpParams);
...
const createEnsemble = function(name, members, dict) {
  ...
  res.nameModifierFunctions = [function(o, list) {
    if (o.members[0].getWorn() && o.isAllTogether() && o.members[0].isAtLoc(game.player.name)) list.push(QuestJs.LANG.invModifiers.worn)
  }]
  ...
  const res = {
    ...
    purchase:function(isMultiple, char) {
      ...
      if (!this.isForSale(char.loc)) {
        if (this.doNotClone && this.isAtLoc(char.name)) {
          return failedmsg(QuestJs.LANG.cannot_purchase_again, tpParams)
        }
        else {
          return failedmsg(QuestJs.LANG.cannot_purchase_here, tpParams)
        }
      }
      
      const cost = this.getBuyingPrice(char);
      tpParams.money = cost
      if (char.money < cost) return failedmsg(prefix(this, isMultiple) + QuestJs.LANG.cannot_afford, tpParams);
      this.purchaseScript(isMultiple, char, cost, tpParams)
    },
    
    purchaseScript:function(isMultiple, char, cost, tpParams) {
      char.money -= cost;
      msg(prefix(this, isMultiple) + QuestJs.LANG.purchase_successful, tpParams);
      if (this.doNotClone) {
    ...
    sell:function(isMultiple, char) {
      ...
      if (!this.canBeSoldHere(char.loc)) {
        return failedmsg(prefix(this, isMultiple) + QuestJs.LANG.cannot_sell_here, tpParams);
      }
      const cost = this.getSellingPrice(char);
      tpParams.money = cost
      char.money += cost;
      msg(prefix(this, isMultiple) + QuestJs.LANG.sell_successful, tpParams);
      if (this.doNotClone) {
        this.moveToFrom(char.loc, char.name);
...
const COUNTABLE = function(countableLocs) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!sourceLoc) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.none_here, tpParams);
      return false;
    }
  ...
  res.drop = function(isMultiple, char) {
    ...
    if (m === 0) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.none_held, tpParams);
      return false;
    }
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      if (!o.isAtLoc(game.player.name)) {
        verbList.push(QuestJs.LANG.verbs.take)
      }
      else if (o.getWorn()) {
        if (!o.getWearRemoveBlocker(game.player, false)) verbList.push(QuestJs.LANG.verbs.remove)
      }
      else {
        verbList.push(QuestJs.LANG.verbs.drop)
        if (!o.getWearRemoveBlocker(game.player, true)) verbList.push(QuestJs.LANG.verbs.wear)
      }
    })
    //console.log(o.verbFunctions.length)

    o.nameModifierFunctions.push(function(o, list) {
      if (o.worn && o.isAtLoc(game.player.name)) list.push(QuestJs.LANG.invModifiers.worn)
    })
  }
  ...
  res.canWearRemove = function(char, toWear) {
    ...
    if (outer) {
      ...
      if (toWear) {
        msg(QuestJs.LANG.cannot_wear_over, tpParams);
      }
      else {
        msg(QuestJs.LANG.cannot_remove_under, tpParams);
      }
      return false;
    }
    return true;
  ...
  res.wear = function(isMultiple, char) {
    ...
    if (this.wearMsg) {
      ...
      msg(prefix(this, isMultiple) + QuestJs.LANG.wear_successful, {garment:this, char:char});
    }
    this.worn = true;
  ...
  res.remove = function(isMultiple, char) {
    ...
    if (this.removeMsg) {
      ...
      msg(prefix(this, isMultiple) + QuestJs.LANG.remove_successful, {garment:this, char:char});
    }
    this.worn = false;
...
const OPENABLE_DICTIONARY = {
  open:function(isMultiple, char) {
    ...
    if (!this.openable) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.cannot_open, {item:this});
      return false;
    }
    else if (!this.closed) {
    ...
    if (this.locked) {
      if (this.testKeys(char)) {
        this.locked = false;
        this.closed = false;
        msg(prefix(this, isMultiple) + QuestJs.LANG.unlock_successful, tpParams);
        this.openMsg(isMultiple, tpParams);
        return true;
      }
      else {
        msg(prefix(this, isMultiple) + QuestJs.LANG.locked, tpParams);
        return false;
      }
    }
    this.closed = false;
  ...
  close:function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (!this.openable) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.cannot_close, {item:this});
      return false;
    }
    else if (this.closed) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.already, {item:this});
  ...
  closeMsg:function(isMultiple, tpParams) {
    msg(prefix(this, isMultiple) + QuestJs.LANG.close_successful, tpParams);
  },
...
const CONTAINER = function(openable) {
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      if (o.openable) {
        verbList.push(o.closed ? QuestJs.LANG.verbs.open : QuestJs.LANG.verbs.close);
      }
    })
    //console.log(o.verbFunctions.length)
  ...
  res.lookinside = function(isMultiple, char) {
    ...
    if (this.closed && !this.transparent) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.nothing_inside, {char:char});
      return false;
    }
    //tpParams.list = formatList(this.getContents(world.LOOK), {article:INDEFINITE, lastJoiner:QuestJs.LANG.list_and, nothing:QuestJs.LANG.list_nothing})
    tpParams.list = this.listContents(world.LOOK, true)
    msg(prefix(this, isMultiple) + QuestJs.LANG.look_inside, tpParams);
    return true;
  };
  
  res.openMsg = function(isMultiple, tpParams) {
    tpParams.list = this.listContents(world.LOOK)
    msg(prefix(this, isMultiple) + QuestJs.LANG.open_successful + " " + (tpParams.list === QuestJs.LANG.list_nothing ? QuestJs.LANG.it_is_empty : QuestJs.LANG.look_inside), tpParams)
  };
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.onCreation = function(o) {
    ...
    o.nameModifierFunctions.push(function(o, list) {
      if (!o.closed) list.push(QuestJs.LANG.invModifiers.open)
    })
  }

  res.openMsg = function(isMultiple, tpParams) {
    msg(prefix(this, isMultiple) + QuestJs.LANG.open_successful, tpParams);
  };
...
const LOCKED_WITH = function(keyNames) {
  ...
  const res = {
    ...
    lock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (this.locked) return falsemsg(QuestJs.LANG.already, tpParams)
      if (!this.testKeys(char, true)) return falsemsg(QuestJs.LANG.no_key, tpParams)

      if (!this.closed) {
        this.closed = true
        this.locked = true
        msg(QuestJs.LANG.close_and_lock_successful, tpParams)
      }
      else {
        this.locked = true
        msg(QuestJs.LANG.lock_successful, tpParams)
      }
      return true
    },
    ...
    unlock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (!this.locked) return falsemsg(QuestJs.LANG.already, {item:this})
      if (!this.testKeys(char, false)) return falsemsg(QuestJs.LANG.no_key, tpParams)
      msg(QuestJs.LANG.unlock_successful, tpParams)
      this.locked = false
...
const LOCKED_DOOR = function(key, loc1, loc2, name1, name2) {
  ...
  res._setup = function() {
    ...
    room1[this.dir1].doorName = this.name1 || 'door to ' + QuestJs.LANG.getName(w[this.loc2], {article:DEFINITE})

    room2[this.dir2].use = util.useWithDoor
    room2[this.dir2].door = this.name
    room2[this.dir2].doorName = this.name2 || 'door to ' + QuestJs.LANG.getName(w[this.loc1], {article:DEFINITE})
  }
...
const READABLE = function(mustBeHeld) {
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (o.loc === game.player.name || !o.mustBeHeld) verbList.push(QuestJs.LANG.verbs.read)
    })
  }
  return res;
}  
...
const FURNITURE = function(options) {
  const res = {
    ...
    getoff:function(isMultiple, char) {
      if (!char.posture) {
        char.msg(QuestJs.LANG.already, {item:char});
        return false;
      }
      if (char.posture) {
        char.msg(QuestJs.LANG.stop_posture(char));  // stop_posture handles details
        return true;
      }  
    },
  }
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (game.player.posture && game.player.postureFurniture === o.name) {
        verbList.push(QuestJs.LANG.verbs.getOff)
        return
      }
      if (game.player.posture) return
      if (o.siton) verbList.push(QuestJs.LANG.verbs.sitOn)
      if (o.standon) verbList.push(QuestJs.LANG.verbs.standOn)
      if (o.reclineon) verbList.push(QuestJs.LANG.verbs.reclineOn)
    })
  }
  ...
  res.assumePosture = function(isMultiple, char, posture, name, adverb) {
    ...
    if (char.posture && char.postureFurniture !== this.name) {
      ...
      char.msg(QuestJs.LANG[name + '_on_successful'], tpParams)
    }
...
const SWITCHABLE = function(alreadyOn, nameModifier) {
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (!o.mustBeHeldToOperate || o.isAtLoc(game.player)) {
        verbList.push(o.switchedon ? QuestJs.LANG.verbs.switchoff : QuestJs.LANG.verbs.switchon)
      }
    })
    o.nameModifierFunctions.push(function(o, list) {
  ...
  res.switchon = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.switchedon) {
      char.msg(prefix(this, isMultiple) + QuestJs.LANG.already, {item:this});
      return false;
    }
    if (!this.checkCanSwitchOn()) {
      return false;
    }
    char.msg(QuestJs.LANG.turn_on_successful, tpParams);
    this.doSwitchon();
    return true;
  };
  ...
  res.switchoff = function(isMultiple, char) {
    ...
    char.msg(QuestJs.LANG.turn_off_successful, tpParams);
    this.doSwitchoff();
    return true;
  };
...
const COMPONENT = function(nameOfWhole) {
  const res = {
    ...
    take:function(isMultiple, char) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.cannot_take_component, {char:char, item:this, whole:w[this.loc]});
      return false;
    },
  };
...
const EDIBLE = function(isLiquid) {
  ...
  res.eat = function(isMultiple, char) {
    ...
    if (this.isLiquid) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.cannot_eat, tpParam);
      return false;
    }
    msg(prefix(this, isMultiple) + QuestJs.LANG.eat_successful, tpParam);
    this.loc = null;
  ...
  res.drink = function(isMultiple, char) {
    const tpParam = {char:char, item:this}
    if (!this.isLiquid) {
      msg(prefix(this, isMultiple) + QuestJs.LANG.cannot_drink, tpParam);
      return false;
    }
    msg(prefix(this, isMultiple) + QuestJs.LANG.drink_successful, tpParam);
    this.loc = null;
    if (this.onIngesting) this.onIngesting(char);
    return true;
  };
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(game.player.name) ? QuestJs.LANG.verbs.drop : QuestJs.LANG.verbs.take)
      if (o.isAtLoc(game.player)) verbList.push(o.isLiquid ? QuestJs.LANG.verbs.drink : QuestJs.LANG.verbs.eat)
    })
    //console.log(o.verbFunctions.length)
  }
  return res;
...
const VESSEL = function(capacity) {
  ...
  res.fill = function(isMultiple, char, liquid) {
    ...
    if (!source) return falsemsg(QuestJs.LANG.none_here, {char:char, item:liquid});
    if (!this.mix && this.containedLiquidName !== liquid.name && this.volumeContained > 0) return falsemsg(QuestJs.LANG.cannot_mix(char, this));
    if (this.volumeContained >= this.capacity) return falsemsg(prefix(this, isMultiple) + QuestJs.LANG.already, {item:this});
    ...
    if (this.mix && liquid.name !== this.containedLiquidName !== liquid.name) {
      ...
      msg(prefix(this, isMultiple) + QuestJs.LANG.fill_successful, tpParams)
    }
    return true;
  };
  ...
  res.empty = function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + QuestJs.LANG.empty_successful, tpParams);
    return true;
  };
...
const LIQUID = function(locs) {
  ...
  res.liquid = true;
  res.pronouns = QuestJs.LANG.pronouns.massnoun;
  res.pluralAlias = '*';
...
const ROPE = function(tetheredTo) {
  const res = $.extend({
    ...
    tiedTo1:tetheredTo,
    locs:[],
    attachVerb:QuestJs.LANG.rope_attach_verb,
    attachedVerb:QuestJs.LANG.rope_attached_verb,
    detachVerb:QuestJs.LANG.rope_detach_verb,
    isAtLoc:function(loc, situation) {
      if (this.loc) {
        this.locs = [this.loc]
        delete this.loc
    ...
    examineAddendum:function() {
      ...
      if (this.locs.length === 1) {
        if (obj1 && obj2) return processText(QuestJs.LANG.examineAddBothEnds, {rope:this, obj1:obj1, obj2:obj2})
        if (obj1) return processText(QuestJs.LANG.rope_examine_attached_one_end, {rope:this, obj1:obj1})
        if (obj2) return processText(QuestJs.LANG.rope_examine_attached_one_end, {rope:this, obj1:obj2})
      ...
      if (obj1 || holder1 || loc1) {
        s += ' ' + QuestJs.LANG.rope_one_end + ' '
        flag = true
      }
      if (obj1) {
        s += QuestJs.LANG.rope_examine_end_attached.replace('obj', 'obj1')
      }
      else if (holder1) {
        s += QuestJs.LANG.rope_examine_end_held.replace('holder', 'holder1')
      }
      else if (loc1) {
        s += QuestJs.LANG.rope_examine_end_headed.replace('loc', 'loc1')
      }
      
      if (obj2 || holder2 || loc2) {
        s += ' ' + (flag ? QuestJs.LANG.rope_other_end : QuestJs.LANG.rope_one_end) + ' '
        flag = true
      }
      if (obj2) {
        s += QuestJs.LANG.rope_examine_end_attached.replace('obj', 'obj2')
      }
      else if (holder2) {
        s += QuestJs.LANG.rope_examine_end_held.replace('holder', 'holder2')
      }
      else if (loc2) {
        s += QuestJs.LANG.rope_examine_end_headed.replace('loc', 'loc2')
      }
...
/*  res.drop = function(isMultiple, char) {
    msg(prefix(this, isMultiple) + QuestJs.LANG.drop_successful(char, this));
  ...
  res.take = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.isAtLoc(char.name)) return falsemsg(prefix(this, isMultiple) + QuestJs.LANG.already_have, tpParams)
    if (!char.canManipulate(this, "take")) return false;
...
const BUTTON_DICTIONARY = {
  ...
  onCreation:function(o) {
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(QuestJs.LANG.verbs.push)
    })
  },
}
...
const BUTTON = function() {
  ...
  res.push = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    msg(QuestJs.LANG.push_button_successful, tpParams)
...
const CHARACTER = function() {
  const res = {
    ...
    getStatusDesc:function() {
      ...
      return this.posture + " " + this.postureAdverb + " " + QuestJs.LANG.getName(w[this.postureFurniture], {article:DEFINITE});
    },
    ...
    onCreation:function(o) {
      o.nameModifierFunctions.push(function(o, l) {
        ...
        if (held.length > 0) {
          list.push(QuestJs.LANG.invHoldingPrefix + ' ' + formatList(held, {article:INDEFINITE, lastJoiner:QuestJs.LANG.list_and, modified:false, nothing:QuestJs.LANG.list_nothing, loc:o.name, npc:true}));
        }
        if (worn.length > 0) {
          list.push(QuestJs.LANG.invWearingPrefix + ' ' + formatList(worn, {article:INDEFINITE, lastJoiner:QuestJs.LANG.list_and, modified:false, nothing:QuestJs.LANG.list_nothing, loc:o.name, npc:true}));
        }
        if (list.length > 0) l.push(list.join('; '))
      })
      o.verbFunctions.push(function(o, verbList) {
        verbList.shift()
        verbList.push(QuestJs.LANG.verbs.lookat)
        if (!settings.noTalkTo) verbList.push(QuestJs.LANG.verbs.talkto)
      })
    },
...
const PLAYER = function() {
  const res = CHARACTER()
  res.pronouns = QuestJs.LANG.pronouns.secondperson
  res.player = true
  return res;
}
