...
const FURNITURE = function(options) {
  const res = {
    ...
    getoff:function(isMultiple, char) {
      if (!char.posture) {
        char.QuestJs._io.msg(QuestJs._lang.already, {item:char});
        return false;
      }
      if (char.posture) {
        char.QuestJs._io.msg(QuestJs._lang.stop_posture(char));  // stop_posture handles details
        return true;
      }  
    },
  }
  ...
  res.assumePosture = function(isMultiple, char, posture, name, adverb) {
    ...
    if (char.posture && char.postureFurniture !== this.name) {
      char.QuestJs._io.msg(stop_posture(char))
      char.QuestJs._io.msg(QuestJs._lang[name + '_on_successful'], tpParams)
    }
    else if (char.posture && this[char.posture + "_to_" + posture] && this.postureChangesImplemented) {
      char.QuestJs._io.msg(this[char.posture + "_to_" + posture], {actor:char, item:this})
    }
    else {
      char.QuestJs._io.msg(QuestJs._lang[name + '_on_successful'], tpParams)
    }
...
const SWITCHABLE = function(alreadyOn, nameModifier) {
  ...
  res.switchon = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.switchedon) {
      char.QuestJs._io.msg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
      return false;
    }
    if (!this.checkCanSwitchOn()) {
      return false;
    }
    char.QuestJs._io.msg(QuestJs._lang.turn_on_successful, tpParams);
    this.doSwitchon();
    return true;
  };
  ...
  res.switchoff = function(isMultiple, char) {
    ...
    char.QuestJs._io.msg(QuestJs._lang.turn_off_successful, tpParams);
    this.doSwitchoff();
    return true;
  };
  ...
  const res = {
    ...
    onCreation:function(o) {
      ...
      o.verbFunctions.push(function(o, verbList) {
        ...
        if (!settings.noTalkTo) verbList.push(QuestJs._lang.verbs.talkto)
      })
    },
