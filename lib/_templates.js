...
const COUNTABLE = function (countableLocs) {
  ...
  res.getListAlias = function (loc) {
    return (
      QuestJs._tools.sentenceCase(this.pluralAlias ? this.pluralAlias : this.listalias + 's') +
      ' (' +
      this.countAtLoc(loc) +
      ')'
    );
...
const LOCKED_DOOR = function (key, loc1, loc2, name1, name2) {
  ...
  res._setup = function () {
    ...
    if (!room1[this.dir1])
      return QuestJs._io.errormsg(
        "Bad exit '" +
          ...
          "' for door: " +
          this.name +
          ' (possibly because the room is defined after the door?)',
      );
    ...
    room1[this.dir1].doorName =
      this.name1 ||
      'door to ' +
        QuestJs._lang.getName(QuestJs._w[this.loc2], { article: QuestJs._consts.DEFINITE });
    ...
    room2[this.dir2].doorName =
      this.name2 ||
      'door to ' +
        QuestJs._lang.getName(QuestJs._w[this.loc1], { article: QuestJs._consts.DEFINITE });
  };
  ...
  const res = {
    ...
    take: function (isMultiple, char) {
      QuestJs._io.msg(
        ...
        { char: char, item: this, whole: QuestJs._w[this.loc] },
      );
      return false;
    },
  };
    ...
    update: function (transitButton, callEvent) {
      ...
      if (typeof map !== 'undefined') {
        ...
        if (this.mapCurrentConnection === -1) {
          QuestJs._io.errormsg(
            'Failed to find a location called "' + transitButton.transitDest + '"',
          );
  ...
  const res = {
    ...
    getStatusDesc: function () {
      ...
      return (
        ...
        this.postureAdverb +
        ' ' +
        QuestJs._lang.getName(QuestJs._w[this.postureFurniture], {
    ...
    onCreation: function (o) {
      o.nameModifierFunctions.push(function (o, l) {
        ...
        if (held.length > 0) {
          list.push(
            QuestJs._lang.invHoldingPrefix +
              ' ' +
              QuestJs._tools.formatList(held, {
        ...
        if (worn.length > 0) {
          list.push(
            QuestJs._lang.invWearingPrefix +
              ' ' +
              QuestJs._tools.formatList(worn, {
                article: QuestJs._consts.INDEFINITE,
                lastJoiner: QuestJs._lang.list_and,
                modified: false,
                nothing: QuestJs._lang.list_nothing,
