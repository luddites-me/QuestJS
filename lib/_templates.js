...
const TAKEABLE_DICTIONARY = {
  onCreation:function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(game.player.name) ? QuestJs._lang.verbs.drop : QuestJs._lang.verbs.take)
    })
...
const createEnsemble = function(name, members, dict) {
  ...
  res.nameModifierFunctions = [function(o, list) {
    if (o.members[0].getWorn() && o.isAllTogether() && o.members[0].isAtLoc(game.player.name)) list.push(QuestJs._lang.invModifiers.worn)
  }]
...
const MERCH = function(value, locs) {
  const res = {
    ...
    isAtLoc:function(loc, situation) {
      ...
      if (!w[loc]) QuestJs._io.errormsg("The location name `" + loc + "`, does not match anything in the game.");
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      if (!o.isAtLoc(game.player.name)) {
        ...
        if (!o.getWearRemoveBlocker(game.player, false)) verbList.push(QuestJs._lang.verbs.remove)
      }
      else {
        verbList.push(QuestJs._lang.verbs.drop)
        if (!o.getWearRemoveBlocker(game.player, true)) verbList.push(QuestJs._lang.verbs.wear)
      }
    })
    //QuestJs._log.info(o.verbFunctions.length)

    o.nameModifierFunctions.push(function(o, list) {
      if (o.worn && o.isAtLoc(game.player.name)) list.push(QuestJs._lang.invModifiers.worn)
    })
  }
...
const CONTAINER = function(openable) {
  ...
  res.lookinside = function(isMultiple, char) {
    ...
    //tpParams.list = QuestJs._tools.formatList(this.getContents(world.LOOK), {article:QuestJs._consts.INDEFINITE, lastJoiner:QuestJs._lang.list_and, nothing:QuestJs._lang.list_nothing})
...
const LOCKED_DOOR = function(key, loc1, loc2, name1, name2) {
  ...
  res._setup = function() {
    ...
    room1[this.dir1].doorName = this.name1 || 'door to ' + QuestJs._lang.getName(w[this.loc2], {article:QuestJs._consts.DEFINITE})

    room2[this.dir2].use = QuestJs._util.useWithDoor
    room2[this.dir2].door = this.name
    room2[this.dir2].doorName = this.name2 || 'door to ' + QuestJs._lang.getName(w[this.loc1], {article:QuestJs._consts.DEFINITE})
  }
...
const READABLE = function(mustBeHeld) {
  ...
  res.onCreation = function(o) {
    o.verbFunctions.push(function(o, verbList) {
      if (o.loc === game.player.name || !o.mustBeHeld) verbList.push(QuestJs._lang.verbs.read)
    })
  }
  return res;
}  
...
const COMPONENT = function(nameOfWhole) {
  const res = {
    ...
    take:function(isMultiple, char) {
      QuestJs._io.msg(QuestJs._tools.prefix(this, isMultiple) + QuestJs._lang.cannot_take_component, {char:char, item:this, whole:w[this.loc]});
      return false;
    },
  };
  if (!w[nameOfWhole]) QuestJs._io.debugmsg("Whole is not define: " + nameOfWhole);
...
const EDIBLE = function(isLiquid) {
  ...
  res.onCreation = function(o) {
    ...
    o.verbFunctions.push(function(o, verbList) {
      verbList.push(o.isAtLoc(game.player.name) ? QuestJs._lang.verbs.drop : QuestJs._lang.verbs.take)
      if (o.isAtLoc(game.player)) verbList.push(o.isLiquid ? QuestJs._lang.verbs.drink : QuestJs._lang.verbs.eat)
    })
    //QuestJs._log.info(o.verbFunctions.length)
  }
  return res;
...
const ROPE = function(tetheredTo) {
  ...
  res.moveToFrom = function(toLoc, fromLoc) {
    ...
    if (!w[fromLoc]) QuestJs._io.errormsg("The location name `" + fromLoc + "`, does not match anything in the game.")
    if (!w[toLoc]) QuestJs._io.errormsg("The location name `" + toLoc + "`, does not match anything in the game.")
...
const TRANSIT_BUTTON = function(nameOfTransit) {
  ...
  res.push = function(isMultiple, char) {
    ...
    if (this.locked) {
      QuestJs._tools.printOrRun(game.player, this, "transitLocked");
      return false;
    }
    else if (exit.name === this.transitDest) {
      QuestJs._tools.printOrRun(game.player, this, "transitAlreadyHere");
      return false;
    }
    else {
      QuestJs._tools.printOrRun(game.player, this, "transitGoToDest")
        ...
        for (let button of buttons) {
          if (buttons[i].transitDestAlias === result) {
            if (room[transitDoorDir].name === button.transitDest) {
              QuestJs._tools.printOrRun(game.player, button, "transitAlreadyHere");
            }
            else {
              QuestJs._tools.printOrRun(game.player, button, "transitGoToDest")
...
const CHARACTER = function() {
  const res = {
    ...
    getStatusDesc:function() {
      ...
      return this.posture + " " + this.postureAdverb + " " + QuestJs._lang.getName(w[this.postureFurniture], {article:QuestJs._consts.DEFINITE});
    },
