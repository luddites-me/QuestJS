...
const TAKEABLE_DICTIONARY = {
  ...
  drop:function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + QuestJs._lang.drop_successful, tpParams);
  ...
  const res = {
    ...
    purchase:function(isMultiple, char) {
      ...
      if (!this.isForSale(char.loc)) {
        if (this.doNotClone && this.isAtLoc(char.name)) {
          return failedmsg(QuestJs._lang.cannot_purchase_again, tpParams)
        }
        else {
          return failedmsg(QuestJs._lang.cannot_purchase_here, tpParams)
        }
      }
      
      const cost = this.getBuyingPrice(char);
      tpParams.money = cost
      if (char.money < cost) return failedmsg(prefix(this, isMultiple) + QuestJs._lang.cannot_afford, tpParams);
      this.purchaseScript(isMultiple, char, cost, tpParams)
    },
    
    purchaseScript:function(isMultiple, char, cost, tpParams) {
      char.money -= cost;
      msg(prefix(this, isMultiple) + QuestJs._lang.purchase_successful, tpParams);
      if (this.doNotClone) {
    ...
    sell:function(isMultiple, char) {
      ...
      if (!this.canBeSoldHere(char.loc)) {
        return failedmsg(prefix(this, isMultiple) + QuestJs._lang.cannot_sell_here, tpParams);
      }
      const cost = this.getSellingPrice(char);
      tpParams.money = cost
      char.money += cost;
      msg(prefix(this, isMultiple) + QuestJs._lang.sell_successful, tpParams);
      if (this.doNotClone) {
        this.moveToFrom(char.loc, char.name);
...
const COUNTABLE = function(countableLocs) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!sourceLoc) {
      msg(prefix(this, isMultiple) + QuestJs._lang.none_here, tpParams);
      return false;
    }
  ...
  res.drop = function(isMultiple, char) {
    ...
    if (m === 0) {
      msg(prefix(this, isMultiple) + QuestJs._lang.none_held, tpParams);
      return false;
    }
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.canWearRemove = function(char, toWear) {
    ...
    if (outer) {
      ...
      if (toWear) {
        msg(QuestJs._lang.cannot_wear_over, tpParams);
      }
      else {
        msg(QuestJs._lang.cannot_remove_under, tpParams);
      }
      return false;
    }
    return true;
  ...
  res.wear = function(isMultiple, char) {
    ...
    if (this.wearMsg) {
      ...
      msg(prefix(this, isMultiple) + QuestJs._lang.wear_successful, {garment:this, char:char});
    }
    this.worn = true;
  ...
  res.remove = function(isMultiple, char) {
    ...
    if (this.removeMsg) {
      ...
      msg(prefix(this, isMultiple) + QuestJs._lang.remove_successful, {garment:this, char:char});
    }
    this.worn = false;
...
const OPENABLE_DICTIONARY = {
  open:function(isMultiple, char) {
    ...
    if (!this.openable) {
      msg(prefix(this, isMultiple) + QuestJs._lang.cannot_open, {item:this});
      return false;
    }
    else if (!this.closed) {
    ...
    if (this.locked) {
      if (this.testKeys(char)) {
        this.locked = false;
        this.closed = false;
        msg(prefix(this, isMultiple) + QuestJs._lang.unlock_successful, tpParams);
        this.openMsg(isMultiple, tpParams);
        return true;
      }
      else {
        msg(prefix(this, isMultiple) + QuestJs._lang.locked, tpParams);
        return false;
      }
    }
    this.closed = false;
  ...
  close:function(isMultiple, char) {
    const tpParams = {char:char, container:this}
    if (!this.openable) {
      msg(prefix(this, isMultiple) + QuestJs._lang.cannot_close, {item:this});
      return false;
    }
    else if (this.closed) {
      msg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
  ...
  closeMsg:function(isMultiple, tpParams) {
    msg(prefix(this, isMultiple) + QuestJs._lang.close_successful, tpParams);
  },
...
const CONTAINER = function(openable) {
  ...
  res.lookinside = function(isMultiple, char) {
    ...
    if (this.closed && !this.transparent) {
      msg(prefix(this, isMultiple) + QuestJs._lang.nothing_inside, {char:char});
      return false;
    }
    ...
    msg(prefix(this, isMultiple) + QuestJs._lang.look_inside, tpParams);
    return true;
  };
  
  res.openMsg = function(isMultiple, tpParams) {
    tpParams.list = this.listContents(world.LOOK)
    msg(prefix(this, isMultiple) + QuestJs._lang.open_successful + " " + (tpParams.list === QuestJs._lang.list_nothing ? QuestJs._lang.it_is_empty : QuestJs._lang.look_inside), tpParams)
  };
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.openMsg = function(isMultiple, tpParams) {
    msg(prefix(this, isMultiple) + QuestJs._lang.open_successful, tpParams);
  };
...
const LOCKED_WITH = function(keyNames) {
  ...
  const res = {
    ...
    lock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (this.locked) return falsemsg(QuestJs._lang.already, tpParams)
      if (!this.testKeys(char, true)) return falsemsg(QuestJs._lang.no_key, tpParams)

      if (!this.closed) {
        this.closed = true
        this.locked = true
        msg(QuestJs._lang.close_and_lock_successful, tpParams)
      }
      else {
        this.locked = true
        msg(QuestJs._lang.lock_successful, tpParams)
      }
      return true
    },
    ...
    unlock:function(isMultiple, char) {
      const tpParams = {char:char, container:this}
      if (!this.locked) return falsemsg(QuestJs._lang.already, {item:this})
      if (!this.testKeys(char, false)) return falsemsg(QuestJs._lang.no_key, tpParams)
      msg(QuestJs._lang.unlock_successful, tpParams)
      this.locked = false
...
const COMPONENT = function(nameOfWhole) {
  const res = {
    ...
    take:function(isMultiple, char) {
      msg(prefix(this, isMultiple) + QuestJs._lang.cannot_take_component, {char:char, item:this, whole:w[this.loc]});
      return false;
    },
  };
...
const EDIBLE = function(isLiquid) {
  ...
  res.eat = function(isMultiple, char) {
    ...
    if (this.isLiquid) {
      msg(prefix(this, isMultiple) + QuestJs._lang.cannot_eat, tpParam);
      return false;
    }
    msg(prefix(this, isMultiple) + QuestJs._lang.eat_successful, tpParam);
    this.loc = null;
  ...
  res.drink = function(isMultiple, char) {
    const tpParam = {char:char, item:this}
    if (!this.isLiquid) {
      msg(prefix(this, isMultiple) + QuestJs._lang.cannot_drink, tpParam);
      return false;
    }
    msg(prefix(this, isMultiple) + QuestJs._lang.drink_successful, tpParam);
    this.loc = null;
    if (this.onIngesting) this.onIngesting(char);
    return true;
  };
...
const VESSEL = function(capacity) {
  ...
  res.fill = function(isMultiple, char, liquid) {
    ...
    if (!source) return falsemsg(QuestJs._lang.none_here, {char:char, item:liquid});
    if (!this.mix && this.containedLiquidName !== liquid.name && this.volumeContained > 0) return falsemsg(QuestJs._lang.cannot_mix(char, this));
    if (this.volumeContained >= this.capacity) return falsemsg(prefix(this, isMultiple) + QuestJs._lang.already, {item:this});
    ...
    if (this.mix && liquid.name !== this.containedLiquidName !== liquid.name) {
      ...
      msg(prefix(this, isMultiple) + QuestJs._lang.fill_successful, tpParams)
    }
    return true;
  };
  ...
  res.empty = function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + QuestJs._lang.empty_successful, tpParams);
    return true;
  };
...
/*  res.drop = function(isMultiple, char) {
    msg(prefix(this, isMultiple) + QuestJs._lang.drop_successful(char, this));
  ...
  res.take = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    if (this.isAtLoc(char.name)) return falsemsg(prefix(this, isMultiple) + QuestJs._lang.already_have, tpParams)
    if (!char.canManipulate(this, "take")) return false;
...
const BUTTON = function() {
  ...
  res.push = function(isMultiple, char) {
    const tpParams = {char:char, item:this}
    msg(QuestJs._lang.push_button_successful, tpParams)
