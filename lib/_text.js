...
function processText(str, params) {
  ...
  //} catch (err) {
  //  QuestJs._io.errormsg("Text processor string caused an error, returning unmodified (reported error: " + err + ")");
  //  return str;
  //}
}

QuestJs._text.processText = processText;
...
tp.processText = function(str, params) {
  ...
  if (s) {
    ...
    if (typeof tp.text_processors[left] !== "function") {
      if (left === "player") {
        arr.unshift(QuestJs._game.player.name);
        left = "show";
      }
      else if (QuestJs._w[left]) {
        arr.unshift(left);
        left = "show";
      }
        ...
        QuestJs._io.errormsg("Attempting to use unknown text processor directive '" + left + "' (<i>" + params.tpOriginalString + "</i>)");
        return str;
      }
    }
...
tp.findFirstToken = function (s) {
  ...
  if (start === -1) {
    QuestJs._io.errormsg("Failed to find starting curly brace in text processor (<i>" + s + "</i>)");
    return false;
  }
...
tp.text_processors.rainbow = function(arr, params) {
  ...
  for (let i = 0; i < s.length; i++) {
    result += '<span style="color:' + QuestJs._random.fromArray(colours) + '">' + s.charAt(i) + '</span>'
  }
  return result
};
...
tp.text_processors.select = function(arr, params) {
  const o = QuestJs._w[arr[0]]
  if (!o) QuestJs._io.errormsg('Failed to find an object called "' + arr[0] + '" in text processor select.')
  const l = o[arr[1]]
  if (!l) QuestJs._io.errormsg('Failed to find an attribute called "' + arr[1] + '" for "' + arr[0] + '" in text processor select.')
  const n = o[arr[2]]
  if (!l) QuestJs._io.errormsg('Failed to find an attribute called "' + arr[2] + '" for "' + arr[0] + '" in text processor select.')
  return l[n]
};

tp._findObject = function(name, params, arr) {
  if (params && params[name]) return typeof params[name] === 'string' ? QuestJs._w[params[name]] : params[name]
  if (name === "player") return QuestJs._game.player
  if (QuestJs._w[name]) return QuestJs._w[name]
  const ary = name.split('.')
  if (ary.length === 1) return undefined
  if (ary.length > 2) {
    QuestJs._log.info("The text process cannot handle attributes of attributes, so failed to deal with: " + name)
    QuestJs._log.info(ary)
    return undefined
  }
  arr.unshift(ary[1])
  return QuestJs._w[ary[0]]
}
...
tp.text_processors.show = function(arr, params) {
  ...
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'show' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  name = arr.shift();
  const val = obj[name];
...
tp.text_processors.number = function(arr, params) {
  let name = arr.shift()
  if (name.match(/^\d+$/)) return QuestJs._lang.toWords(parseInt(name))
  if (typeof params[name] === 'number') return QuestJs._lang.toWords(params[name])
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'number' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  
  if (typeof obj[arr[0]] === 'number') {
    return QuestJs._lang.toWords(obj.obj[arr[0]])
  }
  QuestJs._io.errormsg("Failed to find a number for object '" + name + "' in text processor (<i>" + params.tpOriginalString + "</i>)");
  return false;
};

tp.text_processors.ordinal = function(arr, params) {
  let name = arr.shift()
  if (name.match(/^\d+$/)) return QuestJs._lang.toOrdinal(parseInt(name))
  if (typeof params[name] === 'number') return QuestJs._lang.toOrdinal(params[name])
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'number' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  ...
  if (typeof obj[arr[0]] === 'number') {
    return QuestJs._lang.toOrdinal(obj.obj[arr[0]])
  }
  QuestJs._io.errormsg("Failed to find a number for object '" + name + "' in text processor (<i>" + params.tpOriginalString + "</i>)");
  return false;
};
...
tp.text_processors.money = function(arr, params) {
  let name = arr.shift()
  if (name.match(/^\d+$/)) return QuestJs._tools.displayMoney(parseInt(name))
  if (typeof params[name] === 'number') return QuestJs._tools.displayMoney(params[name])
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'money' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  if (obj.loc === QuestJs._game.player.name && obj.getSellingPrice) {
    return QuestJs._tools.displayMoney(obj.getSellingPrice(QuestJs._game.player))
  }
  if (obj.loc === QuestJs._game.player.name && obj.getBuyingPrice) {
    return QuestJs._tools.displayMoney(obj.getBuyingPrice(QuestJs._game.player))
  }
  if (obj.getPrice) {
    return QuestJs._tools.displayMoney(obj.getPrice())
  }
  if (obj.price) {
    return QuestJs._tools.displayMoney(obj.price)
  }
  if (obj.money) {
    return QuestJs._tools.displayMoney(obj.money)
  }
  QuestJs._io.errormsg("Failed to find a price for object '" + name + "' in text processor (<i>" + params.tpOriginalString + "</i>)");
  return false;
};
...
tp.text_processors.handleIf = function(arr, params, reverse) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'if' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  name = arr.shift();
  if (obj[name] === undefined || typeof obj[name] === "boolean") {
    ...
    if (typeof obj[name] === "number") {
      if (isNaN(value)) {
        QuestJs._io.errormsg("Trying to compare a numeric attribute, '" + name + "' with a string.");
        return false;
      }
...
tp.text_processors.handleIfHere = function(arr, params, reverse) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'ifHere' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  let flag = obj.isAtLoc(QuestJs._game.player.loc, world.ALL);
  if (reverse) flag = !flag;
  return (flag ? arr[0] : (arr[1] ? arr[1] : ""));
};
...
tp.text_processors.handleIfLessMoreThan = function(arr, params, moreThan, orEqual) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    QuestJs._io.errormsg("Failed to find object '" + name + "' in text processor 'ifLessMoreThan' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  }
  name = arr.shift();
  if (typeof obj[name] !== "number") {
    QuestJs._io.errormsg("Trying to use ifLessThan with a non-numeric (or nonexistent) attribute, '" + name + "'.");
    return false;
  }
  let value = arr.shift();
  if (isNaN(value)) {
...
tp.text_processors.transitDest = function(arr, params) {
  const transit = arr[0] ? QuestJs._w[arr[0]] : QuestJs._w[QuestJs._game.player.loc]
  if (!transit.transitDoorDir) return QuestJs._io.errormsg("Trying to use the 'transitDest' text process directive when the player is not in a transit location.")
  if (transit.currentButtonName) { 
    const button = QuestJs._w[transit.currentButtonName]
    if (button.title) return button.title
  }
  const destName = transit[transit.transitDoorDir].name
  return QuestJs._lang.getName(QuestJs._w[destName], {capital:true})
};
...
tp.text_processors.cmd = function(arr, params) {
  if (arr.length === 1) {
    return QuestJs._IO.cmdlink(arr[0], arr[0]);
  }
  else {
    return QuestJs._IO.cmdlink(arr[0], arr[1]);
  }
};
...
tp.addDirective("hour", function(arr, params) {
  const hour = QuestJs._util.getDateTimeDict().hour
...
tp.text_processors.popup = function(arr, params) {
  let s1 = arr.shift()
  let s2 = arr.join(':')
  let id = s1.replace(/[^a-zA-Z_]/, '') + QuestJs._random.int(0, 999999999)
...
tp.text_processors.param = function(arr, params) {
  ...
  if (x === undefined) {
    //QuestJs._io.errormsg("In text processor param, could not find a value with the key '" + arr[0] + "'. Check the console (F12) to see what params is. [" + params.tpOriginalString + "]");
    QuestJs._log.info("params:");
    QuestJs._log.info(params);
    return false;
  }
    ...
    const att = (typeof x === "string" ? QuestJs._w[x][arr[1]] : x[arr[1]]);
...
tp.text_processors.terse = function(arr, params) {
  if ((QuestJs._game.verbosity === world.TERSE && QuestJs._game.room.visited === 0) || QuestJs._game.verbosity === world.VERBOSE) {
    return QuestJs._tools.sentenceCase(arr.join(":"))
  }
  else {
    return ''
  }
...
tp.text_processors.hereDesc = function(arr, params) {
  ...
  if (typeof room.desc === 'string') {
    ...
    s = room.desc()
    if (s === undefined) {
      QuestJs._io.errormsg("This room description is not set up properly. It has a 'desc' function that does not return a string.")
...
tp.text_processors.hereName = function(arr, params) {
  const room = QuestJs._w[QuestJs._game.player.loc]
  return QuestJs._lang.getName(room, {article:QuestJs._consts.DEFINITE})
};

tp.text_processors.objectsHere = function(arr, params) {
  const listOfOjects = QuestJs._scope.scopeHereListed();
...
tp.text_processors.objects = function(arr, params) {
  const listOfOjects = QuestJs._scope.scopeHereListed();
  return QuestJs._tools.formatList(listOfOjects, {article:QuestJs._consts.INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:true, nothing:QuestJs._lang.list_nothing, loc:QuestJs._game.player.loc});
}
  
tp.text_processors.exits = function(arr, params) {
  const list = QuestJs._util.exitList();
  return QuestJs._tools.formatList(list, {lastJoiner:QuestJs._lang.list_or, nothing:QuestJs._lang.list_nowhere});
}
...
tp.findSubject = function(arr, params) {
  ...
  if (params[arr[0]]) {
    ...
    if (typeof subject === "string") subject = QuestJs._w[subject]
    if (subject === undefined) {
      QuestJs._io.errormsg("In text processor findSubject, could not find a subject with '" + arr[0] + "'. Check the console (F12) to see what params is. [" + params.tpOriginalString + "]");
      QuestJs._log.info("params:");
      QuestJs._log.info(params);
...
tp.text_processors.nm = function(arr, params) {
  ...
  const opt = {};
  if (arr[1] === 'the') opt.article = QuestJs._consts.DEFINITE;
  if (arr[1] === 'a') opt.article = QuestJs._consts.INDEFINITE;
  if (params[subject.name + '_count']) opt[subject.name + '_count'] = params[subject.name + '_count']
  return arr[2] === 'true' ? QuestJs._tools.sentenceCase(QuestJs._lang.getName(subject, opt)) : QuestJs._lang.getName(subject, opt);
};
...
tp.text_processors.nv = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs._lang.nounVerb(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.pv = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs._lang.pronounVerb(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.vn = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs._lang.verbNoun(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.vp = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs._lang.verbPronoun(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.cj = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return arr[2] === 'true' ? QuestJs._tools.sentenceCase(QuestJs._lang.conjugate (subject, arr[1])) : QuestJs._lang.conjugate (subject, arr[1]);
};
...
tp.handlePronouns = function(arr, params, pronoun) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return arr[1] === 'true' ? QuestJs._tools.sentenceCase(subject.pronouns[pronoun]) : subject.pronouns[pronoun];
};
...
tp.text_processors.pa2 = function(arr, params) {
  ...
  if (chr1.pronouns === chr2.pronouns) {
    const opt = {article:QuestJs._consts.DEFINITE, possessive:true};
    return arr[1] === 'true' ? QuestJs._tools.sentenceCase(QuestJs._lang.getName(chr1, opt)) : QuestJs._lang.getName(chr1, opt)
  }
  
  return arr[1] === 'true' ? QuestJs._tools.sentenceCase(chr1.pronouns.poss_adj) : chr1.pronouns.poss_adj;
};
...
tp.text_processors.pa3 = function(arr, params) {
  ...
  if (chr1 !== chr2) {
    const opt = {article:QuestJs._consts.DEFINITE, possessive:true};
    return arr[1] === 'true' ? QuestJs._tools.sentenceCase(QuestJs._lang.getName(subject, opt)) : QuestJs._lang.getName(subject, opt);
  }
