...
tp.findFirstToken = function (s) {
  ...
  if (start === -1) {
    QuestJs._io.errormsg(
      `Failed to find starting curly brace in text processor (<i>${s}</i>)`,
    );
    return false;
  }
...
tp.text_processors.select = function (arr, params) {
  ...
  if (!o)
    QuestJs._io.errormsg(
      `Failed to find an object called "${arr[0]}" in text processor select.`,
    );
  const l = o[arr[1]];
  if (!l)
    QuestJs._io.errormsg(
...
tp.text_processors.number = function (arr, params) {
  ...
  if (typeof params[name] === 'number')
    return QuestJs._lang.toWords(params[name]);
  const obj = tp._findObject(name, params, arr);
  if (!obj) {
    QuestJs._io.errormsg(
...
tp.text_processors.ordinal = function (arr, params) {
  ...
  if (typeof params[name] === 'number')
    return QuestJs._lang.toOrdinal(params[name]);
  const obj = tp._findObject(name, params, arr);
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'number' (<i>${params.tpOriginalString}</i>)`,
...
tp.text_processors.money = function (arr, params) {
  ...
  if (typeof params[name] === 'number')
    return QuestJs._tools.displayMoney(params[name]);
  const obj = tp._findObject(name, params, arr);
  if (!obj) {
    QuestJs._io.errormsg(
  ...
  if (obj.loc === QuestJs._game.player.name && obj.getSellingPrice) {
    return QuestJs._tools.displayMoney(
      obj.getSellingPrice(QuestJs._game.player),
    );
  }
  if (obj.loc === QuestJs._game.player.name && obj.getBuyingPrice) {
    return QuestJs._tools.displayMoney(
      obj.getBuyingPrice(QuestJs._game.player),
    );
  }
  if (obj.getPrice) {
...
tp.text_processors.handleIf = function (arr, params, reverse) {
  ...
  if (obj[name] === undefined || typeof obj[name] === 'boolean') {
    ...
    if (typeof obj[name] === 'number') {
      if (isNaN(value)) {
        QuestJs._io.errormsg(
          `Trying to compare a numeric attribute, '${name}' with a string.`,
        );
        return false;
      }
...
tp.text_processors.handleIfLessMoreThan = function (
  arr,
  params,
  moreThan,
  orEqual,
...
tp.text_processors.transitDest = function (arr, params) {
  const transit = arr[0]
    ? QuestJs._w[arr[0]]
    : QuestJs._w[QuestJs._game.player.loc];
...
tp.text_processors.terse = function (arr, params) {
  if (
    (QuestJs._game.verbosity === world.TERSE &&
      QuestJs._game.room.visited === 0) ||
