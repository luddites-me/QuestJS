    ...
    if (typeof tp.text_processors[left] !== 'function') {
      if (left === 'player') {
        ...
        QuestJs._io.errormsg(
          "Attempting to use unknown text processor directive '" +
            left +
            "' (<i>" +
...
tp.text_processors.rainbow = function (arr, params) {
  ...
  for (let i = 0; i < s.length; i++) {
    result +=
      '<span style="color:' + QuestJs._random.fromArray(colours) + '">' + s.charAt(i) + '</span>';
  }
  return result;
};
...
tp.text_processors.blur = function (arr, params) {
  const n = arr.shift();
  return (
    '<span style="color:transparent;text-shadow: 0 0 ' +
    n +
    'px rgba(0,0,0,1);">' +
    arr.join(':') +
    '</span>'
...
tp.text_processors.select = function (arr, params) {
  ...
  if (!l)
    QuestJs._io.errormsg(
      'Failed to find an attribute called "' +
        arr[1] +
        '" for "' +
        arr[0] +
        '" in text processor select.',
    );
  const n = o[arr[2]];
  if (!l)
    QuestJs._io.errormsg(
...
tp.text_processors.number = function (arr, params) {
  ...
  QuestJs._io.errormsg(
    "Failed to find a number for object '" +
      name +
      "' in text processor (<i>" +
      params.tpOriginalString +
      '</i>)',
...
tp.text_processors.money = function (arr, params) {
  ...
  QuestJs._io.errormsg(
    "Failed to find a price for object '" +
      name +
      "' in text processor (<i>" +
      params.tpOriginalString +
      '</i>)',
...
tp.text_processors.handleIfLessMoreThan = function (arr, params, moreThan, orEqual) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      "Failed to find object '" +
        name +
        "' in text processor 'ifLessMoreThan' (<i>" +
        params.tpOriginalString +
        '</i>)',
    );
    return false;
...
tp.findSubject = function (arr, params) {
  ...
  if (params[arr[0]]) {
    ...
    if (subject === undefined) {
      QuestJs._io.errormsg(
        "In text processor findSubject, could not find a subject with '" +
          arr[0] +
          "'. Check the console (F12) to see what params is. [" +
          params.tpOriginalString +
          ']',
      );
      QuestJs._log.info('params:');
    ...
    if (subject === undefined) {
      const s =
        'In text processor findSubject, could not find a key called `' +
        arr[0] +
        '` in the string `' +
        params.tpOriginalString +
        '`';
