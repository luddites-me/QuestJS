...
tp.text_processors.hereName = function(arr, params) {
  const room = w[game.player.loc]
  return QuestJs._lang.getName(room, {article:DEFINITE})
};
...
tp.text_processors.objects = function(arr, params) {
  const listOfOjects = scopeHereListed();
  return formatList(listOfOjects, {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:true, nothing:QuestJs._lang.list_nothing, loc:game.player.loc});
}
  
tp.text_processors.exits = function(arr, params) {
  const list = util.exitList();
  return formatList(list, {lastJoiner:QuestJs._lang.list_or, nothing:QuestJs._lang.list_nowhere});
}
...
tp.text_processors.nm = function(arr, params) {
  ...
  return arr[2] === 'true' ? sentenceCase(QuestJs._lang.getName(subject, opt)) : QuestJs._lang.getName(subject, opt);
};
...
tp.text_processors.cj = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return arr[2] === 'true' ? sentenceCase(QuestJs._lang.conjugate (subject, arr[1])) : QuestJs._lang.conjugate (subject, arr[1]);
};
...
tp.text_processors.pa2 = function(arr, params) {
  ...
  if (chr1.pronouns === chr2.pronouns) {
    ...
    return arr[1] === 'true' ? sentenceCase(QuestJs._lang.getName(chr1, opt)) : QuestJs._lang.getName(chr1, opt)
  }
...
tp.text_processors.pa3 = function(arr, params) {
  ...
  if (chr1 !== chr2) {
    const opt = {article:DEFINITE, possessive:true};
    return arr[1] === 'true' ? sentenceCase(QuestJs._lang.getName(subject, opt)) : QuestJs._lang.getName(subject, opt);
  }
