    ...
    if (typeof tp.text_processors[left] !== 'function') {
      if (left === 'player') {
        ...
        QuestJs._io.errormsg(
          `Attempting to use unknown text processor directive '${left}' (<i>${params.tpOriginalString}</i>)`,
        );
        return str;
      }
    }
...
tp.text_processors.blur = function (arr, params) {
  const n = arr.shift();
  return `<span style="color:transparent;text-shadow: 0 0 ${n}px rgba(0,0,0,1);">${arr.join(
    ':',
  )}</span>`;
};
...
tp.text_processors.select = function (arr, params) {
  ...
  if (!o)
    QuestJs._io.errormsg(`Failed to find an object called "${arr[0]}" in text processor select.`);
  const l = o[arr[1]];
  if (!l)
    QuestJs._io.errormsg(
      `Failed to find an attribute called "${arr[1]}" for "${arr[0]}" in text processor select.`,
    );
  const n = o[arr[2]];
  if (!l)
    QuestJs._io.errormsg(
      `Failed to find an attribute called "${arr[2]}" for "${arr[0]}" in text processor select.`,
    );
  return l[n];
};
...
tp._findObject = function (name, params, arr) {
  if (params && params[name])
    return typeof params[name] === 'string' ? QuestJs._w[params[name]] : params[name];
  ...
  if (ary.length > 2) {
    QuestJs._log.info(
      `The text process cannot handle attributes of attributes, so failed to deal with: ${name}`,
    );
...
tp.text_processors.show = function (arr, params) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'show' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
  name = arr.shift();
...
tp.text_processors.number = function (arr, params) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'number' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
  ...
  QuestJs._io.errormsg(
    `Failed to find a number for object '${name}' in text processor (<i>${params.tpOriginalString}</i>)`,
  );
  return false;
};
...
tp.text_processors.money = function (arr, params) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'money' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
  ...
  QuestJs._io.errormsg(
    `Failed to find a price for object '${name}' in text processor (<i>${params.tpOriginalString}</i>)`,
  );
  return false;
};
...
tp.text_processors.handleIf = function (arr, params, reverse) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'if' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
  name = arr.shift();
...
tp.text_processors.handleIfHere = function (arr, params, reverse) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'ifHere' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
...
tp.text_processors.handleIfLessMoreThan = function (arr, params, moreThan, orEqual) {
  ...
  if (!obj) {
    QuestJs._io.errormsg(
      `Failed to find object '${name}' in text processor 'ifLessMoreThan' (<i>${params.tpOriginalString}</i>)`,
    );
    return false;
  }
  name = arr.shift();
  if (typeof obj[name] !== 'number') {
    QuestJs._io.errormsg(
      `Trying to use ifLessThan with a non-numeric (or nonexistent) attribute, '${name}'.`,
    );
    return false;
  }
  let value = arr.shift();
...
tp.text_processors.transitDest = function (arr, params) {
  ...
  if (!transit.transitDoorDir)
    return QuestJs._io.errormsg(
      "Trying to use the 'transitDest' text process directive when the player is not in a transit location.",
    );
...
tp.addDirective('hour', (arr, params) => {
...
tp.text_processors.terse = function (arr, params) {
  if (
    (QuestJs._game.verbosity === world.TERSE && QuestJs._game.room.visited === 0) ||
    QuestJs._game.verbosity === world.VERBOSE
  ) {
...
tp.text_processors.hereDesc = function (arr, params) {
  ...
  if (typeof room.desc === 'string') {
    ...
    if (s === undefined) {
      QuestJs._io.errormsg(
        "This room description is not set up properly. It has a 'desc' function that does not return a string.",
      );
...
tp.text_processors.objects = function (arr, params) {
  const listOfOjects = QuestJs._scope.scopeHereListed();
  return QuestJs._tools.formatList(listOfOjects, {
...
tp.text_processors.exits = function (arr, params) {
  const list = QuestJs._util.exitList();
  return QuestJs._tools.formatList(list, {
    lastJoiner: QuestJs._lang.list_or,
    nothing: QuestJs._lang.list_nowhere,
  });
};
...
tp.findSubject = function (arr, params) {
  ...
  if (params[arr[0]]) {
    ...
    if (subject === undefined) {
      QuestJs._io.errormsg(
        `In text processor findSubject, could not find a subject with '${arr[0]}'. Check the console (F12) to see what params is. [${params.tpOriginalString}]`,
      );
      QuestJs._log.info('params:');
      QuestJs._log.info(params);
...
tp.text_processors.nm = function (arr, params) {
  ...
  return arr[2] === 'true'
    ? QuestJs._tools.sentenceCase(QuestJs._lang.getName(subject, opt))
    : QuestJs._lang.getName(subject, opt);
};
...
tp.text_processors.cj = function (arr, params) {
  ...
  if (!subject) return false;
  return arr[2] === 'true'
    ? QuestJs._tools.sentenceCase(QuestJs._lang.conjugate(subject, arr[1]))
    : QuestJs._lang.conjugate(subject, arr[1]);
};
...
tp.handlePronouns = function (arr, params, pronoun) {
  ...
  if (!subject) return false;
  return arr[1] === 'true'
    ? QuestJs._tools.sentenceCase(subject.pronouns[pronoun])
...
tp.text_processors.pa2 = function (arr, params) {
  ...
  if (chr1.pronouns === chr2.pronouns) {
    ...
    return arr[1] === 'true'
      ? QuestJs._tools.sentenceCase(QuestJs._lang.getName(chr1, opt))
      : QuestJs._lang.getName(chr1, opt);
  }

  return arr[1] === 'true'
    ? QuestJs._tools.sentenceCase(chr1.pronouns.poss_adj)
