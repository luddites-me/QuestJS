...
tp.text_processors.number = function(arr, params) {
  let name = arr.shift()
  if (name.match(/^\d+$/)) return QuestJs.LANG.toWords(parseInt(name))
  if (typeof params[name] === 'number') return QuestJs.LANG.toWords(params[name])
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
  ...
  if (typeof obj[arr[0]] === 'number') {
    return QuestJs.LANG.toWords(obj.obj[arr[0]])
  }
...
tp.text_processors.ordinal = function(arr, params) {
  let name = arr.shift()
  if (name.match(/^\d+$/)) return QuestJs.LANG.toOrdinal(parseInt(name))
  if (typeof params[name] === 'number') return QuestJs.LANG.toOrdinal(params[name])
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'number' (<i>" + params.tpOriginalString + "</i>)");
    return false;
  ...
  if (typeof obj[arr[0]] === 'number') {
    return QuestJs.LANG.toOrdinal(obj.obj[arr[0]])
  }
  errormsg("Failed to find a number for object '" + name + "' in text processor (<i>" + params.tpOriginalString + "</i>)");
  return false;
};
...
tp.text_processors.transitDest = function(arr, params) {
  ...
  return QuestJs.LANG.getName(w[destName], {capital:true})
};
...
tp.text_processors.hereName = function(arr, params) {
  const room = w[game.player.loc]
  return QuestJs.LANG.getName(room, {article:DEFINITE})
};
...
tp.text_processors.objects = function(arr, params) {
  const listOfOjects = scopeHereListed();
  return formatList(listOfOjects, {article:INDEFINITE, lastJoiner:QuestJs.LANG.list_and, modified:true, nothing:QuestJs.LANG.list_nothing, loc:game.player.loc});
}
  
tp.text_processors.exits = function(arr, params) {
  const list = util.exitList();
  return formatList(list, {lastJoiner:QuestJs.LANG.list_or, nothing:QuestJs.LANG.list_nowhere});
}
...
tp.text_processors.nm = function(arr, params) {
  ...
  return arr[2] === 'true' ? sentenceCase(QuestJs.LANG.getName(subject, opt)) : QuestJs.LANG.getName(subject, opt);
};
...
tp.text_processors.nv = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs.LANG.nounVerb(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.pv = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs.LANG.pronounVerb(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.vn = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs.LANG.verbNoun(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.vp = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return QuestJs.LANG.verbPronoun(subject, arr[1], arr[2] === 'true');
};

tp.text_processors.cj = function(arr, params) {
  const subject = tp.findSubject(arr, params);
  if (!subject) return false;
  return arr[2] === 'true' ? sentenceCase(QuestJs.LANG.conjugate (subject, arr[1])) : QuestJs.LANG.conjugate (subject, arr[1]);
};
...
tp.text_processors.pa2 = function(arr, params) {
  ...
  if (chr1.pronouns === chr2.pronouns) {
    ...
    return arr[1] === 'true' ? sentenceCase(QuestJs.LANG.getName(chr1, opt)) : QuestJs.LANG.getName(chr1, opt)
  }
...
tp.text_processors.pa3 = function(arr, params) {
  ...
  if (chr1 !== chr2) {
    const opt = {article:DEFINITE, possessive:true};
    return arr[1] === 'true' ? sentenceCase(QuestJs.LANG.getName(subject, opt)) : QuestJs.LANG.getName(subject, opt);
  }
