

// Should all be language neutral
QuestJs._defaults.DEFAULT_OBJECT = {
  pronouns:QuestJs._lang.pronouns.thirdperson,
  
  isAtLoc:function(loc, situation) {
    if (typeof loc !== "string") loc = loc.name;
    if (!QuestJs._w[loc]) QuestJs._io.errormsg("The location name `" + loc + "`, does not match anything in the game.");
    if (this.complexIsAtLoc) {
      if (!this.complexIsAtLoc(loc, situation)) return false;
    }
    else {
      if (this.loc !== loc) return false;
    }
    if (situation === undefined) return true;
    if (situation === QuestJs._world.LOOK && this.scenery) return false;
    if (situation === QuestJs._world.SIDE_PANE && this.scenery) return false;
    return true;
  },
  
  isHere:function() {
    return this.isAtLoc(QuestJs._game.player.loc);
  },
  
  isHeld:function() {
    return this.isAtLoc(QuestJs._game.player.name);
  },
  
  isHereOrHeld:function() {
    return this.isHere() || this.isHeld();
  },
  
  countAtLoc:function(loc) {
    if (typeof loc !== "string") loc = loc.name;
    return this.isAtLoc(loc) ? 1 : 0;
  },
  
  scopeSnapshot:function(visible) {
    if (this.scopeStatus) return  // already done this one
    
    this.scopeStatus = visible ? QuestJs._world.VISIBLE : QuestJs._world.REACHABLE  // set the value
    
    if (!this.getContents && !this.componentHolder) return // no lower levels so done

    let l    
    if (this.getContents) {
      // this is a container, so get the contents
      if (!this.canSeeThrough() && !this.scopeStatusForRoom && this !== QuestJs._game.player) {
        // cannot see or reach contents
        return
      }
      if (!this.canReachThrough() && this.scopeStatusForRoom !== QuestJs._world.REACHABLE && this !== QuestJs._game.player) {
        // can see but not reach contents
        visible = true
      }
      l = this.getContents(QuestJs._world.SCOPING)
    }
    else {
      // this has components, so get them
      l = []
      for (let key in QuestJs._w) {
        if (QuestJs._w[key].loc === this.name) l.push(QuestJs._w[key]);
      }
    }
    for (let el of l) {
      // go through them
      el.scopeSnapshot(visible)
    }
  },
  
  canReachThrough:() => false,
  canSeeThrough:() => false,
  itemTaken:QuestJs._consts.NULL_FUNC,
  itemDropped:QuestJs._consts.NULL_FUNC,
  canTalkPlayer:() => false,
  getExits:function() { return []; },
  hasExit:dir => false,
  getWorn:() => false,
  
  
  moveToFrom:function(toLoc, fromLoc) {
    if (fromLoc === undefined) fromLoc = this.loc;
    if (fromLoc === toLoc) return;
    
    if (!QuestJs._w[fromLoc]) QuestJs._io.errormsg("The location name `" + fromLoc + "`, does not match anything in the game.");
    if (!QuestJs._w[toLoc]) QuestJs._io.errormsg("The location name `" + toLoc + "`, does not match anything in the game.");
    this.loc = toLoc; 
    QuestJs._w[fromLoc].itemTaken(this);
    QuestJs._w[toLoc].itemDropped(this);
    if (this.onMove !== undefined) this.onMove(toLoc, fromLoc)
  },
    
  postLoad:QuestJs._consts.NULL_FUNC,

  templatePostLoad:function() {
    this.postLoad();
  },
  
  preSave:QuestJs._consts.NULL_FUNC,

  templatePreSave:function() {
    this.preSave();
  },
  
  getSaveString:function() {
    this.templatePreSave();
    let s = "Object=";
    for (let key in this) {
      if (typeof this[key] !== "function") {
        if (key !== "name" && key !== "gameState") {
          s += QuestJs._saveLoad.encode(key, this[key]);
        }
      }
    }
    return s;
  },

  eventActive:false,
  eventCountdown:0,
  eventIsActive:function() { return this.eventActive},
  doEvent:function(turn) {
    //QuestJs._log.info("this=" + this.name);
    // Not active, so stop
    if (!this.eventIsActive()) return;
    // Countdown running, so stop
    if (this.eventCountdown > 1) {
      this.eventCountdown--;
      return;
    }
    // If there is a condition and it is not met, stop
    //QuestJs._log.info("this=" + this.name);
    if (this.eventCondition && !this.eventCondition(turn)) return;
    //QuestJs._log.info("this=" + this.name);
    this.eventScript(turn);
    if (typeof this.eventPeriod === "number") {
      this.eventCountdown = this.eventPeriod;
    }
    else {
      this.eventActive = false;
    }
  },
};

QuestJs._defaults.DEFAULT_ROOM = {
  room:true,
  beforeEnter:QuestJs._consts.NULL_FUNC,
  beforeFirstEnter:QuestJs._consts.NULL_FUNC,
  afterEnter:QuestJs._consts.NULL_FUNC,
  afterEnterIf:{},
  afterEnterIfFlags:'',
  afterFirstEnter:QuestJs._consts.NULL_FUNC,
  onExit:QuestJs._consts.NULL_FUNC,
  visited:0,
  
  lightSource:() => QuestJs._world.LIGHT_FULL,

  description:function() {
    if (QuestJs._game.dark) {
      QuestJs._tools.printOrRun(QuestJs._game.player, this, "darkDesc");
      return true;
    }
    for (let line of QuestJs._settings.roomTemplate) {
      QuestJs._io.msg(line);
    }
    return true;
  },
  
  examine:function() {
    if (QuestJs._game.dark) {
      QuestJs._tools.printOrRun(QuestJs._game.player, this, "darkDesc");
      return true;
    }
    QuestJs._io.msg(typeof this.desc === 'string' ? this.desc : this.desc())
    return true;
  },
  
  darkDescription:() => QuestJs._io.msg("It is dark."),
  
  getContents:QuestJs._util.getContents,
  
  getExits:function(options) {
    const list = [];
    for (let exit of QuestJs._lang.exit_list) {
      if (this.hasExit(exit.name, options)) {
        list.push(this[exit.name]);
      }
    }
    return list; 
  },
  
  // returns null if there are no exits
  getRandomExit:function(options) { return QuestJs._random.fromArray(this.getExits(options)) },
  
  hasExit:function(dir, options) {
    //QuestJs._log.info(this.name)
    //QuestJs._log.info(dir)
    if (options === undefined) options = {};
    if (!this[dir]) return false;
    //QuestJs._log.info(this[dir])
    if (options.excludeLocked && this[dir].isLocked()) return false;
    if (options.excludeScenery && this[dir].scenery) return false;
    return !this[dir].isHidden();
  },
  
  findExit:function(dest, options) {
    if (typeof dest === "object") dest = dest.name;
    for (let exit of QuestJs._lang.exit_list) {
      if (this.hasExit(exit.name, options) && this[exit.name].name === dest) {
        return this[exit.name];
      }
    }
    return null;
  },

  // Lock or unlock the exit indicated
  // Returns false if the exit does not exist or is not an Exit object
  // Returns true if successful
  setExitLock:function(dir, locked) {
    if (!this[dir]) { return false; }
    const ex = this[dir];
    this[dir].locked = locked;
    return true;  
  },

  // Hide or unhide the exit indicated
  // Returns false if the exit does not exist or is not an Exit object
  // Returns true if successful
  setExitHide:function(dir, hidden) {
    if (!this[dir]) { return false; }
    this[dir].hidden = hidden;
    return true;  
  },
  
  
  templatePreSave:function() {
    /*for (let i = 0; i < QuestJs._lang.exit_list.length; i++) {
      const dir = QuestJs._lang.exit_list[i].name;
      if (this[dir] !== undefined) {
        this["customSaveExit" + dir] = (this[dir].locked ? "locked" : "");
        this["customSaveExit" + dir] += "/" + (this[dir].hidden ? "hidden" : "");
        if (this.saveExitDests) this["customSaveExitDest" + dir] = this[dir].name;
      }
    }*/
    this.preSave();
  },

  templatePostLoad:function() {
    for (let exit of QuestJs._lang.exit_list) {
      const dir = exit.name;
      if (this["customSaveExit" + dir]) {
        this[dir].locked = /locked/.test(this["customSaveExit" + dir]);
        this[dir].hidden = /hidden/.test(this["customSaveExit" + dir]);
        delete this["customSaveExit" + dir];
        if (this.saveExitDests) {
          this[dir].name = this["customSaveExitDest" + dir];
          //QuestJs._log.info("Just set " + dir + " in " + this.name + " to " + this["customSaveExitDest" + dir])
          delete this["customSaveExitDest" + dir];
        }
      }
    }
    this.postLoad();
  },
  
  getReverseExit:function(dir) {
    const reverseDir = QuestJs._lang.exit_list.find(el => el.name === dir)
    const dest = this[dir]
    return dest[reverseDir]
  },

//    


};

QuestJs._defaults.DEFAULT_ITEM = {
  lightSource:() => QuestJs._world.LIGHT_NONE,
  icon:() => "",
  testKeys:(char, toLock) => false,
  getVerbs:function() {
    const verbList = []
    //QuestJs._log.info('verbs for ' + this.name)
    //QuestJs._log.info('count ' + this.verbFunctions.length)
    //QuestJs._log.info(verbList)
    for (let f of this.verbFunctions) f(this, verbList)

    //QuestJs._log.info(verbList)
    if (!this.isAtLoc(QuestJs._game.player.name)) {
      if (this.hereVerbs) {
        for (let s of this.hereVerbs) verbList.push(s)
      }
    }
    else if (this.getWorn()) {
      if (this.wornVerbs) {
        for (let s of this.wornVerbs) verbList.push(s)
      }
    }
    else {
      if (this.heldVerbs) {
        for (let s of this.heldVerbs) verbList.push(s)
      }
    }
    if (this.verbFunction) this.verbFunction(verbList)
    return verbList;
  },
};

