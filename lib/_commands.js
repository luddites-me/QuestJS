...
QuestJs._commands = [
  ...
  new QuestJs._command.Cmd('MetaPronouns', {
    script:function() {
      ...
      console.log(QuestJs._parser.pronouns)
    },
  }),
  ...
  new QuestJs._command.Cmd('Look', {
    script:function() {
      ...
      return QuestJs._settings.lookCountsAsTurn ? world.SUCCESS : world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
  ...
  new QuestJs._command.Cmd('Inv', {
    script:function() {
      ...
      QuestJs._io.msg(QuestJs._lang.inventory_prefix + " " + formatList(listOfOjects, {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:true, nothing:QuestJs._lang.list_nothing, loc:game.player.name}) + ".");
      return QuestJs._settings.lookCountsAsTurn ? world.SUCCESS : world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
  ...
  new QuestJs._command.Cmd('PurchaseFromList', {
    script:function() {
      const l = [];
      for (let key in w) {
        if (QuestJs._parser.isForSale(w[key])) {
      ...
      QuestJs._io.msg(current_money + ": " + world.Money(game.player.money));
  ...
  new QuestJs._command.Cmd('Say', {
    script:function(arr) {
      ...
      if (QuestJs._settings.givePlayerSayMsg) QuestJs._io.msg(QuestJs._lang.nounVerb(game.player, arr[0], true) + ", '" + sentenceCase(arr[1]) + ".'");
      for (let chr of l) {
...
if (QuestJs._settings.playMode === 'dev') {
    ...
    script:function(objects) {
      ...
      for (let el of wt) {
        if (typeof el === "string") {
          ...
          console.log(el.cmd)
          console.log(QuestJs._settings.walkthroughMenuResponses)
  ...
  QuestJs._commands.push(new QuestJs._command.Cmd('DebugInspect', {
    script:function(arr) {
      const item = arr[0][0];
      QuestJs._io.debugmsg("See the console for details on the object " + item.name + " (press F12 to world. the console)");
      console.log(item);
      return world.SUCCESS_NO_TURNSCRIPTS; 
    },
    objects:[
  ...
  QuestJs._commands.push(new QuestJs._command.Cmd('DebugInspectByName', {
    script:function(arr) {
      ...
      QuestJs._io.debugmsg("See the console for details on the object " + item_name + " (press F12 to world. the console)");
  ...
  QuestJs._commands.push(new QuestJs._command.Cmd('DebugTest', {
    script:function() {
      if (!QuestJs._settings.tests) {
        ...
        console.log('To activate testing in your game, set QuestJs._settings.tests to true. More details here: https://github.com/ThePix/QuestJS/wiki/Unit-testing')
        return world.SUCCESS_NO_TURNSCRIPTS;
      }
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(char.name)) {
        QuestJs._io.failedmsg(prefix(obj, multiple) + QuestJs._lang.not_carrying, {char:char, item:obj});
      }
      else {
        obj.moveToFrom(container.name, char.name);
        QuestJs._io.msg(prefix(obj, multiple) + QuestJs._lang.done_msg);
        success = true;
      }
    }
  }
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(container.name)) {
        QuestJs._io.failedmsg(prefix(obj, multiple) + QuestJs._lang.not_inside(container, obj));
      }
      else {
...
const handleTieTo = function(char, rope, obj2) {
  ...
  if (rope.tiedTo1 && rope.tiedTo2) return QuestJs._io.failedmsg("It is already {rope.attachedVerb} to {nm:obj1:the} and {nm:obj12:the}.", {rope:w[rope.tiedTo1], obj1:w[rope.tiedTo2], obj2:w[rope.tiedTo2]})
...
const handleUntieFrom = function(char, rope, obj2) {
  if (rope !== w.rope) return QuestJs._io.failedmsg("You cannot attach that to - or detach it from - anything.", {rope:rope})
  if (obj2 === undefined) {
