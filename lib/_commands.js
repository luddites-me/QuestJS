...
QuestJs._commands = [
  ...
  new QuestJs._command.Cmd('MetaSilent', {
    script:function() {
      if (settings.silent) {
        metamsg(QuestJs._lang.mode_silent_off)
        settings.silent = false
      }
      else {
        metamsg(QuestJs._lang.mode_silent_on)
  ...
  new QuestJs._command.Cmd('MetaSpoken', {
    script:function() {
      if (io.spoken) {
        io.spoken = false
        metamsg(QuestJs._lang.spoken_off)
      }
      else {
        io.spoken = true
        metamsg(QuestJs._lang.spoken_on)
      }
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  }),
  ...
  new QuestJs._command.Cmd('MetaBrief', {
    script:function() {
      ...
      metamsg(QuestJs._lang.mode_brief)
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  }),
  new QuestJs._command.Cmd('MetaTerse', {
    script:function() {
      ...
      metamsg(QuestJs._lang.mode_terse)
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  }),
  ...
  new QuestJs._command.Cmd('MetaTranscriptOn', {
    script:function() {
      if (io.transcript) {
        metamsg(QuestJs._lang.transcript_already_on)
        return world.FAILED
      }
      io.scriptStart()
      return world.SUCCESS_NO_TURNSCRIPTS
  ...
  new QuestJs._command.Cmd('MetaTranscriptOff', {
    script:function() {
      if (!io.transcript) {
        metamsg(QuestJs._lang.transcript_already_off)
        return world.FAILED
      }
      io.scriptEnd()
      return world.SUCCESS_NO_TURNSCRIPTS
  ...
  new QuestJs._command.Cmd('MetaUndo', {
    script:function() {
      if (settings.maxUndo === 0) {
        metamsg(QuestJs._lang.undo_disabled)
        return world.FAILED
      }
      if (game.gameState.length < 2) {
        metamsg(QuestJs._lang.undo_not_available)
        return world.FAILED
      }
      game.gameState.pop()
      const gameState = game.gameState[game.gameState.length - 1]
      metamsg(QuestJs._lang.undo_done)
  ...
  new QuestJs._command.Cmd('MetaRestart', {
    script:function() {
      askQuestion(QuestJs._lang.restart_are_you_sure, function(result) {
        if (result.match(QuestJs._lang.yes_regex)) {
          location.reload()
        }
        else {
          metamsg(QuestJs._lang.restart_no)
        }
      });
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  ...
  new QuestJs._command.Cmd('MetaScore', {
    script:function() {
      metamsg(QuestJs._lang.scores_not_implemented)
    },
  }),
  ...
  new QuestJs._command.Cmd('Exits', {
    script:function() {
      msg(QuestJs._lang.can_go);
      return settings.lookCountsAsTurn ? world.SUCCESS : world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
  new QuestJs._command.Cmd('Wait', {
    script:function() {
      msg(QuestJs._lang.wait_msg);
  ...
  new QuestJs._command.Cmd('Inv', {
    script:function() {
      ...
      msg(QuestJs._lang.inventory_prefix + " " + formatList(listOfOjects, {article:INDEFINITE, lastJoiner:QuestJs._lang.list_and, modified:true, nothing:QuestJs._lang.list_nothing, loc:game.player.name}) + ".");
      return settings.lookCountsAsTurn ? world.SUCCESS : world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
  new QuestJs._command.Cmd('Map', {
    script:function() {
      if (typeof showMap !== 'undefined') {
        ...
        if (!zone.zone) {
          return failedmsg(QuestJs._lang.no_map);
        }
        else {
          const flag = zone.drawMap()
          if (!flag) return failedmsg(QuestJs._lang.no_map);
          return world.SUCCESS_NO_TURNSCRIPTS
        }
      }
    },
  ...
  new QuestJs._command.Cmd('Smell', {
    script:function() {
      if (game.room.onSmell) {
        ...
        msg(QuestJs._lang.no_smell, {char:game.player});
      }
      return world.SUCCESS;
    },
  }),
  new QuestJs._command.Cmd('Listen', {
    script:function() {
      if (game.room.onListen) {
        ...
        msg(QuestJs._lang.no_listen, {char:game.player});
      }
      return world.SUCCESS;
    },
  }),
  new QuestJs._command.Cmd('PurchaseFromList', {
    script:function() {
      ...
      if (l.length === 0) {
        return failedmsg(QuestJs._lang.nothing_for_sale);
      }
  ...
  new QuestJs._command.Cmd('TalkTo', {
    ...
    default:function(item) {
      return failedmsg(QuestJs._lang.cannot_talk_to, {char:game.player, item:item});
    },
  }),
  ...
  new QuestJs._command.Cmd('Topics', {
    ...
    default:function(item) {
      return failedmsg(QuestJs._lang.no_topics, {char:game.player, item:item});
    },
  }),
  ...
  new QuestJs._command.Cmd('Say', {
    script:function(arr) {
      ...
      if (l.length === 0) {
        msg(QuestJs._lang.say_no_one_here(game.player, arr[0], arr[1]));
        return world.SUCCESS;
      }

      if (settings.givePlayerSayMsg) msg(QuestJs._lang.nounVerb(game.player, arr[0], true) + ", '" + sentenceCase(arr[1]) + ".'");
      for (let chr of l) {
      ...
      if (settings.givePlayerSayMsg) {
        msg(QuestJs._lang.say_no_response(game.player, arr[0], arr[1]));
      }
      else {      
        msg(QuestJs._lang.say_no_response_full(game.player, arr[0], arr[1]));
      }
      return world.SUCCESS;
    },
    objects:[
  ...
  new QuestJs._command.Cmd('NpcFillWith', {
    ...
    script:function(objects) {
      ...
      if (!npc.npc) return failedmsg(QuestJs._lang.not_npc, {char:game.player, item:npc})
      objects.shift()
  ...
  new QuestJs._command.Cmd('AskAbout', {
    ...
    script:function(arr) {
      ...
      if (!arr[0][0].askabout) return failedmsg(QuestJs._lang.cannot_ask_about, {char:game.player, item:arr[0][0], text:arr[2]})
...
if (settings.playMode === 'dev') {
  ...
  QuestJs._commands.push(new QuestJs._command.Cmd('DebugListCommands', {
    script:function(arr) {
      ...
      debugmsg("... Found " + count + " QuestJs._commands.");
      return world.SUCCESS_NO_TURNSCRIPTS; 
    },
    objects:[
    ],
...
function handleFillWithLiquid(char, vessel, liquid) {
  ...
  if (!vessel.vessel) return failedmsg(QuestJs._lang.not_vessel, tpParams);
  if (vessel.closed) return  failedmsg(QuestJs._lang.container_closed, tpParams);
  if (!char.canManipulate(vessel, "fill")) return world.FAILED;
  if (!char.getAgreement("Fill", vessel)) return world.FAILED;
  if (!vessel.isAtLoc(char.name)) return failedmsg(QuestJs._lang.not_carrying, {char:char, item:obj});
...
function handlePutInContainer(char, objects) {
  ...
  if (!container.container) return failedmsg(QuestJs._lang.not_container, {char, container})
  if (container.closed) return failedmsg(QuestJs._lang.container_closed, tpParams)
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(char.name)) {
        failedmsg(prefix(obj, multiple) + QuestJs._lang.not_carrying, {char:char, item:obj});
      }
      else {
        obj.moveToFrom(container.name, char.name);
        msg(prefix(obj, multiple) + QuestJs._lang.done_msg);
        success = true;
      }
    }
  }
...
function handleTakeFromContainer(char, objects) {
  ...
  if (!container.container) {
    failedmsg(QuestJs._lang.not_container(char, container));
    return world.FAILED; 
  }
  if (container.closed) {
    failedmsg(QuestJs._lang.container_closed, tpParams);
    return world.FAILED; 
  }
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(container.name)) {
        failedmsg(prefix(obj, multiple) + QuestJs._lang.not_inside(container, obj));
      }
      else {
...
function handleGiveToNpc(char, objects) {
  ...
  if (!npc.npc && npc !== game.player) {
    failedmsg(QuestJs._lang.not_npc_for_give, {char:char, item:npc});
    return world.FAILED; 
  }
  for (let obj of objects[0]) {
    let flag = true;
...
function handleStandUp(objects) {
  ...
  if (objects.length === 0) {
    ...
    const npc = objects[0][0]
    if (!npc.npc) {
      failedmsg(QuestJs._lang.not_npc, {char:game.player, item:npc});
      return world.FAILED; 
    }
    if (!npc.posture) {
      failedmsg(QuestJs._lang.already, {item:npc});
      return world.FAILED;
    }
...
function handlePushExit(char, objects) {
  ...
  if (!obj.shiftable && obj.takeable) {
    msg(QuestJs._lang.take_not_push, tpParams);
    return world.FAILED;
  }
  if (!obj.shiftable) {
    msg(QuestJs._lang.cannot_push, tpParams);
    return world.FAILED;
  }
  if (!room[dir] || room[dir].isHidden()) {
    msg(QuestJs._lang.not_that_way, tpParams);
    return world.FAILED;
  }
  if (room[dir].isLocked()) {
    msg(QuestJs._lang.locked_exit(char, room[dir]));
    return world.FAILED;
  }
  ...
  if (dir === "up") {
    msg(QuestJs._lang.cannot_push_up, tpParams);
    return world.FAILED;
  }
  ...
  msg(QuestJs._lang.push_exit_successful, tpParams);
  return world.SUCCESS;
}
