...
QuestJs._commands = [
  ...
  new QuestJs._command.Cmd('Inv', {
    script: function () {
      ...
      QuestJs._io.msg(
        QuestJs._lang.inventory_prefix +
          ' ' +
          QuestJs._tools.formatList(listOfOjects, {
    ...
    script: function () {
      ...
      for (let key in QuestJs._w) {
        if (QuestJs._parser.isForSale(QuestJs._w[key])) {
          ...
          row.push(
            price > game.player.money ? '-' : '{cmd:buy ' + QuestJs._w[key].alias + ':' + buy + '}',
          );
          l.push(row);
        }
      }
  ...
  new QuestJs._command.Cmd('Say', {
    script: function (arr) {
      ...
      if (QuestJs._settings.givePlayerSayMsg)
        QuestJs._io.msg(
          QuestJs._lang.nounVerb(game.player, arr[0], true) +
            ", '" +
    ...
    new QuestJs._command.Cmd('DebugInspect', {
      script: function (arr) {
        ...
        QuestJs._io.debugmsg(
          'See the console for details on the object ' +
            item.name +
            ' (press F12 to QuestJs._world. the console)',
        );
...
function handleFillWithLiquid(char, vessel, liquid) {
  ...
  if (!vessel.isAtLoc(char.name))
    return QuestJs._io.failedmsg(QuestJs._lang.not_carrying, { char: char, item: obj });
...
const handleTieTo = function (char, rope, obj2) {
  if (!rope.rope)
    return QuestJs._io.failedmsg('You cannot attach that to anything.', { rope: rope });
  if (!obj2.attachable)
