...
QuestJs._commands = [
  ...
  new QuestJs._command.Cmd('PurchaseFromList', {
    script() {
      ...
      QuestJs._io.msg(
        `${current_money}: ${QuestJs._world.Money(game.player.money)}`,
      );
    ...
    script(arr) {
      ...
      for (const key in QuestJs._w) {
        if (
          ...
          QuestJs._w[key].sayCanHear(game.player, arr[0])
        )
      ...
      if (l.length === 0) {
        QuestJs._io.msg(
          QuestJs._lang.say_no_one_here(game.player, arr[0], arr[1]),
        );
        return QuestJs._world.SUCCESS;
      }
      ...
      if (QuestJs._settings.givePlayerSayMsg) {
        QuestJs._io.msg(
          QuestJs._lang.say_no_response(game.player, arr[0], arr[1]),
        );
      } else {
        QuestJs._io.msg(
          QuestJs._lang.say_no_response_full(game.player, arr[0], arr[1]),
        );
      }
      return QuestJs._world.SUCCESS;
    },
