...
map.init = function() {
  ...
  $("<style>")
      .prop("type", "text/css")
      .html(".map-text " + util.dictionaryToCss(QuestJs._settings.mapLabelStyle, true))
      .appendTo("head")
...
map.redraw = function(offX, offY) {
  ...
  if (w[game.player.loc].mapX) game.player.mapX = w[game.player.loc].mapX / QuestJs._settings.mapScale
  if (w[game.player.loc].mapY) game.player.mapY = w[game.player.loc].mapY / QuestJs._settings.mapScale
  if (w[game.player.loc].mapRegion) game.player.mapRegion = w[game.player.loc].mapRegion
  
  if (!game.player.mapRegion) game.player.mapRegion = QuestJs._settings.mapImages[0].name
  const mapImage = QuestJs._settings.mapImages.find(el => el.name === game.player.mapRegion)
  if (!mapImage) return QuestJs._io.errormsg("Failed to find a map region called '" + game.player.mapRegion + "'")
  ...
  result.push(QuestJs._settings.mapMarker(game.player))
