...
const map = {
  ...
  defaults:{
    ...
    mapDrawPointOfInterest:function(point) {
      let s = '<g>'
      s += '<text x="' + (point.mapX/QuestJs._settings.mapScale+18) + '" y="' + (point.mapY/QuestJs._settings.mapScale-23) + '" fill="' + point.fill + '">'
      s += point.text + '</text>'
      s +=  map.polygon({
...
map.init = function() {
  ...
  map.questMapDiv.addEventListener("wheel", function(e) {
    e.preventDefault()
    QuestJs._settings.mapScale -= e.deltaY * -0.01 * QuestJs._settings.mapScale / 4;
    QuestJs._settings.mapScale = Math.min(Math.max(.2, QuestJs._settings.mapScale), 2.5);
    map.redraw()
  })
...
map.redraw = function(offX, offY) {
  ...
  if (QuestJs._w[QuestJs._game.player.loc].mapX) QuestJs._game.player.mapX = QuestJs._w[QuestJs._game.player.loc].mapX / QuestJs._settings.mapScale
  if (QuestJs._w[QuestJs._game.player.loc].mapY) QuestJs._game.player.mapY = QuestJs._w[QuestJs._game.player.loc].mapY / QuestJs._settings.mapScale
  if (QuestJs._w[QuestJs._game.player.loc].mapRegion) QuestJs._game.player.mapRegion = QuestJs._w[QuestJs._game.player.loc].mapRegion
  
  if (!QuestJs._game.player.mapRegion) QuestJs._game.player.mapRegion = QuestJs._settings.mapImages[0].name
  const mapImage = QuestJs._settings.mapImages.find(el => el.name === QuestJs._game.player.mapRegion)
  if (!mapImage) return QuestJs._io.errormsg("Failed to find a map region called '" + QuestJs._game.player.mapRegion + "'")

  const result = []
  result.push('<g id="map-top">')
  result.push('<image width="' + (mapImage.width / QuestJs._settings.mapScale) + '" height="' + (mapImage.height / QuestJs._settings.mapScale) + '", x="0", y="0" href="' + mapImage.file + '"/>')
  result.push(QuestJs._settings.mapMarker(QuestJs._game.player))
  
  for (let point of QuestJs._settings.mapPointsOfInterest) {
    ...
    if (!point.isActive || point.isActive()) result.push(QuestJs._settings.mapDrawPointOfInterest(point))  
  }
  result.push('</g>')
  ...
  QuestJs._io.draw(QuestJs._settings.mapWidth, QuestJs._settings.mapHeight, result, {destination:'quest-map', x:x, y:y, background:'black'})
}
