const map = {
  ...
  defaults: {
    ...
    mapDrawPointOfInterest: function (point) {
      ...
      s +=
        '<text x="' +
        (point.mapX / QuestJs._settings.mapScale + 18) +
        '" y="' +
...
map.init = function () {
  ...
  map.questMapDiv.addEventListener('wheel', function (e) {
    ...
    QuestJs._settings.mapScale = Math.min(Math.max(0.2, QuestJs._settings.mapScale), 2.5);
    map.redraw();
  });
...
map.redraw = function (offX, offY) {
  ...
  if (QuestJs._w[QuestJs._game.player.loc].mapRegion)
    QuestJs._game.player.mapRegion = QuestJs._w[QuestJs._game.player.loc].mapRegion;
  ...
  result.push(
    '<image width="' +
      mapImage.width / QuestJs._settings.mapScale +
      '" height="' +
