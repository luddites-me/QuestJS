...
function createItem() {
  ...
  args.unshift(QuestJs._defaults.DEFAULT_ITEM);
  return createObject(name, args);
}

QuestJs._create.createItem = createItem;
...
function createRoom() {
  const args = Array.prototype.slice.call(arguments);
  const name = args.shift();
  args.unshift(QuestJs._defaults.DEFAULT_ROOM);
  return createObject(name, args);
}

QuestJs._create.createRoom = createRoom;
...
function cloneObject(item, loc, newName) {
  if (item === undefined) { QuestJs._log.info("Item is not defined.") }
  if (typeof item === 'string') {
    const o = w[item]
    if (o === undefined) { QuestJs._log.info("No item called '" + item + "' found in cloneObject.") }
    item = o
  }
  const clone = {};
...
QuestJs._create.cloneObject = cloneObject;

//@DOC
// Creates a basic object. Generally it is better to use CreateItem or CreateRoom.
function createObject(name, listOfHashes) {
  if (world.isCreated && !QuestJs._settings.saveDisabled) {
    QuestJs._log.info("Attempting to use createObject with `" + name + "` after set up. To ensure games save properly you should use cloneObject to create ites during play.")
    QuestJs._io.errormsg("Attempting to use createObject with `" + name + "` after set up. To ensure games save properly you should use cloneObject to create ites during play.");
    return null;
  }

  if (/\W/.test(name)) {
    QuestJs._log.info("Attempting to use the disallowed name `" + name + "`; a name can only include letters and digits - no spaces or accented characters. Use the 'alias' attribute to give an item a name with other characters.")
    QuestJs._io.errormsg("Attempting to use the disallowed name `" + name + "`; a name can only include letters and digits - no spaces or accented characters. Use the 'alias' attribute to give an item a name with other characters.");
    return null;
  }
  if (w[name]) {
    QuestJs._log.info("Attempting to use the name `" + name + "` when there is already an item with that name in the world.")
    QuestJs._io.errormsg("Attempting to use the name `" + name + "` when there is already an item with that name in the world.");
    return null;
  }
  if (typeof listOfHashes.unshift !== 'function') {
    QuestJs._log.info("The list of hashes for `" + name + "` is not what I was expecting. Found:")
    QuestJs._log.info(listOfHashes)
    QuestJs._log.info('Maybe you meant to use createItem?')
    QuestJs._io.errormsg("The list of hashes for `" + name + "` is not what I was expecting. Look at the console for more.");
    return null;
  }

  listOfHashes.unshift(QuestJs._defaults.DEFAULT_OBJECT);
  ...
  if (!item.listalias) item.listalias = QuestJs._tools.sentenceCase(item.alias);
  ...
  item.verbFunctions = [function(o, verbList) {
    verbList.push(QuestJs._lang.verbs.examine)
    if (o.use !== undefined) verbList.push(QuestJs._lang.verbs.use)
  }]
...
const world = {
  ...
  init:function() {
    ...
    if (w.background === undefined) {
      w.background = createItem("background", {
        scenery:true,
        examine:QuestJs._lang.default_description,
        background:true,
        name:'default_background_object',
        lightSource:function() { return world.LIGHT_NONE; },
        isAtLoc:function(loc, situation) {
          if (typeof loc !== "string") loc = loc.name
          if (!w[loc]) QuestJs._io.errormsg("Unknown location: " + loc)
          return w[loc] && w[loc].room && situation === world.PARSER; 
        },
      });
    }
    if (!w.background.background) {
        QuestJs._io.errormsg("It looks like an item has been named 'background`, but is not set as the background item. If you intended to do this, ensure the background property is set to true.");
    }
    ...
    // Set up the UI
    //QuestJs._io.endTurnUI();
    if (QuestJs._settings.playMode === 'beta') {
      QuestJs._lang.betaTestIntro()
    }
    QuestJs._io.msgHeading(QuestJs._settings.title, 2)
    if (QuestJs._settings.subtitle) QuestJs._io.msgHeading(QuestJs._settings.subtitle, 3)
    QuestJs._IO.setTitleAndInit(QuestJs._settings.title)
  ...
  initItem:function(item) {
    if (QuestJs._settings.playMode === 'dev' && item.loc && !w[item.loc]) {
      QuestJs._log.info("ERROR: The item `" + item.name + "` is in an unknown location (" + item.loc + ")");
    }
    ...
    if (QuestJs._settings.playMode === 'dev') {
      const dirs = QuestJs._lang.exit_list.filter(el => el.type !== 'nocmd').map(el => el.name)
      //QuestJs._log.info(dirs)
      for (let key in item) {
        if (dirs.includes(key)) {
          if (!item[key] instanceof Exit) QuestJs._log.info("ERROR: Exit " + key + " of " + item.name + " is not an Exit instance.")
          if (item[key].name !== '_' && !w[item[key].name]) QuestJs._log.info("ERROR: Exit " + key + " of " + item.name + " goes to an unknown location (" + item[key].name + ").")
        }
        else {
          if (item[key] instanceof Exit) QuestJs._log.info("ERROR: Attribute " + key + " of " + item.name + " is an Exit instance and probably should not.")
        }
      }
    }
  },
  ...
  endTurn:function(result) {
    if (result === true) QuestJs._io.debugmsg("That command returned 'true', rather than the proper result code.");
    if (result === false) QuestJs._io.debugmsg("That command returned 'false', rather than the proper result code.");
    QuestJs._util.handleChangeListeners()
    if (result === world.SUCCESS || (QuestJs._settings.failCountsAsTurn && result === world.FAILED)) {
      game.turnCount++
      game.elapsedTime += QuestJs._settings.dateTime.secondsPerTurn;
  ...
  scopeSnapshot:function() {
    ...
    if (room === undefined) {
      QuestJs._io.errormsg("Error in scopeSnapshot; the location assigned to the player does not exist.")
      QuestJs._log.info("Error in scopeSnapshot; the location assigned to the player does not exist ('" + game.player.loc + "').")
      QuestJs._log.info("Is it possible the location is in a file not loaded? Loaded files are: " + QuestJs._settings.files)
      return
    }
  ...
  setRoom:function(char, roomName, dir, forced) {
    ...
    if (typeof roomName === 'string') {
      room = w[roomName];
      if (room === undefined) {
        QuestJs._io.errormsg("Failed to find room: " + roomName + ".");
        return false;
      }
    }
    else {
      if (roomName.name === undefined) {
        QuestJs._io.errormsg("Not sure what to do with this room: " + roomName + " (a " + (typeof roomName) + ").");
        return false;
      }
      room = roomName
      roomName = room.name
    ...
    if (QuestJs._settings.clearScreenOnRoomEnter) QuestJs._io.clearScreen();
  ...
  enterRoom:function() {
    if (game.room.beforeEnter === undefined) {
      QuestJs._io.errormsg("This room, " + game.room.name + ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the QuestJs._defaults.DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.");
      return;
    }
...
const game = createObject("game", [{
  ...
  elapsedTime:0,
  elapsedRealTime:0,
  startTime:QuestJs._settings.dateTime.start,
  gameState:[],
  ...
  begin:function() {
    if (QuestJs._settings.startingDialogEnabled) return
    if (typeof QuestJs._settings.intro === "string") {
      QuestJs._io.msg(QuestJs._settings.intro)
    }
    else if (QuestJs._settings.intro) {
      for (let el of QuestJs._settings.intro) QuestJs._io.msg(el)
    }
    if (typeof QuestJs._settings.setup === "function") QuestJs._settings.setup()
    world.enterRoom()
  },
  ...
  update:function(player) {
    //QuestJs._io.debugmsg("update");
    if (player !== undefined) {
      this.player = player;
    }
    
    if (!this.player) {
      QuestJs._io.errormsg("No player object found. This will not go well...");
      return;
    }
    if (!this.player.loc || !w[this.player.loc]) {
      QuestJs._io.errormsg(this.player.loc === undefined ? "No player location set." : "Player location set to '" + this.player.loc + "', which does not exist.");
      QuestJs._io.errormsg("If this is when you load a game: This is likely to be because of an error in one of the .js files. Press F12, and go to the 'Console' tab (if not already open), to see the error. Look at the very first error (but ignore any that mentions 'jquery'). It should tell you the file and line number that is causing the problem.");
      QuestJs._io.errormsg("If this is when player moves: This is likely to be because of an error in the exit being used.");
    }
  ...
  saveGameState:function() {
    if (QuestJs._settings.maxUndo > 0) {
      this.gameState.push(QuestJs._saveLoad.getSaveBody());
      if (this.gameState.length > QuestJs._settings.maxUndo) this.gameState.shift();
    }
  },
  ...
  registerEvent:function(eventName, func) {
    if (world.isCreated && !QuestJs._settings.saveDisabled) {
      QuestJs._io.errormsg("Attempting to use registerEvent after set up.")
      return
    }
  ...
  registerTimedEvent:function(eventName, triggerTime, interval) {
    if (world.isCreated && !QuestJs._settings.saveDisabled) {
      QuestJs._io.errormsg("Attempting to use registerTimedEvent after set up.")
      return
    }
    ...
    for (let el of game.timerEvents) {
      if (el.triggerTime && el.triggerTime < game.elapsedRealTime) {
        if (typeof(game.eventFunctions[el.eventName]) === 'function') {
          ...
          QuestJs._io.errormsg("A timer is trying to call event '" + el.eventName + "' but no such function is registered.");
          //QuestJs._log.info(game.eventFunctions);
        }
      }
    }
    if (somethingHappened) QuestJs._util.handleChangeListeners()
  },
  preSave:function() {
    const arr = [];
    for (let el of game.timerEvents) {
...
function Exit(name, hash) {
  if (!hash) hash = {}
  this.name = name
  this.use = QuestJs._util.defaultExitUse
