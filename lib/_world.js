...
const world = {
  ...
  enterRoom:function() {
    if (game.room.beforeEnter === undefined) {
      errormsg("This room, " + game.room.name + ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the QuestJs._defaults.DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.");
      return;
    }
