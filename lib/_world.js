...
function cloneObject(item, loc, newName) {
  if (item === undefined) {
    QuestJs._log.info('Item is not defined.');
  }
  if (typeof item === 'string') {
    const o = w[item];
    if (o === undefined) {
      QuestJs._log.info(`No item called '${item}' found in cloneObject.`);
    }
    item = o;
  }
  const clone = {};
...
function createObject(name, listOfHashes) {
  ...
  if (!item.getListAlias)
    item.getListAlias = function (loc) {
...
const world = {
      ...
      for (const key in item) {
        if (dirs.includes(key)) {
          if (!item[key] instanceof Exit)
            QuestJs._log.info(`ERROR: Exit ${key} of ${item.name} is not an Exit instance.`);
  ...
  endTurn(result) {
    if (result === true)
      QuestJs._io.debugmsg("That command returned 'true', rather than the proper result code.");
    if (result === false)
      QuestJs._io.debugmsg("That command returned 'false', rather than the proper result code.");
    QuestJs._util.handleChangeListeners();
    if (
      result === world.SUCCESS ||
      (QuestJs._settings.failCountsAsTurn && result === world.FAILED)
    ) {
      game.turnCount += 1;
      game.elapsedTime += QuestJs._settings.dateTime.secondsPerTurn;
      for (const key in w) w[key].doEvent();
      QuestJs._util.handleChangeListeners();
      world.resetPauses();
      game.update();
      game.saveGameState();
  ...
  scopeSnapshot() {
    ...
    if (room === undefined) {
      ...
      QuestJs._log.info(
        `Error in scopeSnapshot; the location assigned to the player does not exist ('${game.player.loc}').`,
      );
      QuestJs._log.info(
        `Is it possible the location is in a file not loaded? Loaded files are: ${QuestJs._settings.files}`,
      );
      return;
    }
  ...
  setRoom(char, roomName, dir, forced) {
    let room;
    if (typeof roomName === 'string') {
      ...
      if (roomName.name === undefined) {
        QuestJs._io.errormsg(
          `Not sure what to do with this room: ${roomName} (a ${typeof roomName}).`,
        );
        return false;
      }
      room = roomName;
  ...
  enterRoom() {
    if (game.room.beforeEnter === undefined) {
      QuestJs._io.errormsg(
        `This room, ${game.room.name}, has no 'beforeEnter\` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the QuestJs._defaults.DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.`,
      );
      return;
    }
  ...
  enterRoomAfterScripts() {
    ...
    for (const follower of game.player.followers) {
