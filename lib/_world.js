...
function cloneObject(item, loc, newName) {
  ...
  clone.name =
    newName === undefined ? world.findUniqueName(item.name) : newName;
...
const world = {
  ...
  init() {
    ...
    if (QuestJs._settings.subtitle)
      QuestJs._io.msgHeading(QuestJs._settings.subtitle, 3);
  ...
  initItem(item) {
    ...
    if (QuestJs._settings.playMode === 'dev') {
      const dirs = QuestJs._lang.exit_list
  ...
  enterRoomAfterScripts() {
    ...
    for (const follower of game.player.followers) {
      if (follower.loc !== game.player.loc)
...
const game = createObject('game', [
  {
    ...
    begin() {
      ...
      if (typeof QuestJs._settings.setup === 'function')
        QuestJs._settings.setup();
      world.enterRoom();
    },
    ...
    saveGameState() {
      if (QuestJs._settings.maxUndo > 0) {
        ...
        if (this.gameState.length > QuestJs._settings.maxUndo)
