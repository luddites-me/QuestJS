...
const world = {
  ...
  init:function() {
    ...
    QuestJs._io.msgHeading(settings.title, 2)
    if (settings.subtitle) QuestJs._io.msgHeading(settings.subtitle, 3)
    QuestJs._IO.setTitleAndInit(settings.title)
  ...
  setRoom:function(char, roomName, dir, forced) {
    ...
    if (settings.clearScreenOnRoomEnter) QuestJs._io.clearScreen();
...
const game = createObject("game", [{
  ...
  begin:function() {
    ...
    if (typeof settings.intro === "string") {
      QuestJs._io.msg(settings.intro)
    }
    else if (settings.intro) {
      for (let el of settings.intro) QuestJs._io.msg(el)
    }
