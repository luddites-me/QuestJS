...
function createObject(name, listOfHashes) {
  ...
  item.verbFunctions = [function(o, verbList) {
    verbList.push(QuestJs.LANG.verbs.examine)
    if (o.use !== undefined) verbList.push(QuestJs.LANG.verbs.use)
  }]
...
const world = {
  ...
  init:function() {
    ...
    if (w.background === undefined) {
      w.background = createItem("background", {
        scenery:true,
        examine:QuestJs.LANG.default_description,
        background:true,
    ...
    QuestJs.command.initCommands();
    
    // Set up the UI
    //endTurnUI();
    if (settings.playMode === 'beta') {
      QuestJs.LANG.betaTestIntro()
    }
  ...
  initItem:function(item) {
    ...
    if (settings.playMode === 'dev') {
      const dirs = QuestJs.LANG.exit_list.filter(el => el.type !== 'nocmd').map(el => el.name)
      //console.log(dirs)
      for (let key in item) {
