...
function createObject(name, listOfHashes) {
  if (world.isCreated && !QuestJs._settings.saveDisabled) {
    QuestJs._log.info(
      'Attempting to use createObject with `' +
        name +
  ...
  if (/\W/.test(name)) {
    QuestJs._log.info(
      'Attempting to use the disallowed name `' +
        name +
  ...
  if (typeof listOfHashes.unshift !== 'function') {
    ...
    QuestJs._io.errormsg(
      'The list of hashes for `' +
        name +
        '` is not what I was expecting. Look at the console for more.',
    );
    return null;
  }
...
const world = {
          ...
          if (item[key] instanceof Exit)
            QuestJs._log.info(
              'ERROR: Attribute ' +
                ...
                ' of ' +
                item.name +
                ' is an Exit instance and probably should not.',
            );
        }
      }
    }
  ...
  scopeSnapshot: function () {
    ...
    if (room === undefined) {
      ...
      QuestJs._log.info(
        'Is it possible the location is in a file not loaded? Loaded files are: ' +
          QuestJs._settings.files,
      );
      return;
    }
  ...
  enterRoom: function () {
    if (game.room.beforeEnter === undefined) {
      QuestJs._io.errormsg(
        'This room, ' +
          game.room.name +
          ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the QuestJs._defaults.DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.",
      );
      return;
    }
      ...
      for (let el of game.timerEvents) {
        if (el.triggerTime && el.triggerTime < game.elapsedRealTime) {
          if (typeof game.eventFunctions[el.eventName] === 'function') {
            ...
            QuestJs._io.errormsg(
              "A timer is trying to call event '" +
                el.eventName +
