...
test.tests = function() {
  ...
  test.assertEqual("you", QuestJs.LANG.getName(w.Buddy));
  test.assertEqual("You", QuestJs.LANG.getName(w.Buddy, {article:INDEFINITE, capital:true}));
...
/*
  ...
  if (this.check_lang) {
    ...
    for (let el in QuestJs.LANG) {
      if (typeof el !== 'string') continue
      if (langSkips.find(e => el.match(e))) continue
      if (tp.usedStrings.includes(QuestJs.LANG[el])) {
        countDone++
        continue
      }
      console.log(el)
