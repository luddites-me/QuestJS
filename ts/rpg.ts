...
const elements = {
    ...
    opposed: function (s: any) {
        if (!s)
            errormsg("elements.opposed was sent something that evaulates to false (type is " + (typeof s) + ")");
...
const skills = {
    list: [defaultSkill],
    add: function (skill: any) { this.list.push(skill); },
    find: function (skillName: any) {
        return this.list.find(el => skillName === el.name.toLowerCase() || ((el as any).regex && skillName.match((el as any).regex)));
    },
    findName: function (skillName: any) {
        return this.list.find(el => skillName === el.name);
    },
    terminate: function (spell: any, item: any) {
        (array as any).remove(item.activeEffects, spell.name);
        delete item['countdown_' + spell.name];
        let s = "The {i:" + spell.name + "} spell" + (item === (game as any).player ? '' : " on " + lang.getName(item, { article: DEFINITE })) + " terminates.";
        if (spell.terminatingScript)
            s += spell.terminatingScript(item);
        return s;
    }
};
...
class Attack {
    ...
    target: any;
    weapon: any;
    static createAttack(attacker: any, target: any, skill: any) {
        const attack = new Attack();
        attack.attacker = attacker;
        attack.skill = skill;
        attack.target = target;
        // Find the skill
        if (attacker === (game as any).player && skill === undefined) {
            attack.skill = skillUI.getSkillFromButtons();
            skillUI.resetButtons();
        }
        if (!attack.skill)
            attack.skill = defaultSkill;
        // Set targets
        ...
        if (attack.skill.noWeapon) {
            (attack as any).damage = attack.skill.damage;
            if ((attack as any).damage)
                attack.setDamageAtts((attack as any).damage);
            ...
            (attack as any).damage = attack.weapon.damage;
            attack.setDamageAtts((attack as any).damage);
        }
        ...
        for (let el of items) {
            if (el.modifyOutgoingAttack)
                el.modifyOutgoingAttack(attack);
        }
        // Now take into account the target's room
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        const room = (target ? w[target.loc] : w[attacker.loc]);
        if (room.modifyOutgoingAttack)
            room.modifyOutgoingAttack(attack);
    ...
    resolve(target: any, isPrimary: any, count: any) {
        this.target = target;
        // Now take into account the target's active spell
        this.applyActiveEffects(target);
        const items = scopeHeldBy(target);
        for (let el of items) {
            if (el.modifyIncomingAttack)
                el.modifyIncomingAttack(this);
        }
        // Now take into account the target
        if (target.modifyIncomingAttack)
            target.modifyIncomingAttack(this);
        ...
        if (this.skill.automaticSuccess) {
            ...
            this.roll = (random as any).int(1, 20);
        ...
        if (this.skill.incompatible) {
            for (let spellName of target.activeEffects) {
                for (let regex of this.skill.incompatible) {
                    if (spellName.match(regex))
                        terminatedSpells.push(spellName);
                }
            }
        }
        if (this.skill.ongoing)
            target.activeEffects.push(this.skill.name);
    ...
    output() {
        (settings as any).output(this.report);
    }
    clone() {
    ...
    applyDamage(target: any, prefix = 'damage') {
        ...
        if (this[prefix + 'Bonus'] || this[prefix + 'Number']) {
            ...
            for (let i = 0; i < this[prefix + 'Number']; i++) {
                // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
                const roll = (random as any).int(1, this[prefix + 'Sides']);
                damage += roll;
                // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    applyActiveEffects(source: any) {
        if (!source || !source.activeEffects)
            return;
        for (let el of source.activeEffects) {
            const effect = skills.findName(el);
            if (!effect)
                return errormsg("applyActiveEffects: Failed to find skill [" + el + "]");
            if ((effect as any).modifyOutgoingAttack)
                (effect as any).modifyOutgoingAttack(this, source);
        }
    }
}
const rpg = {};
// Get a list of foes in the current room, with target first (whether a foe or not)
(rpg as any).getFoes = function (target: any) {
...
(rpg as any).getAll = function (target: any) {
    const l = scopeHereListed().filter(function (el) {
...
// Get a list of foes in the current room, with target first (whether a foe or not)
(rpg as any).getFoesBut = function (target: any) {
    const l = scopeHereListed().filter(function (el) {
        return el.npc && el.isHostile() && el !== target;
    });
    return l;
...
// Get a list of NPCs in the current room, with target first
(rpg as any).getAllBut = function (target: any) {
    const l = scopeHereListed().filter(function (el) {
        return el.npc && el !== target;
    });
    return l;
...
const RPG_TEMPLATE = {
    ...
    activeEffects: [],
    skillsLearnt: [],
    attack: function (isMultiple: any, char: any) {
        // Create an attack, based on the current skill, weapon and given target
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
        const attack = Attack.createAttack((game as any).player, this).apply().output();
    ...
    modifyOutgoingAttack: function (attack: any) {
        ...
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            ...
            errormsg("ERROR: Trying to use a stat the character " + (this as any).name + " does not have: " + attack.statForOffensiveBonus);
        }
    },
...
const RPG_PLAYER = function () {
    const res = PLAYER();
    for (let key in RPG_TEMPLATE)
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        res[key] = RPG_TEMPLATE[key];
    //res.getEquippedWeapon = function() { return this.equipped ? w[this.equipped] : w.weapon_unarmed; }
    (res as any).getEquippedWeapon = function () {
        const carried = scopeHeldBy(this);
        return carried.find((el: any) => el.equipped && el.weapon) || (w as any).weapon_unarmed;
    };
    (res as any).getArmour = function () {
        const garments = scopeHeldBy(this).filter((el: any) => el.worn);
        let armour = 0;
        for (let el of garments)
            armour += el.getArmour();
        return armour / (settings as any).armourScaling;
    };
    return res;
};
const RPG_NPC = function (female: any) {
    ...
    for (let key in RPG_TEMPLATE)
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        res[key] = RPG_TEMPLATE[key];
    (res as any).oldRpgOnCreation = res.onCreation;
    res.onCreation = function (o) {
        ...
        o.nameModifierFunctions.push(function (o: any, list: any) {
            if (o.dead)
                list.push(lang.invModifiers.dead);
        });
    };
    return res;
};
const RPG_CORPOREAL_UNDEAD = function (female: any) {
    const res = RPG_NPC(female);
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'necrotic'.
    (res as any).element = necrotic;
    return res;
};
const RPG_NON_CORPOREAL_UNDEAD = function (female: any) {
    const res = RPG_CORPOREAL_UNDEAD(female);
    (res as any).noCorpse = true;
    (res as any).modifyIncomingAttack = function (attack: any) {
...
(agenda as any).attack = function (npc: any, arr: any) {
    // do stuff
    return false;
};
const EQUIPPABLE = function () {
    ...
    res.activeEffects = [],
        res.getObstructing = function (char: any) {
            return scopeHeldBy(char).filter((el: any) => el.equipped && this.match(el));
        };
    ...
    res.onCreation = function (o: any) {
        ...
        o.nameModifierFunctions.push(function (o: any, list: any) {
            if (o.equipped && o.isAtLoc((game as any).player.name))
                list.push(lang.invModifiers.equipped);
        });
    };
    ...
    res.equip = function (isMultiple: any, char: any) {
        ...
        if (equipped.length === 0) {
            ...
            for (let el of equipped)
                delete el.equipped;
        }
        this.equipped = true;
        return true;
    };
    res.unequip = function (isMultiple: any, char: any) {
        if (!this.equipped) {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            msg("It already is.");
            return false;
...
const WEAPON = function (damage: any) {
    // @ts-expect-error ts-migrate(2581) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
...
const LIMITED_USE_WEAPON = function (damage: any, ammo: any) {
    // @ts-expect-error ts-migrate(2581) FIXME: Cannot find name '$'. Do you need to install type ... Remove this comment to see the full error message
...
const SPELLBOOK = function (list: any) {
    ...
    res.examine = function () {
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        msg(this.examineX + ' It contains the spells ' + formatList(this.spellsAvailableToLearn.map((el: any) => '<i>' + el + '</i>'), { lastJoiner: lang.list_and }) + '.');
    };
    res.icon = () => 'spell12';
    return res;
};
...
createItem("spell_handler", {
    ...
    eventScript: function () {
        const objs = scopeBy(function (o: any) { return o.activeEffects !== undefined; });
        //console.log(objs)
        for (let el of objs) {
...
commands.push(new Cmd('Attack', {
    ...
    default: function (item: any, isMultiple: any, char: any) {
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
(world as any).isSpellAvailable = function (char: any, spell: any) {
...
commands.push(new Cmd('LearnSpell', {
    ...
    script: function (objects: any) {
        ...
        if (!spell || !(spell as any).spell)
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("There is no spell called " + objects[0] + ".");
        const source = (world as any).isSpellAvailable((game as any).player, spell);
        if (!source)
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("You do not have anything you can learn {i:" + spell.name + "} from.");
        (game as any).player.skillsLearnt.push(spell.name);
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
commands.push(new Cmd('CastSpell', {
    ...
    script: function (objects: any) {
        ...
        if (!spell || !(spell as any).spell)
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("There is no spell called " + objects[0] + ".");
        if (!(game as any).player.skillsLearnt.includes(spell.name))
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("You do not know the spell {i:" + spell.name + "}.");
        if (!(spell as any).noTarget)
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("You need a target for the spell {i:" + spell.name + "}.");
        const attack = Attack.createAttack((game as any).player, (game as any).player, spell).apply().output();
...
commands.push(new Cmd('CastSpellAt', {
    ...
    script: function (objects: any) {
        ...
        // check target
        if ((spell as any).damage && target.health === undefined)
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            return failedmsg("You can't attack that.");
        const attack = Attack.createAttack((game as any).player, target, spell).apply().output();
        return world.SUCCESS;
    },
}));
// @ts-expect-error ts-migrate(7009) FIXME: 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
commands.push(new Cmd('DebugRPG', {
    ...
    script: function (objects: any) {
        (settings as any).attackOutputLevel = 10;
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
