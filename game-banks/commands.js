...
function handlePressurise(char, objects, pressurise) {
  ...
  if (char === QuestJs._game.player) {
    QuestJs._io.metamsg("You need to ask Xsansi to pressurise or depressurise any part of the ship.");
    return QuestJs._world.FAILED;
  }
  ...
  if (baseRoom.isSpace) {
    QuestJs._io.msg("'Scientists estimates the volume of space to be infinite. The ship does not have sufficient air to pressure space.'");
    return QuestJs._world.SUCCESS;
  }
  ...
  if (!QuestJs._w.Xsansi.pressureOverride && mainRoom.name !== "airlock" && !pressurise) {
    QuestJs._io.msg("'Safety interlocks prevent depressurising parts of the ship while the crew are active.'");
    return QuestJs._world.SUCCESS;
  }
  if (!pressurise) {
...
QuestJs._commands.push(new QuestJs._command.Cmd('Approach', {
  ...
  script:function(objects) {
    if (!objects[0][0].isShip) {
      QuestJs._io.metamsg("The APPROACH command is for piloting the ship to a specific destination; a satellite or vessel for example.")
      return QuestJs._world.FAILED;
    }
    ...
    QuestJs._io.msg("You sit at the controls, and unlock the console. You type the co-ordinates into the system, and feel a noticeable pull as the ship accelerates to the target. At the half way point, the ship swings around, so the rockets are firing towards the target, slowing the ship down, so it comes to a stop, relative to the other ship.");
...
QuestJs._commands.push(new QuestJs._command.Cmd('Scan', {
  ...
  script:function(objects) {
    if (!objects[0][0].isShip) {
      QuestJs._io.metamsg("The SCAN command is for scanning a target nearby in space, having approached it; a satellite or vessel for example.")
      return QuestJs._world.FAILED;
    }
    if (QuestJs._game.player.loc !== "flightdeck") {
    ...
    QuestJs._io.msg("While you await the results, you look at the image on the screen. It is not big, less than half the length of the Joseph Banks, and a dull grey colour. It is all curves, without a straight edge anywhere, but it nevertheless looks lumpy rather than sleek. There is no obvious propulsion system, but you can see what might be an opening. There are no marking as far as you can see, and  no obvious weapons.");
    QuestJs._io.msg("The results of the scan appear on the screen. Unsurprisingly, the ship is not in the database. An XDR scan of the hull indicates it is made of an unknown intermetallic alloy of aluminium, nickel and arsenic.");
    QuestJs._io.msg("A look at the infrared camera shows the ship is radiating low level thermal energy, especially from the aft area (relative to the Joseph Banks). The radio signal is emanating from a point lower port forward section.");
    QuestJs._io.msg("There are no other electromagnetic emissions detected, and no significant magnetic, electrical or gravity fields detected.");
    QuestJs._io.msg("There are no other electromagnetic emissions detected, and no significant magnetic, electrical or gravity fields detected.");
    QuestJs._w.alienShip.status = 2;
    return QuestJs._world.SUCCESS;
  },
...
QuestJs._command.findCmd('MetaHelp').script = function() {
  ...
  QuestJs._io.metamsg("{color:red:HELP STASIS}: How to use stasis pods (and hence travel to the next planet)");
  if (QuestJs._w.Xsansi.currentPlanet !== 0) QuestJs._io.metamsg("{color:red:HELP VACUUM}: How to handle the cold vacuum of space");
  if (QuestJs._w.alienShip.status > 0) QuestJs._io.metamsg("{color:red:HELP DOCKING}: How to dock with another ship");
...
QuestJs._commands.push(new QuestJs._command.Cmd('HelpGame', {
  regex:/^(?:\?|help) game$/,
  script:function() {
    QuestJs._io.metamsg("At each planet, you need to assess how many bio-probes and how many geo-probes to launch. Do {color:red:HELP PROBES} for details on that. You can {color:red:ASK AI ABOUT SHIP} to find how many of each probe is left.");
    QuestJs._io.metamsg("You have five planets to visit, before returning to Earth. Return to the stasis pod to go back into stasis. Xsansi will then navigate the ship to the next destination.");
    QuestJs._io.metamsg("As the captain, the welfare of the crew is important, so {color:red:ASK KYLE ABOUT HIS HEALTH}, etc.");
    QuestJs._io.metamsg("You can talk to Xsansi anywhere on the ship (and can just call her \"ai\"). Do {color:red:ASK AI ABOUT CREW} to find out where the crew are. Do {color:red:ASK AI ABOUT KYLE}, for example, for more specific information; the last crew mate yoiu asked about will appear in blue on the map, helping you find him or her (until you meet, where the room will turn green for one turn).");
    return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('HelpProbes', {
  ...
  QuestJs._io.metamsg("{b:Using probes:}");
    QuestJs._io.metamsg("Kyle will automatically deploy a satellite on arrival at a new planet, but you need to tell your crew to deploy probes for the surface. Wait for Xsansi to announce that the satellite is in orbit, then {color:red:ASK XSANSI ABOUT PLANET}. You can then assess what probes you want to deploy.");
    QuestJs._io.metamsg("For a bio-probe, talk to Ostap, for a geo-probe, talk to Aada. They will then walk to the probe hanger, and launch the probe. You can tell them to launch several at once (eg {color:red:OSTAP, LAUNCH 3 PROBES}), but remember, you only have sixteen of each for all five planets.");
    QuestJs._io.metamsg("Once a probe has been launched, it is on its own; you cannot control it.");
    QuestJs._io.metamsg("After a probe has landed, it will send data back to the ship, for your crew to analyse. If the data has value, your bonus will automatically increase. The first probe on a planet might get you two or three bonuses, but the third may not get you any and by the tenth, it is not going to find anything new. Ask the crew about the planet once the probes have explored it. You may decide you want to launch more.")
    QuestJs._io.metamsg("After thirty turns a probe will have got everything it can - and usually much sooner. Why not get to know your crew while you wait?");
    return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('HelpVacuum', {
  ...
  script:function() {
    ...
    QuestJs._io.metamsg("Each section of the ship can be pressurised or depressurised by Xsansi, just ask {color:red:XSANSI, PRESSURIZE THE CARGO BAY} or {color:red:AI, DEPRESSURISE ENGINEERING}. Note that safety overrides may prevent Xsansi from complying.");
    QuestJs._io.metamsg("To find out what areas are pressurised, {color:red: ASK XSANSI ABOUT WHERE IS PRESSURISED} or {color:red:ASK AI ABOUT VACUUM}.");
    return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('HelpDock', {
  ...
  script:function() {
    ...
    QuestJs._io.metamsg("From the flight-deck, you can get closer to another ship, either to get a better look or to dock with it; {color:red:XSANSI, APPROACH SHUTTLE} or {color:red:AI, APPROACH SHIP}. Obviously there must be an vessel around.");
    QuestJs._io.metamsg("Once adjacent, you can scan it or dock with it; {color:red:XSANSI, DOCK WITH SHUTTLE} or {color:red:AI, SCAN SHIP}.");
    return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
  },
}));
...
QuestJs._commands.push(new QuestJs._command.Cmd('HelpUniverse', {
  ...
  script:function() {
    ...
    QuestJs._io.metamsg("I originally {i:tried} to go hard science fiction; these are real stars the ship visits! However, I have assumed artificial gravity, which is required to orientate the game (once you have down, you have port, up and starboard).");
    QuestJs._io.metamsg("I am also assuming people can be held in stasis, and presumably this is like freezing time (cf Niven's stasis field, in his \"Known Space\" series). I need that to preserve the food so the crew have something to eat 80 years after leaving Earth.");
    QuestJs._io.metamsg("Also, probes are {i:fast}! It just takes a few turns to travel from orbit to the planet surface, which has to be at least 100 miles, and likely considerably more. They work fast on the planet too. It is a game; we need stuff to happened quickly to keep players interested. So maybe not so hard science fiction afterall.");
    return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
  },
}));
