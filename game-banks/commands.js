...
function handlePressurise(char, objects, pressurise) {
  ...
  const mainRoom =
    typeof baseRoom.vacuum === 'string'
      ? QuestJs._w[baseRoom.vacuum]
      : baseRoom;
  ...
  if (
    !QuestJs._w.Xsansi.pressureOverride &&
...
QuestJs._command.findCmd('MetaHelp').script = function () {
  ...
  QuestJs._io.metamsg(
    '{color:red:HELP GENERAL} or {color:red:HELP GEN}: How to play parser games'
  );
  QuestJs._io.metamsg('{b:Commands to help you play this game:}');
  QuestJs._io.metamsg(
    '{color:red:HELP GAME}: Suggestions on what to actually do'
  );
  QuestJs._io.metamsg(
    '{color:red:HELP NPC}: Interacting with other characters'
  );
  ...
  if (QuestJs._w.Xsansi.currentPlanet !== 0)
    QuestJs._io.metamsg(
      '{color:red:HELP VACUUM}: How to handle the cold vacuum of space'
    );
  if (QuestJs._w.alienShip.status > 0)
    QuestJs._io.metamsg(
      '{color:red:HELP DOCKING}: How to dock with another ship'
    );
  QuestJs._io.metamsg(
    '{b:Commands that give meta-information about the game:}'
  );
  QuestJs._io.metamsg(
    '{color:red:HELP UNIVERSE}: Notes about the universe the game is set in'
  );
  ...
  QuestJs._io.metamsg(
    'You can use {color:red:?} as a shorthand for {color:red:HELP}'
  );
  return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
};
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('HelpCredits', {
    ...
    script() {
      ...
      QuestJs._io.metamsg(
        'This was written by The Pixie, on a game system created by The Pixie.'
      );
      return QuestJs._world.SUCCESS_NO_TURNSCRIPTS;
    },
  }),
