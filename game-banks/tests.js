...
QuestJs._test.tests = function() {
  ...
  QuestJs._test.assertCmd("ask ai about crew", QuestJs._test.padArray(["'Tell me about the crew, Xsansi,' you say."], 4));
  QuestJs._test.assertCmd("o", ["You climb out of the stasis pod.", /^There are six/, "A drawer under the pod slides open to reveal your jumpsuit."]);
  QuestJs._test.assertCmd("get jumpsuit", ["You take your jumpsuit.", "The stasis pod drawer slides shut."]);
  ...
  QuestJs._test.assertCmd("u", ["You walk up the narrow stair way to the top deck.", /^The top deck is where the living quarters/]);
  QuestJs._test.assertCmd("f", ["You head forward.", /^You are stood at the forward end /, "'Satellite I has successfully entered orbit around the planet.' announces Xsansi."]);
  QuestJs._test.assertCmd("ask ostap about probes", "You can't see anything you might call 'ostap' here.");
  ...
  QuestJs._test.assertCmd("x table", "The table is plastic, attached to the wall at one end, and held up by a single leg at the other end. The table is bare.");
  QuestJs._test.assertCmd("ask ostap about bio-probes", QuestJs._test.padArray(["'How does a bio-probe work?' you ask Ostap."], 2));
  QuestJs._test.assertCmd("ask ostap about his health", ["'How are you feeling?' you ask Ostap.", "'I am feeling good.'"]);


  QuestJs._test.title("Order launch");
  QuestJs._test.assertCmd("ostap, launch 19 probes", ["'Launch 19 bio-probes,' you say to Ostap.", "'We only have 16 and we should save some for the other planets on our itinerary.'"]);
  QuestJs._test.assertCmd("ostap, launch 2 bio-probe", ["'Launch 2 bio-probes,' you say to Ostap.", "'Okay captain.'"]);
  QuestJs._test.assertCmd("z", ["You wait one turn.", "Ostap leaves the canteen, heading port."]);
  QuestJs._test.assertCmd("p", ["You head port.", "You are stood at the forward end of a narrow corridor, with your cabin to port, and the canteen to starboard. Ahead, is the lounge.", "You can see Ostap here.", "Ostap leaves the top deck forward, heading down."]);
  QuestJs._test.assertCmd("d", ["You head down.", /^This is, in a sense, the central nexus of the ship./, "You can see Ostap here.", "Ostap leaves the hallway, heading down."]);
  QuestJs._test.assertCmd("d", ["You head down.", /^The forward probe hanger is where the satellites/, "You can see Kyle and Ostap here.", "Ostap leaves the Forward probe hanger, heading aft."]);
  QuestJs._test.assertCmd("a", ["You head aft.", /^The aft probe hanger has/, "You can see Ostap here.", "'Okay, two probes to deploy...' mutters Ostap as he types at the console."]);


  QuestJs._test.title("Launching");
  QuestJs._test.assertCmd("z", ["You wait one turn.", "Ostap prepares the first bio-probe."]);
  QuestJs._test.assertCmd("ask ostap about lost probes", ["'Do we ever lose probes?' you ask Ostap.", /^'We are exploring the unknown, we have to expect /])
  ...
  QuestJs._test.assertCmd("ask ostap about lost probes", ["'Do we ever lose probes?' you ask Ostap.", /^'We are exploring the unknown/]);
  QuestJs._test.assertCmd("topics for ostap", ["Some suggestions for what to ask Ostap about: background; expertise; health; planet; probes."]);
  ...
  QuestJs._test.assertCmd("z", ["You wait one turn.", "'Okay, two probes launched,' says Ostap as he stands up.", "'Bio-probe I has successfully landed on the planet.' announces Xsansi."]);
  
  
  QuestJs._test.title("Waiting")
  QuestJs._test.assertCmd("z", ["You wait one turn."]);
  QuestJs._test.assertCmd("z", ["You wait one turn.", "'Contact with Bio-probe II has been lost as it attempted to land on the planet.' announces Xsansi."]);
  ...
  QuestJs._test.assertCmd("ostap, go in stasis pod", ["'Ostap, you're work here is done; you can go get in your stasis pod.'", "'Right, okay then.'"]);
  QuestJs._test.assertCmd("z", ["You wait one turn.", "Ostap leaves the Aft probe hanger, heading forward."]);
  ...
  QuestJs._test.assertCmd("ostap, stop", ["'Ostap, forget what I said; don't get in your stasis pod yet.'", "'Oh, okay.'"]);
  QuestJs._test.assertCmd("ostap, stop", ["'Ostap, stop what you're doing.'", "'Not really doing anything.'"]);
  ...
  QuestJs._test.assertCmd("ostap, go in stasis pod", ["'Ostap, you're work here is done; you can go get in your stasis pod.'", "'Right, okay then.'"]);
  QuestJs._test.assertCmd("z", ["You wait one turn.", "Just in his underwear, Ostap climbs into his stasis pod."]);
  QuestJs._test.assertCmd("x ostap", ["Ostap is a big guy; not fat, but broad and tall. He keeps his dark hair in a short ponytail. He is in his underwear. He is lying in his stasis pod.", "'Close the pod, Xsansi,' Ostap says. The stasis pod lid smoothly lowers, and Xsansi operates the stasis field."]);
