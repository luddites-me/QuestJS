...
export class Cmd extends Base {
  ...
  default(item, isMultiple, char) {
    if (typeof this.defmsg === "string") {
      this.io.failedmsg(prefix(item, isMultiple) + this.defmsg, {
        char,
        item
      });
    } else if (typeof this.defmsg === "function") {
      this.io.failedmsg(prefix(item, isMultiple) + this.defmsg(char, item), {
        char,
        item
      });
    } else {
  ...
  script(objects, matches) {
    ...
    if (objects.length > 1)
      verb = objects.shift();
    const multi = objects[0] && (objects[0].length > 1 || this.parser.currentCommand.all);
    ...
    for (let i = 0; i < objects[0].length; i += 1) {
      if (!objects[0][i][this.attName]) {
        ...
        let result = this.processCommand(this.game.player, objects[0][i], multi, matches[0][i], verb);
    ...
    if (success) {
      return this.noTurnscripts || suppressEndturn ? WorldStates.SUCCESS_NO_TURNSCRIPTS : WorldStates.SUCCESS;
    }
    return WorldStates.FAILED;
  }
  processCommand(char, item, multi = false, match = false, verb = "") {
    this.rules.forEach((rule) => {
      if (typeof rule !== "function") {
        this.io.errormsg(`Failed to process command '${this.name}' as one of its rules is not a function (F12 for more).`);
        this.log.info(`Failed to process command '${this.name}' as one of its rules is not a function:`);
        this.log.info(this);
        this.log.info(rule);
      }
    ...
    if (typeof result !== "boolean" && result !== WorldStates.SUCCESS_NO_TURNSCRIPTS) {
      result = true;
    }
    return result;
  }
  ...
  init() {
    ...
    if (!this.regex && !this.regexes) {
      this.regexes = Array.isArray(this.lexicon.regex[this.name]) ? this.lexicon.regex[this.name] : [this.lexicon.regex[this.name]];
    }
  }
}
