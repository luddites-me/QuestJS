...
export class SaveLoad extends Base {
  ...
  saveGame(filename, comment = "-") {
    ...
    if (this.settings.afterSave)
      this.settings.afterSave(filename);
    return true;
  }
  saveTheWorld(comment) {
    return this.getSaveHeader(comment) + this.getSaveBody();
  }
  getHeader(s) {
    const arr = s.split("!");
    return {
  ...
  loadGame(filename) {
    ...
    if (s != null) {
      ...
      if (this.settings.afterLoad)
        this.settings.afterLoad(filename);
    } else {
  ...
  setLoadString(s) {
    ...
    this.io.errormsg(`Unknown save type for object '${name}' (${this.saveType})`);
  }
  decode(hash, str) {
  ...
  encode(key, value) {
    ...
    if (Array.isArray(value)) {
      ...
      if (typeof value[0] === "number")
        return `${key}:numberarray:${this.encodeNumberArray(value)};`;
      return "";
    }
    if (value instanceof Exit) {
      if (value.name)
        return `${key}:exit:${value.name}:${value.locked ? "l" : "u"}:${value.hidden ? "h" : "v"};`;
      return "";
    }
    if (attType === "object") {
      if (value.name)
  ...
  decodeString(s) {
    if (typeof s !== "string") {
      this.log.info("Expecting a string there, but found this instead (did you add an object to a list rather than its name?):");
      this.log.info(s);
    }
    for (const d of this.replacements) {
