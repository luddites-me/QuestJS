import {WorldStates} from "../../src/index.js";
import {Player} from "../../node/actors/player.js";
import {Exit} from "../../node/exit.js";
import {Item} from "../../node/items/item.js";
import {Surface} from "../../node/items/surface.js";
import {Wearable} from "../../node/items/wearable.js";
import {Room} from "../../node/locations/room.js";
import {Quest as Quest2} from "../../Quest.js";
const quest = new Quest2();
const cloakHere = () => {
  const cloak2 = quest.state.get("cloak");
  if (cloak2.isAtLoc("me"))
    return true;
  if (cloak2.isHere())
    return true;
  const cloakRoom2 = quest.state.get("cloakroom");
  if (cloak2.isAtLoc("hook") && quest.game.player.isAtLoc(cloakRoom2))
    return true;
  return false;
};
quest.init({
  ...
  commands: {
    ...
    HangUp: {
      ...
      script(objects) {
        if (!objects[0][0].isAtLoc(player2)) {
...
const player2 = new Player(quest, "me", {
  loc: lobby,
...
new Item(quest, "posters", {
  ...
  read: "You spend a few minutes reading about the shows they put on twenty-odd years ago.... {i:Die Zauberfl\xF6te}... {i:Guillaume Tell}... {i:A Streetcar Named Desire}. Wasn't that a play?",
  scenery: true,
  plural: true,
  loc: lobby
});
...
const message = new Item(quest, "message", {
  ...
  eventPeriod: 1,
  eventIsActive() {
    return player2.isAtLoc(bar) && !this.finished;
  },
  eventScript() {
    message.count += 1;
    if (message.count > 1) {
      if (message.disturbed === 0) {
        this.io.msg("You think it might be a bad idea to disturb things in the dark.");
      } else if (!player2.suppress_background_sounds) {
...
new Item(quest, "walls", {
  examine() {
    if (cloakHere() && player2.isAtLoc(bar)) {
