...
export class Lang {
  ...
  constructor(opts) {
    const { msg, errormsg, w, metamsg, world } = opts;
    this.msg = msg;
  ...
  object_unknown_msg = (name) => {
    return this.nounVerb(game.player, "can't", true) + " see anything you might call '" + name + "' here.";
  }; 
  ...
  stop_posture = (char) => {
    ...
    if (char.postureFurniture) {
      s = this.nounVerb(char, "get", true) + " off " + this.getName(this.w[char.postureFurniture], {article:DEFINITE}) + ".";
    }
    else {
  ...
  tell_about_intro = (char, text1, text2) => {
    return "You tell " + this.getName(char, {article:DEFINITE}) + " " + text2 + " " + text1 + ".";
  };
  // If the player does ASK MARY ABOUT HOUSE this will appear before the response.
  ask_about_intro = (char, text1, text2) => {
    return "You ask " + this.getName(char, {article:DEFINITE}) + " " + text2 + " " + text1 + ".";
  };
  ...
  npc_leaving_msg = (npc, dest) => {
    let s = "";
    let flag = false;
    if (this.w[game.player.loc].canViewLocs && this.w[game.player.loc].canViewLocs.includes(npc.loc)) {
      s = this.w[game.player.loc].canViewPrefix;
      flag = true;
    }
    if (flag || npc.inSight()) {
      s += this.nounVerb(npc, "leave", !flag) + " " + this.getName(this.w[npc.loc], {article:DEFINITE});
  ...
  niceDirection = (dir) => {
    const dirObj = this.exit_list.find((el) => { return el.name === dir })
  ...
  npc_entering_msg = (npc, origin) => {
    ...
    if (flag || npc.inSight()) {
      s += this.nounVerb(npc, "enter", !flag) + " " + this.getName(this.w[npc.loc], {article:DEFINITE});
  ...
  helpScript = () => {
    if (settings.textInput) {
      this.metamsg("Type commands in the command bar to interact with the world. Using the arrow keys you can scroll up and down though your previous commands.");      
      this.metamsg("{b:Movement:} To move, use the eight compass directions (or just N, NE, etc.). Up/down and in/out may be options too. When \"Num Lock\" is on, you can use the number pad for all eight compass directions, - and + for UP and DOWN, / and * for IN and OUT.");
      this.metamsg("{b:Other commands:} You can also LOOK (or just L or 5 on the number pad), HELP (or ?) or WAIT (or Z or the dot on the number pad). Other commands are generally of the form GET HAT or PUT THE BLUE TEAPOT IN THE ANCIENT CHEST. Experiment and see what you can do!");
      ...
      this.metamsg("{b:Meta-commands:} Type ABOUT to find out about the author, SCRIPT to learn about transcripts or SAVE to learn about saving games. Use WARNINGS to see any applicable sex, violence or trigger warnings.")
      ...
      this.metamsg(s)
      this.metamsg("{b:Shortcuts:}You can often just type the first few characters of an item's name and Quest will guess what you mean.  If fact, if you are in a room with Brian, who is holding a ball, and a box, Quest should be able to work out that B,PUT B IN B mean you want Brian to put the ball in the box.")
    ...
    if (settings.panes !== "none") {
      this.metamsg("{b:User Interface:} To interact with an object, click on its name in the side pane, and a set of possible actions will appear under it. Click on the appropriate action.");
      if (settings.compassPane) {
        if (settings.symbolsForCompass) {
          this.metamsg("You can also use the compass rose at the top to move around. Click the eye symbol, &#128065;, to look at you current location, the pause symbol, &#9208;, to wait or &#128712; for help.");
        }
        else {
          this.metamsg("You can also use the compass rose at the top to move around. Click 'Lk' to look at you current location, 'Z' to wait or '?' for help.");
        }
      }
    }
    if (settings.additionalHelp !== undefined) {
      for (let s of settings.additionalHelp) this.metamsg(s)
    }
  ...
  aboutScript = () => {
    this.metamsg("{i:{param:settings:title} version {param:settings:version}} was written by {param:settings:author} using Quest 6 AKA Quest JS version {param:settings:questVersion}.", {settings:settings})
    if (settings.ifdb) this.metamsg("IFDB number: " + settings.ifdb)
    if (settings.thanks && settings.thanks.length > 0) {
      this.metamsg("Thanks to " + formatList(settings.thanks, {lastJoiner:this.list_and}) + ".")
    }
    if (settings.additionalAbout !== undefined) {
      for (let s of settings.additionalAbout) this.metamsg(s)
    }
    return this.world.SUCCESS_NO_TURNSCRIPTS
  };
  ...
  warningsScript = () => {
    switch (typeof settings.warnings) {
      case 'undefined' : this.metamsg('No warning have been set for this game.'); break;
      case 'string' : this.metamsg(settings.warnings); break;
      default: for (let el of settings.warnings) this.metamsg(el)
    }
    return this.world.SUCCESS_NO_TURNSCRIPTS;
  };

  saveLoadScript = () => {
    this.metamsg("To save your progress, type SAVE followed by the name to save with.");
    this.metamsg("To load your game, refresh/reload this page in your browser, then type LOAD followed by the name you saved with.");
    this.metamsg("To see a list of save games, type DIR.");
    return this.world.SUCCESS_NO_TURNSCRIPTS;
  };

  transcriptScript = () => {
    this.metamsg("The TRANSCRIPT or SCRIPT command can be used to handle saving the input and output. This can be very useful when testing a game, as the author can go back through it and see exactly what happened, and how the player got there.");
    this.metamsg("Use SCRIPT ON to turn on recording and SCRIPT OFF to turn it off. Use SCRIPT SHOW to display it (it will appear in a new tab; you will not lose your place inthe game). To empty the file, use SCRIPT CLEAR.");
    ...
    this.metamsg("You can add a comment to the transcript by starting your text with an asterisk (*).")
    this.metamsg("You can do TRANSCRIPT WALKTHROUGH or just SCRIPT W to copy the transcript to the clipboard formatted for a walk-through. You can then paste it straight into the code.")
  ...
  topicsScript = () => {
    this.metamsg("Use TOPICS FOR [name] to see a list of topic suggestions to ask a character about (if implemented in this game).");
    return this.world.SUCCESS_NO_TURNSCRIPTS;
  };
  
  betaTestIntro = () => {
    this.metamsg("This version is for beta-testing (" + settings.version + "). A transcript will be automatically recorded. When you finish, do Ctrl-Enter or type SCRIPT SHOW to open the transcript in a new tab; it can then be copy-and-pasted into an e-mail.")
    if (settings.textInput) this.metamsg("You can add your own comments to the transcript by starting a command with *.")
    io.scriptStart()
  };
...
//@UNDOC
  ...
  getName = (item, options) => {
    ...
    if (item.pronouns === this.pronouns.firstperson || item.pronouns === this.pronouns.secondperson) {
  ...
  toWords = (number) => {
    ...
    if (number < 2000) {
      ...
      if (number < 20) {
        if (number !== 0 || s === "") {
          s = s + this.numberUnits[number];
        }
      }
      else {
        let units = number % 10;
        let tens = Math.floor(number / 10) % 10;
        s = s + this.numberTens[tens - 2];
        if (units !== 0) {
          s = s + this.numberUnits[units];
        }
      }
    }
    else {
  ...
  toOrdinal = (number) => {
    ...
    let s = this.toWords(number);
    for (let or of this.ordinalReplacements) {
      if (or.regex.test(s)) {
  ...
  convertNumbers = (s) => {
    for (let i = 0; i < this.numberUnits.length; i++) {
      let regex = new RegExp("\\b" + this.numberUnits[i] + "\\b");
  ...
  conjugate = (item, verb) => {
    ...
    if (!arr) {
      this.errormsg("No conjugations found: conjugations_" + gender.toLowerCase());
      return verb;
    }
    for (let conj of arr) {
      if (conj.name === verb) {
  ...
  nounVerb = (item, verb, capitalise) => {
    ...
    let s = this.getName(item, {article:DEFINITE}) + " " + this.conjugate (item, verb);
    s = s.replace(/ +\'/, "'");  // yes this is a hack!
    return capitalise ? sentenceCase(s) : s;
  };
  ...
  verbNoun = (item, verb, capitalise) => {
    ...
    let s = this.conjugate (item, verb) + " " + this.getName(item, {article:DEFINITE});
    s = s.replace(/ +\'/, "'");  // yes this is a hack!
    return capitalise ? sentenceCase(s) : s;
  };
}
