QuestJs._create.createItem('robot', QuestJs._npc.NPC(false), {
  loc: 'laboratory',
  examine:
    'The robot is approximately man-shaped, if a little squat. It looks a little... clunky, like might have been designed in the fifties.',
  strong: true,
  attachable: true,
  eventIsActive() {
  ...
  getAgreementGo(dir) {
    if (QuestJs._w.rope.isAttachedTo(this)) {
      ...
      if (!this.ropeMoveFlag) {
        ...
        tmsg(
          "Let's be honest here, ropes are tricky to code. Be thankful you can tie it to the robot and forget about him walking around dragging the rope behind him.",
        );
      }
      return false;
    }
  ...
  askOptions: [
    {
      ...
      script() {
        ...
        QuestJs._io.msg(
          "'This is Professor Kleinscope's zeta-particle laboratory,' says the robot grandly.",
        );
    ...
    },
    {
      ...
      script() {
        ...
        QuestJs._io.msg(
          "'Professor Kleinscope is the pre-eminent authority on zeta-particles,' says the robot, not to helpfully.",
        );
        QuestJs._io.msg("'Okay... go on.'");
        QuestJs._io.msg(
          "'Oh. Professor Kleinscope graduated from Keele University with a B.Sc. in physics, and later from Alabama Evangelical Bible College with a Ph.D. in an unspecified subject. Since then, he has been engaged in important post-graduate work under the Tutorial House.'",
        );
      },
    },
    {
      ...
      script() {
        ...
        QuestJs._io.msg(
          "'Zeta-particles were discovered in Atlantis over 300 years ago, but knowledge of them was lost when that great city disappeared. They offer unlimited power and a cure for all diseases,' says the robot.",
        );
        if (QuestJs._w.me.hints < 230) {
          tmsg(
            "Great, you are having a conversation with someone. But it is Professor Kleinscope we want to talk to, so let's take the lift up to him.",
          );
    ...
    },
    {
      name: 'reactor',
      test(p) {
        return p.text.match(/\b(reactor)\b/) && QuestJs._w.me.hints >= 250;
      },
      script() {
        QuestJs._io.msg(
          "'How do I turn on the reactor?' you ask the robot. 'And where is it, anyway?'",
        );
        QuestJs._io.msg(
          "'The control rod must be placed in the reactor core,' says the robot. 'The reactor is through the door to the north.'",
        );
    ...
    },
    {
      ...
      script() {
        ...
        if (QuestJs._w.reactor_room.reactorRunning) {
          QuestJs._io.msg(
            "'The zeta-lift - or zeta-elevator - is operating normally,' says the robot.",
          );
        } else {
          QuestJs._io.msg(
            "'The zeta-lift - or zeta-elevator - is powered by the zeta-reactor, which is currently not running,' says the robot.",
          );
        }
    ...
    },
    {
      ...
      script() {
        ...
        QuestJs._io.msg(
          "'Atlantis is the name of a technologically-advanced city that disappeared 2954 years ago,'  says the robot. 'It supposedly sunk, but Professor Kleinscope believes it actually moved into another dimension, using zeta-particles.'",
        );
      },
    },
    {
...
QuestJs._create.createItem('Professor_Kleinscope', QuestJs._npc.NPC(false), {
  loc: 'office',
  examine:
    'The Professor is a slim tall man, perhaps in his forties, dressed, inevitably in a lab coat. Curiously his hair is neither white nor wild.{ifNot:Professor_Kleinscope:flag: He is sat at he desk, engrossed in his work.}',
  talkto() {
    if (!this.flag) {
      ...
      QuestJs._io.msg(
        "'Not now, my boy,' he says. He looks at you again. 'You are a boy, aren't you? Important appointment with my dinner!'",
      );
    }
  },
  getAgreement() {
