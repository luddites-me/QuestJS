...
hint.data = [
  ...
  },
  {
    ...
    hint: 'GET TORCH.',
    tutorial:
      'This room has a torch, but it is described in the room description as part of the scenery, so not as obvious as the hat. But you can still pick it up just the same. And if you then drop it again, you will see it is just an ordinary item (though that may not be the case in all games).|Incidentally, you can call it a flashlight if you prefer.',
  },
  {
    name: 'torchOn',
    hint:
      'TURN ON TORCH, then head to the basement (out, southwest, then down). If you are in the basement and it is dark, you will need to go back up, and then turn the torch on before coming back down.',
    tutorial:
      'Now it is calling it a flashlight? So anyway, we have a torch, we can now take a proper look in the basement (go down from the kitchen).|The torch can  be turned on and off, with TURN ON TORCH or SWITCH FLASHLIGHT OFF or whatever.',
  },
  {
    name: 'turnOnLight',
    hint: 'SWITCH ON THE LIGHT, and then TURN OFF THE TORCH (in that order!).',
    tutorial:
      'Great, at last we can see down here. And it turns out there is a light switch, but we needed the torch to see the switch.|It is quite common for torch batteries to run out after so many turns, and then you have to re-charge it or find a battery. Hopefully that will not happen here, but it would be a good idea to save the battery just in case, so turn the light on, and turn off the torch.',
  },
  {
    name: 'getAll',
    hint: 'Try GET ALL to pick up everything.',
    tutorial:
  ...
  {
    name: 'moveCrates',
    hint: 'You cannot take the crates, but you might be able to MOVE CRATES.',
    tutorial:
  ...
  { name: 'enterPassage', hint: 'Head WEST.', tutorial: 'Now we are getting somewhere!' },
  {
    ...
    hint: 'Type SAVE.',
    tutorial:
      "Not much in this room, so let's pause for a moment. It is a good idea to save occasionally whilst playing, just in case you die (not possible in this game) or lose the connection to the server (not an issue for Quest 6) or your PC crashes or you just have some else to do and want to return later. You really do not want to have to start from the beginning, so save your QuestJs._game.|Different systems have different ways to handle saving and loading (and some games may not support it at all), but a good start is to type SAVE.",
  },
  {
    name: 'saveGame',
    hint: 'Type SAVE TUTORIAL (or some other name).',
    tutorial:
  ...
  {
    name: 'press3',
    hint: 'Try PRESS 3 to operate the lift.',
    tutorial:
      'The lift (or elevator) is a special location in that it moves. You probably already knew that! To get it to go, just press one of the buttons.',
  },
  {
    name: 'askRLift',
    hint: 'Go back to the laboratory, and ASK ROBOT ABOUT LIFT.',
    tutorial: 'Something is wrong... Perhaps we should ask the robot about it?',
  },
  ...
  },
  {
    ...
    hint: 'Go back SOUTH.',
    tutorial:
      "Well that's a bother! But there must be some way around. We need to use the lift, and to do that we need to get the reactor going. Given the author has gone to trouble to set this up, there must be some way to get the control rod.|We can get the robot to do it!|You will need to go back to the other room, get the robot to come here, then tell the robot to pick up the control rod.",
  },
  {
    name: 'rGoNorth',
    hint:
      'Tell the robot to go north (ROBOT,N), then go NORTH yourself and tell the robot to get the rod (ROBOT,GET ROD).',
    tutorial:
      'You can tell the robot to do something by prefixing a normal command with either TELL ROBOT TO or just ROBOT and a comma. To have it go north, them, you could do TELL ROBOT TO GO NORTH or ROBOT,N.',
  },
  {
    name: 'rGetRod',
  ...
  },
  {
    ...
    tutorial:
      'Now we are getting somewhere. At last the lift that we saw in the lounge right at the start is working, and we can use it to get to the top of the house.',
  },
  {
    name: 'eastOffice',
    hint: 'GO EAST.',
    tutorial: 'Hopefully when we exit the lift we will be somewhere new...',
  },
  {
    name: 'useComputer',
    hint: 'Try to USE THE COMPUTER.',
    tutorial: 'Great we must be nearly done! Just use the computer...',
  },
  {
    name: 'talkProf',
    hint: 'TALK TO PROF.',
    tutorial:
      "Bother! We need to shift the professor. You could tell the robot to do something and, being a robot, it would just do it. Professor Kleinscope will not. In fact, you cannot ASK or TELL him stuff either. To converse with the Professor, you need to TALK TO him. Let's see what happens...",
  },
  {
    name: '',
    hint: 'Wait until the professor goes, then try to USE COMPUTER again.',
    tutorial: '',
  },
  {
    name: 'findCode',
    hint:
      'Look for the code for the computer. Try LOOK BEHIND PAINTING, then read what you find. Once you have the number, try to USE THE COMPUTER again.',
    tutorial:
      'Oh heck!|Just occasionally a game will ask you for some specific text, which it will understand to be different to a command. In this case it is wanting a specific six-digit number (by the way, that number was randomly generated when you started the game, so you cannot cheat by asking someone on the internet what it is).|But worry not! This guy is a professor, therefore he necessarily must be absent-minded, therefore he will have the number written down somewhere. We just need to find it.|By the way, this is a small example of a "Babel fish puzzle". The name comes from the Hitch-Hiker\'s Guide to the Galaxy, and is when you have a seemingly simple task, but there is an obstacle; each time you resolve one obstacle a new one is apparent.',
  },
  {
    name: 'smashWindow',
    hint:
      'You need to escape, and the only way out is through the window. What happens if you SMASH THE WINDOW?',
    tutorial: '',
  },
  {
    name: 'wrapFist',
    hint:
      'You need to escape, and the only way out is through the window, but you need to wrap your hand in something first. How about that old newspaper (WRAP FIST IN NEWSPAPER)?',
    tutorial: '',
  },
  {
    name: 'smashWindow2',
    hint:
      'You need to escape, and the only way out is through the window. What happens if you SMASH THE WINDOW now your fist is wrapped in newspaper?',
    tutorial: '',
  },
  {
    name: 'out',
    hint:
      'You need to escape, and the only way out is through the broken window. What happens if you try OUT?',
    tutorial: '',
  },
  {
    name: 'climbOut',
    hint:
      'You need to escape, and the only way out is through the broken window, but it is a long way down and you will need to use the rope. To do that, TIE QuestJs._templates.ROPE TO DESK, then THROW QuestJs._templates.ROPE OUT WINDOW.',
  },
...
const walkthroughs = {
  a: [
    ...
    'QuestJs._w',
    'hint',
    'ask robot about the laboratory',
    'hint',
    'topics for robot',
    'ask robot about zeta-particles',
    'hint',
    'QuestJs._w',
    'hint',
    'press 3',
    ...
    'e',
    'hint',
    'ask robot about lift',
    'hint',
    'ask robot about reactor',
    'hint',
    'ask robot to open door',
    'hint',
    'n',
    'hint',
    'get rod',
...
const wrapScript = function (obj1, obj2) {
  if (obj2 !== QuestJs._w.old_newspaper)
    return QuestJs._io.failedmsg('You cannot wrap that round anything.');
  if (obj1 !== QuestJs._w.fist)
    return QuestJs._io.failedmsg("You don't think that will achieve anything.");
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('ThrowThrough', {
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('SmashWith', {
    // throw rope out window
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('UseToSmash', {
    // throw rope out window
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('Attack', {
    // throw rope out window
...
QuestJs._commands.unshift(
  new QuestJs._command.Cmd('TieUp', {
    // throw rope out window
    ...
    script: function (objects) {
      ...
      if (!QuestJs._w.rope.isAtLoc(QuestJs._game.player)) {
        return QuestJs._io.failedmsg(
          'What were you thinking you could tie {ob:item} up with it exactly?',
          tpParams,
        );
      }
      ...
      if (objects[0][0] === QuestJs._w.robot) {
        QuestJs._io.msg(
          "'I am not into the kinky stuff,' says the robot. Despite its metallic face, you still feel it is looking at you with disapproval.",
        );
      } else if (objects[0][0] === QuestJs._w.Professor_Kleinscope) {
        QuestJs._io.msg(
          "'I don;t have time for that sort of thing now,' says the Professor irritably. He looks at yoi thoughtfully. 'Though maybe later...'",
        );
      } else {
        QuestJs._io.msg("That's not going to achieve anything.");
      }
...
QuestJs._commands.push(
  new QuestJs._command.Cmd('RudeCommand', {
    // throw rope out window
